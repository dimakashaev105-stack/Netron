...
def init_db():
    """Создать таблицу driver_stats если нет (идентично боту)"""
    with get_db() as conn:
        conn.execute('''
...
def taxi_complete():
    """Завершить поездку — начисляет баланс + опыт, обновляет driver_stats."""
    data    = request.json
    user_id = data.get('user_id')
    order   = data.get('order')  # может не прийти — берём из active_trips
    if not user_id:
        return jsonify({'error':'bad data'}), 400

    with get_db() as conn:
        # Если order не пришёл — читаем из active_trips
        if not order:
            row = conn.execute(
    ...
    with get_db() as conn:
        ...
        return jsonify({
            'ok':             True,
            'balance':        upd['balance'],
            'new_balance':    upd['balance'],   # alias for frontend
            'earned':         price,            # for toast "+X 🌸"
            'exp_gain':       experience,       # for toast "+X XP"
...
# Переопределяем taxi_start чтобы принимать route_id и возвращать trip + duration_seconds
@app.route('/api/taxi/start', methods=['POST'])
def taxi_start():
    data    = request.json
    user_id = data.get('user_id')
    route_id= data.get('route_id')
    order   = data.get('order')  # fallback если отправляют order напрямую
...
def roulette_spin():
    ...
    user_id   = data.get('user_id')
    bet       = int(data.get('bet',0))
    bet_type  = data.get('bet_type','red')  # red, black, green, number
...
def bj_deal():
    ...
    with get_db() as conn:
        ...
        if bal is None: return jsonify({'error':'user not found'}),404
        if bal < bet: return jsonify({'error':'недостаточно средств'}),400
    deck = make_deck()
    ...
    if pv==21 and dv==21:
        win_amount = bet
        with get_db() as conn:
            res = apply_game_result(conn, user_id, 0, 10)
        del bj_sessions[user_id]
        return jsonify({'ok':True,'player':player,'dealer':dealer,'player_value':pv,'dealer_value':dv,'status':'push','win_amount':win_amount,**res})
    if pv==21:
        win_amount = int(bet*1.5)
        with get_db() as conn:
            res = apply_game_result(conn, user_id, win_amount, 15)
        del bj_sessions[user_id]
...
def bj_hit():
    ...
    if pv > 21:
        bet = sess['bet']
        with get_db() as conn:
            res = apply_game_result(conn, user_id, -bet, 5)
        del bj_sessions[user_id]
...
def bj_stand():
    ...
    pv = hand_value(sess['player']); dv = hand_value(sess['dealer'])
    bet = sess['bet']
    if dv > 21 or pv > dv:
        status='win'; delta=bet; xp=15; win_amount=bet*2
    elif pv == dv:
        status='push'; delta=0; xp=10; win_amount=bet
    else:
        status='lose'; delta=-bet; xp=5; win_amount=0
    with get_db() as conn:
        res = apply_game_result(conn, user_id, delta, xp)
    del bj_sessions[user_id]
