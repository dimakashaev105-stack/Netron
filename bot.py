...
def _pvp_battle_text(b, last_event=None):
    a_name = _mkt_user_name_by_id(b["attacker_id"])
    d_name = _mkt_user_name_by_id(b["defender_id"])
    a_stats = _eq_calc_stats(b["attacker_id"])
    d_stats = _eq_calc_stats(b["defender_id"])
    a_hp = b["attacker_hp"]
    d_hp = b["defender_hp"]
    a_guard = b.get("attacker_guard", "")
    d_guard = b.get("defender_guard", "")

    def guard_hint(guard):
        if guard and guard in BODY_PARTS:
            bp = BODY_PARTS[guard]
            return f"  {bp['emoji']} защищает {bp['name']}"
        return "  🔓 без блока"

    lines_out = [
        "⚔️ <b>— П В П   Б О Й —</b>\n",
        f"🔴 <b>{a_name}</b>{guard_hint(a_guard)}",
        f"{_pvp_hp_bar(a_hp)}  <b>{a_hp}</b>/100",
        f"<i>⚔️ {a_stats['damage']} атк  •  🛡 {a_stats['defense']} защ</i>",
        "",
        f"🔵 <b>{d_name}</b>{guard_hint(d_guard)}",
        f"{_pvp_hp_bar(d_hp)}  <b>{d_hp}</b>/100",
        f"<i>⚔️ {d_stats['damage']} атк  •  🛡 {d_stats['defense']} защ</i>",
    ]
    if b["bet"] > 0:
        lines_out.append(f"\n💰 Ставка: <b>{format_balance(b['bet'] * 2)}</b>")
    if last_event:
        lines_out.append(f"\n┌─ Последний ход\n│  {last_event}\n└──────────────")
    if b["status"] == "active":
        turn_name = a_name if b["whose_turn"] == b["attacker_id"] else d_name
        indicator = "🔴" if b["whose_turn"] == b["attacker_id"] else "🔵"
        lines_out.append(f"\n{indicator} Ход <b>{turn_name}</b>  ⏳ {PVP_TURN_TIMEOUT}с")
    return "\n".join(lines_out)

def _pvp_action_kb(battle_id, user_id):
    atk_row = []
    for part_id, bp in BODY_PARTS.items():
        atk_row.append({"text": f"{bp['emoji']} {bp['name']}", "callback_data": f"pvp_action_{battle_id}_{user_id}_atk_{part_id}"})
    grd_row = []
    for part_id, bp in BODY_PARTS.items():
        grd_row.append({"text": f"🛡 {bp['name']}", "callback_data": f"pvp_action_{battle_id}_{user_id}_guard_{part_id}"})
    return {
        "inline_keyboard": [
            [{"text": "⚔️ — АТАКОВАТЬ —", "callback_data": "pvp_noop"}],
            atk_row,
            [{"text": "🛡️ — ЗАЩИТИТЬ —", "callback_data": "pvp_noop"}],
            grd_row,
        ]
    }

def _pvp_send_dm(battle_id, b, text, kb=None):
    def _send_one(uid):
        try:
            result = _tg_api("sendMessage", chat_id=uid, text=text, parse_mode="HTML", reply_markup=kb or {})
            if result.get("ok"):
                return result["result"]["message_id"]
        except Exception as e:
            print(f"[pvp_dm] send error uid={uid}: {e}")
        return 0
    a_mid = _send_one(b["attacker_id"])
    d_mid = _send_one(b["defender_id"])
    _pvp_update(battle_id, attacker_dm_msg_id=a_mid, defender_dm_msg_id=d_mid)
    return a_mid, d_mid

def _pvp_edit_dm(b, text, kb=None):
    whose_turn = b.get("whose_turn")
    a_mid = b.get("attacker_dm_msg_id", 0)
    d_mid = b.get("defender_dm_msg_id", 0)
    def _edit(uid, msg_id, show_kb):
        if not msg_id:
            return
        payload = dict(chat_id=uid, message_id=msg_id, text=text, parse_mode="HTML")
        payload["reply_markup"] = kb if (show_kb and kb) else {"inline_keyboard": []}
        try:
            _tg_api("editMessageText", **payload)
        except Exception as e:
            print(f"[pvp_dm] edit error uid={uid}: {e}")
    _edit(b["attacker_id"], a_mid, whose_turn == b["attacker_id"])
    _edit(b["defender_id"], d_mid, whose_turn == b["defender_id"])

def _pvp_edit(b, text, kb=None):
    a_mid = b.get("attacker_dm_msg_id", 0)
    d_mid = b.get("defender_dm_msg_id", 0)
    if not a_mid and not d_mid:
        try:
            if kb:
                bot.edit_message_text(text, chat_id=b["chat_id"], message_id=b["message_id"], parse_mode="HTML", reply_markup=kb)
            else:
                bot.edit_message_text(text, chat_id=b["chat_id"], message_id=b["message_id"], parse_mode="HTML")
        except Exception as e:
            print(f"[pvp] edit fallback error: {e}")
        return
    _pvp_edit_dm(b, text, kb)
...
def handle_pvp_challenge(message):
    ...
    def _cancel_pending():
        ...
        if bet > 0:
            update_balance(uid, bet)
        _pvp_edit(b, f"⏰ Вызов от <b>{a_name}</b> истёк — никто не принял (60 сек)")
...
def cb_pvp_accept(call):
    ...
    b = _pvp_get(battle_id)
    text = _pvp_battle_text(b, "🔔 Бой начался! Первый ход за атакующим.")
    kb = _pvp_action_kb(battle_id, b["attacker_id"])
    a_mid, d_mid = _pvp_send_dm(battle_id, b, text, kb)
...
def cb_pvp_action(call):
    ...
    text = _pvp_battle_text(b, event)
    kb = _pvp_action_kb(battle_id, b["whose_turn"])
    _pvp_edit_dm(b, text, kb)
    _pvp_start_timeout(battle_id)
def _pvp_finish(battle_id, b, last_event=""):
    ...
    w_indicator = "🔴" if winner_id == attacker_id else "🔵"
    l_indicator = "🔵" if winner_id == attacker_id else "🔴"

    text = (
        f"⚔️ <b>— Б О Й   З А В Е Р Ш Ё Н —</b>\n\n"
        f"{w_indicator} 🏆 <b>{w_name}</b>  победил!\n"
        f"{l_indicator} 💀 <b>{l_name}</b>  повержен\n\n"
        f"🔴 HP: <b>{b['attacker_hp']}</b>/100  •  🔵 HP: <b>{b['defender_hp']}</b>/100\n"
        f"{prize_note}\n"
        f"┌─ Финальный удар\n│  {last_event}\n└──────────────\n"
        f"🏆 Побед у {w_name}: <b>{w_wins}</b>\n"
        f"💀 Поражений у {l_name}: <b>{l_losses}</b>"
    )
    _pvp_edit_dm(b, text, kb=None)
    try:
        if b.get("message_id"):
            _tg_api("editMessageText",
                    chat_id=b["chat_id"],
                    message_id=b["message_id"],
                    text=(f"⚔️ <b>{w_name}</b> победил в бою против <b>{l_name}</b>! 🏆"
                          + (f"\n💰 Приз: <b>{format_balance(b['bet'] * 2)}</b>" if b["bet"] > 0 else "")),
                    parse_mode="HTML")
    except Exception:
        pass
def _pvp_start_timeout(battle_id):
    def _timeout():
        ...
        _pvp_edit(b,
            f"⏰ <b>Время вышло!</b>\n\n"
            f"💀 <b>{l_name}</b> не сделал ход за {PVP_TURN_TIMEOUT}с\n"
            f"🏆 Победа засчитана <b>{w_name}</b>!{prize_note}"
        )
