...
ADMIN_IDS = [8139807344, 5255608302]  # Только реальные ID
...
def handle_delete_item(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not items:
                bot.send_message(message.chat.id, f"💀 Вещь '{target}' отсутствуета!")
                return
            ...
            bot.send_message(message.chat.id,
                           ...
                           f"Подтвердить что хотите удалить эту вещь?",
                           reply_markup=markup,
                           parse_mode='HTML')
...
def confirm_delete_item(call):
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not item_info:
                bot.answer_callback_query(call.id, "💀 Вещь отсутствуета!")
                return
...
def get_daily_attempts(user_id, today):
    """Получает количество почкиок за сегодня"""
    with get_db_cursor() as cursor:
...
def update_math_stats(user_id, is_correct, earned, answer_time):
    ...
    with get_db_cursor() as cursor:
        # Увеличиваем счётчик почкиок за день
        cursor.execute('''
            UPDATE math_stats SET
...
def handle_math_start(message):
    ...
    info_text = (
        ...
        f"• На ответ: {MATH_SYSTEM['time_limit']}с\n"
        ...
        f"📝 <b>Просто впиши ответ числом в чат</b>"
    )
...
def handle_math_answer(message):
    ...
    try:
        ...
        bot.reply_to(message, "💀 Незачто, впиши только число (например: 42)")
        threading.Timer(2, delete_message, args=[message.chat.id, message.message_id + 1]).start()
...
def handle_item_info(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            info_text += f"📦 Побет: {item['supply'] if item['supply'] != -1 else '∞'}\n"
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not items:
                ...
                help_text = f"💀 Вещь '{item_name}' отсутствуета!\n\n"
...
def handle_find_item(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not items:
                bot.send_message(message.chat.id, f"💀 Вещи по запросу '{search_term}' отсутствуеты!")
                return
        ...
        if target.startswith('@'):
            ...
            with get_db_cursor() as cursor:
                ...
                if result:
                    ...
                    bot.send_message(message.chat.id, f"💀 Пользователь {target} отсутствует!")
                    return
        else:
            # По ID
            try:
...
def create_new_order(chat_id, user_id):
    ...
    markup.add(
        types.InlineKeyboardButton("✅ БЕРУ", callback_data="taxi_accept"),
        types.InlineKeyboardButton("❌ НЕ БЕРУ", callback_data="taxi_decline")
    )
...
def handle_admin_clear_taxi(message):
    ...
    if cleared > 0:
        bot.reply_to(message, f"✅ Очищено активных поездок: {cleared}")
    else:
...
# Функция для проверки активного займа
def has_active_loan(user_id):
    """Проверяет, есть ли у пользователя активный займ"""
    with get_db_cursor() as cursor:
        cursor.execute('SELECT status FROM loans WHERE user_id = ? AND status = "active"', (user_id,))
        return cursor.fetchone() is not None

def get_loan_details(user_id):
    """Получает детальную информацию о займе"""
    with get_db_cursor() as cursor:
...
def calculate_interest():
    ...
    with get_db_cursor() as cursor:
        ...
        # 3. Начисляем штраф за просрочку займов (старше 3д)
        cursor.execute('''
            UPDATE loans 
            SET loan_amount = loan_amount + (loan_amount * ?)
            WHERE status = 'active' 
...
def handle_deposit_command(message):
    ...
    # ПРОВЕРКА АКТИВНОГО ЗАЙМА
    if has_active_loan(user_id):
        loan_details = get_loan_details(user_id)
        bot.send_message(
            message.chat.id,
            f"💀 <b>БАНК ЗАБЛОКИРОВАН!</b>\n\n"
            f"У вас есть активный займ!\n"
            f"💸 Сумма долга: {format_balance(loan_details['amount_due'])}\n"
            f"📆 Дней прошло: {loan_details['days_passed']:.1f}\n\n"
            f"⚠️ <b>Услуги банка недоступны</b>\n"
            f"Погасите займ, чтобы продолжить пользоваться банком.",
            parse_mode='HTML'
        )
        return
...
def handle_loan_command(message):
    ...
    if has_active_loan(user_id):
        ...
        bot.send_message(
            message.chat.id,
            f"💀 <b>У ВАС УЖЕ ЕСТЬ АКТИВНЫЙ ЗАЙМ!</b>\n\n"
            f"💸 Сумма долга: {format_balance(loan_details['amount_due'])}\n"
            f"📆 Дней прошло: {loan_details['days_passed']:.1f}\n"
            f"📊 Начислено процентов: {format_balance(loan_details['total_interest'])}\n\n"
            f"{'🚨 <b>ЗАЙМ ПРОСРОЧЕН!</b> Штраф: ' + format_balance(loan_details['penalty']) if loan_details['is_overdue'] else ''}\n\n"
            f"⚠️ <b>Вы не можете взять новый займ, пока не погасите текущий!</b>\n"
            f"💡 Используйте 'погасить займ' для погашения долга.",
            parse_mode='HTML'
        )
        return
    ...
    try:
        ...
        bot.send_message(
            ...
            f"<b>ВАЖНО:</b> Пока у вас есть активный займ, услуги банка будут заблокированы!\n\n"
            f"Подтверждаете?",
            reply_markup=markup,
            parse_mode='HTML'
        )
...
def handle_repay_loan_command(message):
    ...
    message_text += f"💳 Твой баланс: {format_balance(balance)}\n"
...
def handle_cmd_deposit(call):
    ...
    # ПОВТОРНАЯ ПРОВЕРКА АКТИВНОГО ЗАЙМА
    if has_active_loan(user_id):
        bot.answer_callback_query(call.id, "💀 У вас активный займ! Банк заблокирован!")
        bot.edit_message_text(
            "💀 Операция отменена: у вас есть активный займ!\n\n"
            "Погасите займ, чтобы продолжить пользоваться банком.",
            call.message.chat.id,
            call.message.message_id
        )
        return
    ...
    try:
        ...
        bot.edit_message_text(
            ...
            f"💳 Твой баланс: {format_balance(new_balance)}\n"
...
def handle_cmd_withdraw(call):
    ...
    try:
        ...
        if deposit_amount < withdraw_amount:
            bot.answer_callback_query(call.id, "💀 Не хватает монет на вкладе!")
            return
...
def handle_cmd_loan(call):
    ...
    try:
        ...
        # ИСПОЛЬЗУЕМ БЛОКИРОВКУ ЧЕРЕЗ ТРАНЗАКЦИИ SQLITE
        with get_db_cursor() as cursor:
            ...
            if existing_loan:
                bot.answer_callback_query(call.id, "💀 У вас уже есть активный займ!")
                bot.edit_message_text(
                    "💀 Операция отменена: у вас уже есть активный займ!\n\n"
            ...
            # Выдаем займ
            cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (loan_amount, user_id))
            
            # ИСПРАВЛЕНО: Используем INSERT OR REPLACE вместо простого INSERT
            cursor.execute('''
        ...
        bot.edit_message_text(
            ...
            f"💳 Твой баланс: {format_balance(new_balance)}\n"
            f"📆 Срок займа: {LOAN_CONFIG['max_term']}д\n"
            f"📊 Проценты: {int(LOAN_CONFIG['interest_rate'] * 100)}% в день\n\n"
            f"⚠️ <b>ВНИМАНИЕ!</b>\n"
            f"• Пока у вас есть активный займ, банк ЗАБЛОКИРОВАН\n"
            f"• Вы не можете класть/снимать деньги со вклада\n"
            f"• Не забудьте погасить займ вовремя!\n\n"
            f"💡 Используйте 'погасить займ' для погашения долга.",
            call.message.chat.id,
            call.message.message_id,
            parse_mode='HTML'
...
def handle_cmd_repay(call):
    ...
    try:
        ...
        if not loan_info:
            bot.answer_callback_query(call.id, "💀 Займ отсутствует!")
            return
        
        loan_amount, taken_at = loan_info
        
        # Рассчитываем актуальную сумму долга
        current_time = int(time.time())
        days_passed = (current_time - taken_at) / 86400
        daily_interest = int(loan_amount * LOAN_CONFIG["interest_rate"])
        total_interest = int(daily_interest * days_passed)
        ...
        # Проверяем просрочку
        if days_passed > LOAN_CONFIG["max_term"]:
            penalty = int(amount_due * LOAN_CONFIG["penalty_rate"])
            amount_due += penalty
        ...
        if repay_amount >= amount_due:
            ...
            bot.edit_message_text(
                ...
                f"💳 Твой баланс: {format_balance(get_balance(user_id))}\n\n"
                f"🎉 <b>БАНК РАЗБЛОКИРОВАН!</b>\n"
                f"Теперь вы снова можете пользоваться всеми услугами банка.",
                call.message.chat.id,
                call.message.message_id,
                parse_mode='HTML'
            )
            ...
            # Частичное погашение
            remaining_debt = amount_due - repay_amount
            
            with get_db_cursor() as cursor:
                # Обновляем сумму займа и время (чтобы проценты пересчитывались от новой суммы)
                cursor.execute('UPDATE loans SET loan_amount = ? WHERE user_id = ?', (remaining_debt, user_id))
            
            bot.edit_message_text(
                f"🏆 <b>ЗАЙМ ЧАСТИЧНО ПОГАШЕН</b>\n\n"
                f"💸 Погашено: {format_balance(repay_amount)}\n"
                f"📊 Остаток долга: {format_balance(remaining_debt)}\n"
                f"💳 Твой баланс: {format_balance(get_balance(user_id))}\n\n"
                f"⚠️ Банк все еще заблокирован!\n"
                f"💡 Погасите остаток полностью, чтобы разблокировать услуги.",
                call.message.chat.id,
                call.message.message_id,
                parse_mode='HTML'
            )
...
def handle_bank_info(message):
    ...
    if loan_details:
        message_text += f"• Текущий займ: {format_balance(loan_details['loan_amount'])}\n"
        message_text += f"• Дней прошло: {loan_details['days_passed']:.1f}\n"
        message_text += f"• Начислено %: {format_balance(loan_details['total_interest'])}\n"
        message_text += f"• Всего к возврату: {format_balance(loan_details['amount_due'])}\n"
        
        if loan_details['is_overdue']:
            message_text += f"🚨 <b>ЗАЙМ ПРОСРОЧЕН!</b>\n"
            message_text += f"• Штраф: {format_balance(loan_details['penalty'])}\n"
        
        message_text += f"\n⚠️ <b>СТАТУС БАНКА: ЗАБЛОКИРОВАН</b>\n"
        message_text += f"Погасите займ для доступа к услугам.\n"
    else:
        message_text += f"• Нет активных займов\n"
        message_text += f"\n✅ <b>СТАТУС БАНКА: РАБОТАЕТ</b>\n"
    ...
    message_text += f"• Срок займа: {LOAN_CONFIG['max_term']}д\n"
    message_text += f"• Штраф за просрочку: {int(LOAN_CONFIG['penalty_rate'] * 100)}%"
    
    # Показываем разные кнопки в зависимости от наличия займа
    markup = InlineKeyboardMarkup()
    if loan_details:
        markup.add(
            InlineKeyboardButton("📋 Инфо по займу", callback_data="show_loan_info"),
            InlineKeyboardButton("💵 Погасить займ", callback_data="show_repay_menu")
        )
    else:
        markup.add(
...
def handle_bank(message):
    ...
    has_loan = has_active_loan(user_id)
    loan_details = get_loan_details(user_id) if has_loan else None
    
    # Проверяем наличие картинки банка
    try:
        with open('bank.jpg', 'rb') as photo:
            caption = f"🏛 <b>Приветствуем в Банк!</b>\n\n"
            caption += f"💵 <b>Твой баланс:</b> {format_balance(balance)}\n"
            caption += f"💳 <b>На вкладе:</b> {format_balance(bank_deposit)}\n\n"
            
            if has_loan:
                caption += f"🚨 <b>АКТИВНЫЙ ЗАЙМ!</b>\n"
                caption += f"💸 Долг: {format_balance(loan_details['amount_due'])}\n"
                caption += f"⚠️ <b>БАНК ЗАБЛОКИРОВАН!</b>\n\n"
            else:
                caption += f"✅ <b>БАНК РАБОТАЕТ</b>\n\n"
        ...
        bank_text = f"🏛 <b>Приветствуем в Банк!</b>\n\n"
        bank_text += f"💵 <b>Твой баланс:</b> {format_balance(balance)}\n"
        bank_text += f"💳 <b>На вкладе:</b> {format_balance(bank_deposit)}\n\n"
        
        if has_loan:
            bank_text += f"🚨 <b>АКТИВНЫЙ ЗАЙМ!</b>\n"
            bank_text += f"💸 Долг: {format_balance(loan_details['amount_due'])}\n"
            bank_text += f"⚠️ <b>БАНК ЗАБЛОКИРОВАН!</b>\n\n"
        else:
            bank_text += f"✅ <b>БАНК РАБОТАЕТ</b>\n\n"
        ...
        print("⚠️ Файл bank.jpg отсутствует в корневой папке!")
...
def handle_take_loan(message):
    ...
    if has_active_loan(user_id):
        ...
        bot.send_message(
            ...
            f"💸 Сумма долга: {format_balance(loan_details['amount_due'])}\n"
            f"📆 Дней прошло: {loan_details['days_passed']:.1f}\n\n"
            f"⚠️ Вы не можете взять новый займ!\n"
            f"💡 Используйте 'погасить займ' для погашения.",
            parse_mode='HTML'
        )
        return
    ...
    bot.send_message(
        ...
        f"• Срок: до {LOAN_CONFIG['max_term']}д\n"
        f"• Штраф за просрочку: {int(LOAN_CONFIG['penalty_rate'] * 100)}%\n"
        f"• <b>Банк будет заблокирован до погашения займа</b>\n\n"
        f"<b>Выберите сумму займа:</b>",
        reply_markup=markup,
        parse_mode='HTML'
    )

# Обработчики для кнопок меню банка
...
def handle_deposit_money(message):
    ...
    if has_active_loan(user_id):
        ...
        bot.send_message(
            ...
            f"У вас есть активный займ!\n"
            f"💸 Сумма долга: {format_balance(loan_details['amount_due'])}\n\n"
            f"Погасите займ, чтобы пользоваться вкладами.",
            parse_mode='HTML'
        )
        return
...
def handle_loan_info(message):
    ...
    message_text += f"💸 Сумма займа: {format_balance(loan_details['loan_amount'])}\n"
    message_text += f"⏰ Взят: {loan_details['days_passed']:.1f}д назад\n"
    message_text += f"📆 Макс. срок: {LOAN_CONFIG['max_term']}д\n"
    message_text += f"📊 Проценты: {int(LOAN_CONFIG['interest_rate'] * 100)}% в день\n"
    message_text += f"🎁 Начислено процентов: {format_balance(loan_details['total_interest'])}\n"
    message_text += f"💳 Сумма к возврату: {format_balance(loan_details['amount_due'])}\n\n"
    
    if loan_details['is_overdue']:
        message_text += f"🚨 <b>ЗАЙМ ПРОСРОЧЕН!</b>\n"
        message_text += f"⚠️ Штраф: {format_balance(loan_details['penalty'])}\n\n"
    
    message_text += f"<b>Твой баланс:</b> {format_balance(get_balance(user_id))}\n\n"
    message_text += f"⚠️ <b>Банк заблокирован до полного погашения займа!</b>"
    
    # Создаем кнопки для погашения
    markup = InlineKeyboardMarkup()
    if loan_details['amount_due'] <= get_balance(user_id):
        markup.add(InlineKeyboardButton("💵 ПОГАСИТЬ ЗАЙМ", callback_data=f"repay_loan_{user_id}"))
    else:
        markup.add(InlineKeyboardButton("💵 Недостаточно средств", callback_data="no_money"))
...
def handle_repay_loan(call):
    ...
    balance = get_balance(user_id)
    amount_due = loan_details['amount_due']
    
    if balance < amount_due:
        bot.answer_callback_query(call.id, f"💀 Не хватает монет!")
        return
    ...
    def interest_calculator():
        while True:
            try:
                ...
                time.sleep(300)  # Ждем 5м при ошибке
...
def handle_warn_user(message):
    ...
    try:
        ...
        if not get_user_info(target_user_id):
            bot.send_message(message.chat.id, "💀 Пользователь отсутствует в базе данных!")
            return
...
def handle_unwarn_user(message):
    ...
    try:
        ...
        if success:
            # Уведомляем пользователя
            try:
                bot.send_message(target_user_id, "🏆 Твой варн снят! Ограничения сняты.")
            except:
                pass
...
def handle_download_db(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "✅ База данных отправлена")
...
def handle_document_upload(message):
    """Обрабатывает загрузку файла базы данных"""
    ...
    try:
        ...
        if not file_name.lower().endswith('.db'):
            bot.reply_to(message, "💀 Ошибка: отправьте файл с расширением .db")
            return
        ...
        try:
            ...
            banned_users = set()  # Очищаем список забаненных (они теперь в новой базе)
            ...
            bot.send_message(message.chat.id, "💀 Ошибка: загруженный файл не является базой данных SQLite!")
            
            if backup_path and os.path.exists(backup_path):
                shutil.copy2(backup_path, db_path)
                bot.send_message(message.chat.id, "✅ Старая база восстановлена")
...
def handle_clan_search(message):
    ...
    if user_clan:
        bot.send_message(message.chat.id, 
                       ...
                       f"Используйте 'Выйти из клана' чтобы вступить в новый.")
        return
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not clans:
                bot.send_message(message.chat.id, f"💀 Кланы по запросу '{search_query}' не найдены")
                return
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при поиске кланов")
...
def handle_clans_button(message):
    ...
    if user_clan:
        ...
        if clan_info.get('has_avatar'):
            caption = f'⚔️ {clan_info["name"]} [{clan_info["tag"]}]\n━━━━━━━━━━━━━━━\n'
            caption += f'⭐ Ранг: {clan_info["level"]}\n'
            caption += f'📈 Очки: {format_balance(clan_info["experience"])}\n'
            caption += f'💵 Казна: {format_balance(clan_info["balance"])}\n'
            caption += f'👥 Бойцов: {clan_info["members_count"]}/{clan_info["max_members"]}'
            ...
            message_text = f'⚔️ {clan_info["name"]} [{clan_info["tag"]}]\n━━━━━━━━━━━━━━━\n'
            message_text += f'⭐ Ранг: {clan_info["level"]}\n'
            message_text += f'📈 Очки: {format_balance(clan_info["experience"])}\n'
            message_text += f'💵 Казна: {format_balance(clan_info["balance"])}\n'
            message_text += f'👥 Бойцов: {clan_info["members_count"]}/{clan_info["max_members"]}'
        ...
        message_text = '⚔️ СИСТЕМА ГИЛЬДИЙ\n━━━━━━━━━━━━━━━\n'
        message_text += 'Объединяйся с друзьями для общих целей\n\n'
        message_text += '🎯 Возможности:\n'
        message_text += '• Общая казна\n'
        message_text += '• Клановые войны\n'
        message_text += '• Аватар гильдии\n'
        message_text += '• Еженедельные награды\n\n'
        message_text += f'💵 Создать: {format_balance(CLAN_CONFIG["create_price"])}\n'
        message_text += f'🖼️ Аватар: {format_balance(CLAN_CONFIG["avatar_price"])}'
...
def handle_clan_callbacks(call):
    ...
    try:
        ...
        # Выход из клана
            ...
            markup = types.InlineKeyboardMarkup(row_width=2)
            markup.add(
                types.InlineKeyboardButton('✅ Да, выйти', callback_data='leave_clan_yes'),
                types.InlineKeyboardButton('❌ Отмена', callback_data='back_to_clans')
            )
            
            bot.edit_message_text(
                f'⚠️ ВЫХОД ИЗ ГИЛЬДИИ\n━━━━━━━━━━━━━━━\n\n'
                f'Точно хочешь покинуть {user_clan["name"]}?',
                call.message.chat.id,
                call.message.message_id,
                reply_markup=markup
            )
...
def process_clan_tag_step(message, clan_name):
    ...
    if len(clan_tag) < 2 or len(clan_tag) > 5:
        bot.send_message(message.chat.id, '💀 Тег должен быть от 2 до 5 символов')
        return
    
    if not clan_tag.isalnum():
        bot.send_message(message.chat.id, '💀 Тег может содержать только буквы и цифры')
        return
    ...
    with get_db_connection() as conn:
        ...
        if cursor.fetchone():
            bot.send_message(message.chat.id, '💀 Этот тег уже занят')
            return
...
def process_clan_description_step(message, clan_name, clan_tag):
    ...
    if description == '-':
        ...
        bot.send_message(message.chat.id, '💀 Описание слишком длинное (макс. 100 символов)')
        return
    ...
    if balance < price:
        bot.send_message(message.chat.id, f'💀 Не хватает средств! Нужно: {format_balance(price)}')
        return
...
def process_clan_search(message):
    ...
    if len(search_query) < 2:
        bot.send_message(message.chat.id, '💀 Запрос слишком короткий')
        return
    ...
    if not clans:
        bot.send_message(message.chat.id, f'💀 Гильдии по запросу "{search_query}" не найдены')
        return
...
def process_clan_donate(message, clan_id):
    ...
    try:
        ...
        if not amount or amount < 1000:
            bot.send_message(message.chat.id, '💀 Минимальная сумма: 1K$')
            return
        
        balance = get_balance(user_id)
        if amount > balance:
            bot.send_message(message.chat.id, '💀 Не хватает средств')
            return
...
def process_declare_war(message, attacker_clan_id):
    ...
    with get_db_cursor() as cursor:
        ...
        if not target:
            bot.send_message(message.chat.id, f'💀 Клан с тегом [{target_tag}] отсутствует')
            return
...
def war_declare_command(message):
    ...
    if user_clan['role'] != 'leader':
        bot.send_message(message.chat.id, '💀 Только глава может объявлять войну')
        return
...
def create_business_menu():
    ...
    markup.add(
        ...
        KeyboardButton("🛒 Шоп бизнесов"),
        KeyboardButton("Заприобрести сырье"),
...
def create_top_menu(top_type="balance", page=0):
    ...
    type_buttons = [
        ...
        InlineKeyboardButton("⚔️ Гильдии", callback_data=f"top_type_clans_{page}"),
    ...
    for user_id in expired_captchas:
        try:
            ...
            try:
                bot.send_message(
                    user_id,
                    "⏰ Время на решение капчи истекло\n🔁 Используйте /start для новой почкики"
                )
            except:
                pass
...
def handle_city(message):
    ...
    city_text = """🏙️ <b>Приветствуем в Город!</b>

Здесь вы можете:
🛍️ <b>🛒 Шоп</b> - приобрести новую одежду
🎒 <b>👔 Шкаф</b> - надеть свою одежду  
...
def add_experience(user_id, exp_amount):
    """Начисляет очки и отправляет уведомление о повышении уровня"""
    try:
        with get_db_cursor() as cursor:
            # Получаем старый очки и ранг
            cursor.execute('SELECT experience FROM users WHERE user_id = ?', (user_id,))
            old_exp = cursor.fetchone()[0]
            old_level = int((old_exp / 900) ** 0.4) + 1
            
            # Добавляем новый очки
            ...
            if new_level > old_level:
                ...
                try:
                    ...
                    pass  # Если ошибка отправить - ничего страшного
...
def handle_buy(message):
    ...
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton("💵 10k за 2 звезды", callback_data="stars_5kkk"),
        InlineKeyboardButton("💵 40k за 10 звезд", callback_data="stars_30kkk")
    )
    markup.row(
        InlineKeyboardButton("💵 105k за 15 звезд", callback_data="stars_75kkk"),
        InlineKeyboardButton("💵 300k за 25 звезд", callback_data="stars_100kkk")
    )
    markup.row(
        InlineKeyboardButton("💵 1.5kk за 100 звезд", callback_data="stars_500kkk")
    )
...
def handle_stars_selection(call):
    ...
    packages = {
        "stars_5kkk": {"amount": 10000, "stars": 2, "title": "10k"},
        "stars_30kkk": {"amount": 40000, "stars": 10, "title": "40k"},
        "stars_75kkk": {"amount": 105000, "stars": 15, "title": "105k"},
        "stars_100kkk": {"amount": 300000, "stars": 25, "title": "300k"},
        "stars_500kkk": {"amount": 1500000, "stars": 100, "title": "1.5kk"}
    }
    ...
    if package in packages:
        ...
        try:
            bot.send_invoice(
                ...
                title=f"💫 Приобретение {pkg['title']}",
...
def handle_case(message):
    ...
    try:
        ...
        if balance < total_cost:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно {format_balance(total_cost)}")
            return
        ...
        bot.send_message(message.chat.id,
                       f"🎁 Приобретение кейсов\n\n"
                       f"💵 Стоимость: {format_balance(total_cost)}\n"
                       f"💳 Твой баланс: {format_balance(balance)}\n"
                       f"⚔️ Количество: {quantity} кейсов\n\n"
                       f"📊 Шансы:\n"
...
def show_case_page(call, opened_items, quantity, total_cost, total_items_value, page=0):
    ...
    message_text += f"\n💳 Средства: {format_balance(new_balance)}"
...
def handle_case_navigation(call):
    try:
        ...
        # Проверяем, что игрок имеет доступ к этим данным
        if call.from_user.id != user_id:
...
def handle_bag(message):
    ...
    message_text += f"\n💡 Используй 'сбыть [номер]' или 'сбыть все'"
    
    bot.send_message(message.chat.id, message_text)

# Обработчик команды сбыть
@bot.message_handler(func=lambda message: message.text.lower().startswith('сбыть'))
def handle_sell(message):
    ...
    parts = text.split()
    if len(parts) < 2:
        bot.send_message(message.chat.id, "💀 Используй: сбыть [номер] или сбыть все")
        return
...
def handle_active(message):
    try:
        ...
        bot.send_message(message.chat.id, "💀 Временная ошибка системы. Повторите почкику.")
...
def handle_roulette_photo_delete(message):
    ...
    try:
        ...
        if not os.path.exists(photo_path):
            bot.send_message(message.chat.id, f"💀 Фото для числа {number} отсутствуето")
            return
        ...
        bot.send_message(
            message.chat.id,
            f"🗑️ Подтвердить что хотите удалить фото для числа {number}?\n\n📁 Файл: {photo_path}",
            reply_markup=markup
        )
...
BANNED_CHAT_ID = -1002399401515  # Замените на реальный ID чата @FECTIZ_CHAT
...
def handle_roulette(message):
    ...
    try:
        ...
        if bet_amount > balance:
            bot.send_message(message.chat.id, "💀 Не хватает монет!")
            return
...
def handle_basketball(message):
    try:
        ...
        if bet_amount > balance:
            bot.send_message(message.chat.id, "💀 Не хватает монет для ставки")
            return
        ...
        bot.send_message(message.chat.id, "💀 Ошибка в игре. Повторите почкику.")
...
def handle_darts(message):
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка в игре. Повторите попытку.")
...
def handle_mines(message):
    try:
        ...
        if bet_amount is None or bet_amount < 100:
            bot.send_message(message.chat.id, f"💀 Мин. бет: {format_balance(100)}")
            return
...
def handle_lottery(message):
    ...
    if bet_text.lower() == 'все':
        ...
        if not bet:
            bot.reply_to(message, "💀 Неверная бет!")
            return
    
    # Проверяем лимиты
    if bet < LOTTERY_MACHINE_CONFIG["min_bet"]:
        bot.reply_to(message, f"💀 Мин. бет: {format_balance(LOTTERY_MACHINE_CONFIG['min_bet'])}")
        return
    
    if bet > LOTTERY_MACHINE_CONFIG["max_bet"]:
        bot.reply_to(message, f"💀 Макс. бет: {format_balance(LOTTERY_MACHINE_CONFIG['max_bet'])}")
        return
...
def handle_mines_click(call):
    try:
        ...
        if user_id not in active_mines_games:
            bot.answer_callback_query(call.id, "💀 Игра отсутствуета!")
            return
...
def change_item_type(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not item:
                bot.send_message(message.chat.id, "💀 Вещь отсутствуета!")
                return
...
def handle_add_supply(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not items:
                bot.send_message(message.chat.id, f"💀 Одежда '{item_name}' отсутствуета в магазине!")
                return
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if affected > 0:
                ...
                bot.send_message(message.chat.id, "💀 Pepe Gucci отсутствует в базе!")
...
def show_shop_page(chat_id, user_id, page=0, category=None):
    ...
    try:
        ...
        if not page_items:
            bot.send_message(chat_id, "🛍️ Товары отсутствуеты!")
            return
...
def show_wardrobe_page(chat_id, user_id, page=0):
    ...
    try:
        ...
        if not page_items:
            bot.send_message(chat_id, "🎒 Вещи отсутствуеты!")
            return
...
Введите ответ числом."""
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при запуске. Повторите почкику.")
                ...
                if check_data:
                    ...
                    if ref_user and ref_user[0] != user_id:
                        ...
                        if not current_ref or not current_ref[0]:
                            ...
                            bot.send_message(message.chat.id, "🎉 Ты получил 1 000🌸 за приглашение друга!")
        
        markup = create_main_menu()
        welcome_text = f"""👋 Рады видеть, {first_name}!
...
💡 Используйте кнопки меню для продолжения игры!"""
        ...
        bot.send_message(message.chat.id, "💀 Ошибка. Повторите почкику.")
    ...
    try:
        ...
        if user_answer == correct_answer:
            ...
            if remaining_attempts > 0:
                bot.send_message(message.chat.id, f"💀 Неверно! Осталось почкиок: {remaining_attempts}")
            else:
                # Превышено количество почкиок
                del active_captchas[user_id]
                bot.send_message(message.chat.id, "🚫 Превышено количество почкиок. Попробуйте позже.", reply_markup=ReplyKeyboardRemove())
...
def complete_new_user_registration(user_id, username, first_name, ref_code):
    ...
    try:
        ...
        success_text = f"""🏆 <b>Регистрация окончена!</b>
...
💡 Используйте кнопки меню для навигации!"""
        ...
        bot.send_message(user_id, "💀 Ошибка при регистрации. Повторите почкику.")
            ...
            if check_data:
                ...
                if ref_user and ref_user[0] != user_id:
                    ...
                    if not current_ref or not current_ref[0]:
                        ...
                        cursor.execute('UPDATE users SET referred_by = ? WHERE user_id = ?', (referred_by, user_id))
                        cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (user_bonus, user_id))
                        bot.send_message(user_id, "🎉 Ты получил 1 000🌸 за регистрацию по реферальной ссылке!")
...
def handle_name_change(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"🏆 Твое имя изменено на: {new_name}")
    
    except Exception as e:
        print(f"Ошибка в handle_name_change: {e}")
        bot.send_message(message.chat.id, "💀 Ошибка при изменении имени. Повторите почкику.")
...
def handle_user_create_check(message):
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           "💀 Используйте: выпустить ваучер [сумма] [кол-во активаций] (пароль)\n\n"
                           "Примеры:\n"
                           "• выпустить ваучер 1000000 1 - чек на 1М, 1 активация\n"
                           "• выпустить ваучер 500000 3 mypass - чек на 500к с паролем")
            return
        ...
        if total_amount > balance:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно: {format_balance(total_amount)}, твой баланс: {format_balance(balance)}")
            return
...
def handle_clan_distribute(message):
    ...
    if user_clan['role'] != 'leader':
        bot.send_message(message.chat.id, "💀 Только глава может распределять деньги!")
        return
    ...
    try:
        ...
        if amount > user_clan['balance']:
            bot.send_message(message.chat.id, f"💀 В казне недостаточно средств! Открыто: {format_balance(user_clan['balance'])}")
            return
        ...
        if share_per_member == 0:
            bot.send_message(message.chat.id, "💀 Сумма слишком мала для распределения между боецами!")
            return
...
def handle_clan_upgrade(message):
    ...
    if user_clan['role'] not in ['leader', 'officer']:
        bot.send_message(message.chat.id, "💀 Только глава и офицеры могут улучшать клан!")
        return
    ...
    bot.send_message(message.chat.id,
                   f"🏰 Улучшение клана\n\n"
                   f"📈 Текущий ранг: {user_clan['level']}\n"
...
def place_bid(auction_id, user_id, bid_amount):
    ...
    with get_db_cursor() as cursor:
        ...
        if not auction:
            return False, "Аукцион отсутствует!"
...
def handle_duel_create(message):
    ...
    try:
        ...
        if bet_amount < DUEL_CONFIG["min_bet"]:
            bot.send_message(message.chat.id, 
                           f"Минимальная бет: {format_balance(DUEL_CONFIG['min_bet'])}!")
            return
        
        if bet_amount > DUEL_CONFIG["max_bet"]:
            bot.send_message(message.chat.id, 
                           f"Максимальная бет: {format_balance(DUEL_CONFIG['max_bet'])}!")
            return
        
        total_bet = bet_amount * team_size
        if total_bet > balance:
            bot.send_message(message.chat.id, 
                           f"Не хватает монет! Нужно: {format_balance(total_bet)}")
            return
...
def start_duel_in_dm(duel):
    ...
    bot.send_message(duel.chat_id, 
                   "⚔️ Дуэль начинается! Все боеци получат ЛС с деталями боя.")
...
def cleanup_expired_duels():
    ...
    for duel_id in expired_duels:
        ...
        # Уведомляем в чате
        bot.send_message(duel.chat_id, 
                       f"⚔️ Дуэль истекла! Деньги возвращены всем боецам.")
...
def handle_delete_auction(message):
    ...
    try:
        ...
        if not auction:
            bot.send_message(message.chat.id, "💀 Аукцион отсутствует!")
            return
        ...
        bot.send_message(message.chat.id,
                       ...
                       f"⚔️ Ставок: {auction['bids_count']}\n\n"
                       f"⚠️ <b>Все ставки будут возвращены боецам!</b>\n"
                       f"Подтвердить что хотите удалить этот аукцион?",
                       reply_markup=markup,
                       parse_mode='HTML')
...
def handle_create_auction(message):
    ...
    try:
        ...
        if duration_hours < 1 or duration_hours > 168:
            bot.send_message(message.chat.id, "💀 Время должно быть от 1 до 168ч (7д)")
            return
...
def handle_auction_bid(message, auction_id, bet_text):
    ...
    if not auction:
        bot.reply_to(message, "💀 Аукцион отсутствует")
        return
        ...
        if winner_id:
            ...
            try:
                ...
                bot.send_message(winner_id,
                               ...
                               f"📞 Свяжитесь с админом для получения выигрыша!")
            except:
                pass
            ...
            bot.send_message(message.chat.id,
                           ...
                           "💵 Все средства возвращены боецам")
    
    except Exception as e:
        print(f"Ошибка окончения аукциона: {e}")
        bot.send_message(message.chat.id, "💀 Ошибка при окончении аукциона!")
...
def handle_profile(message):
    ...
    try:
        ...
        message_text = f"🎴 Игровой аккаунт\n\n"
        message_text += f"🆔 Никнейм: {display_name}\n"
        message_text += f"📆 Играет: {age_days} дн.\n"
        message_text += f"💵 Средства: {format_balance(user_info['balance'])}\n"
        message_text += f"⚡ Майнеров: {user_info['video_cards']}\n"
        message_text += f"🏛 Депозит: {format_balance(user_info['bank_deposit'])}\n"
        message_text += f"👆 Кликов: {user_info['total_clicks']}\n"
        message_text += f"🎁 Наград: {click_bonus_count}\n"
        ...
        message_text += f"\n🎲 Игровая статистика:\n"
        message_text += f"⚔️ Партий: {total_games}\n"
        if total_games > 0:
            message_text += f"🏆 Побед: {games_won} ({games_won/total_games*100:.1f}%)\n"
            message_text += f"💀 Поражений: {games_lost} ({games_lost/total_games*100:.1f}%)\n"
        else:
            message_text += f"🏆 Побед: 0\n"
            message_text += f"💀 Поражений: 0\n"
        message_text += f"💵 Заработано: {format_balance(total_won)}\n"
        message_text += f"💸 Потрачено: {format_balance(total_lost)}\n"
...
def handle_reset_stats(message):
    ...
    try:
        ...
        if target.startswith('@'):
            ...
            with get_db_cursor() as cursor:
                ...
                if result:
                    ...
                    bot.reply_to(message, f"💀 Пользователь {target} отсутствует!")
                    return
        else:
            # По ID
            try:
        ...
        if not target_info:
            bot.reply_to(message, "💀 Пользователь отсутствует в базе данных!")
            return
        ...
        bot.reply_to(message, 
                   ...
                   f"• Поражений: {format_balance(target_info.get('total_lost_amount', 0))} → 0")
...
# Обработчик кнопки "🛒 Шоп бизнесов"
@bot.message_handler(func=lambda message: message.text == "🛒 Шоп бизнесов")
...
# Обработчик "Заприобрести сырье"
@bot.message_handler(func=lambda message: message.text == "Заприобрести сырье")
...
def handle_work(message):
    try:
        ...
        print("Файл work.jpg отсутствует!")
    except Exception as e:
...
def handle_clicker(message):
    ...
    bot.send_message(message.chat.id, "⚔️ Найди правильную кнопку:", reply_markup=create_clicker_keyboard())
...
def handle_balance(message):
    try:
        user_id = message.from_user.id
        balance = get_balance(user_id)
        bot.send_message(message.chat.id, f"💵 Твой баланс: {format_balance(balance)}")
    except Exception as e:
...
def handle_slots(message):
    ...
    try:
        ...
        if bet_amount < SLOT_CONFIG["min_bet"]:
            bot.send_message(message.chat.id, f"💀 Минимальная бет: {format_balance(SLOT_CONFIG['min_bet'])}!")
            return
        
        if bet_amount > SLOT_CONFIG["max_bet"]:
            bot.send_message(message.chat.id, f"💀 Максимальная бет: {format_balance(SLOT_CONFIG['max_bet'])}!")
            return
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if result:
                ...
                message_text = f"⚡ Твоя майнинг ферма:\n"
        ...
        if call.data == "mining_collect":
            with get_db_cursor() as cursor:
                ...
                if income > 0:
                    ...
                    message_text += f"💳 Средства: {format_balance(new_balance)}"
    ...
    try:
        ...
        if symbol == "🏆":
            with get_db_cursor() as cursor:
                ...
                # Бонус за ранг (УМЕНЬШЕНО В 1000 РАЗ)
                if new_level > old_level:
                    level_bonuses = {
                        2: 1000,   # Было 1B → стало 1M
                        3: 2500,   # Было 2.5B → стало 2.5M
                        5: 10000,  # Было 10B → стало 10M
                    }
            ...
            display_text += f"👆 Кликов: {new_total_clicks}\n"
            display_text += f"💵 Средства: {format_balance(new_balance)}\n"
            ...
            if bonus > 0:
                display_text += f"⚔️ До следующего бонуса: 100 кликов"
            else:
                next_bonus = 100 - (new_total_clicks % 100)
                display_text += f"⚔️ До следующего бонуса: {next_bonus} кликов"
...
def handle_bonus_broadcast(message):
    ...
    try:
        ...
        bot.reply_to(
            ...
            f"⚔️ <b>Цель:</b> Привлечь подписчиков в канал\n\n"
            f"Подтвердить что хотите разослать бонусы всем?",
            reply_markup=markup,
            parse_mode='HTML'
        )
...
def get_user_level(user_id):
    with get_db_cursor() as cursor:
        ...
        # Формула уровня: sqrt(очки / 1000) + 1
        level = int((experience / 1000) ** 0.5) + 1
        return level, experience

# Функция для добавления очкиа
def add_experience(user_id, exp_amount):
    with get_db_cursor() as cursor:
        cursor.execute('UPDATE users SET experience = experience + ? WHERE user_id = ?', (exp_amount, user_id))
...
def handle_me(message):
    try:
        ...
        if not user_info:
            bot.send_message(message.chat.id, "💀 Пользователь отсутствует")
            return
...
def send_trade_interface(user_id, trade):
    try:
        ...
        markup.add(
            ...
            InlineKeyboardButton("💀 Назад обмен", callback_data=f"TRADE_CNL_{trade.trade_code}_{user_id}")
        )
...
def handle_trade_add(call):
    ...
    if trade_id not in active_trades:
        bot.answer_callback_query(call.id, "💀 Обмен отсутствует!")
        return
...
def handle_trade_cancel(call):
    ...
    try:
        bot.send_message(user_id, "💀 Обмен сброшен")
    except:
        pass
...
def cleanup_expired_trades():
    ...
    for trade_id in expired:
        ...
        # Уведомляем боецов
        try:
            bot.send_message(trade.user1_id, "💀 Обмен сброшен (время вышло)")
        except:
            pass
        
        try:
            bot.send_message(trade.user2_id, "💀 Обмен сброшен (время вышло)")
        except:
            pass
...
def handle_add_clothing(message):
    ...
    try:
        ...
        if price is None or price <= 0:
            bot.send_message(message.chat.id, "💀 Неверная стоимость!")
            return
...
def handle_scratch(call):
    ...
    if target_user_id not in lottery_games:
        bot.answer_callback_query(call.id, "💀 Билет отсутствует!")
        return
...
# Добавляем одежду
    ...
    ("BMB M5 f90 karabasa", 50000000000, "Справа", "BMB M5 f90 karabasa.png"),
        ...
        bot.send_message(message.chat.id, f"🏆 🛒 Шоп создан! Добавлено {added_count} айтемов.")
...
def handle_remove_item_from_shop(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not items:
                bot.send_message(message.chat.id, f"💀 Вещь '{item_identifier}' отсутствуета!")
                return
            ...
            bot.send_message(message.chat.id, 
                           ...
                           f"💡 Вещь осталась в базе данных, но ее нельзя приобрести.")
...
def handle_clothes_price(message):
    ...
    if message.text == "💀 Отмена":
        del clothes_creation_state[message.from_user.id]
        bot.send_message(message.chat.id, "💀 Добавление сброшено", reply_markup=create_main_menu())
        return
    
    try:
        price = parse_bet_amount(message.text, float('inf'))
        if not price or price <= 0:
            bot.send_message(message.chat.id, "💀 Неверная стоимость! Укажи число больше 0")
            return
...
def handle_dice_bet(message):
    ...
    try:
        ...
        if user_balance < bet_amount:
            bot.send_message(message.chat.id, f"Не хватает монет: {format_balance(bet_amount)}")
            return
...
def handle_give_money(message):
    ...
    try:
        ...
        try:
            bot.send_message(target_user_id, 
                           f"🎉 Администратор выдал вам {format_balance(amount)}!\n💳 Твой баланс: {format_balance(get_balance(target_user_id))}")
        except:
            pass
...
def handle_take_money(message):
    ...
    try:
        ...
        if target_balance < amount:
            bot.send_message(message.chat.id, f"💀 У пользователя недостаточно средств! Средства: {format_balance(target_balance)}")
            return
...
def handle_set_balance(message):
    ...
    try:
        ...
        try:
            bot.send_message(target_user_id, 
                           f"⚡ Администратор установил твой баланс: {format_balance(amount)}!")
        except:
            pass
...
def handle_create_check(message):
    ...
    try:
        ...
        # Обработчик команды "очки"
@bot.message_handler(func=lambda message: message.text.lower() == "очки")
def handle_experience(message):
    try:
        user_id = message.from_user.id
        level, experience = get_user_level(user_id)
        
        # Считаем очки для следующего уровня
        ...
        message_text = f"⭐️ Твой прогресс:\n\n"
        message_text += f"⚔️ Текущий ранг: {level}\n"
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при получении информации об очкие")
# Система налогов
TAX_CONFIG = {
...
def handle_collect_tax(message):
    ...
    try:
        ...
        message_text += f"Подтвердить что хотите собрать налоги?"
...
def handle_cancel_tax_collection(call):
    ...
    bot.edit_message_text("💀 Сбор налогов сброшен", call.message.chat.id, call.message.message_id)
    bot.answer_callback_query(call.id, "🏆 Отменено")
...
def handle_transfer(message):
    try:
        ...
        if is_user_warned(user_id):
            bot.send_message(message.chat.id, "💀 Вы не можете транзакцияить деньги, так как у вас активный варн!")
            return
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, 
                           "💀 Используйте:\n"
                           "• `кинуть @username сумма` - транзакция по юзернейму\n"
                           "• `кинуть сумма` (ответом на сообщение) - транзакция по ответу", 
                           parse_mode='Markdown')
            return
        ...
        if user_balance < amount:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно: {format_balance(amount)}")
            return
        ...
        if isinstance(target_identifier, int):
            ...
            if not target_info:
                bot.send_message(message.chat.id, f"💀 Пользователь {target_identifier} отсутствует!")
                return
        ...
        print(f"Ошибка при транзакцияе: {e}")
        bot.send_message(message.chat.id, "💀 Ошибка при обработке транзакцияа")
...
def handle_change_price(message):
    ...
    try:
        ...
        try:
            ...
            bot.send_message(message.chat.id, "💀 Неверная стоимость! Используйте только цифры")
            return
...
def handle_change_price_by_id(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if not item:
                bot.send_message(message.chat.id, f"💀 Одежда с ID {item_id} отсутствуета!")
                return
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при изменении цены!")
# Обработчик подтверждения транзакцияа
...
def handle_confirm_transfer(call):
    try:
        ...
        if success:
            add_experience(user_id, amount // 100000)
            
            # Сначала отправляем эмодзи в чат
            bot.send_message(call.message.chat.id, "💸")
            
            # Затем обновляем сообщение
            result_text = f"🏆 Транзакция окончена\n"
...
# Обработчик отмены транзакцияа
@bot.callback_query_handler(func=lambda call: call.data == "cancel_transfer")
def handle_cancel_transfer(call):
    bot.edit_message_text("💀 Перевод сброшен", call.message.chat.id, call.message.message_id)
    bot.answer_callback_query(call.id, "Отменено")
...
def crash_command(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"⚔️ Множитель: *{multiplier:.2f}x*\n"
            f"💵 Ставка: *{format_balance(bet_amount)}*\n"
            f"⏳ Результат через: *5с*\n\n"
...
def handle_knb_challenge(message):
    ...
    if opponent.is_bot:
        bot.send_message(message.chat.id, "❌ Нельзя играть с ботом!")
        return
    ...
    if bet_amount > challenger_balance:
        bot.send_message(message.chat.id, f"❌ Недостаточно средств! Ваш баланс: {format_balance(challenger_balance)}")
        return
    ...
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton("✅ Принять", callback_data=f"knb_accept_{challenger_id}_{opponent_id}"),
        InlineKeyboardButton("❌ Отклонить", callback_data=f"knb_decline_{challenger_id}_{opponent_id}")
    )
