            ...
            try:
                ...
                if is_private:
                    with get_db_cursor() as _cur:
                        _cur.execute(
                            "UPDATE users SET last_activity=%s, last_private_activity=%s WHERE user_id=%s",
                            (now_ts, now_ts, uid)
                        )
                else:
                    with get_db_cursor() as _cur:
                        _cur.execute(
                            "UPDATE users SET last_activity=%s WHERE user_id=%s AND last_activity > 0",
                            (now_ts, uid)
                        )
            except Exception:
                pass
...
_pg_pool = psycopg2.pool.ThreadedConnectionPool(
    minconn=2, maxconn=10, dsn=_PG_URL, sslmode="require",
)
...
class _PgRow:
    ...
    def __repr__(self): return repr(dict(zip(self._cols, self._vals)))

class _PgCursor:
    def __init__(self, cur, conn=None):
        self._cur = cur; self._conn = conn; self._cols = []
        self.lastrowid = None; self.rowcount = 0
    @property
    ...
    def execute(self, sql, params=None):
        ...
        return self
    def execute_ignore(self, sql, params=None):
        """Execute a statement, silently ignoring errors (uses SAVEPOINT to keep transaction alive)."""
        conn = self._conn
        if conn is None:
            try: self.execute(sql, params)
            except Exception: pass
            return
        self._cur.execute("SAVEPOINT _exec_ignore")
        try:
            self.execute(sql, params)
            self._cur.execute("RELEASE SAVEPOINT _exec_ignore")
        except Exception:
            self._cur.execute("ROLLBACK TO SAVEPOINT _exec_ignore")
    def executemany(self, sql, params_list):
        sql = self._fix(sql)
        for p in params_list: self._cur.execute(sql, list(p))
    def _row(self, raw):
        return _PgRow(self._cols, list(raw)) if raw is not None else None
    def fetchone(self): return self._row(self._cur.fetchone())
    def fetchall(self): return [self._row(r) for r in (self._cur.fetchall() or [])]
...
def get_db_cursor():
    with get_db_connection() as conn:
        cursor = _PgCursor(conn.cursor(), conn)
        try: yield cursor
...
def init_db():
    with get_db_cursor() as cursor:
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            last_mining_collect BIGINT DEFAULT 0,
            ...
            business_start_time BIGINT DEFAULT 0,
...
''')
        ...
        for _sql in [
            'ALTER TABLE checks ADD COLUMN target_username TEXT',
            'ALTER TABLE users ADD COLUMN experience INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN last_private_activity BIGINT DEFAULT 0',
            'ALTER TABLE users ADD COLUMN custom_name TEXT',
            'ALTER TABLE users ADD COLUMN video_cards INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN deposit INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN last_mining_collect BIGINT DEFAULT 0',
            'ALTER TABLE users ADD COLUMN click_streak INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN total_clicks INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN bank_deposit INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN daily_streak INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN last_daily_bonus BIGINT DEFAULT 0',
            'ALTER TABLE users ADD COLUMN last_interest_calc BIGINT DEFAULT 0',
            'ALTER TABLE users ADD COLUMN business_id INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN business_progress INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN business_start_time BIGINT DEFAULT 0',
            'ALTER TABLE users ADD COLUMN business_raw_materials INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN clan_id INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN games_won INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN games_lost INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN total_won_amount INTEGER DEFAULT 0',
            'ALTER TABLE users ADD COLUMN total_lost_amount INTEGER DEFAULT 0',
            'ALTER TABLE clothes_shop ADD COLUMN supply INTEGER DEFAULT -1',
            'ALTER TABLE clothes_shop ADD COLUMN sold_count INTEGER DEFAULT 0',
        ]:
            cursor.execute_ignore(_sql)
        ...
        cursor.execute('INSERT INTO lottery (id, jackpot) VALUES (1, 0) ON CONFLICT DO NOTHING')
        ...
        ''')
        cursor.execute_ignore('ALTER TABLE discounts ADD COLUMN auto_given INTEGER DEFAULT 0')
        ...
        cursor.executemany(
            "INSERT INTO donate_packages (key, stars, amount, emoji) VALUES (%s,%s,%s,%s) ON CONFLICT DO NOTHING",
            defaults
        )
...
def init_dice_tables():
    try:
        with get_db_cursor() as cursor:
            cursor.execute("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename=%s", ('dice_challenges',))
            ...
            if not table_exists:
                ...
                # Get existing columns from pg
                cursor.execute("""
                    SELECT column_name FROM information_schema.columns
                    WHERE table_schema='public' AND table_name='dice_challenges'
                """)
                ...
                for column in required_columns:
                    if column not in columns:
                        if column in ['challenger_id', 'target_id', 'bet_amount', 'chat_id', 'message_id']:
                            cursor.execute_ignore(f'ALTER TABLE dice_challenges ADD COLUMN {column} INTEGER')
                        elif column == 'expires_at':
                            cursor.execute_ignore(f'ALTER TABLE dice_challenges ADD COLUMN {column} TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP')
                        else:
                            cursor.execute_ignore(f'ALTER TABLE dice_challenges ADD COLUMN {column} TIMESTAMP DEFAULT CURRENT_TIMESTAMP')
                        print(f"🏆 Добавлена колонка {column} в dice_challenges")
            
            cursor.execute("SELECT indexname FROM pg_indexes WHERE schemaname='public' AND indexname=%s", ('idx_dice_challenges_expires',))
...
def cleanup_expired_challenges():
    try:
        with get_db_cursor() as cursor:
            ...
            columns = ['expires_at']  # pg: column always exists
            
            if 'expires_at' not in columns:
                print("⚠️ Колонка expires_at не существует, пропускаем очистку")
                return
                
            cursor.execute('DELETE FROM dice_challenges WHERE expires_at < NOW()')
...
def add_warn(user_id, reason, warned_by, duration_hours=24):
    ...
    with get_db_cursor() as cursor:
        ...
        cursor.execute('''
            INSERT INTO warns (user_id, reason, warned_by, expires_at, is_active)
            VALUES (?, ?, ?, ?, 1)
...
def init_taxi_database():
    with get_db_cursor() as cursor:
        ...
        cursor.execute_ignore("ALTER TABLE driver_stats ADD COLUMN driver_class TEXT DEFAULT 'economy'")
        cursor.execute_ignore("ALTER TABLE driver_stats ADD COLUMN unlocked_classes TEXT DEFAULT 'economy'")
...
def get_or_create_user(user_id, username, first_name):
    with get_db_cursor() as cursor:
        ...
        if not user:
            ...
            cursor.execute(
                'INSERT INTO users (user_id, username, first_name, balance, referral_code, video_cards, deposit, last_mining_collect, click_streak, total_clicks, bank_deposit, daily_streak, last_daily_bonus, last_interest_calc, business_id, business_progress, business_start_time, business_raw_materials, clan_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (user_id) DO NOTHING',
                (user_id, username, first_name, 0, referral_code, 0, 0, 0, 0, 0, 0, 0, 0, int(time.time()), 0, 0, 0, 0, 0)
            )
            # Повторно читаем, чтобы вернуть созданного пользователя
            cursor.execute('SELECT * FROM users WHERE user_id = ?', (user_id,))
            user = cursor.fetchone()
...
def handle_db_stats(message):
    ...
    try:
        ...
        stats_text = (
            f"📊 <b>ИНФОРМАЦИЯ О БАЗЕ ДАННЫХ (PostgreSQL)</b>\n\n"
            ...
            f"<b>Команды:</b>\n"
            f"• <code>база</code> - Скачать резервную копию (SQL)"
        )
...
def handle_document_upload(message):
    ...
    try:
        ...
        if file_ext == 'db':
            ...
            try:
                test_conn = psycopg2.connect(dsn=_PG_URL, sslmode="require")
...
def finish_clothes_creation(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            item_id = (cursor.fetchone() or {}).get("id")
...
import threading
# Глобальный словарь активных дуэлей — инициализируем здесь, чтобы поток очистки не падал
if 'active_duels' not in dir():
    active_duels = {}
...
def create_promo(code, reward, max_uses, expires_at, created_by):
    with get_db_cursor() as cursor:
        cursor.execute('''
            INSERT INTO promo_codes (code, reward, max_uses, current_uses, expires_at, created_by, created_at, is_active)
            VALUES (?, ?, ?, 0, ?, ?, ?, 1)
...
def _get_user_context(user_id):
    ...
    try:
        with get_db_cursor() as cur:
            ...
            # История игр
            cur.execute("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='game_history'")
            if cur.fetchone():
                cur.execute("""
                    SELECT column_name FROM information_schema.columns
            ...
            cur.execute("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='transfers'")
            if cur.fetchone():
                cur.execute("""
                    SELECT column_name FROM information_schema.columns
            ...
            # Акции игрока
            cur.execute("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='stock_portfolio'")
            if cur.fetchone():
                try:
...
STOCK_BUY_IMPACT  = 0.0005        # покупка 1 акции = +0.05% к цене
STOCK_SELL_IMPACT = 0.0005        # продажа 1 акции = -0.05% к цене
STOCK_COOLDOWN    = 10 * 60       # 10 минут между сделками
STOCK_UPDATE_SEC  = 15 * 60       # drift каждые 15 минут
STOCK_HISTORY_LEN = 96            # хранить 96 точек (~24ч)
STOCK_DRIFT_MAX   = 0.10          # макс случайный drift ±10% за период
...
def _init_stocks():
    with get_db_cursor() as cur:
        ...
        ''')
        # Миграция: добавляем prev_price если таблица уже существовала без неё
        cur.execute_ignore("ALTER TABLE stocks ADD COLUMN prev_price INTEGER NOT NULL DEFAULT 0")
        cur.execute('''
        ...
        ''')
        cur.execute(
            "INSERT INTO stocks (ticker,name,price,prev_price,updated_at) VALUES (%s,%s,%s,%s,%s) ON CONFLICT DO NOTHING",
...
def _migrate_stocks():
    try:
        with get_db_cursor() as cur:
            # Добавляем колонки если не существуют (PostgreSQL-safe)
            cur.execute_ignore("ALTER TABLE stock_portfolio ADD COLUMN last_trade INTEGER NOT NULL DEFAULT 0")
            cur.execute_ignore("ALTER TABLE stocks ADD COLUMN prev_price INTEGER NOT NULL DEFAULT 0")
            # Перенос кулдаунов из старой таблицы если она есть
            cur.execute("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='stock_cooldowns'")
            if cur.fetchone():
                cur.execute("""
...
def _market_update():
    """
    Обновляет цену на основе реального спроса и предложения за последние 15 минут.
    Если торгов не было — мягкий mean reversion к скользящему среднему (±0.5%).
    """
    ...
    if abs(pressure) < 0.01:
        # Нет торгов — цена плавно тянется к среднему за 24ч (mean reversion)
        history = _stock_history(limit=96)
        if len(history) >= 2:
            avg_price = sum(p for p, _ in history) / len(history)
            reversion = (avg_price - price) / price
            # Ограничиваем reversion: не более ±0.5% за период
            reversion = max(-0.005, min(0.005, reversion * 0.1))
            new_price = _set_stock_price(price * (1 + reversion))
            label = f"mean reversion ({reversion*100:+.3f}%)"
        else:
            new_price = price
            label = "нет данных"
    else:
        # Есть торги — двигаем цену по давлению покупателей/продавцов
        # pressure от -1 до +1, умножаем на STOCK_DRIFT_MAX для масштаба
        impact = pressure * STOCK_DRIFT_MAX
        # Добавляем лёгкий шум ±0.3% чтобы рынок не был идеально предсказуемым
        noise = (random.random() - 0.5) * 0.006
        impact = max(-STOCK_DRIFT_MAX, min(STOCK_DRIFT_MAX, impact + noise))
        new_price = _set_stock_price(price * (1 + impact))
        label = f"pressure={pressure:+.2f}, impact={impact*100:+.2f}%"
...
def _apply_trade_impact(qty, is_buy):
    """
    Моментальное влияние сделки на цену — пропорционально объёму.
    Ступенчатое: первые 100 акций — полный impact, дальше затухает.
    """
    price, _ = _get_stock_price()
    impact = qty * STOCK_BUY_IMPACT if is_buy else -qty * STOCK_SELL_IMPACT
    new_price = _set_stock_price(price * (1 + impact))
    return new_price
...
def _log_stock_trade(user_id, action, qty, price, total, new_price):
    ...
    try:
        ...
        text = (
            f"{icon} <b>{name}</b>{username} {action_text} <b>{qty} {STOCK_TICKER}</b>\n"
        ...
        # Стикер при покупке (любой)
        if action == "buy":
            try:
                bot.send_sticker(STOCK_LOG_CHANNEL, "CAACAgIAAxkBAAEBmZ9nqKQ2vHQ3AAFjYBjlFLxJYCcJ3gACAgADwDZPE_oRUCTn5YQSNAQ")
            except Exception:
                pass
...
def handle_buy_stock(message):
    ...
    if qty <= 0 or qty > 5000:
        bot.send_message(message.chat.id, "❌ От 1 до 5000 акций за раз.", parse_mode="HTML")
        return
    ...
    if balance < total_cost:
        bot.send_message(message.chat.id,
            ...
            f"Есть: {format_balance(balance)}", parse_mode="HTML")
        return

    # Атомарная операция: списываем деньги
    update_balance(user_id, -total_cost)

    # Обновляем портфель и кулдаун
    with get_db_cursor() as cur:
        ...
        cur.execute(
            ...
            (user_id, STOCK_TICKER, qty, new_avg, int(time.time()), qty, new_avg, int(time.time()))
        )
        cur.execute(
...
def handle_sell_stock(message):
    ...
    # Атомарная операция: начисляем деньги
    ...
    with get_db_cursor() as cur:
        ...
        if new_amount == 0:
            ...
            cur.execute(
                ...
                (new_amount, int(time.time()), user_id, STOCK_TICKER)
            )
        cur.execute(
            "INSERT INTO stock_trades (user_id,ticker,action,amount,price,fee,total,created_at) VALUES (?,?,?,?,?,?,?,?)",
