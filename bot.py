...
ВАЖНЫЕ ЛЮДИ:
— Кари / Cary (@Cary_Python) — разраб бота, шарит за всё техническое
— Ево / Evo — владелец, главный
...
def _get_user_context(user_id):
    """Получить данные игрока из БД для контекста."""
    try:
        with get_db_cursor() as cur:
            cur.execute(
        ...
        name = custom_name or first_name or f"@{username}" or str(user_id)
        premium = is_premium(user_id)
        admin = is_admin(user_id)

        lines = [
            f"\n--- ДАННЫЕ ИГРОКА КОТОРЫЙ ПИШЕТ ---",
            f"Ник: {name}" + (" (@" + username + ")" if username else ""),
            f"Баланс: {balance or 0:,} 🌸".replace(",", " "),
            f"Банк: {bank_deposit or 0:,} 🌸".replace(",", " "),
            f"Видеокарты (майнинг): {video_cards or 0} шт.",
