...
@bot.message_handler(func=lambda m: m.text and re.match(r"^бонусвсем\s+\d+", m.text.lower()) and is_admin(m.from_user.id))
def handle_admin_bonus_all(message):
    """бонусвсем <монет> — начислить всем пользователям указанную сумму"""
    try:
        match = re.match(r"^бонусвсем\s+(\d+)", message.text.strip(), re.IGNORECASE)
        if not match:
            bot.reply_to(message, "❌ Формат: <code>бонусвсем 20</code>", parse_mode="HTML")
            return
        ...
        for uid in user_ids:
            try:
                bot.send_message(
                    ...
                    f"Тебе начислено: <b>+{format_balance(amount)}</b>🌸\n\n"
                    f"Приятной игры! 🎮",
                    parse_mode="HTML"
                )
                notified += 1
...
# ══════════════════════════════════════════════════════
# ПРЯМЫЕ SQL-ЗАПРОСЫ (только для админов)
# Использование:
#   sql SELECT * FROM users LIMIT 5
#   sql UPDATE users SET balance = balance + 500 WHERE user_id = 123
#   tables          — список всех таблиц
#   tables users    — колонки таблицы users
# ══════════════════════════════════════════════════════

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower().startswith("sql ") and is_admin(m.from_user.id))
def handle_raw_sql(message):
    query = message.text.strip()[4:].strip()
    if not query:
        bot.reply_to(message, "❌ Пустой запрос", parse_mode="HTML")
        return
    try:
        with get_db_cursor() as cursor:
        ...
        if rows:
            ...
            bot.reply_to(message, "✅ Выполнено успешно", parse_mode="HTML")
    except Exception as e:
        bot.reply_to(message, f"❌ Ошибка:\n<code>{e}</code>", parse_mode="HTML")


@bot.message_handler(func=lambda m: m.text and m.text.strip().lower().startswith("tables") and is_admin(m.from_user.id))
def handle_tables(message):
    parts = message.text.strip().split()
    try:
        with get_db_cursor() as cursor:
            if len(parts) == 1:
                ...
                text = "📋 <b>Таблицы в БД:</b>\n\n" + "\n".join(f"• <code>{t}</code>" for t in tables)
                text += "\n\n💡 <code>tables имя_таблицы</code> — колонки таблицы"
            else:
                ...
                if not cols:
                    bot.reply_to(message, f"❌ Таблица <code>{table_name}</code> не найдена", parse_mode="HTML")
                    return
                lines = ["cid | name | type | notnull | default | pk", "─" * 45]
                for col in cols:
                    lines.append(" | ".join(str(v) for v in col))
                text = f"📋 <b>Таблица {table_name}:</b>\n\n<pre>" + "\n".join(lines) + "</pre>"
...
def cb_event_join(call):
    ...
    # Уведомление в чат только на ключевых отметках
    NOTIFY_AT = {3, 5, 8}
    if count in NOTIFY_AT:
        left = EVENT_REQUIRED - count
        try:
            _tg_api(
                "sendMessage",
                chat_id=EVENT_CHAT_ID,
                text=(
                    f"⚡ <b>Сбор идёт!</b> Уже {count}/{EVENT_REQUIRED} участников\n"
                    f"Не хватает ещё <b>{left}</b> — жми кнопку выше и участвуй!"
                ),
                parse_mode="HTML"
            )
        except Exception:
...
if __name__ == "__main__":
    ...
    try:
        ...
        bot.infinity_polling(num_threads=8, timeout=20, long_polling_timeout=20)
    except Exception as e:
        print(f"Критическая ошибка: {e}")
