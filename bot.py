...
def _eq_set_with_durability(user_id, slot, set_id):
    dur = DURABILITY.get(set_id, 60)
    with get_db_cursor() as cur:
        cur.execute(
            "INSERT INTO equipment (user_id, slot, set_id, durability) VALUES (?,?,?,?)"
            " ON CONFLICT(user_id, slot) DO UPDATE SET set_id=excluded.set_id,"
            " durability=CASE WHEN equipment.set_id=excluded.set_id THEN equipment.durability ELSE excluded.durability END",
            (user_id, slot, set_id, dur)
        )
...
def cb_eq_equip(call):
    ...
    if old_equipped.get(slot):
        _inv_add_eq(uid, old_equipped[slot], slot, 1)
...
def cb_pvp_action(call):
    ...
    if action_type == "guard":
        ...
        event = f"⚔️ <b>{actor_name}</b> → {bp_info['emoji']} {bp_info['name']} · <b>−{reduced}</b>{block_note}{crit_note}"
...
def _arena_queue_ticker(uid, started_at):
    while True:
        ...
        if elapsed >= ARENA_QUEUE_TIMEOUT:
            _arena_leave_queue(uid)
            try:
                _tg_api("sendMessage", chat_id=uid, parse_mode="HTML",
                        text="⏰ <b>Поиск отменён</b> — соперник не найден за 2 минуты.")
            except Exception: pass
            return
