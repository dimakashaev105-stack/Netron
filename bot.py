...
def send_discount_message(user_id, percent=50):
    ...
    try:
        ...
        kb = {
            "inline_keyboard": [[
                {
                    "text": "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Использовать скидку",
...
def _eq_main_kb(user_id):
    ...
    for slot in slot_order:
        ...
        emoji_prefix = slot_emojis[slot] if slot_emojis[slot] else fb
        if set_id:
            s = EQUIPMENT_SETS[set_id]
            btn_text = f"✅ {emoji_prefix} {slot_names[slot]}"
        else:
            btn_text = f"◻️ {emoji_prefix} {slot_names[slot]}"
        row.append({
            "text": btn_text,
...
def cb_craft_set(call):
    ...
    for slot, cost in CRAFT_COST.items():
        ...
        slot_fb = {"helmet":"🪖","chestplate":"🦺","leggings":"👖","boots":"👟","sword":"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"}.get(slot,"⚔️")
