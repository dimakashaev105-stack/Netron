...
def _mine_cd_left(user_id, cooldown=None):
    if cooldown is None:
        cooldown = MINE_COOLDOWN_PRIVATE if _mine_rental(user_id) else MINE_COOLDOWN
    with get_db_cursor() as cur:
...
def handle_mine_dig_text(message):
    ...
    if event["mult"] == 0.0:
        ...
        text_html  = f"⛏️ {where}\n{res_emoji} · +{format_balance(reward)} 🌸{streak_tag}"
...
def cb_mine_list(call):
    ...
    for row in rows:
        ...
        btn = f"🏔 #{mid} {lv_info['label']} x{lv_info['bonus']} · {plain_balance(rent_price)} · {name}"
