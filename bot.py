...
def cb_craft_item(call):
    ...
    # Обновляем меню крафта — подменяем call.data на правильный формат
    call.data = f"craft_set_{set_id}"
    cb_craft_set(call)
...
def _pvp_active_for(user_id):
    """Вернуть активный бой игрока (attacker или defender), или None."""
    with get_db_cursor() as cur:
        cur.execute("""
...
def _pvp_action_kb(battle_id, user_id):
    return {"inline_keyboard": [[
        {"text": "⚔️  Атаковать",  "callback_data": f"pvp_action_{battle_id}_{user_id}_attack"},
        {"text": "🛡️  Защититься", "callback_data": f"pvp_action_{battle_id}_{user_id}_defend"},
    ]]}
...
def handle_pvp_challenge(message):
    ...
    # Парсим: "пвп @юзер 5000" или "пвп @юзер"
    parts = message.text.strip().split()
    if len(parts) < 2:
        bot.send_message(message.chat.id,
    ...
    if not target:
        bot.send_message(message.chat.id, "❌ Укажи игрока через @username или ответь на его сообщение", parse_mode="HTML")
        return
    ...
    existing_t = _pvp_active_for(target_id)
    if existing_t:
    ...
    bet = 0
    for part in parts[1:]:
        if part.isdigit() or (part[:-1].isdigit() and part[-1].lower() in "кмb"):
            try:
    ...
    if bet > 0:
        bal = get_balance(uid)
        if bal < bet:
            bot.send_message(message.chat.id, f"❌ Недостаточно монет! Нужно {format_balance(bet)}", parse_mode="HTML")
            return
    ...
    result = _tg_api("sendMessage", chat_id=message.chat.id,
                     text=text, reply_markup=kb, parse_mode="HTML")
    if result and result.get("ok"):
        msg_id = result["result"]["message_id"]
        _pvp_set_msg(battle_id, msg_id)
    ...
    def _cancel_pending():
        ...
        if b and b["status"] == "pending":
            ...
            _tg_api("editMessageText",
                    ...
                    text=f"⚔️ Вызов от <b>{a_name}</b> истёк (60 сек)",
                    parse_mode="HTML")
...
def cb_pvp_accept(call):
    ...
    # Проверить баланс защищающегося
    if b["bet"] > 0:
        bal = get_balance(uid)
        if bal < b["bet"]:
...
def cb_pvp_decline(call):
    ...
    _tg_api("editMessageText",
            chat_id=b["chat_id"],
            message_id=b["message_id"],
            text=f"❌ <b>{d_name}</b> отклонил вызов от <b>{a_name}</b>",
            parse_mode="HTML")
...
def cb_pvp_action(call):
    ...
    try:
        _, _, bid, expected_uid, action = call.data.split("_", 4)
        battle_id = int(bid)
    ...
    if action == "defend":
        # Ставим флаг блока текущему игроку
        if is_attacker:
        ...
        actor_name = _mkt_user_name_by_id(uid)
        opp_name = _mkt_user_name_by_id(opponent_id)
    ...
    next_turn = b["whose_turn"]
    kb = _pvp_action_kb(battle_id, next_turn)
    _tg_api("editMessageText",
            chat_id=b["chat_id"],
            message_id=b["message_id"],
            text=text, reply_markup=kb, parse_mode="HTML")
...
def _pvp_finish(battle_id, b, last_event=""):
    ...
    if b["bet"] > 0:
        ...
        prize_note = f"\n💰 <b>+{format_balance(prize)}</b> в кошелёк победителя!"
    ...
    text = (
        ...
        f"🔴 HP: <b>{a_hp}</b>/100  •  🔵 HP: <b>{d_hp}</b>/100\n"
        f"{prize_note}\n"
        f"┌─ Финальный удар\n│  {last_event}\n└──────────────\n"
...
def _pvp_start_timeout(battle_id):
    def _timeout():
        ...
        if not b or b["status"] != "active":
            return
        now = int(time.time())
        if now - b["last_action"] < PVP_TURN_TIMEOUT - 2:
            return
        # Проигрывает тот чей ход
