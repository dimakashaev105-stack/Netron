...
def _get_momentum():
    ...
    momentum = streak * 0.007
    return max(-0.03, min(0.03, momentum))

def _market_update():
    """
    Реалистичная рыночная модель через AMM-пул:
    1. Торговое давление покупателей/продавцов → сдвиг цены
    2. Случайный шум с динамической волатильностью
    3. Momentum — усиление тренда
    4. Mean reversion — возврат к среднему
    5. Upward bias — компания слегка растёт в спокойные периоды

    Все изменения применяются к пулу ликвидности x*y=k.
    """
    shares_r, money_r = _get_pool()
    price = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE
    k = shares_r * money_r
    ...
    # Применяем импакт к пулу: меняем money_reserve
    new_price = int(price * (1 + total_impact))
    new_price = max(STOCK_MIN_PRICE, min(STOCK_MAX_PRICE, new_price))
    # Сохраняем k, пересчитываем reserves: price = money/shares, k = money*shares
    # => money = sqrt(k * price), shares = sqrt(k / price)
    import math
    if new_price > 0 and k > 0:
        new_money = int(math.sqrt(k * new_price))
        new_shares = int(math.sqrt(k / new_price)) if new_price > 0 else shares_r
        # Обновляем через _set_pool
        new_price = _set_pool(new_shares, new_money)
    else:
        new_price = price
...
def _apply_trade_impact(qty, is_buy):
    """
    AMM-модель: реальное влияние через пул ликвидности x*y=k.
    Цена = money_reserve / shares_reserve.
    При покупке: money↑ shares↓ → цена↑
    При продаже: shares↑ money↓ → цена↓
    Slippage пропорционален объёму относительно пула.
    """
    shares_r, money_r = _get_pool()
    k = shares_r * money_r
    price_before = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE
    ...
    if is_buy:
        # Имитация: пользователь покупает qty акций по текущей цене
        # В AMM для покупки qty акций нужно внести деньги
        new_shares = shares_r - qty
        new_money = k // new_shares if new_shares > 0 else money_r
        money_in = new_money - money_r
        # Добавляем 1% slippage
        money_in = int(money_in * 1.01)
        new_money = money_r + money_in
        new_shares = k // new_money
    else:
        # Продажа qty акций
        ...
        new_money = money_r - money_out
        new_shares = k // new_money if new_money > 0 else shares_r

    new_price = _set_pool(new_shares, new_money)
    change_pct = (new_price - price_before) / price_before * 100 if price_before else 0
    print(f"[stocks] AMM trade: qty={qty} {'BUY' if is_buy else 'SELL'} "
          f"pool={shares_r:,}/{money_r:,} → "
          f"{new_shares:,}/{new_money:,} "
          f"price={price_before:,} → {new_price:,} ({change_pct:+.2f}%)")
    return new_price
