...
def handle_successful_payment(message):
    try:
        ...
        elif payload_parts[0] == "mine_boost" and len(payload_parts) >= 2:
            target_uid = int(payload_parts[1])
...
PICKAXE_LEVELS = {
    1: {"stars": 0,  "bonus": 1.0, "cooldown": 5*60,  "label": "🪨 Базовая"},
    2: {"stars": 10, "bonus": 1.3, "cooldown": 4*60,  "label": "⚙️ Улучшенная"},
    3: {"stars": 20, "bonus": 1.6, "cooldown": 3*60,  "label": "🔩 Прочная"},
    4: {"stars": 40, "bonus": 1.8, "cooldown": 2*60,  "label": "💎 Алмазная"},
    5: {"stars": 80, "bonus": 2.0, "cooldown": 1*60,  "label": "⭐ Легендарная"},
}
PICKAXE_MAX = 5
...
def cb_mine_dig(call):
    ...
    rental = _mine_rental(uid)
    pk_lvl = _pickaxe_get(uid)
    pk_bonus = PICKAXE_LEVELS[pk_lvl]["bonus"]
    if rental:
        mine  = _mine_by_id(rental[0])
        bonus = (MINE_LEVELS[mine['level']]['bonus'] if mine else 1.0) * pk_bonus
        where = f"шахта #{rental[0]}"
