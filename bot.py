    ...
    try:
        ...
        if result["success"]:
            text = (
                ...
                "<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> В казну: <b>" + format_balance(result["total_collected"]) + " 🌸</b>\n"
...
• продать акции [кол-во] — продать акции (комиссия 2%)
• история акций — история цен и твои сделки
• Без ограничений на кол-во, кулдаун 1 мин между сделками
...
STOCK_MIN_PRICE   = 100
STOCK_MAX_PRICE   = 500_000_000
STOCK_MAX_PER_USER  = 0          # 0 = без ограничений
STOCK_MAX_PER_TRADE = 0          # 0 = без ограничений
STOCK_SELL_FEE    = 0.02         # 2% комиссия
STOCK_PROFIT_TAX  = 0.10         # 10% налог на прибыль
STOCK_COOLDOWN     = 60          # 1 минута между сделками
STOCK_UPDATE_SEC  = 5 * 60       # обновление цены каждые 5 минут
STOCK_HISTORY_LEN = 288          # храним 24 часа истории (288 * 5 мин)

STOCK_LIQUIDITY   = 500          # влияние сделок на цену

# Волатильность — настоящие рыночные движения
STOCK_VOL_CALM    = 0.012        # спокойный рынок ±1.2%
STOCK_VOL_NORMAL  = 0.025        # нормальный ±2.5%
STOCK_VOL_HIGH    = 0.055        # горячий ±5.5%
STOCK_VOL_CRISIS  = 0.12         # кризис/памп ±12%
STOCK_DRIFT_BASE  = 0.0008       # слабый upward bias

# Рыночные события
STOCK_EVENT_CHANCE  = 0.18       # 18% шанс события при каждом тике
STOCK_EVENTS = [
    # (вес, название, мин%, макс%)
    (12, "📰 Позитивные новости",           +0.04, +0.12),
    (12, "📰 Негативные новости",           -0.12, -0.04),
    (8,  "🚀 Памп! Крупный покупатель",     +0.10, +0.25),
    (8,  "📉 Дамп! Крупный продавец",       -0.25, -0.10),
    (6,  "💎 Партнёрство объявлено",        +0.15, +0.30),
    (6,  "⚠️ Скандал в компании",           -0.30, -0.15),
    (4,  "🔥 Памп недели!",                 +0.25, +0.50),
    (4,  "💀 Крах! Паника продавцов",       -0.50, -0.25),
    (3,  "🌊 Волна инвестиций",             +0.20, +0.40),
    (3,  "❄️ Заморозка торгов снята",       -0.15, +0.15),
    (2,  "🏆 Рекордная прибыль квартала",   +0.35, +0.60),
    (2,  "💸 Утечка капитала",              -0.45, -0.20),
]
...
def _get_market_volatility():
    """
    Динамическая волатильность по реальным движениям рынка.
    """
...
def _pick_market_event():
    """Выбирает случайное рыночное событие по весам."""
    total_weight = sum(w for w, *_ in STOCK_EVENTS)
    r = random.uniform(0, total_weight)
    acc = 0
    for w, name, lo, hi in STOCK_EVENTS:
        acc += w
        if r <= acc:
            return name, lo, hi
    return STOCK_EVENTS[0][1], STOCK_EVENTS[0][2], STOCK_EVENTS[0][3]

def _market_update():
    """
    Реалистичная рыночная модель с событиями:
    1. Торговое давление покупателей/продавцов
    2. Случайный шум с динамической волатильностью
    3. Momentum — усиление тренда
    4. Mean reversion — коррекция при перекупленности/перепроданности
    5. Случайные рыночные события (памп, дамп, новости)
    """
    ...
    trade_impact = pressure * 0.06

    noise = random.gauss(0, vol)

    mom_impact = momentum * 0.6
    ...
    if len(history) >= 6:
        ...
        if avg_price > 0:
            ...
            if abs(deviation) > 0.40:
                reversion = -deviation * 0.12

    bias = STOCK_DRIFT_BASE if abs(pressure) < 0.15 else 0.0

    # Рыночное событие
    event_name = None
    event_impact = 0.0
    if random.random() < STOCK_EVENT_CHANCE:
        event_name, lo, hi = _pick_market_event()
        event_impact = random.uniform(lo, hi)

    total_impact = trade_impact + noise + mom_impact + reversion + bias + event_impact
    ...
    label = (
        ...
        + (f" EVENT={event_name}({event_impact*100:+.1f}%)" if event_name else "")
    )
    print(f"[stocks] market update: {price:,} → {new_price:,} ({label})")
    return price, new_price, event_name, event_impact
...
def _apply_trade_impact(qty, is_buy):
    ...
    крупные объёмы ощутимо меняют цену.

    Примеры при STOCK_LIQUIDITY=500:
      10  акций →  ~0.3%
      50  акций →  ~1.2%
     100  акций →  ~2.2%
     500  акций →  ~5.0%
   ...
   10000  акций → ~18.0%
    ...
    import math
    L = STOCK_LIQUIDITY
    REF_QTY = 5000        # референсный объём для калибровки
    MAX_IMPACT = 0.15     # максимум разового влияния ~15%

    impact_pct = (math.log(1 + qty / L) / math.log(1 + REF_QTY / L)) * MAX_IMPACT
    impact_pct = min(impact_pct, 0.30)   # жёсткий кап 30%

    slippage = random.gauss(0, 0.002)
...
def handle_stock_market(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"💸 Комиссия продажи: {int(STOCK_SELL_FEE*100)}% + 10% налог с прибыли\n"
        f"⏱ Обновление цены: каждые {STOCK_UPDATE_SEC//60} мин\n"
        f"{portfolio_line}{cd_line}\n\n"
        f"<code>купить акции 100</code>\n"
        f"<code>продать акции 100</code>\n"
...
def _stock_price_scheduler():
    ...
    while True:
        ...
        try:
            old_price, new_price, event_name, event_impact = _market_update()
            change_pct = (new_price - old_price) / old_price * 100
            _log_stock_price_update(old_price, new_price, reason="market")

            # Уведомление при значительном движении (>= 2%) или событии
            if event_name and abs(event_impact) >= 0.04:
                arrow = "📈" if new_price > old_price else "📉"
                send_alert(
                    f"{event_name}\n"
                    f"{arrow} {STOCK_TICKER}: "
