        ...
        if payload_parts[0] == "premium" and len(payload_parts) >= 3:
            ...
            if target_lvl == cur_lvl + 1 and target_lvl <= PICKAXE_MAX:
                ...
                bot.send_message(
                    ...
                    f"{info['label']}  ·  x{info['bonus']}  ·  КД {info['cooldown']//60}м",
                    parse_mode="HTML"
                )
            else:
...
def handle_mine_dig_text(message):
    ...
    try:
        ...
        print(f"[mine_dig_text] HTML send failed: {e}")
        # fallback без кастомных эмодзи
        fb = resource.get('fallback', '⬛')
        qty_tag = f" ×{total_qty}" if total_qty > 1 else ""
        st_tag  = f" <tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>×{new_streak}" if new_streak >= 10 else ""
        fb_text = f"⛏️ {where}\n{fb} {resource['name']}{qty_tag}  +{_fmt_num(reward // 2)} 🌸{st_tag}"
        bot.send_message(message.chat.id, fb_text)
