...
def _build_donate_text():
    ...
    for key, pkg in DONATE_PACKAGES.items():
        ...
        lines.append(f"{pkg['emoji']} {pkg['stars']} зв — {amt_str}")
    return "\n".join(lines)
    ...
    for i in range(0, len(keys), 2):
        ...
        for k in row_keys:
            ...
            buttons.append(InlineKeyboardButton(
                f"{pkg['emoji']} {pkg['stars']} зв — {amt_str}",
                callback_data=k
            ))
        markup.row(*buttons)
    return markup
...
def handle_buy(message):
    ...
    bot.send_message(
        message.chat.id,
        "🛒 <b>Магазин валюты</b>\n\n"
        f"<blockquote>{_build_donate_text()}</blockquote>\n\n"
...
@bot.message_handler(func=lambda message: message.text in ["📈 Биржа", "Биржа", "биржа"])
...
GROQ_API_KEY = "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"
GROQ_URL = "https://api.groq.com/openai/v1/chat/completions"
_GEMINI_COOLDOWNS = {}
GEMINI_COOLDOWN_SEC = 60

_GEMINI_SYSTEM = """Ты — ИИ-помощник и друг игрового Telegram бота Fectiz 🌸.
Ты общаешься с игроками в чате. Можешь отвечать на вопросы про бот, шутить, болтать, поддерживать разговор — веди себя как живой, дружелюбный собеседник.
Отвечай кратко и по-русски. Не будь занудой.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎮 ЧТО УМЕЕТ БОТ FECTIZ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Валюта бота — 🌸 (сакура, или просто "цветочки").

💰 ЭКОНОМИКА
• Баланс: команда "баланс" или /profile
• Переводы: "перевести [сумма] @юзер" — комиссия 10%
• Банк: хранить деньги (без процентов, просто хранение)
• Срочные вклады: "вклад [сумма] [часы]"
  - до 6ч = 0.30%/час, до 24ч = 0.25%/час, до 72ч = 0.15%/час, до 120ч = 0.10%/час
  - максимум 50 млн 🌸, срок до 5 дней
  - получить: "вклад получить", инфо: "вклад инфо"
• Кредиты: максимум 100 000 🌸, ставка 10%, срок до 3 дней, штраф 20%
• Чеки: создать чек на сумму — "чек [сумма]", активируется промокодом

💼 РАБОТА
• Такси: команда "такси" — возишь пассажиров, зарабатываешь 🌸
• Майнинг: покупаешь видеокарты, они добывают 🌸 пассивно
  - команды: "майнинг", "купить карту", "майнить"
• Кликер: "кликнуть" — кликаешь и зарабатываешь

🎰 ИГРЫ (всё ставки на 🌸)
• Слоты: "слоты [ставка]"
• Рулетка: "рулетка [ставка] [цвет/число]" или "рул [ставка]"
• Монетка: "монетка [ставка] [орёл/решка]"
• Краш: "краш [ставка]" — множитель растёт, жмёшь стоп вовремя
• Кости: "кости [ставка]" — бросок кубиков против бота
• Блэкджек: "блэкджек [ставка]" — 21
• Хайкард: "хайкард [ставка]" — у кого карта старше
• Дуэль: "дуэль [ставка] @игрок" — вызов другого игрока
• Дартс: "дартс [ставка]"
• КНБ (камень-ножницы-бумага): "кнб [ставка]"
• Мины: "мины [ставка]"

📈 АКЦИИ
• Тикер: FECTZ (Fectiz Bot)
• Купить: "купить акции [кол-во]"
• Продать: "продать акции [кол-во]"
• История: "история акций"
• Цена обновляется каждые 15 минут, макс изменение ±15%
• Комиссия при продаже 3%, кулдаун между сделками 10 минут
• Максимум 100 акций на игрока

🛍️ МАГАЗИН И ВЕЩИ
• Магазин одежды: "магазин"
• Инвентарь: "гардероб" или "вещи"
• Надеть/снять вещи, менять образ

👑 PREMIUM
• Команда: /premium
• Даёт доступ к ИИ-помощнику (гугл), бонусы к вкладам +20%
• Пакеты: 7 дней, 30 дней, 90 дней за звёзды Telegram

💳 ДОНАТ (покупка 🌸 за звёзды Telegram)
• Команда: /buy
• Цены автоматически подстраиваются под экономику сервера раз в сутки
• Примерный курс: ~26 000 🌸 за 1 звезду

🏆 РЕЙТИНГИ И ПРОФИЛЬ
• Профиль: "профиль" или /profile
• Топ игроков: "топ"
• Статистика: "стат"

🎁 ПРОМОКОДЫ И СКИДКИ
• Активировать промокод: "промо [код]"
• Скидки на донат выдаются админами

🎪 СОБЫТИЯ И ДРОПЫ
• Био-дроп: иногда бот разыгрывает 🌸 среди активных
• Метеорит: случайное событие с наградой
• Ивенты: запускаются администрацией

⚔️ КЛАНЫ
• Создать/вступить в клан: "кланы"

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👮 КОМАНДЫ АДМИНИСТРАЦИИ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Выдать 🌸: "выдать [сумма] @юзер"
• Бан/разбан: "бан @юзер", "разбан @юзер"
• Варн: "варн @юзер [причина]"
• Рассылка: "рассылка [текст]"
• Собрать налог: "собрать налог" или /налог
• Налоги: >1млн = 10%, >100к = 5%, остальные = 3%
• Скидка на донат: "скидка [%] @юзер"
• Обновить цены доната: /обновитьдонат

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💬 КАК ОБЩАТЬСЯ
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Игроки пишут "гугл [вопрос]" — только для Premium пользователей.
Ты можешь: объяснять механики, советовать стратегии, шутить,
отвечать на общие вопросы, поддерживать разговор.
Важная информация о людях:
— Кари (Cary, @Cary_Python) — разработчик бота, уважай его
— Ево (Evo) — владелец бота, главный

Правила общения:
— Отвечай КРАТКО, максимум 3-4 предложения
— Если игрок шутит или грубит — отвечай в тон, с юмором и дерзостью
— Можно подкалывать в ответ, но без реальных оскорблений
— Не будь роботом, будь живым собеседником
— Матерные намёки можно обыгрывать с юмором (как в примере: написал "хуй" — ответь что-то остроумное про него)
— Если не знаешь — скажи честно и коротко
— Никогда не пиши простыни текста"""


def _ask_gemini(user_question: str) -> str:
    """Отправить вопрос в Groq и получить ответ."""
    try:
        resp = _requests.post(
            GROQ_URL,
            json={
...
def handle_gemini(message):
    ...
    if not is_premium(user_id) and not is_admin(user_id):
        bot.send_message(message.chat.id,
            ...
            "Эта функция доступна только игрокам с премиум подпиской.\n"
            "Купить: /premium",
            parse_mode="HTML")
        return
    ...
    if now - last < GEMINI_COOLDOWN_SEC and not is_admin(user_id):
        left = int(GEMINI_COOLDOWN_SEC - (now - last))
        bot.send_message(message.chat.id,
            "⏳ Подожди ещё <b>" + str(left) + " сек.</b> перед следующим вопросом.",
            parse_mode="HTML")
        return
    ...
    if not question:
        bot.send_message(message.chat.id,
            "❓ Напиши вопрос после слова <b>гугл</b>\n"
            "Пример: <code>гугл как работает майнинг</code>",
            parse_mode="HTML")
        return

    # Показываем что думаем
    thinking = bot.send_message(message.chat.id, "🤔 Думаю...", parse_mode="HTML")
...
STOCK_TICKER      = "FECTZ"
STOCK_NAME        = "Fectiz Bot"
STOCK_INIT_PRICE  = 10_000
STOCK_MAX_PER_USER = 100
STOCK_SELL_FEE    = 0.03        # 3% комиссия при продаже
STOCK_MAX_CHANGE  = 0.15        # макс ±15% за период
STOCK_UPDATE_SEC  = 15 * 60     # обновление цены каждые 15 минут
STOCK_COOLDOWN    = 10 * 60     # кулдаун между сделками 10 минут
...
def _init_stocks():
    with get_db_cursor() as cursor:
        # Таблица акций (одна строка на тикер)
        cursor.execute('''
...
def _get_stock_price(ticker=STOCK_TICKER):
    with get_db_cursor() as cursor:
        cursor.execute("SELECT price, prev_price FROM stocks WHERE ticker=?", (ticker,))
        row = cursor.fetchone()
        return (row[0], row[1]) if row else (STOCK_INIT_PRICE, STOCK_INIT_PRICE)

def _get_portfolio(user_id, ticker=STOCK_TICKER):
    with get_db_cursor() as cursor:
        cursor.execute(
            "SELECT amount, avg_price FROM stock_portfolio WHERE user_id=? AND ticker=?",
            (user_id, ticker)
        )
        row = cursor.fetchone()
...
def _get_cooldown_left(user_id):
    with get_db_cursor() as cursor:
        cursor.execute("SELECT last_trade FROM stock_cooldowns WHERE user_id=?", (user_id,))
        row = cursor.fetchone()
        if not row:
            return 0
        return max(0, row[0] + STOCK_COOLDOWN - int(time.time()))

def _set_cooldown(user_id):
    with get_db_cursor() as cursor:
        cursor.execute(
            "INSERT OR REPLACE INTO stock_cooldowns (user_id, last_trade) VALUES (?,?)",
            (user_id, int(time.time()))
        )

def _stock_history(ticker=STOCK_TICKER, limit=24):
    with get_db_cursor() as cursor:
        cursor.execute(
            "SELECT price, recorded_at FROM stock_history WHERE ticker=? ORDER BY recorded_at DESC LIMIT ?",
            (ticker, limit)
        )
        return cursor.fetchall()[::-1]  # от старых к новым

def _total_shares_in_market(ticker=STOCK_TICKER):
    with get_db_cursor() as cursor:
        cursor.execute(
            "SELECT COALESCE(SUM(amount),0) FROM stock_portfolio WHERE ticker=?", (ticker,)
        )
        return cursor.fetchone()[0]

# ── Обновление цены ─────────────────────────────────────────

def _update_stock_price():
    """Пересчитывает цену на основе активности за последние 15 минут."""
    now      = int(time.time())
    since    = now - STOCK_UPDATE_SEC

    with get_db_cursor() as cursor:
        # Активность: игры + переводы + новые юзеры
        try:
            cursor.execute("SELECT COUNT(*), COALESCE(SUM(win_amount),0) FROM game_history WHERE created_at >= ?", (since,))
            row = cursor.fetchone(); game_count = row[0]; game_vol = row[1]
        except Exception:
            game_count = game_vol = 0

        try:
            cursor.execute("SELECT COUNT(*), COALESCE(SUM(amount),0) FROM transfers WHERE created_at >= ?", (since,))
            row = cursor.fetchone(); tr_count = row[0]; tr_vol = row[1]
        except Exception:
            tr_count = tr_vol = 0

        try:
            cursor.execute("SELECT COUNT(*) FROM users WHERE last_activity >= ?", (since,))
            active_users = cursor.fetchone()[0]
        except Exception:
            active_users = 0

        cursor.execute("SELECT price FROM stocks WHERE ticker=?", (STOCK_TICKER,))
        current_price = cursor.fetchone()[0]

    # Формула активности → изменение цены
    # Нормализуем: 10 игр, 5 переводов, 10 юзеров = нейтрально
    activity_score = (
        (game_count  / 10.0) * 0.4 +
        (tr_count    /  5.0) * 0.3 +
        (active_users / 10.0) * 0.3
    )
    # activity_score = 1.0 → нейтро, >1 → рост, <1 → падение
    raw_change = (activity_score - 1.0) * 0.10   # ±10% при сильном отклонении
    # Добавляем шум ±2%
    noise = (random.random() - 0.5) * 0.04
    total_change = raw_change + noise
    # Ограничиваем ±15%
    total_change = max(-STOCK_MAX_CHANGE, min(STOCK_MAX_CHANGE, total_change))
    # Цена не может упасть ниже 1000
    new_price = max(1000, int(current_price * (1 + total_change)))
    ...
    print(f"[stocks] цена обновлена: {current_price} → {new_price} ({total_change*100:+.1f}%)")
    return current_price, new_price

# ── Команды игроков ─────────────────────────────────────────

def _stock_chart(history):
    """Мини-график из символов."""
    if len(history) < 2:
        return ""
    ...
    chart  = ""
    for p in prices[-16:]:
        idx = int((p - mn) / (mx - mn) * 8)
        chart += blocks[idx]
    return chart

@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() in ("акции", "акция", "биржа"))
def handle_stock_market(message):
    ...
    history     = _stock_history(limit=24)
    chart       = _stock_chart(history)

    amount, avg = _get_portfolio(user_id)
    cd_left     = _get_cooldown_left(user_id)

    portfolio_line = ""
    if amount > 0:
        cur_val  = amount * price
        avg_val  = amount * avg
        pnl      = cur_val - avg_val
        pnl_sign = "+" if pnl >= 0 else ""
        portfolio_line = (
            f"\n📂 <b>Твой портфель</b>\n"
            f"  Акций: <b>{amount} шт.</b> × {format_balance(price)} = <b>{format_balance(cur_val)} 🌸</b>\n"
            f"  Средняя цена покупки: {format_balance(avg)} 🌸\n"
            f"  P&L: <b>{pnl_sign}{format_balance(pnl)} 🌸</b>\n"
        )

    cd_line = f"\n⏳ Кулдаун: <b>{cd_left//60}м {cd_left%60}с</b>" if cd_left > 0 else ""

    total_shares = _total_shares_in_market()

    bot.send_message(message.chat.id,
        f"📈 <b>{STOCK_NAME} [{STOCK_TICKER}]</b>\n\n"
        f"{arrow} Цена: <b>{format_balance(price)} 🌸</b>  {sign}{change_pct:.1f}%\n"
        f"📊 График (24 точки): <code>{chart}</code>\n\n"
        f"📦 Акций в рынке: <b>{total_shares}</b> шт.\n"
        f"💹 Макс на игрока: {STOCK_MAX_PER_USER} шт.\n"
        f"💸 Комиссия продажи: {int(STOCK_SELL_FEE*100)}%\n"
        f"{portfolio_line}{cd_line}\n\n"
        f"Купить: <code>купить акции 10</code>\n"
        f"Продать: <code>продать акции 10</code>\n"
        f"История: <code>история акций</code>",
        parse_mode="HTML"
    )
...
def handle_buy_stock(message):
    ...
    if cd > 0:
        bot.send_message(message.chat.id,
            f"⏳ Кулдаун! Подожди <b>{cd//60}м {cd%60}с</b>", parse_mode="HTML")
        return

    try:
        qty = int(parts[2])
    except:
        bot.send_message(message.chat.id, "❌ Укажи количество числом.", parse_mode="HTML")
        return
    ...
    # Проверка лимита
    owned, avg = _get_portfolio(user_id)
    ...
    if balance < total_cost:
        bot.send_message(message.chat.id,
            ...
            f"Нужно: {format_balance(total_cost)} 🌸\n"
            f"У тебя: {format_balance(balance)} 🌸",
            parse_mode="HTML"
        )
        return
    ...
    with get_db_cursor() as cursor:
        ...
        if owned > 0:
            new_avg = (owned * avg + qty * price) // (owned + qty)
        else:
            new_avg = price

        cursor.execute(
            "INSERT INTO stock_portfolio (user_id, ticker, amount, avg_price) VALUES (?,?,?,?) "
            "ON CONFLICT(user_id, ticker) DO UPDATE SET amount=amount+?, avg_price=?",
            (user_id, STOCK_TICKER, qty, new_avg, qty, new_avg)
        )
        cursor.execute(
            "INSERT INTO stock_trades (user_id, ticker, action, amount, price, fee, total, created_at) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (user_id, STOCK_TICKER, "buy", qty, price, 0, total_cost, int(time.time()))
        )
    ...
    bot.send_message(message.chat.id,
        ...
        f"📂 Итого у тебя: {owned + qty} акций\n\n"
...
def handle_sell_stock(message):
    ...
    if owned < qty:
        bot.send_message(message.chat.id,
            f"❌ У тебя только {owned} акций.", parse_mode="HTML")
        return
    ...
    with get_db_cursor() as cursor:
        ...
        if new_amount == 0:
            ...
            cursor.execute(
                "UPDATE stock_portfolio SET amount=? WHERE user_id=? AND ticker=?",
                (new_amount, user_id, STOCK_TICKER)
            )
        cursor.execute(
            "INSERT INTO stock_trades (user_id, ticker, action, amount, price, fee, total, created_at) "
            "VALUES (?,?,?,?,?,?,?,?)",
            (user_id, STOCK_TICKER, "sell", qty, price, fee, net, int(time.time()))
        )
    ...
    bot.send_message(message.chat.id,
        ...
        f"💰 Цена продажи: {format_balance(price)} 🌸/шт.\n"
        f"💸 Комиссия (3%): -{format_balance(fee)} 🌸\n"
        f"💵 Получено: <b>{format_balance(net)} 🌸</b>\n"
        f"📊 P&L: <b>{pnl_sign}{format_balance(pnl)} 🌸</b>\n"
        f"📂 Осталось акций: {new_amount}\n\n"
        f"⏳ Следующая сделка через 10 минут",
        parse_mode="HTML"
    )
...
def handle_stock_history(message):
    ...
    with get_db_cursor() as cursor:
        ...
        my_trades = cursor.fetchall()

    trade_lines = []
    for action, amt, price, fee, total, ts in my_trades:
        dt   = _dt.datetime.utcfromtimestamp(ts).strftime("%d.%m %H:%M")
        icon = "🛒" if action == "buy" else "💰"
        fee_str = f" (комиссия -{format_balance(fee)})" if fee else ""
        trade_lines.append(
            f"{icon} {dt}  {amt} шт. × {format_balance(price)}{fee_str}"
        )
...
def _stock_price_scheduler():
    print(f"[stocks] планировщик запущен, обновление каждые {STOCK_UPDATE_SEC//60} мин")
    while True:
        time.sleep(STOCK_UPDATE_SEC)
        try:
            old_price, new_price = _update_stock_price()
            change_pct = (new_price - old_price) / old_price * 100

            # Алерт если резкое движение > 10%
            if abs(change_pct) >= 10:
                arrow = "📈" if new_price > old_price else "📉"
                send_alert(
                    f"{arrow} Резкое движение акции {STOCK_TICKER}!\n"
                    f"{format_balance(old_price)} → <b>{format_balance(new_price)} 🌸</b> "
                    f"({change_pct:+.1f}%)"
                )
        except Exception as e:
            print(f"[stocks] ошибка обновления цены: {e}")
