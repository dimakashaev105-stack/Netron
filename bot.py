...
def handle_collect_tax(message):
    ...
    try:
        ...
        if not stats:
            bot.send_message(message.chat.id, "❌ Не удалось получить статистику.", parse_mode="HTML")
            return

        pot_high   = int(stats["high_wealth"]["total"]   * TAX_CONFIG["high_wealth_tax"])
        pot_medium = int(stats["medium_wealth"]["total"] * TAX_CONFIG["medium_wealth_tax"])
        pot_low    = int(stats["low_wealth"]["total"]    * TAX_CONFIG["general_tax"])
        pot_total  = pot_high + pot_medium + pot_low

        text = (
            "🏛️ <b>СБОР НАЛОГОВ</b>\n\n"
            "<blockquote>"
            "👥 Граждан: " + str(stats["total_users"]) + "\n"
            "💵 В системе: " + format_balance(stats["total_wealth"]) + " 🌸\n"
            "👛 На балансах: " + format_balance(stats["total_balance"]) + " 🌸\n"
            "🏦 В банке: " + format_balance(stats["total_deposits"]) + " 🌸"
            "</blockquote>\n\n"
            "📈 <b>Категории:</b>\n"
            "🏆 Элита (&gt;1млн): " + str(stats["high_wealth"]["count"]) + " чел. — 10% (~" + format_balance(pot_high) + " 🌸)\n"
            "💎 Состоятельные (&gt;100к): " + str(stats["medium_wealth"]["count"]) + " чел. — 5% (~" + format_balance(pot_medium) + " 🌸)\n"
            "👤 Граждане: " + str(stats["low_wealth"]["count"]) + " чел. — 3% (~" + format_balance(pot_low) + " 🌸)\n\n"
            "💰 <b>Итого к сбору: ~" + format_balance(pot_total) + " 🌸</b>\n\n"
            "⚠️ Подтвердите сбор налогов!"
        )
        kb = {
            "inline_keyboard": [[
                {"text": "✅ СОБРАТЬ", "callback_data": "confirm_tax_collection", "style": "success"},
