...
def _parse_pg_url(url):
    ...
    return dict(user=m[1], password=m[2], host=m[3], port=int(m[4]), database=m[5])

_pg_params = _parse_pg_url(_PG_URL)
_pg_pool = []
...
class _PgCursor:
    """Makes pg8000 cursor behave like sqlite3 dict cursor."""
    ...
    def executemany(self, sql, params_list):
        ...
        for p in params_list:
            self._cur.execute(sql, list(p))

    def _row(self, raw):
        return dict(zip(self._cols, raw)) if raw is not None else None

    def fetchone(self):
        return self._row(self._cur.fetchone())

    def fetchall(self):
        return [self._row(r) for r in (self._cur.fetchall() or [])]
...
''')
        ...
        # Безопасное добавление колонок через try/except
        _safe_alters = [
...
def handle_document_upload(message):
    ...
    try:
        ...
        if file_ext == 'db':
            ...
            try:
                test_conn = pg8000.connect(db_path)
