...
# Cooldown для /start — защита от спама реферальными ссылками
_start_cooldowns = {}
...
def start(message):
    try:
        user_id = message.from_user.id
        # Rate limit: /start не чаще 1 раза в 5 секунд на пользователя
        now = time.time()
        with _start_lock:
            last = _start_cooldowns.get(user_id, 0)
            if now - last < 5:
            ...
            with get_db_cursor() as cursor:
                ...
                if check_data:
                    ...
                    if ref_user and ref_user[0] != user_id:
                        ...
                        # Атомарный UPDATE — срабатывает только если referred_by ещё NULL
                        # Защита от race condition при параллельных /start
                        cursor.execute(
                            'UPDATE users SET referred_by = ? WHERE user_id = ? AND referred_by IS NULL',
                            (referred_by, user_id)
                        )
                        if cursor.rowcount > 0:
                            # Только если мы первые записали — начисляем бонусы
                            referrer_bonus = 5000
                            user_bonus = 1000
                            referrer_exp = 500
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if check_data:
                ...
                if ref_user and ref_user[0] != user_id:
                    referred_by = ref_user[0]
                    # Атомарный UPDATE — защита от повторных активаций
                    cursor.execute(
                        'UPDATE users SET referred_by = ? WHERE user_id = ? AND referred_by IS NULL',
                        (referred_by, user_id)
                    )
