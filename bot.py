...
class DatabasePool:
    def __init__(self, db_name=None, pool_size=10):
        import os
        self.db_name = db_name or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'game.db')
        self.pool = []
        ...
        for column in required_columns:
            if column not in columns:
                if column in ['bank_deposit', 'click_streak', 'total_clicks', 'daily_streak', 
                    ...
                    cursor.execute(f'ALTER TABLE users ADD COLUMN {column} TEXT DEFAULT NULL')
                else:
                    cursor.execute(f'ALTER TABLE users ADD COLUMN {column} INTEGER DEFAULT 0')
...
def handle_sql_console(message):
    ...
    """
    try:
        query = message.text.strip()[4:].strip()  # убираем 'SQL '
        if not query:
...
def handle_existing_user(message):
    ...
    try:
        ...
        # Deep link: уведомления об ивентах
...
def handle_tower_callback(call):
    ...
    try:
        ...
        # Формат: tower_{game_id}_{level}_{tower}_{type}
        if not data.startswith('tower_'):
            return
            
        parts = data[6:].split('_')  # Пропускаем префикс 'tower_'
...
def event_launch():
    ...
    # Уведомляем подписчиков о старте ивента
    ...
    def force_finish():
        if state["active"] and len(state["participants"]) >= EVENT_WINNERS:
            ...
            try:
                ...
                pass
            # Уведомляем подписчиков об отмене
...
def start_event_scheduler():
    # Создаём таблицу подписок на уведомления
    with get_db_cursor() as cursor:
        cursor.execute("""
...
def cb_toggle_event_notify(call):
    ...
    # Обновляем кнопку прямо в том же сообщении
    try:
