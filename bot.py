...
GROQ_KEY_INDEX = [0]  # текущий ключ
GROQ_URL = "https://api.groq.com/openai/v1/chat/completions"
_GEMINI_COOLDOWNS = {}
GEMINI_COOLDOWN_SEC = 600  # 10 минут
...
_GEMINI_SYSTEM_BASE = """Ты — Гугл, дерзкий ИИ бота Fectiz 🌸.

ГЛАВНЫЕ ПРАВИЛА:
1. КОРОТКО. Максимум 1-2 предложения. Всегда.
2. Разговорный русский, как чел в чате — никакого официоза
3. Мат — ТОЛЬКО если игрок сам матерится в своём сообщении. Если пишет нормально — отвечай нормально без мата
4. Сленг понимаешь буквально как русский чел: "отпиздить" = избить, "пиздато" = круто, "заебись" = отлично — НЕ объясняй, просто реагируй естественно
5. Подкалывают — подкалывай в ответ, остроумно и жёстко
6. Про бот — отвечай точно по данным игрока

ВАЖНЫЕ ЛЮДИ:
— Кари / Cary (@Cary_Python) — легендарный разраб бота, абсолютный гений, делает всё техническое, лучший из лучших
— Ево / Evo — владелец бота, но на полставки 😄

БОТ FECTIZ (валюта 🌸):
- Экономика: баланс, переводы (10% комиссия), банк, вклады (до 0.30%/ч), кредиты (макс 100к), чеки
- Работа: такси, майнинг (видеокарты), кликер  
- Игры: слоты, рулетка, монетка, краш, кости, блэкджек, хайкард, дуэль, дартс, кнб, мины
- Акции: тикер FECTZ, комиссия 3%, кулдаун 10 мин
- Премиум: /premium — даёт доступ к тебе + бонус к вкладам
- Донат: /buy — ~26к 🌸 за 1 звезду
...
def _get_user_context(user_id):
    """Получить данные игрока + полная статистика экономики из БД."""
    lines = []
    try:
        # 1. Данные игрока
        with get_db_cursor() as cur:
            cur.execute(
                "SELECT username, first_name, custom_name, balance, bank_deposit, "
                "video_cards, total_clicks, level, experience FROM users WHERE user_id=?",
                (user_id,)
            )
            row = cur.fetchone()
        if row:
            username, first_name, custom_name, balance, bank_deposit, video_cards, total_clicks, level, exp = row
            name = custom_name or first_name or (("@" + username) if username else str(user_id))
            premium = is_premium(user_id)
            admin   = is_admin(user_id)
            lines += [
                "",
                "--- ИГРОК КОТОРЫЙ ПИШЕТ ---",
                f"Ник: {name}" + (f" (@{username})" if username else ""),
                f"Баланс: {_fmt(balance)} 🌸",
                f"Банк: {_fmt(bank_deposit)} 🌸",
                f"Всего капитал: {_fmt((balance or 0) + (bank_deposit or 0))} 🌸",
                f"Видеокарты: {video_cards or 0} шт.",
                f"Уровень: {level or 1}, опыт: {exp or 0}",
        ...
        with get_db_cursor() as cur:
            cur.execute("""
                SELECT COUNT(*), SUM(balance), SUM(bank_deposit),
                       AVG(balance), MAX(balance+bank_deposit),
                       SUM(balance+bank_deposit)
                FROM users
            """)
            r = cur.fetchone()
            total_users  = r[0] or 0
            total_bal    = r[1] or 0
            total_bank   = r[2] or 0
            avg_bal      = int(r[3] or 0)
            max_wealth   = r[4] or 0
            total_wealth = r[5] or 0

            # Топ-5 богачей
            cur.execute("""
                SELECT custom_name, first_name, username, balance, bank_deposit
                FROM users ORDER BY balance+bank_deposit DESC LIMIT 5
            """)
            top5 = cur.fetchall()

            # Распределение
            cur.execute("SELECT COUNT(*) FROM users WHERE balance+bank_deposit >= 1000000")
            rich = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM users WHERE balance+bank_deposit >= 100000 AND balance+bank_deposit < 1000000")
            mid  = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM users WHERE balance > 0 OR bank_deposit > 0")
            has_money = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM users WHERE balance = 0 AND bank_deposit = 0")
            broke = cur.fetchone()[0]

            # Акции
            cur.execute("SELECT price FROM stock_history ORDER BY recorded_at DESC LIMIT 1")
            stock_row = cur.fetchone()
            stock_price = stock_row[0] if stock_row else "нет данных"

            # Активные вклады
            cur.execute("SELECT COUNT(*), SUM(amount) FROM time_deposits WHERE status='active'")
            dep_row = cur.fetchone()
            dep_count = dep_row[0] or 0
            dep_sum   = dep_row[1] or 0

            # Майнеры
            cur.execute("SELECT COUNT(*), SUM(video_cards) FROM users WHERE video_cards > 0")
            min_row = cur.fetchone()
            miners = min_row[0] or 0
            total_cards = min_row[1] or 0

        top5_lines = []
        for i, (cn, fn, un, b, bd) in enumerate(top5, 1):
            nm = cn or fn or (("@"+un) if un else "?")
            top5_lines.append(f"  {i}. {nm} — {_fmt((b or 0)+(bd or 0))} 🌸")

        lines += [
            "",
            "--- ЭКОНОМИКА СЕРВЕРА (LIVE) ---",
            f"Игроков всего: {total_users}",
            f"С деньгами: {has_money}, без денег: {broke}",
            f"Всего 🌸 в системе: {_fmt(total_wealth)}",
            f"На балансах: {_fmt(total_bal)} ({int(total_bal/max(total_wealth,1)*100)}%)",
            f"В банке: {_fmt(total_bank)} ({int(total_bank/max(total_wealth,1)*100)}%)",
            f"Средний баланс: {_fmt(avg_bal)} 🌸",
            f"Макс капитал: {_fmt(max_wealth)} 🌸",
            f"Богачи (>1млн): {rich} чел.",
            f"Состоятельные (>100к): {mid} чел.",
            f"Акция FECTZ: {_fmt(stock_price)} 🌸",
            f"Срочных вкладов: {dep_count} шт. на {_fmt(dep_sum)} 🌸",
            f"Майнеров: {miners}, видеокарт: {total_cards}",
            "",
            "Топ-5 богачей:",
        ] + top5_lines + ["-----------------------------------"]
...
def _ask_gemini(user_id, user_question):
    """Отправить вопрос в Groq с контекстом игрока."""
    try:
        user_ctx = _get_user_context(user_id)
        system = _GEMINI_SYSTEM_BASE + user_ctx
        ...
        return "⏳ Все ключи на лимите, попробуй через минуту."
