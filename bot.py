        ...
        with get_db_cursor() as cursor:
            ...
            for user in users:
                ...
                if tax_from_balance > 0:
                    parts.append(f"  • Баланс: -{format_balance(tax_from_balance)} 🌸 ({format_balance(balance)} → {format_balance(new_balance)})")
                if tax_from_bank > 0:
                    parts.append(f"  • Банк: -{format_balance(tax_from_bank)} 🌸 ({format_balance(bank_deposit)} → {format_balance(new_bank)})")

                try:
                    bot.send_message(
                        user_id,
                        f"🏛️ <b>НАЛОГОВЫЙ СБОР</b>\n\n"
                        f"💸 Всего списано: <b>{format_balance(actual_tax)} 🌸</b>\n"
                        f"📊 Ставка: {tax_reason}\n\n"
                        "<blockquote>" + "\n".join(parts) + "</blockquote>\n\n"
                        f"🏛 Общее состояние: {format_balance(total_wealth - actual_tax)} 🌸",
                        parse_mode='HTML'
                    )
                except:
                    pass
...
def handle_collect_tax(message):
    ...
    try:
        ...
        message_text += f"• Богачи (&gt;1млн): {stats['high_wealth']['count']} чел. — 10%\n"
        message_text += f"• Состоятельные (&gt;100к): {stats['medium_wealth']['count']} чел. — 5%\n"
