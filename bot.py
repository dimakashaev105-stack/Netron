...
def handle_warn_user(message):
    ...
    try:
        ...
        if success:
            ...
            bot.send_message(message.chat.id,
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Варн выдан пользователю ID: {target_user_id}\n"
                           f"📝 Причина: {reason}\n"
...
MINE_RENT_DURATION = 60 * 60  # 1 час
...
def handle_mine_dig_text(message):
    ...
    if rental:
        mine  = _mine_by_id(rental[0])
        bonus = (MINE_LEVELS[mine['level']]['bonus'] if mine else 1.0) * pk_bonus
        where = f"шахта #{rental[0]}"
    elif my_mine:
        bonus = MINE_LEVELS[my_mine['level']]['bonus'] * pk_bonus
