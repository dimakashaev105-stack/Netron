...
MINE_RESOURCES = [
    {"id": "coal",      "name": "Уголь",        "fallback": "🪨", "emoji_id": 5456408804541340493, "chance": 32,   "price": 1200},
    {"id": "earth",     "name": "Земля",         "fallback": "🟫", "emoji_id": 5458822138075028493, "chance": 18,   "price": 800},
    {"id": "stone",     "name": "Камень",        "fallback": "🪨", "emoji_id": 5458781211331665562, "chance": 14,   "price": 1000},
    {"id": "copper",    "name": "Медь",          "fallback": "🔶", "emoji_id": 5458520300658368161, "chance": 10,   "price": 2800},
    {"id": "iron",      "name": "Железо",        "fallback": "⚙️", "emoji_id": 5458833133191306560, "chance": 9,    "price": 4500},
    ...
    {"id": "lapis",     "name": "Лазурит",       "fallback": "🔵", "emoji_id": 5458825788797231419, "chance": 1.5,  "price": 30000},
    {"id": "emerald",   "name": "Изумруд",       "fallback": "💚", "emoji_id": 5458517053663093203, "chance": 0.9,  "price": 70000},
    {"id": "diamond",   "name": "Алмаз",         "fallback": "💎", "emoji_id": 5458420940884942467, "chance": 0.4,  "price": 140000},
    {"id": "gold_ore",  "name": "Золотая руда",  "fallback": "🟡", "emoji_id": 5458617543012918174, "chance": 0.25, "price": 250000},
    {"id": "netherite", "name": "Незерит",       "fallback": "🌑", "emoji_id": 6280349132968168995, "chance": 0.05, "price": 1000000},
]
...
def cb_mkt_sell_pick(call):
    ...
    for rid, amt in inv.items():
        ...
        pair.append({"text": f"{mark}{icon} {r['name']} ·{amt}", "callback_data": f"mkt_sell_res:{rid}"})
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:
...
def cb_mkt_buy_pick(call):
    ...
    for r in MINE_RESOURCES:
        ...
        icon = r.get("fallback", "📦")
        if cnt:
            label = f"{icon} {r['name']} ·{cnt}"
        else:
            label = f"{icon} {r['name']}"
        pair.append({"text": label, "callback_data": f"mkt_buy_res:{r['id']}"})
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    if pair:
    ...
    for slot in slot_order:
        ...
        if set_id:
            ...
            if em_id:
                ...
                tge = fb
            lines_out.append(f"{tge} <b>{SLOT_NAMES[slot]}</b>  <i>{s['name']}</i>")
        else:
...
def _eq_main_kb(user_id):
    ...
    base_icons = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "⚔️"}
    rows = []
    row = []
    for slot in slot_order:
        set_id = equipped.get(slot)
        if set_id:
            s = EQUIPMENT_SETS[set_id]
            item_icon = s["slots"][slot]["fallback"]
            mat_emoji = s["emoji"]
            label = f"{item_icon} {mat_emoji} {slot_names[slot]}"
        else:
            label = f"{base_icons[slot]} {slot_names[slot]}"
        row.append({"text": label, "callback_data": f"eq_slot_{slot}"})
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
def _craft_menu_kb():
    ...
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        row.append({"text": f"{s['emoji']} {s['name']}", "callback_data": f"craft_set_{set_id}"})
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
def cb_craft_set(call):
    ...
    for slot, cost in CRAFT_COST.items():
        ...
        rows.append([{"text": btn_text, "callback_data": f"craft_item_{set_id}_{slot}"}])
