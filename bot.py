...
CRASH_ADMIN_CHAT = -5261308794  # Чат для решений админа
# Если еще нет
...
def format_balance(balance):
    ...
    return f"{balance:,}".replace(",", " ") + flower
...
def process_crash_game_ls_only(user_id, bet_amount, target_multiplier):
    ...
    # Получаем имя игрока
    try:
        user_info = bot.get_chat_member(CRASH_ADMIN_CHAT, user_id)
        user_name = user_info.user.first_name or str(user_id)
    except:
...
def send_crash_result(game_id, admin_decision):
    ...
    if r < 0.23:  # 23% шанс
        ...
        result_multiplier = random.uniform(1.50, 2.10)
    elif r < 0.57:  # 20% шанс
        result_multiplier = random.uniform(2.10, 3.30)
    elif r < 0.77:  # 20% шанс
        result_multiplier = random.uniform(3.30, 5.50)
    elif r < 0.93:  # 16% шанс
        result_multiplier = random.uniform(5.50, 11.00)
    else:  # 7% шанс
        result_multiplier = random.uniform(11.00, 27.00)
    
    result_multiplier = round(result_multiplier, 2)
    
    # Если админ решил выигрыш - гарантируем что игрок выиграет
    if won and admin_decision is True:
        result_multiplier = target_mult - 0.01
    
    # Если админ решил проигрыш - гарантируем что игрок проиграет
    if not won and admin_decision is False:
        result_multiplier = target_mult + 0.01
    ...
    try:
        if won:
            bot.send_message(
                ...
                f"📊 Результат краша: {result_multiplier:.2f}x\n"
                f"💵 Ставка: {int(bet)} 🌸\n"
                f"💵 Выигрыш: {int(win_amount)} 🌸\n"
                f"📈 Прибыль: +{int(profit)} 🌸\n\n"
                f"🔄 Новый баланс: {int(current_balance)} 🌸",
                parse_mode='Markdown'
            )
        else:
            bot.send_message(
                ...
                f"📊 Результат краша: {result_multiplier:.2f}x\n"
                f"💵 Потеряно: {int(bet)} 🌸\n\n"
                f"🔄 Текущий баланс: {int(current_balance)} 🌸\n"
                f"💡 Удачи в следующий раз!",
                parse_mode='Markdown'
            )
    except Exception as e:
...
def handle_crash_win(call):
    ...
    bot.answer_callback_query(call.id, "✅ Выигрыш", show_alert=False)
    bot.edit_message_reply_markup(CRASH_ADMIN_CHAT, call.message.message_id, reply_markup=None)
...
def handle_crash_lose(call):
    ...
    bot.answer_callback_query(call.id, "❌ Проигрыш", show_alert=False)
    bot.edit_message_reply_markup(CRASH_ADMIN_CHAT, call.message.message_id, reply_markup=None)
