...
# ──────────────────────────────────────────────────────────
# Антиспам для callback-кнопок и сообщений
# ──────────────────────────────────────────────────────────
_cb_last: dict[int, float] = {}   # user_id → timestamp последнего callback
_msg_last: dict[int, float] = {}  # user_id → timestamp последнего сообщения
_CB_COOLDOWN  = 0.8   # секунд между нажатиями кнопок
_MSG_COOLDOWN = 0.5   # секунд между командами

def _cb_throttle(call, cooldown=_CB_COOLDOWN) -> bool:
    """True = разрешить, False = заблокировать (слишком быстро)."""
    uid = call.from_user.id
    now = time.time()
    last = _cb_last.get(uid, 0)
    if now - last < cooldown:
        try:
            bot.answer_callback_query(call.id)  # тихо подтверждаем чтобы не крутился кружок
        except Exception:
            pass
        return False
    _cb_last[uid] = now
...
def _msg_throttle(message, cooldown=_MSG_COOLDOWN) -> bool:
    """True = разрешить."""
    uid = message.from_user.id
    now = time.time()
    last = _msg_last.get(uid, 0)
    if now - last < cooldown:
        return False
    _msg_last[uid] = now
    return True

# Патч: перехватываем все callback через middleware
_orig_process_new_updates = bot.process_new_updates
def _patched_process_new_updates(updates):
    _orig_process_new_updates(updates)
bot.process_new_updates = _patched_process_new_updates
...
def cb_premium_buy_menu(call):
    if not _cb_throttle(call): return
    markup = InlineKeyboardMarkup()
...
def handle_me(message):
    if not _msg_throttle(message, cooldown=2.0):
        return
    def _do():
      try:
        user_id = message.from_user.id
