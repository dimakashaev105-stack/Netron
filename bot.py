...
def handle_mine_dig_text(message):
    ...
    if event["mult"] > 0:
        res_qty = 2 if event["mult"] >= 2.0 else 1
        _inv_add(uid, resource["id"], res_qty)
    ...
    if event["mult"] == 0.0:
        ...
        res_qty    = 2 if event["mult"] >= 2.0 else 1
        qty_tag    = f" ×{res_qty}" if res_qty > 1 else ""
        streak_tag = (f" 🔥×{new_streak}") if new_streak >= 10 else ""
        res_emoji  = f"<tg-emoji emoji-id='{resource['emoji_id']}'>{resource['name']}</tg-emoji>"
        text_html  = f"⛏️ {where}\n{res_emoji}{qty_tag} · +{format_balance(reward)} 🌸{streak_tag}"
        text_plain = f"⛏️ {where}\n{resource['name']}{qty_tag} · +{format_balance(reward)} 🌸{streak_tag}"
        try:
            bot.send_message(message.chat.id, text_html, parse_mode="HTML")
        except Exception:
            bot.send_message(message.chat.id, text_plain)
...
def cb_mine_dig(call):
    ...
    # Добавляем ресурс в инвентарь (1 шт., при двойной жиле — 2 шт.)
    if event["mult"] > 0:
        res_qty = 2 if event["mult"] >= 2.0 else 1
        _inv_add(uid, resource["id"], res_qty)
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, plain)
    try:
        bot.edit_message_text(_mine_main_text(uid), call.message.chat.id, call.message.message_id,
                              reply_markup=_mine_main_kb(uid), parse_mode="HTML")
    except Exception:
...
def _pickaxe_text(uid):
    ...
    lines = [f"⛏️ <b>Кирка</b>\n"]
    lines.append(f"{info['label']}  ·  x{info['bonus']}  ·  КД {info['cooldown']//60}м")
    lines.append("")
    for lv, d in PICKAXE_LEVELS.items():
        mark = "▶️" if lv == lvl else ("✅" if lv < lvl else "  ")
        cd_m = d["cooldown"] // 60
        lines.append(f"{mark} Лвл {lv}  x{d['bonus']}  КД {cd_m}м  —  {d['stars']} ⭐" if lv > 1 else f"{mark} Лвл {lv}  x{d['bonus']}  КД {cd_m}м  — бесплатно")
    return "\n".join(lines)
