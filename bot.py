    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id,
                           ...
                           reply_markup=json.dumps(del_item_keyboard),
                           parse_mode='HTML')
...
def create_new_order(chat_id, user_id):
    ...
    sent = bot.send_message(chat_id, text, reply_markup=json.dumps(taxi_keyboard), parse_mode='HTML')
...
@bot.message_handler(func=lambda message: message.text.lower() == 'удалить все займы' and is_admin(message.from_user.id))
def handle_delete_all_loans(message):
    """Удалить все активные займы (обнулить долги)"""
    if not is_admin(message.from_user.id):
        return
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            # Удаляем все займы (или помечаем как оплаченные)
            cursor.execute('UPDATE loans SET status = "deleted" WHERE status = "active"')
            # Альтернатива: полностью удалить записи
            # cursor.execute('DELETE FROM loans WHERE status = "active"')
        
        bot.send_message(message.chat.id, 
                       f"✅ Все займы удалены!\n\n"
                       f"📊 Удалено: {active_loans} займов\n"
                       f"💸 Общая сумма долга: {format_balance(total_debt)}")
        
    except Exception as e:
        print(f"Ошибка при удалении займов: {e}")
...
def handle_deposit_command(message):
    ...
    try:
        ...
        # Создаем JSON клавиатуру с цветными кнопками
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "✅",
                        "callback_data": f"cmd_deposit_{deposit_amount}",
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "✅",
                        "callback_data": f"cmd_withdraw_{withdraw_amount}",
                        "style": "success"  # 🟢 Зеленая
                    },
                    {
                        "text": "❌",
...
def handle_loan_command(message):
    ...
    try:
        ...
        loan_keyboard = {
            "inline_keyboard": [[
                {"text": "✅ ВЗЯТЬ ЗАЙМ", "callback_data": f"cmd_loan_{loan_amount}", "style": "success"},
                {"text": "❌ ОТМЕНА", "callback_data": "cmd_cancel", "style": "danger"}
            ]]
        }
        ...
        bot.send_message(
            ...
            f"<b>ВАЖНО:</b> Банк заблокируется до погашения займа!\n\n"
            f"Подтверждаете?",
            reply_markup=json.dumps(loan_keyboard),
            parse_mode='HTML'
        )
...
def handle_repay_loan_command(message):
    ...
    repay_keyboard = {
        "inline_keyboard": [[
            {"text": "✅ ПОГАСИТЬ", "callback_data": f"cmd_repay_{repay_amount}", "style": "success"},
            {"text": "❌ ОТМЕНА", "callback_data": "cmd_cancel", "style": "danger"}
        ]]
    }
    ...
    bot.send_message(
        message.chat.id,
        message_text,
        reply_markup=json.dumps(repay_keyboard),
        parse_mode='HTML'
    )
...
def handle_bank(message):
    ...
    try:
        with open('bank.jpg', 'rb') as photo:
            ...
            caption += f"📋 <b>Команды банка:</b>\n"
            caption += f"• <code>займ [сумма]</code> - взять займ\n"
            caption += f"• <code>погасить займ</code> - погасить долг\n"
            caption += f"• <code>вклад [сумма]</code> - положить на вклад\n"
            caption += f"• <code>снять [сумма]</code> - снять со вклада\n"
            caption += f"• <code>банк инфо</code> - подробная информация"
        ...
        bank_text += f"📋 <b>Команды банка:</b>\n"
        bank_text += f"• <code>займ [сумма]</code> - взять займ\n"
        bank_text += f"• <code>погасить займ</code> - погасить долг\n"
        bank_text += f"• <code>вклад [сумма]</code> - положить на вклад\n"
        bank_text += f"• <code>снять [сумма]</code> - снять со вклада\n"
        bank_text += f"• <code>банк инфо</code> - подробная информация"
...
def handle_loan_info(message):
    ...
    if loan_details['amount_due'] <= get_balance(user_id):
        repay_btn = {"text": "💵 ПОГАСИТЬ ЗАЙМ", "callback_data": f"repay_loan_{user_id}", "style": "success"}
    else:
        repay_btn = {"text": "💵 Недостаточно средств", "callback_data": "no_money", "style": "secondary"}
    repay_admin_keyboard = {"inline_keyboard": [[repay_btn]]}
    
    bot.send_message(
        message.chat.id,
        message_text,
        reply_markup=json.dumps(repay_admin_keyboard),
        parse_mode='HTML'
    )
    ...
    try:
        ...
        # Выход из клана
            ...
            bot.edit_message_text(
                ...
                call.message.chat.id,
                call.message.message_id,
                reply_markup=json.dumps(leave_clan_keyboard),
                parse_mode='HTML'
            )
...
def handle_broadcast(message):
    ...
    try:
        ...
        bot.reply_to(message,
                    ...
                    f"Подтвердить?",
                    reply_markup=json.dumps(broadcast_keyboard),
                    parse_mode='HTML')
...
def handle_buy(message):
    ...
    buy_keyboard = {
        "inline_keyboard": [
            [
                {"text": "⭐ 1 зв — 10,000", "callback_data": "stars_1", "style": "secondary"},
                {"text": "⭐ 5 зв — 66,000", "callback_data": "stars_5", "style": "secondary"}
            ],
            [
                {"text": "🔥 15 зв — 266,000", "callback_data": "stars_15", "style": "success"},
                {"text": "🔥 50 зв — 1,000,000", "callback_data": "stars_50", "style": "success"}
            ],
            [
                {"text": "💎 150 зв — 4,000,000", "callback_data": "stars_150", "style": "secondary"},
                {"text": "👑 250 зв — 8,000,000", "callback_data": "stars_250", "style": "secondary"}
            ]
        ]
    }
    ...
    bot.send_message(
        ...
        "🔥 15 зв — 266,000  (~22.5₽)\n"
        "🔥 50 зв — 1,000,000  (~75₽)\n"
        "💎 150 зв — 4,000,000  (~225₽)\n"
        "👑 250 зв — 8,000,000  (~375₽)"
        "</blockquote>\n\n"
        "🔥 — самое выгодное соотношение\n"
        "Чем больше пакет — тем больше монет за звезду",
        reply_markup=json.dumps(buy_keyboard),
        parse_mode='HTML'
    )
...
def handle_case(message):
    ...
    try:
        ...
        case_keyboard = {
            "inline_keyboard": [[
                {"text": f"✅ Купить {quantity} кейс{'ов' if quantity > 1 else ''} за {plain_balance(total_cost)}", "callback_data": f"buy_case_{quantity}", "style": "success"},
                {"text": "❌ Отмена", "callback_data": "buy_case_cancel", "style": "danger"}
            ]]
        }
        
        bot.send_message(message.chat.id,
                       f"🎁 <b>Приобретение кейсов</b>\n\n"
                       f"<blockquote>"
                       ...
                       reply_markup=json.dumps(case_keyboard), parse_mode='HTML')
...
def handle_roulette_photo_delete(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            reply_markup=json.dumps(roulette_del_keyboard),
            parse_mode='HTML'
        )
    ...
    try:
        if call.data.startswith('confirm_buy_'):
            ...
            bot.send_message(
                ...
                reply_markup=json.dumps(confirm_kb),
                parse_mode='HTML'
            )
...
def handle_clan_upgrade(message):
    ...
    bot.send_message(message.chat.id,
                   ...
                   reply_markup=json.dumps(clan_upgrade_keyboard), parse_mode='HTML')
...
def send_duel_invitation(chat_id, duel):
    ...
    msg = bot.send_message(chat_id, message_text, reply_markup=json.dumps(duel_keyboard), parse_mode='HTML')
...
def handle_delete_auction(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,
                       ...
                       f"Подтвердить удаление?",
                       reply_markup=json.dumps(del_auc_keyboard),
                       parse_mode='HTML')
...
def handle_reset_all_stats(message):
    ...
    try:
        ...
        bot.reply_to(message,
                   ...
                   reply_markup=json.dumps(reset_stats_keyboard),
                   parse_mode='HTML')
...
def handle_sell_business(message):
    ...
    bot.send_message(message.chat.id,
                   ...
                   f"Продать бизнес?",
                   reply_markup=json.dumps(sell_biz_keyboard), parse_mode='HTML')
...
def handle_convert_balances(message):
    ...
    bot.send_message(
        ...
        reply_markup=json.dumps(convert_keyboard),
        parse_mode='HTML'
    )
...
def handle_bonus_broadcast(message):
    ...
    try:
        ...
        bot.reply_to(
            ...
            reply_markup=json.dumps(bonus_broadcast_keyboard),
            parse_mode='HTML'
        )
...
def handle_dice_bet(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, challenge_text, reply_markup=json.dumps(dice_keyboard), parse_mode='HTML')
...
def handle_collect_tax(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, message_text, reply_markup=json.dumps(tax_keyboard), parse_mode='HTML')
...
def handle_transfer(message):
    try:
        ...
        sender_name = sender_info.get('custom_name') or message.from_user.first_name or f"ID: {user_id}"
        ...
        # Создаем JSON клавиатуру с цветными кнопками и data + sender_id для проверки
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "✅",
                        "callback_data": f"confirm_transfer_{user_id}_{target_user_id}_{amount}",
                        "style": "success"  # 🟢 Зеленая
                    },
                    {
                        "text": "❌",
                        "callback_data": f"cancel_transfer_{user_id}",
                        "style": "danger"  # 🔴 Красная
                    }
                ]
            ]
        ...
        # Формируем сообщение с премиум-галочкой и blockquote
        transfer_text = f"<tg-emoji emoji-id='5206607081334906820'>✅</tg-emoji> <b>Подтвердите перевод</b>\n\n"
        transfer_text += f"<blockquote>Кто: {sender_name}\n"
        transfer_text += f"Кому: {target_display_name}\n"
        transfer_text += f"Сумма: {format_balance(receive_amount)}</blockquote>"
        
        # Отправляем сообщение с HTML и цветными кнопками
        bot.send_message(
            message.chat.id, 
            transfer_text, 
            reply_markup=json.dumps(keyboard),
...
def handle_knb_challenge(message):
    ...
    bot.send_message(
        ...
        parse_mode='HTML',
        reply_markup=json.dumps(knb_keyboard)
    )
