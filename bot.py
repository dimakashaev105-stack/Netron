...
def init_taxi_database():
    with get_db_cursor() as cursor:
        ...
        # Миграция: добавить status если нет
        try:
...
def handle_start_shift(message):
    ...
    # Если есть старый pending — удаляем его
    with get_db_cursor() as _cur:
        _cur.execute(
...
def handle_taxi_actions(call):
    ...
    if call.data == "taxi_start_shift":
        ...
        # Удаляем pending-заказ (поездка ещё не началась — деньги не списывались)
        with get_db_cursor() as _cur:
            _cur.execute(
                "UPDATE active_trips SET finish_time=? WHERE user_id=? AND status='pending' AND finish_time IS NULL",
                (int(time.time()), user_id)
...
def accept_order(call):
    ...
    # Берём pending-заказ (ещё не принятый)
    order = get_pending_trip(user_id)
    if not order:
        # Может уже принят или истёк
...
MINE_COOLDOWN         = 10 * 60  # 10 минут — общая шахта (базовый, Лвл 1 кирки)
# Кулдаун частной шахты по уровню шахты — берётся минимум с кулдауном кирки
...
def handle_mine_dig_text(message):
    ...
    # Монеты — половина от базовой награды
    if reward > 0:
        update_balance(uid, reward // 2)
    # Ресурсы: кирка * событие * буст
...
MARKET_COMMISSION = 0.05  # 5% комиссия сгорает

_market_fsm = {}  # uid -> {"step":..., "rid":..., ...}
...
def cb_mkt_sell_res(call):
    ...
    # Чужие заявки на покупку этого ресурса
...
def _execute_market_sell(message, uid, order_id, amount):
    ...
    # Атомарно: снимаем ресурс у продавца, возвращаем часть замороженных денег покупателю если частичное
    ...
    # Частично возвращаем незамороженные деньги если продали меньше
...
def cb_mkt_sell_cancel(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    # Защита от двойного нажатия: SELECT + DELETE в одном блоке
    with get_db_cursor() as cur:
...
def handle_market_fsm(message):
    ...
    if step == "sell_amount":
        ...
        amount = fsm["amount"]
        r      = _res_by_id(rid)
        # Проверяем инвентарь ещё раз (защита от гонки)
        inv_amt = _inv_get(uid, rid)
        if inv_amt < amount:
...
# Быстрые кнопки: подставить значение в FSM
...
# Кол-во ресурсов для крафта каждого слота (как в Minecraft)
CRAFT_COST = {
    "helmet":     5,
    "chestplate": 8,
    "leggings":   7,
...
# Бонус за полный сет одного материала
FULL_SET_BONUS = 0.15  # +15% к урону и защите
...
def _eq_calc_stats(user_id):
    ...
    # Бонусы от каждого предмета
    # Меч даёт весь урон сета, броня делится по 3 слотам (шлем, нагрудник, поножи, сапоги)
    for slot, set_id in equipped.items():
        ...
        if slot == "sword":
            ...
            # 4 слота брони делят защиту равномерно
            base_def += s["stats"]["defense"] // 4

    # Бонус полного сета: все 5 слотов одного материала
...
def _eq_main_kb(user_id):
    ...
    rows = []
    slot_order = ["helmet", "chestplate", "leggings", "boots", "sword"]
    icons = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "⚔️"}
    row = []
    for slot in slot_order:
        icon = icons[slot]
        if equipped.get(slot):
            ...
            label = f"{icon} {mat_emoji} {SLOT_NAMES[slot]}"
        else:
            label = f"{icon} {SLOT_NAMES[slot]}"
...
# ── Инвентарь скрафченных предметов ──────────────────────────
# Хранится в resource_inventory как "eq_{set_id}_{slot}"
...
def cb_craft_item(call):
    ...
    # Списываем ресурс, добавляем предмет в инвентарь
...
PVP_BASE_DMG_MIN = 10
PVP_BASE_DMG_MAX = 20
PVP_BLOCK_REDUCTION = 0.50   # блок снижает урон на 50%
PVP_TURN_TIMEOUT = 60        # секунд на ход
PVP_MAX_HP = 100

_pvp_timeout_timers = {}  # battle_id -> Timer
...
def _pvp_battle_text(b, last_event=None):
    ...
    a_hp = b["attacker_hp"]
    d_hp = b["defender_hp"]
    a_block_hint = "  🛡" if b.get("attacker_blocked") else ""
    d_block_hint = "  🛡" if b.get("defender_blocked") else ""

    lines_out = [
        "⚔️ <b>— П В П   Б О Й —</b>\n",
        f"🔴 <b>{a_name}</b>{a_block_hint}",
        f"{_pvp_hp_bar(a_hp)}  <b>{a_hp}</b>/100",
        f"<i>⚔️ {a_stats['damage']} атк  •  🛡 {a_stats['defense']} защ</i>",
        "",
        f"🔵 <b>{d_name}</b>{d_block_hint}",
...
def _pvp_action_kb(battle_id, user_id):
    return InlineKeyboardMarkup(keyboard=[[
        InlineKeyboardButton("⚔️ Атаковать",  callback_data=f"pvp_action_{battle_id}_{user_id}_attack"),
        InlineKeyboardButton("🛡️ Защититься", callback_data=f"pvp_action_{battle_id}_{user_id}_defend"),
    ]])

def _pvp_edit(b, text, kb=None):
    """Редактировать сообщение боя через bot. Логирует ошибки."""
    try:
        if kb:
            bot.edit_message_text(
        ...
        print(f"[pvp] edit_message_text ошибка: {e}")

# ── Вызов ─────────────────────────────────────────────────────

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower().startswith("пвп") and m.chat.type in ("group", "supergroup"))
def handle_pvp_challenge(message):
    ...
    # Команда "пвп стат" / "пвп статистика" — передаём дальше
    if len(parts) >= 2 and parts[1].lower() in ("стат", "статистика"):
        return _pvp_send_stats(message)

    if len(parts) < 2:
        bot.send_message(message.chat.id,
            "⚔️ <b>ПВП</b>\nИспользование: <code>пвп @игрок [ставка]</code>",
            parse_mode="HTML")
        return

    # Проверить что нет активного боя
    ...
    # Создаём бой и резервируем ставку
    ...
    text = (
        f"⚔️ <b>— В Ы З О В   Н А   П В П —</b>\n\n"
        f"🔴 <b>{a_name}</b>  бросает вызов  🔵 <b>{d_name}</b>\n\n"
        f"🔴 {a_name}\n"
        f"   ⚔️ {a_stats['damage']} атк  •  🛡 {a_stats['defense']} защ\n\n"
        f"🔵 {d_name}\n"
        f"   ⚔️ {d_stats['damage']} атк  •  🛡 {d_stats['defense']} защ\n"
    )
    if bet > 0:
        text += f"\n💰 Ставка: <b>{format_balance(bet)}</b> с каждого\n"
    text += f"\n<b>{d_name}</b>, принимаешь бой?"

    kb = InlineKeyboardMarkup(keyboard=[[
        InlineKeyboardButton("✅ Принять",   callback_data=f"pvp_accept_{battle_id}"),
        InlineKeyboardButton("❌ Отклонить", callback_data=f"pvp_decline_{battle_id}"),
    ]])
...
def cb_pvp_accept(call):
    ...
    if b["bet"] > 0:
        if get_balance(uid) < b["bet"]:
            bot.answer_callback_query(
                call.id,
                f"❌ Недостаточно монет! Нужно {format_balance(b['bet'])}",
                show_alert=True
            )
            return
    ...
    bot.answer_callback_query(call.id, "⚔️ Бой начался!")
...
def cb_pvp_action(call):
    uid = call.from_user.id
    # format: pvp_action_{battle_id}_{expected_uid}_{action}
    try:
        parts = call.data.split("_")
        # pvp_action_<id>_<uid>_<action>  →  parts = ['pvp','action',id,uid,action]
    ...
    if action == "defend":
        if is_attacker:
            _pvp_update(battle_id, attacker_blocked=1,
                        whose_turn=opponent_id, last_action=int(time.time()))
        else:
            _pvp_update(battle_id, defender_blocked=1,
                        whose_turn=opponent_id, last_action=int(time.time()))
        actor_name = _mkt_user_name_by_id(uid)
        event = f"🛡️ <b>{actor_name}</b> уходит в защиту!"

    elif action == "attack":
        stats_atk = _eq_calc_stats(uid)
        stats_def = _eq_calc_stats(opponent_id)
        base = random.randint(PVP_BASE_DMG_MIN, PVP_BASE_DMG_MAX)
        raw_dmg = base + stats_atk["damage"]
        reduced = max(1, raw_dmg - stats_def["defense"])

        opp_blocked = b["defender_blocked"] if is_attacker else b["attacker_blocked"]
        if opp_blocked:
            reduced = max(1, int(reduced * (1 - PVP_BLOCK_REDUCTION)))
            if is_attacker:
                _pvp_update(battle_id, defender_blocked=0)
            else:
                _pvp_update(battle_id, attacker_blocked=0)

        actor_name = _mkt_user_name_by_id(uid)
        block_note = " (🛡️ блок!)" if opp_blocked else ""
        event = f"⚔️ <b>{actor_name}</b> атакует — <b>{reduced}</b> урона{block_note}"

        if is_attacker:
            defender_hp = max(0, defender_hp - reduced)
            _pvp_update(battle_id, defender_hp=defender_hp,
                        attacker_blocked=0,
                        whose_turn=opponent_id, last_action=int(time.time()))
        else:
            attacker_hp = max(0, attacker_hp - reduced)
            _pvp_update(battle_id, attacker_hp=attacker_hp,
                        defender_blocked=0,
                        whose_turn=opponent_id, last_action=int(time.time()))
    else:
        bot.answer_callback_query(call.id, "❌")
