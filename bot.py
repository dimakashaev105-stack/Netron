...
CRASH_ADMIN_CHAT = -1003520330846  # Чат для решений админа
# Если еще нет
...
def handle_deposit_command(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка! Используйте: вклад [сумма]", parse_mode='HTML')
...
def handle_withdraw_command(message):
    ...
    if deposit_amount <= 0:
        bot.send_message(message.chat.id, "💀 На вкладе нет денег!", parse_mode='HTML')
        return
    ...
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка! Используйте: снять [сумма] или снять все", parse_mode='HTML')
...
def handle_loan_command(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка! Используйте: займ [сумма]", parse_mode='HTML')
...
def handle_repay_loan_command(message):
    ...
    if not loan_info:
        bot.send_message(message.chat.id, "🏆 У вас нет активных займов", parse_mode='HTML')
        return
    ...
    if len(parts) >= 3:
        ...
        if amount_text.lower() in ['все', 'all']:
            ...
            if repay_amount > amount_due:
                bot.send_message(message.chat.id, f"💀 Сумма больше долга! Долг: {format_balance(amount_due)}", parse_mode='HTML')
                return
    else:
...
@bot.message_handler(func=lambda message: message.text in ["🏛 Банк", "Банк"])
def handle_bank(message):
    ...
    try:
        with open('bank.jpg', 'rb') as photo:
            caption = f"🏛 <b>Банк</b>\n\n"
            caption += f"💵 Баланс: {format_balance(balance)}\n"
            caption += f"💳 На вкладе: {format_balance(bank_deposit)}\n\n"
            
            if has_loan:
                caption += f"⚠️ <b>Активный займ</b>\n"
                caption += f"Долг: {format_balance(loan_details['amount_due'])}\n\n"
            else:
                caption += f"✅ Банк работает\n\n"
            
            caption += f"Вклады — 0.01% в час · Займы до {format_balance(LOAN_CONFIG['max_loan'])}"
        ...
        bank_text += f"💵 Баланс: {format_balance(balance)}\n"
        bank_text += f"💳 На вкладе: {format_balance(bank_deposit)}\n\n"
        
        if has_loan:
            bank_text += f"⚠️ <b>Активный займ</b>\n"
            bank_text += f"Долг: {format_balance(loan_details['amount_due'])}\n\n"
        else:
            bank_text += f"✅ Банк работает\n\n"
        
        bank_text += f"Вклады — 0.01% в час · Займы до {format_balance(LOAN_CONFIG['max_loan'])}"
...
def handle_deposit_money(message):
    ...
    if balance <= 0:
        bot.send_message(message.chat.id, "💀 У вас нет денег для вклада!", parse_mode='HTML')
        return
...
def handle_withdraw_deposit(message):
    ...
    if deposit_amount <= 0:
        bot.send_message(message.chat.id, "💀 У вас нет денег на вкладе!", parse_mode='HTML')
        return
...
def handle_document_upload(message):
    ...
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "💀 У вас нет прав для этой команды", parse_mode='HTML')
        return
    ...
    try:
        ...
        if not file_name.lower().endswith('.db'):
            bot.reply_to(message, "💀 Ошибка: отправьте файл с расширением .db", parse_mode='HTML')
            return
...
def handle_case(message):
    ...
    try:
        ...
        if balance < total_cost:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно {format_balance(total_cost)}", parse_mode='HTML')
            return
        
        markup = InlineKeyboardMarkup()
        if quantity == 1:
            markup.add(
                InlineKeyboardButton(f"🏆 Купить 1 кейс за {plain_balance(total_cost)}", callback_data=f"buy_case_{quantity}"),
                InlineKeyboardButton("💀 Отмена", callback_data="buy_case_cancel")
            )
        else:
            markup.add(
                InlineKeyboardButton(f"🏆 Купить {quantity} кейсов за {plain_balance(total_cost)}", callback_data=f"buy_case_{quantity}"),
                InlineKeyboardButton("💀 Отмена", callback_data="buy_case_cancel")
            )
...
def handle_sell(message):
    ...
    if not components:
        bot.send_message(message.chat.id, "💀 В мешке нет компонентов", parse_mode='HTML')
        return
    ...
    if text == 'сбыть все':
        ...
        bot.send_message(message.chat.id, f"🏆 Продано все за {format_balance(total_income)}", parse_mode='HTML')
        return
    
    parts = text.split()
    if len(parts) < 2:
        bot.send_message(message.chat.id, "💀 Используй: сбыть [номер] или сбыть все", parse_mode='HTML')
        return
    
    try:
        index = int(parts[1]) - 1
        if index < 0 or index >= len(components):
            bot.send_message(message.chat.id, "💀 Неверный номер компонента", parse_mode='HTML')
            return
        ...
        bot.send_message(message.chat.id, f"🏆 Продан {component_name} за {format_balance(price)}", parse_mode='HTML')
...
def handle_user_create_check(message):
    ...
    if is_user_warned(user_id):
        bot.send_message(message.chat.id, "💀 Вы не можете создавать чеки, так как у вас активный варн!", parse_mode='HTML')
        return
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           ...
                           "• выпустить ваучер 500000 3 mypass - чек на 500к с паролем", parse_mode='HTML')
            return
        ...
        if max_activations <= 0:
            bot.send_message(message.chat.id, "💀 Количество активаций должен быть больше 0!", parse_mode='HTML')
            return
        ...
        if total_amount > balance:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно: {format_balance(total_amount)}, твой баланс: {format_balance(balance)}", parse_mode='HTML')
            return
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при создании чека!", parse_mode='HTML')
...
def handle_cheque_command(message):
    try:
        ...
        if not is_admin(message.from_user.id):
            bot.send_message(message.chat.id, "💀 Только админы могут создавать чеки", parse_mode='HTML')
            return
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           ...
                           "/чек 5000000 5 secret - чек на 5М с паролем", parse_mode='HTML')
            return
        ...
        try:
            ...
            if max_activations <= 0:
                bot.send_message(message.chat.id, "💀 Количество активаций должно быть больше 0!", parse_mode='HTML')
                return
        except ValueError:
            bot.send_message(message.chat.id, "💀 Неверное количество активаций!", parse_mode='HTML')
            return
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           ...
                           "• чек 5000000 @username secret123 - чек на 5М для @username с паролем", parse_mode='HTML')
            return
        ...
        if parts[2].startswith('@'):
            ...
            try:
                ...
                bot.send_message(message.chat.id, "💀 Неверный формат! Укажите число активаций или юзернейм с @", parse_mode='HTML')
                return
...
def handle_daily_bonus(message):
    try:
        ...
        bonus_text = (
            ...
            f"<tg-emoji emoji-id='5929466926408404919'>⏱</tg-emoji> Каждые 15 мин"
        )
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, 
                           ...
                           "Примеры:\n"
                           "• чек 1000000 10 - чек на 1М, 10 активаций\n"
                           "• чек 5000000 5 secret123 - чек на 5М с паролем", parse_mode='HTML')
            return
...
def handle_experience(message):
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при получении информации об очкие", parse_mode='HTML')
# Система налогов
TAX_CONFIG = {
...
def handle_confirm_tax_collection(call):
    ...
    try:
        ...
        if result['success']:
            ...
            bot.edit_message_text(f"💀 Ошибка при сборе налогов: {result['error']}", call.message.chat.id, call.message.message_id, parse_mode='HTML')
            bot.answer_callback_query(call.id, "💀 Ошибка!")
...
def handle_cancel_tax_collection(call):
    ...
    bot.edit_message_text("💀 Сбор налогов сброшен", call.message.chat.id, call.message.message_id, parse_mode='HTML')
    bot.answer_callback_query(call.id, "🏆 Отменено")
...
def handle_transfer(message):
    try:
        ...
        if is_user_warned(user_id):
            bot.send_message(message.chat.id, "💀 Вы не можете транзакцияить деньги, так как у вас активный варн!", parse_mode='HTML')
            return
        ...
        if user_balance < amount:
            bot.send_message(message.chat.id, f"💀 Не хватает монет! Нужно: {format_balance(amount)}", parse_mode='HTML')
            return
        ...
        if isinstance(target_identifier, int):
            ...
            if not target_info:
                bot.send_message(message.chat.id, f"💀 Пользователь {target_identifier} не найден!", parse_mode='HTML')
                return
            target_user_id = target_info['user_id']
        
        if target_user_id == user_id:
            bot.send_message(message.chat.id, "💀 Нельзя перевести деньги самому себе", parse_mode='HTML')
            return
        ...
        bot.send_message(message.chat.id, "💀 Ошибка при обработке транзакцияа", parse_mode='HTML')
...
def handle_confirm_transfer(call):
    try:
        ...
        if success:
            ...
            try:
                bot.send_message(target_user_id, f"💸 Вам перевели {format_balance(receive_amount)}", parse_mode='HTML')
            except:
                pass
...
def handle_cancel_transfer(call):
    bot.edit_message_text("💀 Перевод сброшен", call.message.chat.id, call.message.message_id, parse_mode='HTML')
    bot.answer_callback_query(call.id, "Отменено")
...
def crash_command(message):
    ...
    try:
        ...
        if bet_amount is None:
            bot.send_message(message.chat.id, "💀 Неверный формат суммы", parse_mode='HTML')
            return
        
        # Минимальная бет
        if bet_amount < 50:
            bot.send_message(message.chat.id, f"💵 Мин. бет: 50$", parse_mode='HTML')
            return
...
def send_crash_result(game_id, admin_decision):
    ...
    if admin_decision is None:
        # Динамический шанс: 1.1x=~40%, 2x=~29%, 5x=~17%, 10x=~12%
...
@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() == 'пиздаэко' and is_admin(m.from_user.id))
def handle_eco_reset(message):
    with get_db_cursor() as cursor:
        cursor.execute('SELECT user_id, username, first_name, balance, bank_deposit, deposit FROM users')
    ...
    affected_1kkkk = []
    negodyai = []
    MAX_TOTAL = 1_000_000  # Кап 1кк после чистки

    for row in all_users:
        user_id, username, first_name, balance, bank_deposit, deposit = row
        balance = balance or 0
        bank_deposit = bank_deposit or 0
        deposit = deposit or 0
        total = balance + bank_deposit + deposit
        name = f"@{username}" if username else (first_name or f"id{user_id}")
        new_balance = balance
        new_bank = bank_deposit
        new_dep = deposit
        changed = False

        if total >= 10_000_000_000_000:
            new_balance = balance // 100_000_000
            new_bank = bank_deposit // 100_000_000
            new_dep = deposit // 100_000_000
            affected_1kkkk.append((user_id, name, total))
            negodyai.append((user_id, name, total))
            changed = True
        elif total >= 1_000_000_000_000:
            new_balance = balance // 1_000_000
            new_bank = bank_deposit // 1_000_000
            new_dep = deposit // 1_000_000
            affected_1kkk.append((user_id, name, total))
            negodyai.append((user_id, name, total))
            changed = True
        elif total >= 10_000_000:
            new_balance = balance // 1_000
            new_bank = bank_deposit // 1_000
            new_dep = deposit // 1_000
            affected_10kk.append((user_id, name, total))
            negodyai.append((user_id, name, total))
            changed = True

        final_total = new_balance + new_bank + new_dep
        if final_total > MAX_TOTAL:
            ratio = MAX_TOTAL / final_total
            new_balance = int(new_balance * ratio)
            new_bank = int(new_bank * ratio)
            new_dep = int(new_dep * ratio)
            changed = True

        if changed:
            with get_db_cursor() as cursor:
                cursor.execute(
                    'UPDATE users SET balance = ?, bank_deposit = ?, deposit = ?, video_cards = 0 WHERE user_id = ?',
                    (new_balance, new_bank, new_dep, user_id)
                )
        else:
            with get_db_cursor() as cursor:
    ...
    for uid, name, total in negodyai:
        try:
            bot.send_message(uid,
                f"🚨 <b>ВНИМАНИЕ! ЭКОНОМИЧЕСКАЯ РЕФОРМА!</b>\n\n"
                f"Ты накопил слишком много бабла, НЕГОДЯЙ 😤\n"
                f"Твой баланс был обрезан в рамках антиинфляционной чистки!\n\n"
                f"💸 Экономика должна жить, а не у тебя в кармане!\n"
                f"🃏 Видеокарты тоже обнулены, сорян 😂",
                parse_mode='HTML')
        except Exception:
            pass

    report = (
        f"✅ <b>ПИЗДАЭКО ВЫПОЛНЕНО!</b>\n\n"
        f"🔴 <b>Срезано 000</b> (>10кк): {len(affected_10kk)} чел.\n"
        f"🔴 <b>Срезано 000 000</b> (>1ккк): {len(affected_1kkk)} чел.\n"
        f"🔴 <b>Срезано 000 000 0</b> (>1кккк): {len(affected_1kkkk)} чел.\n"
        f"💾 <b>Видеокарты:</b> обнулены у всех\n"
        f"🗑 <b>Чеки удалены:</b> {checks_count} шт.\n"
        f"📉 <b>Кап после чистки:</b> 1кк суммарно\n\n"
        f"😈 <b>Негодяям отправлено:</b> {len(negodyai)} чел.\n\n"
    )
    if negodyai:
        report += "👥 <b>Список негодяев:</b>\n"
        for uid, name, total in negodyai[:20]:
            report += f"• {name} — {total:,}🌸\n"
        if len(negodyai) > 20:
            report += f"...и ещё {len(negodyai) - 20} чел."
