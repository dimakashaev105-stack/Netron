...
# ── Turso HTTP API (без внешних библиотек) ───────────────────
# Работает через встроенный urllib — ничего устанавливать не надо.
# Если TURSO_URL не задан — используем локальный SQLite.
...
if _USE_TURSO:
    ...
    print(f"[db] Turso HTTP mode: {_TURSO_HTTP[:50]}...")
else:
    print("[db] SQLite mode (локальный файл)")

# ── Turso HTTP курсор ─────────────────────────────────────────

class _TursoCursor:
    """Минимальный cursor-like объект поверх Turso HTTP API."""

    def __init__(self):
        self._pending  = []   # запросы накапливаем до commit
        self.description = None
        self._rows     = []
        self._pos      = 0

    def execute(self, sql, params=()):
    # Подставляем параметры вручную (Turso принимает named args)
    stmt = {"sql": sql, "args": [_turso_val(p) for p in params]}
    self._pending.append(stmt)
    # Для SELECT сразу выполняем чтобы вернуть данные
    if sql.strip().upper().startswith("SELECT") or \
       sql.strip().upper().startswith("PRAGMA"):
        result = _turso_exec([stmt])
        rows_data = result[0] if result else {}
        cols = [c["name"] for c in rows_data.get("cols", [])]
        self.description = [(c, None, None, None, None, None, None) for c in cols]
        
        # ИСПРАВЛЕНИЕ: правильно обрабатываем строки результата
        rows_raw = rows_data.get("rows", [])
        self._rows = []
        for r in rows_raw:
            # Если r — список, берём как есть
            if isinstance(r, list):
                values = r
            else:
                # Если r — словарь, извлекаем значения
                values = [v.get("value") if isinstance(v, dict) else v for v in r.get("values", [])]
            self._rows.append(_TursoRow(cols, values))
        
        self._pos  = 0
        self._pending = []  # SELECT не накапливаем
    return self
    ...
    def _flush(self):
        """Отправить все накопленные write-запросы."""
        if self._pending:
            _turso_exec(self._pending)
            self._pending = []
...
class _TursoRow:
    """Имитирует sqlite3.Row — доступ по индексу и по имени колонки."""
    def __init__(self, cols, values):
        self._cols = cols
        # values уже список значений, не нужно повторно обрабатывать
        self._vals = values
...
def _turso_exec(stmts, retries=3):
    """Отправить список запросов в Turso, вернуть список результатов."""
    requests_payload = [{"type": "execute", "stmt": s} for s in stmts]
    requests_payload.append({"type": "close"})
    body = _json.dumps({"requests": requests_payload}).encode("utf-8")
    headers = {
    ...
    for attempt in range(retries):
        try:
            req  = _urllib_req.Request(_TURSO_HTTP, data=body, headers=headers, method="POST")
            ...
            for r in data.get("results", []):
                if r.get("type") == "error":
                    ...
                    if "already exists" not in msg and "UNIQUE" not in msg:
                        print(f"[turso] SQL error: {msg}")
                elif r.get("type") == "ok":
                    results.append(r.get("response", {}).get("result", {}))
            return results
        except Exception as e:
            print(f"[turso] attempt {attempt+1} error: {e}")
            time.sleep(1)
    return []


class _TursoConn:
    """Имитирует sqlite3.Connection поверх Turso HTTP."""
    def __init__(self):
        self._cursor = _TursoCursor()
...
def get_db_cursor():
    with get_db_connection() as conn:
        ...
        try:
            ...
            conn.rollback()
            _msg = str(_dbe)
            if _USE_TURSO and any(x in _msg for x in ("duplicate column", "already exists", "UNIQUE")):
                pass
            else:
                raise
...
# Ликвидность рынка: сколько акций нужно купить/продать чтобы
# сдвинуть цену на 1%. Меньше значение = более волатильный рынок.
# При 500 акциях сдвиг ~2.5%, при 1000 акциях ~4.5%, при 5000 ~14%
STOCK_LIQUIDITY   = 400           # "глубина стакана" (базовый объём за 1%)

# Режимы волатильности рынка (переключаются автоматически)
STOCK_VOL_NORMAL  = 0.008         # обычный шум ±0.8% за период
STOCK_VOL_HIGH    = 0.02          # повышенная волатильность ±2%
STOCK_VOL_CRISIS  = 0.04          # кризис ±4%
STOCK_DRIFT_BASE  = 0.002         # небольшой upward bias
...
def _apply_trade_impact(qty, is_buy):
    ...
    Модель ликвидности: impact = qty / (qty + STOCK_LIQUIDITY) * max_impact

    Примеры при STOCK_LIQUIDITY=200:
      10  акций → ~0.24% сдвиг
      50  акций → ~1.1%
      100 акций → ~2.0%
      200 акций → ~3.5%
      500 акций → ~6.5%
     1000 акций → ~9.5%
     5000 акций → ~17%  (максимум — китовая сделка)

    Без жёсткого кепа — большие покупки реально двигают рынок.
    """
    price, _ = _get_stock_price()

    # Нелинейный impact: логистическое насыщение без жёсткого ограничения
    # Каждые STOCK_LIQUIDITY акций = +~3.5% дополнительного влияния
    impact_pct = qty / (qty + STOCK_LIQUIDITY) * 0.10  # max ~10% при огромных объёмах

    # Небольшой случайный шум ±0.3% (слиппедж)
    slippage = random.gauss(0, 0.003)

    total_impact = impact_pct + slippage if is_buy else -(impact_pct - slippage)
...
# ── Константы ────────────────────────────────────────────────
CLAN_CREATE_COST   = 1_000_000   # цена создания клана
CLAN_MAX_MEMBERS   = 30          # макс участников
CLAN_WAR_DURATION  = 86400       # длительность войны (24ч)
CLAN_WAR_COST      = 50_000      # взнос в казну за объявление войны
CLAN_WAR_WIN_PCT   = 0.20        # победитель забирает 20% казны проигравшего
CLAN_WEEKLY_PRIZES = [500_000, 250_000, 100_000]  # призы топ-3
    ...
    with get_db_cursor() as cur:
        ...
        cur.execute('''
            CREATE TABLE IF NOT EXISTS clan_war_snapshots (
                ...
                balance_end   INTEGER DEFAULT NULL,
        ...
        ''')
        # Добавляем колонку war_score в clans если нет
        cur.execute("PRAGMA table_info(clans)")
        cols = [r[1] for r in cur.fetchall()]
        if "war_wins" not in cols:
            cur.execute("ALTER TABLE clans ADD COLUMN war_wins INTEGER DEFAULT 0")
        if "war_losses" not in cols:
            cur.execute("ALTER TABLE clans ADD COLUMN war_losses INTEGER DEFAULT 0")
        if "war_score" not in cols:
            cur.execute("ALTER TABLE clans ADD COLUMN war_score INTEGER DEFAULT 0")
...
@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "клан")
...
def handle_join_clan(message):
    ...
    if _get_member_count(clan['id']) >= CLAN_MAX_MEMBERS:
...
def handle_accept_app(message):
    ...
    with get_db_cursor() as cur:
        ...
        if _get_member_count(cid) >= CLAN_MAX_MEMBERS:
    ...
    bot.send_message(message.chat.id,
        f"✅ <b>{name_str}</b> принят в клан <b>[{clan['tag']}]</b>!",
        parse_mode="HTML")
    try:
        bot.send_message(target_uid,
            f"🎉 Твоя заявка в клан <b>[{clan['tag']}] {clan['name']}</b> одобрена!\n"
    ...
    if clan['role'] == 'owner':
        ...
        if len(members) > 1:
            bot.send_message(message.chat.id,
                ...
                "<code>клан передать @username</code>\n"
...
def handle_clan_deposit(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"💰 Казна теперь: <b>{format_balance(clan['balance'] + amount)}</b>",
        parse_mode="HTML")
...
def handle_declare_war(message):
    ...
    if len(parts) < 3:
        bot.send_message(message.chat.id,
            "❌ Формат: <code>клан война ТЕГ</code>", parse_mode="HTML")
        return
    ...
    mk = InlineKeyboardMarkup()
    mk.add(
        InlineKeyboardButton(f"⚔️ Объявить войну [{enemy_tag}]",
    ...
    bot.send_message(message.chat.id,
        ...
        f"Атакующий: <b>[{clan['tag']}] {clan['name']}</b>\n"
        f"Враг: <b>[{enemy['tag']}] {enemy['name']}</b>\n\n"
        f"💰 Взнос из казны: <b>{format_balance(CLAN_WAR_COST)}</b>\n"
        f"⏱ Длительность: <b>24 часа</b>\n\n"
        f"Победитель определяется по суммарному приросту\n"
        f"баланса всех участников за время войны.\n"
        f"Победивший клан заберёт <b>{int(CLAN_WAR_WIN_PCT*100)}%</b> казны проигравшего.",
        reply_markup=mk,
        parse_mode="HTML"
    )
...
def cb_war_declare(call):
    uid = call.from_user.id
    _, _, _, att_id, def_id = call.data.split("_")
    ...
    if _active_war(att_id) or _active_war(def_id):
    ...
    with get_db_cursor() as cur:
        # Списываем взнос из казны атакующего
        ...
        cur.execute(
            ...
            "VALUES (?,?,?,?,'active',?)",
            (att_id, def_id, now, ends, CLAN_WAR_COST)
        )
        war_id = cur.lastrowid

        # Снапшот балансов всех участников обоих кланов
        cur.execute(
            "SELECT cm.user_id, cm.clan_id, u.balance FROM clan_members cm "
            "JOIN users u ON u.user_id=cm.user_id WHERE cm.clan_id IN (?,?)",
            (att_id, def_id)
        )
        for row in cur.fetchall():
            cur.execute(
                "INSERT INTO clan_war_snapshots (war_id, user_id, clan_id, balance_start) VALUES (?,?,?,?)",
                (war_id, row[0], row[1], row[2])
            )
    ...
    bot.edit_message_text(
        ...
        f"⚔️ <b>[{attacker['tag']}]</b> vs <b>[{defender['tag']}]</b>\n"
        f"⏱ Конец войны: <b>{ends_str}</b>\n\n"
        f"💪 Зарабатывайте как можно больше — побеждает клан\n"
        f"с наибольшим суммарным приростом баланса!",
        call.message.chat.id, call.message.message_id, parse_mode="HTML"
    )
    bot.answer_callback_query(call.id)

    # Уведомляем защищающийся клан
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT user_id FROM clan_members WHERE clan_id=? AND role IN ('owner','officer')",
            (def_id,)
    ...
    for mid in managers:
        try:
            bot.send_message(mid,
                ...
                f"Война продлится 24 часа до {ends_str}\n"
                f"Зарабатывайте монеты — побеждает клан с лучшим приростом баланса!\n\n"
                f"Приз победителю: {int(CLAN_WAR_WIN_PCT*100)}% казны проигравшего",
                parse_mode="HTML")
        except Exception:
            pass
...
# ── Завершение войн (планировщик) ────────────────────────────

def _resolve_wars():
    """Проверяет и завершает истёкшие войны."""
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute(
    ...
    for war in wars:
        try:
            ...
            print(f"[clan_war] ошибка завершения войны {war['id']}: {e}")
...
def _finish_war(war):
    ...
    # Берём снапшоты начала
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT s.user_id, s.clan_id, s.balance_start, u.balance as balance_now "
            "FROM clan_war_snapshots s JOIN users u ON u.user_id=s.user_id "
            "WHERE s.war_id=?",
            (war_id,)
        )
        snaps = [dict(r) for r in cur.fetchall()]

    # Считаем прирост по каждому клану
    att_gain = sum(max(0, s['balance_now'] - s['balance_start']) for s in snaps if s['clan_id'] == att_id)
    def_gain = sum(max(0, s['balance_now'] - s['balance_start']) for s in snaps if s['clan_id'] == def_id)

    attacker = _get_clan_by_id(att_id)
    defender = _get_clan_by_id(def_id)

    if att_gain >= def_gain:
    ...
    # Приз — 20% казны проигравшего
    prize = int(loser['balance'] * CLAN_WAR_WIN_PCT)

    with get_db_cursor() as cur:
        cur.execute("UPDATE clan_wars SET status='finished', winner_id=?, prize=? WHERE id=?",
    ...
    # Уведомление обоим кланам
    is_draw = att_gain == def_gain
    result_text = (
        f"⚔️ <b>Война завершена!</b>\n\n"
        f"[{attacker['tag']}] прирост: <b>{format_balance(att_gain)}</b>\n"
        f"[{defender['tag']}] прирост: <b>{format_balance(def_gain)}</b>\n\n"
    )
    if is_draw:
    ...
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT user_id FROM clan_members WHERE clan_id IN (?,?)", (att_id, def_id)
        )
...
def cb_war_menu(call):
    ...
    if war:
        ...
        with get_db_cursor() as cur:
            cur.execute(
                "SELECT s.clan_id, s.balance_start, u.balance FROM clan_war_snapshots s "
                "JOIN users u ON u.user_id=s.user_id WHERE s.war_id=?",
                (war['id'],)
            )
            snaps = cur.fetchall()

        att_gain = sum(max(0, r[2]-r[1]) for r in snaps if r[0] == war['attacker_id'])
        def_gain = sum(max(0, r[2]-r[1]) for r in snaps if r[0] == war['defender_id'])
        our_gain = att_gain if war['attacker_id'] == clan['id'] else def_gain
        their_gain = def_gain if war['attacker_id'] == clan['id'] else att_gain

        text = (
            f"⚔️ <b>Война: [{clan['tag']}] vs [{enemy['tag']}]</b>\n\n"
            f"⏱ До конца: {ends_str}\n\n"
            f"📈 Наш прирост: <b>{format_balance(our_gain)}</b>\n"
            f"📉 Их прирост: <b>{format_balance(their_gain)}</b>\n\n"
            f"{'🟢 Мы лидируем!' if our_gain >= their_gain else '🔴 Мы проигрываем!'}"
        )
    else:
        text = (
            f"⚔️ <b>Клановые войны</b>\n\n"
            f"Твой клан сейчас не воюет.\n\n"
            f"Чтобы объявить войну:\n"
            f"<code>клан война ТЕГ</code>\n\n"
            f"💰 Стоимость: <b>{format_balance(CLAN_WAR_COST)}</b> из казны\n"
            f"🏆 Победитель забирает {int(CLAN_WAR_WIN_PCT*100)}% казны врага"
        )
...
def cb_clan_manage(call):
    ...
    mk.add(
        ...
        InlineKeyboardButton("◀️ Назад",              callback_data="clan_back"),
    )
    bot.edit_message_text(
        f"⚙️ <b>Управление [{clan['tag']}]</b>\n\n"
        f"Команды:\n"
