...
# ── База данных: только локальный SQLite ─────────────────────
# Храним game.db локально на Render (персистентный диск или /data)
...
STOCK_LIQUIDITY   = 2000          # "глубина стакана"

# Режимы волатильности рынка (переключаются автоматически)
STOCK_VOL_NORMAL  = 0.005         # обычный шум ±0.5% за период
STOCK_VOL_HIGH    = 0.012         # повышенная волатильность ±1.2%
STOCK_VOL_CRISIS  = 0.025         # кризис ±2.5%
...
def _apply_trade_impact(qty, is_buy):
    ...
    # Небольшой случайный шум ±0.15% (слиппедж)
    slippage = random.gauss(0, 0.0015)

    if is_buy:
        total_impact = impact_pct + slippage
    else:
        total_impact = -(impact_pct - slippage)
