...
MINE_COOLDOWN         = 5 * 60   # 5 минут — общая шахта
MINE_COOLDOWN_PRIVATE = 1 * 60   # 1 минута — частная шахта
...
def _mine_cd_left(user_id, cooldown=None):
    if cooldown is None:
        if _mine_rental(user_id):
            cooldown = MINE_COOLDOWN_PRIVATE
        else:
            pk_lvl = _pickaxe_get(user_id)
            cooldown = PICKAXE_LEVELS[pk_lvl]["cooldown"]
    with get_db_cursor() as cur:
...
def handle_mine_dig_text(message):
    ...
    flower    = "<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji>"
    pk_emoji  = "<tg-emoji emoji-id='" + str(pk_info['emoji_id']) + "'>⛏</tg-emoji>"
    res_emoji = "<tg-emoji emoji-id='" + str(resource['emoji_id']) + "'>●</tg-emoji>"
    try:
        if event["mult"] == 0.0:
            bot.send_message(message.chat.id,
        ...
        print(f"[mine_dig] ошибка отправки: {e}")
        bot.send_message(message.chat.id, "⛏️ Копание прошло успешно!")
...
def cb_mkt_sell_pick(call):
    ...
    if not inv:
        ...
        return
    items = [(rid, amt) for rid, amt in inv.items() if _res_by_id(rid)]
    rows = []
    for i in range(0, len(items), 2):
        row = []
        for rid, amt in items[i:i+2]:
            r = _res_by_id(rid)
            has_order = "✅ " if _sell_order_get(uid, rid) else ""
            row.append({
                "text": f"{has_order}{r['name']} {amt}шт",
