...
# Кулдаун частной шахты зависит от уровня шахты (мин → от 9 до 5 минут)
...
def _mine_resource(bonus=1.0, rare_boost=0.0):
    if rare_boost > 0:
        ...
        for r in MINE_RESOURCES:
            r2 = dict(r)
            if r2["chance"] < 5:
                r2["chance"] = r2["chance"] * (1 + rare_boost)
            resources.append(r2)
    else:
    ...
    for r in resources:
        ...
        if roll <= cum:
            ...
            return r, reward
    return resources[0], int(resources[0]["price"] * bonus)

def _get_cooldown(user_id):
    """Возвращает кулдаун для пользователя: частная шахта по уровню, общая по кирке."""
    rental = _mine_rental(user_id)
    if rental:
        mine = _mine_by_id(rental[0])
        lvl = mine["level"] if mine else 1
        return MINE_COOLDOWN_PRIVATE_BY_LEVEL.get(lvl, 9 * 60)
    else:
        pk_lvl = _pickaxe_get(user_id)
        return PICKAXE_LEVELS[pk_lvl]["cooldown"]
