...
def _build_donate_text():
    ...
    for key, pkg in DONATE_PACKAGES.items():
        ...
        lines.append(f"<tg-emoji emoji-id=\'5204347567759956677\'><tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji></tg-emoji> <b>{pkg['stars']} зв</b> → {amt_str} 🌸")
    return "\n".join(lines)
...
def handle_buy(message):
    ...
    disc_line = f"\n\n<blockquote><tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> У тебя скидка <b>{discount}%</b> на следующую покупку</blockquote>" if discount else ""
    bot.send_message(
        message.chat.id,
        f"<tg-emoji emoji-id='5204347567759956677'><tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji></tg-emoji> <b>Пополнение баланса</b>\n\n"
...
def activate_check(user_id, ref_code, amount, max_activations, current_activations, password, target_username):
    ...
    return True, f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> <b>Вы активировали чек на {format_balance(amount)}</b> <tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji>!", created_by
