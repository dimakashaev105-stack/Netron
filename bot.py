...
# Читаем код бота один раз при запуске — Gemini будет знать всё о боте
try:
    with open(__file__, "r", encoding="utf-8") as _f:
        _BOT_SOURCE = _f.read()
    # Обрезаем до 60к символов чтобы не превысить лимит токенов
    if len(_BOT_SOURCE) > 60000:
        _BOT_SOURCE = _BOT_SOURCE[:60000] + "\n... (код обрезан)"
except Exception:
    _BOT_SOURCE = "(код недоступен)"

_GEMINI_SYSTEM = """Ты — умный помощник бота Fectiz. 
Ты знаешь весь код этого бота и можешь отвечать на вопросы по нему.
Отвечай кратко, по-русски, без лишней воды.
Если вопрос про команды или механики бота — отвечай точно по коду.
Если вопрос общий — отвечай как умный ассистент.
