...
def show_taxi_menu(chat_id, user_id):
    ...
    text = (
        ...
        f"📊 Поездок: <b>{trips}</b>   💰 Заработано: <b>{format_balance(earned)}</b>\n"
        f"{status_line}"
        f"{progress_line}"
    )
...
def _show_taxi_stats_inline(chat_id, user_id):
    ...
    text = (
        ...
        f"💰 Заработано: <b>{format_balance(stats['total_earned'])}</b>\n"
        f"💵 Средний чек: <b>{format_balance(avg)}</b>\n"
    )
    if active:
...
def handle_clicker(message):
    ...
    bot.send_message(message.chat.id, "⚔️ Найди правильную кнопку:", reply_markup=create_clicker_keyboard(message.from_user.id), parse_mode='HTML')
            ...
            if new_level > old_level:
                try:
                    ...
                    bot.send_message(user_id,
                        ...
                        f"🎁 Бонус к ежедневке: +{daily_b}%", parse_mode='HTML')
                except Exception as e:
...
# ═══════════════════════════════════════════════════════════════
# 🖥️ ВИДЕОКАРТЫ — пассивный доход за редкие ресурсы
# Макс 3 GPU на игрока. Работают пока не сломаются (wear).
# Начисление каждые 30 минут автоматически.
# ═══════════════════════════════════════════════════════════════
...
GPU_MODELS = {
    "rx580": {
        "name": "RX 580",
        "emoji": "🟧",
        "cost": {"redstone": 5, "lapis": 3},
        "income_per_tick": 800,       # монет каждые 30 мин
        "max_wear": 100,              # зарядов до поломки
        "income_type": "balance",
    },
    "rtx3060": {
        "name": "RTX 3060",
        "emoji": "🟩",
        "cost": {"redstone": 10, "lapis": 8, "emerald": 3},
        "income_per_tick": 2500,
        "max_wear": 120,
        "income_type": "balance",
    },
    "rtx4090": {
        "name": "RTX 4090",
        "emoji": "🟦",
        "cost": {"redstone": 20, "lapis": 15, "emerald": 10},
        "income_per_tick": 8000,
        "max_wear": 150,
        "income_type": "balance",
    },
}
GPU_TICK_SECONDS = 30 * 60   # раз в 30 минут
GPU_MAX_SLOTS    = 3         # макс видеокарт
...
def _gpu_list(uid):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM user_gpus WHERE user_id=? ORDER BY id", (uid,))
        return [dict(r) for r in cur.fetchall()]

def _gpu_buy(uid, model):
    """Покупает GPU за ресурсы. Возвращает (ok, reason)."""
    if model not in GPU_MODELS:
        return False, "Неизвестная модель"
    gpus = _gpu_list(uid)
    if len(gpus) >= GPU_MAX_SLOTS:
        return False, f"Максимум {GPU_MAX_SLOTS} видеокарты"
    cost = GPU_MODELS[model]["cost"]
    # Проверяем ресурсы
    for res_id, need in cost.items():
        have = _inv_get(uid, res_id)
        if have < need:
            res_name = next((r["name"] for r in MINE_RESOURCES if r["id"] == res_id), res_id)
            return False, f"Нужно {need} {res_name}, у тебя {have}"
    # Списываем
    for res_id, need in cost.items():
        _inv_add(uid, res_id, -need)
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute(
            "INSERT INTO user_gpus (user_id, model, wear, last_tick) VALUES (?,?,0,?)",
            (uid, model, now)
        )
    return True, "ok"

def _gpu_tick_user(uid):
    """Начисляет доход со всех GPU пользователя. Вызывается фоновым потоком."""
    now = int(time.time())
    gpus = _gpu_list(uid)
    total_earned = 0
    broken = []
    for gpu in gpus:
        model_info = GPU_MODELS.get(gpu["model"])
        if not model_info:
            continue
        if now - gpu["last_tick"] < GPU_TICK_SECONDS:
            continue
        ticks = (now - gpu["last_tick"]) // GPU_TICK_SECONDS
        ticks = min(ticks, 3)  # не более 3 тиков за раз (защита от накопления)
        earned = model_info["income_per_tick"] * ticks
        new_wear = gpu["wear"] + ticks
        with get_db_cursor() as cur:
            if new_wear >= model_info["max_wear"]:
                cur.execute("DELETE FROM user_gpus WHERE id=?", (gpu["id"],))
                broken.append(gpu["model"])
            else:
                cur.execute(
                    "UPDATE user_gpus SET wear=?, last_tick=? WHERE id=?",
                    (new_wear, now, gpu["id"])
                )
        _add_balance(uid, earned)
        total_earned += earned
    return total_earned, broken

def _gpu_ticker():
    """Фоновый поток — каждые 5 мин проверяет все GPU."""
    while True:
        try:
            time.sleep(5 * 60)
            with get_db_cursor() as cur:
                cur.execute("SELECT DISTINCT user_id FROM user_gpus")
                uids = [r[0] for r in cur.fetchall()]
            for uid in uids:
                try:
                    earned, broken = _gpu_tick_user(uid)
                    if earned > 0:
                        broken_txt = ""
                        if broken:
                            names = ", ".join(GPU_MODELS[m]["name"] for m in broken if m in GPU_MODELS)
                            broken_txt = f"\n\n💀 Сломались: <b>{names}</b> — купи новые!"
                        try:
                            bot.send_message(uid,
                                f"🖥️ <b>Видеокарты</b>\n\n"
                                f"💵 Начислено: <b>+{format_balance(earned)} 🌸</b>{broken_txt}",
                                parse_mode="HTML")
                        except Exception:
                            pass
                except Exception as e:
                    print(f"[gpu] tick error uid={uid}: {e}")
        except Exception as e:
            print(f"[gpu] ticker error: {e}")

threading.Thread(target=_gpu_ticker, daemon=True, name="gpu-ticker").start()

def _gpu_menu_text(uid):
    gpus = _gpu_list(uid)
    lines = ["🖥️ <b>Видеокарты</b>\n"]
    lines.append(f"Слоты: {len(gpus)}/{GPU_MAX_SLOTS}\n")
    if gpus:
        lines.append("<b>Твои GPU:</b>")
        for g in gpus:
            m = GPU_MODELS.get(g["model"])
            if not m: continue
            wear_pct = int(g["wear"] / m["max_wear"] * 100)
            bar_filled = int((1 - g["wear"]/m["max_wear"]) * 8)
            bar = "█" * bar_filled + "░" * (8 - bar_filled)
            lines.append(f"{m['emoji']} {m['name']}  [{bar}] {100-wear_pct}%  +{format_balance(m['income_per_tick'])}/30м")
    else:
        lines.append("Нет видеокарт. Покупай за редкие ресурсы!\n")
    lines.append("\n<b>Доступно купить:</b>")
    for model_id, m in GPU_MODELS.items():
        cost_parts = []
        for res_id, need in m["cost"].items():
            have = _inv_get(uid, res_id)
            res = next((r for r in MINE_RESOURCES if r["id"] == res_id), None)
            name = res["name"] if res else res_id
            fb   = res.get("fallback","⬛") if res else "⬛"
            mark = "✅" if have >= need else "❌"
            cost_parts.append(f"{mark}{fb}{name}: {have}/{need}")
        income = format_balance(m["income_per_tick"])
        lines.append(f"\n{m['emoji']} <b>{m['name']}</b> — {income} 🌸/30м")
        lines.append("  " + "  ".join(cost_parts))
    return "\n".join(lines)

def _gpu_menu_kb(uid):
    gpus = _gpu_list(uid)
    rows = []
    if len(gpus) < GPU_MAX_SLOTS:
        for model_id, m in GPU_MODELS.items():
            cost = m["cost"]
            can_buy = all(_inv_get(uid, r) >= n for r, n in cost.items())
            rows.append([{
                "text": f"{'✅' if can_buy else '❌'} Купить {m['name']}",
                "callback_data": f"gpu_buy:{model_id}"
            }])
    rows.append([{"text": "◀️ Назад", "callback_data": "mine_back"}])
    return {"inline_keyboard": rows}

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("видеокарты", "видюхи", "gpu", "🖥️ видеокарты"))
def handle_gpu_menu_text(message):
    uid = message.from_user.id
    if not is_registered(uid): return
    bot.send_message(message.chat.id, _gpu_menu_text(uid),
                     reply_markup=_gpu_menu_kb(uid), parse_mode="HTML")

@bot.callback_query_handler(func=lambda c: c.data == "gpu_menu")
def cb_gpu_menu(call):
    uid = call.from_user.id
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text(_gpu_menu_text(uid), call.message.chat.id,
                              call.message.message_id, reply_markup=_gpu_menu_kb(uid), parse_mode="HTML")
    except Exception:
        bot.send_message(call.message.chat.id, _gpu_menu_text(uid),
                         reply_markup=_gpu_menu_kb(uid), parse_mode="HTML")

@bot.callback_query_handler(func=lambda c: c.data.startswith("gpu_buy:"))
def cb_gpu_buy(call):
    uid = call.from_user.id
    model = call.data[len("gpu_buy:"):]
    ok, reason = _gpu_buy(uid, model)
    if ok:
        m = GPU_MODELS[model]
        bot.answer_callback_query(call.id, f"✅ {m['name']} куплена!", show_alert=True)
    else:
...
def _mine_main_kb(uid):
    ...
    kb["inline_keyboard"].append([{"text": "🖥️ Видеокарты", "callback_data": "gpu_menu"}])
...
def cb_mine_dig(call):
    ...
    boost_mult = 2 if _boost_active(uid) else 1
    if reward > 0:
        _add_balance(uid, reward)
        event_qty  = random.randint(1, 3) if event["mult"] > 1 else 1
        new_streak = streak + 1
        new_best   = max(best_streak, new_streak)
        total_qty  = pk_res_qty * event_qty * boost_mult
        _inv_add(uid, resource["id"], total_qty)
    ...
    try:
        ...
        # fallback без tg-emoji (личка без Premium)
        if event["mult"] == 0.0:
            fb_text = f"⛏️ {where}\n{event['text']}"
        else:
            res_fb  = resource.get('fallback', '⬛')
            qty_tag = f" ×{total_qty}" if total_qty > 1 else ""
            st_tag  = f" 🔥×{new_streak}" if new_streak >= 10 else ""
            bt_tag  = " ⚡×2" if boost_mult == 2 else ""
            ev_tag  = f"\n{event['text']}" if event.get("text") else ""
            fb_text = f"⛏️ {where}{ev_tag}\n{res_fb} {resource['name']}{qty_tag}{bt_tag}  +{_fmt_num(reward // 2)} 🌸{st_tag}"
        try:
            bot.send_message(call.message.chat.id, fb_text)
        except Exception:
            pass
    try:
...
def cb_mkt_sell_to_buy(call):
    ...
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"⚡ {_res_emoji(r)} <b>{r['name']}</b> — выбери заявку покупателя:",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)
...
def cb_mkt_buy_from_sell(call):
    ...
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"⚡ {_res_emoji(r)} <b>{r['name']}</b> — выбери заявку:",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)
...
EQUIPMENT_SETS = {
    "iron": {
        ...
        "slots": {
            ...
            "sword":      {"emoji_id": 6278389670103491005, "fallback": "⚔️"},
        },
    ...
    "gold": {
        ...
        "slots": {
            ...
            "sword":      {"emoji_id": 6280626858438432759, "fallback": "⚔️"},
        },
    ...
    "diamond": {
        ...
        "slots": {
            ...
            "sword":      {"emoji_id": 6278347003898368427, "fallback": "⚔️"},
        },
    ...
    "netherite": {
        ...
        "slots": {
            ...
            "sword":      {"emoji_id": 6278398831268729149, "fallback": "⚔️"},
        },
...
def _eq_main_text(user_id):
    ...
    fallbacks = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "⚔️"}
...
def _eq_main_kb(user_id):
    ...
    base_emoji_ids = {
        "helmet":     "6280459251634672193",
        "chestplate": "6280373850504958136",
        "leggings":   "6280310409543028056",
        "boots":      "6278320826072699252",
        "sword":      "6278389670103491005",
    }
    rows = []
    row = []
    for slot in slot_order:
        ...
        if set_id:
            s = EQUIPMENT_SETS[set_id]
            emoji_id = str(s["slots"][slot]["emoji_id"])
        else:
            emoji_id = base_emoji_ids[slot]
        row.append({
...
def cb_eq_slot(call):
    ...
    if set_id:
        ...
        slot_dmg = s["stats"]["damage"] if slot == "sword" else 0
        slot_def = s["stats"]["defense"] // 4 if slot != "sword" else 0
        stat_line = f"⚔️ +{slot_dmg} урон" if slot == "sword" else f"🛡️ +{slot_def} защита"
        text = (
...
def _eq_set_pick_rows(slot, user_id=None):
    """Список кнопок для выбора сета под слот с премиум emoji и индикатором наличия."""
    rows = []
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        ...
        if em_id:
            btn["icon_custom_emoji_id"] = str(em_id)
        row.append(btn)
        if len(row) == 2:
            rows.append(row)
            row = []
...
def _craft_menu_kb():
    SET_EMOJI_IDS = {
        "iron":      "6280459251634672193",
        "gold":      "6280607844618211753",
        "diamond":   "6278271090351411403",
        "netherite": "6278466403989197908",
    }
    rows = []
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        btn = {"text": s["name"], "callback_data": f"craft_set_{set_id}"}
        if set_id in SET_EMOJI_IDS:
            btn["icon_custom_emoji_id"] = SET_EMOJI_IDS[set_id]
        row.append(btn)
        if len(row) == 2:
            rows.append(row)
            row = []
...
def cb_craft_set(call):
    ...
    for slot, cost in CRAFT_COST.items():
        ...
        btn_text = f"{SLOT_NAMES[slot]}  ({cost} ед.)"
        rows.append([{
            "text": btn_text,
            "callback_data": f"craft_item_{set_id}_{slot}",
            "icon_custom_emoji_id": str(s["slots"][slot]["emoji_id"])
        }])
...
def _pvp_battle_text(b, last_event=None, viewer_id=None):
    ...
    lines_out = [
        ...
        _pvp_eq_line(a_id),
        _pvp_hp_bar(a_hp),
        f"<i>{a_stats['damage']} ⚔️  {a_stats['defense']} 🛡</i>",
        "",
        f"🔵 <b>{d_name}</b>{d_hint}",
        _pvp_eq_line(d_id),
        _pvp_hp_bar(d_hp),
        f"<i>{d_stats['damage']} ⚔️  {d_stats['defense']} 🛡</i>",
    ]
    if b["bet"] > 0:
        lines_out.append(f"\n💰 {format_balance(b['bet'] * 2)}")
    if last_event:
        lines_out.append(f"\n{last_event}")
    if b["status"] == "active":
        if viewer_id:
...
def handle_pvp_menu(message):
    ...
    text = (
        ...
        f"⚔️ {stats['damage']} атк   🛡️ {stats['defense']} защ\n\n"
...
def cb_pvp_menu_stat(call):
    ...
    text = (
        f"⚔️ <b>ПВП Статистика</b>\n\n"
        f"🏆 Побед: <b>{wins}</b>\n"
        f"💀 Поражений: <b>{losses}</b>\n"
        f"📊 {bar} <b>{winrate}%</b>\n\n"
        f"🛡️ <b>Снаряжение</b>\n" + "\n".join(eq_lines) +
        f"\n\n⚔️ <b>{stats['damage']}</b> атк   🛡️ <b>{stats['defense']}</b> защ"
    )
...
def handle_pvp_challenge(message):
    ...
    if len(parts) < 2:
        bot.send_message(message.chat.id,
            ...
            "⚔️ Удары по частям тела: Голова 💥×1.5, Корпус ×1.0, Ноги ×0.8\n"
    ...
    bet_line = f"\n💰 Ставка: <b>{format_balance(bet)}</b> с каждого" if bet > 0 else ""
    ...
    if dm_ok:
        ...
        chat_note = (
            f"⚔️ <b>{a_name}</b> вызывает <b>{d_name}</b>!\n"
...
def cb_pvp_accept(call):
    ...
    try:
        _tg_api("editMessageText",
                ...
                text=(f"⚔️ <b>{a_name}</b> vs <b>{d_name}</b>\n\n"
    ...
    b = _pvp_get(battle_id)
    a_mid, d_mid = _pvp_send_dm(battle_id, b, last_event="⚔️ Бой начался!")
...
def cb_pvp_action(call):
    ...
    if action_type == "guard":
        ...
        try:
            ...
            event = f"⚔️ <b>{actor_name}</b> → {bp_info['emoji']} {bp_info['name']} · <b>−{reduced}</b>{block_note}{armor_note}{crit_note}"
...
def _pvp_finish(battle_id, b, last_event=""):
    ...
    if b["bet"] > 0:
        ...
        prize_note = f"\n💰 <b>+{format_balance(prize)}</b> победителю!"
...
def _pvp_start_timeout(battle_id):
    def _timeout():
        ...
        w_name = _mkt_user_name_by_id(winner_id)
        l_name = _mkt_user_name_by_id(loser_id)
        prize_note = f"\n💰 <b>+{format_balance(b['bet'] * 2)}</b> победителю!" if b["bet"] > 0 else ""
...
def _pvp_send_stats(message):
    ...
    text = (
        f"⚔️ <b>— П В П   С Т А Т И С Т И К А —</b>\n\n"
        f"🏆 Побед:      <b>{wins}</b>\n"
        f"💀 Поражений:  <b>{losses}</b>\n"
        f"📊 Винрейт:    {wrate_bar}  <b>{winrate}%</b>\n\n"
        f"╠══ 🛡️ <b>СНАРЯЖЕНИЕ</b> ══╣\n"
        + "\n".join(eq_lines) +
        f"\n\n  ⚔️ Урон: <b>{stats['damage']}</b>   🛡️ Защита: <b>{stats['defense']}</b>\n"
        f"╚══════════════════════╝"
    )
    bot.send_message(message.chat.id, text, parse_mode="HTML")
...
def _arena_start_match(uid1, uid2):
    ...
    header = (f"🏟 <b>БОЙ НАЙДЕН!</b>\n\n"
              ...
              f"{_pvp_eq_line(uid1)}\n<i>{s1['damage']} ⚔️  {s1['defense']} 🛡</i>\n\n"
              f"🔵 <b>{n2}</b>  {_arena_rank(r2['elo'])}  <i>({r2['elo']} ELO)</i>\n"
              f"{_pvp_eq_line(uid2)}\n<i>{s2['damage']} ⚔️  {s2['defense']} 🛡</i>\n\n")

    b = _pvp_get(battle_id)
    kb1 = _pvp_action_kb(battle_id, uid1, b)
    r1a = _tg_api("sendMessage", chat_id=uid1, text=header+"⚔️ <b>Твой ход!</b>",
...
def _arena_main_text(uid):
    ...
    return (f"🏟 <b>П В П — А Р Е Н А</b>\n\n{_arena_rank(r['elo'])}\n"
            f"📊 ELO: <b>{r['elo']}</b>   🏆 #{place} в рейтинге\n\n"
            f"Победы: <b>{r['wins']}</b>   Поражения: <b>{r['losses']}</b>\n"
            f"Винрейт: {wr_bar} <b>{wr}%</b>\n"
            f"🔥 Серия: <b>{r['streak']}</b>   Рекорд: <b>{r['best_streak']}</b>\n"
            f"⭐ Пик ELO: <b>{r['peak_elo']}</b>\n\n"
        ...
        if found:
            try:
                _tg_api("sendMessage", chat_id=uid, parse_mode="HTML",
                        text="⚔️ <b>Соперник найден!</b> Бой начинается 👇")
            except Exception: pass
            return
