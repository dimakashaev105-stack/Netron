...
# Если TURSO_URL не задан — падаем обратно на локальный SQLite
...
if _USE_TURSO:
    try:
        ...
        print(f"[db] Turso mode: {TURSO_URL[:40]}...")
    except ImportError:
        _USE_TURSO = False
        print("[db] libsql_experimental не установлен, падаем на SQLite")
...
class DatabasePool:
    def __init__(self, db_name='game.db', pool_size=10):
        ...
        # Для Turso держим одно постоянное соединение (libsql не thread-safe в пуле)
    ...
    def _make_turso(self):
        conn = libsql.connect(
            self.db_name,
            sync_url=TURSO_URL,
            auth_token=TURSO_TOKEN,
        )
        conn.sync()   # тянем актуальное состояние из облака при старте
        return conn

    def get_connection(self):
        if _USE_TURSO:
            with self._turso_lock:
                if self._turso_conn is None:
                    self._turso_conn = self._make_turso()
                return self._turso_conn   # всегда одно соединение
        with self._lock:
            if self.pool:
    ...
    def return_connection(self, conn):
        if _USE_TURSO:
            return   # Turso-соединение не возвращаем в пул, оно постоянное
        with self._lock:
...
def get_db_cursor():
    with get_db_connection() as conn:
        ...
        try:
            ...
            if _USE_TURSO:
                try:
                    conn.sync()   # пушим изменения в облако после каждого коммита
                except Exception as _se:
                    print(f"[turso] sync error: {_se}")
        except:
            conn.rollback()
            raise
