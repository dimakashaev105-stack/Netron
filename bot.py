...
STOCK_BUY_IMPACT  = 0.001        # покупка 1 акции = +0.05% к цене
STOCK_SELL_IMPACT = 0.001        # продажа 1 акции = -0.05% к цене
STOCK_COOLDOWN    = 3 * 60       # 10 минут между сделками
...
def _apply_trade_impact(qty, is_buy):
    ...
    """
    price, _ = _get_stock_price()
    # Ступенчатый impact: уменьшаем эффект для крупных сделок
    effective_qty = 100 * (1 - 1 / (1 + qty / 100))  # логистическое затухание
    impact = effective_qty * STOCK_BUY_IMPACT if is_buy else -effective_qty * STOCK_SELL_IMPACT
    impact = max(-0.05, min(0.05, impact))
    new_price = _set_stock_price(price * (1 + impact))
    return new_price
