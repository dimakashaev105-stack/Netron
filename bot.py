...
def handle_bank(message):
    ...
    try:
        with open('bank.jpg', 'rb') as photo:
            ...
            caption += f"• Процентная бет: {int(LOAN_CONFIG['interest_rate'] * 100)}% в день\n\n"
        ...
        bank_text += f"• Процентная бет: {int(LOAN_CONFIG['interest_rate'] * 100)}% в день\n\n"
...
def handle_loan_amount(message):
    ...
    bot.send_message(
        ...
        f"📌 Займ автоматически спишется через {LOAN_CONFIG['max_term']}д\n\n"
        f"Подтвердить что хотите взять займ?",
        reply_markup=markup,
        parse_mode='HTML'
    )
...
def handle_loan_info(message):
    ...
    message_text += f"⏰ Взят: {days_passed:.1f}д назад\n"
...
def handle_repay_loan(call):
    ...
    bot.edit_message_text(
        ...
        f"💳 Твой баланс: {format_balance(get_balance(user_id))}\n\n"
        f"🎉 Благодарим что пользуетесь нашими услугами!",
        call.message.chat.id,
        call.message.message_id,
        parse_mode='HTML'
    )
...
def handle_withdraw_amount(message):
    ...
    bot.send_message(
        ...
        f"📊 Твой баланс после: {format_balance(get_balance(user_id) + withdraw_amount)}\n\n"
...
# Замените существующий обработчик join_clan_ на этот:
            ...
            # Проверяем, не подавал ли уже заявку (если есть таблица заявок)
            try:
                with get_db_cursor() as cursor:
                ...
                pass  # Таблицы нет - пропускаем
            
            # Сразу добавляем в клан (если нет системы заявок)
            with get_db_cursor() as cursor:
                cursor.execute('''
...
# Добавьте кнопку поиска в меню кланов
"""
В функции create_clans_menu() найдите строку с кнопками
и добавьте кнопку поиска:
"""
...
def create_clans_menu():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton('🔍 Поиск гильдий', callback_data='search_clans'),  # Уже есть?
    ...
    # Добавьте если нет:
    # markup.add(types.InlineKeyboardButton('🔍 Поиск', callback_data='search_clans'))
    return markup
...
def handle_clan_callbacks(call):
    ...
    try:
        ...
        # Остальные обработчики (уже существующие)
            ...
            # Получаем всех боецов войн клана
            with get_db_cursor() as cursor:
                cursor.execute('''
        ...
        # Вступление в клан
            ...
            text += f'🏰 Гильдия: {clan_info["name"]} `[{clan_info["tag"]}]`\n'
            text += f'⭐ Уровень: {clan_info["level"]}\n'
            text += f'👥 Участников: {clan_info["members_count"] + 1}/{clan_info["max_members"]}\n'
            text += f'💵 Казна: {format_balance(clan_info["balance"])}\n\n'
            text += f'Теперь ты часть команды!'
...
def handle_create_bonus(message):
    ...
    example_bonus = random.randint(100, 50000)
    ...
    def create_bonus_handler(bonus_id):
        ...
        def handle_specific_bonus(call):
            ...
            user_bonus_amount = random.randint(1000, 50000)  # 1ккк - 50ккк
...
# НОВАЯ ЛОГИКА ДАРТСА
# Замени функцию handle_darts (строки 17039-17100) на эту:

@bot.message_handler(func=lambda message: message.text.lower().startswith('дартс '))
def handle_darts(message):
    """
    Игра в дартс с новой механикой:
    - Центр (6) = x5 выигрыш
    - Близкое кольцо (4-5) = x1 возврат ставки
    - Промах (1-3) = -x2 потеря удвоенной ставки
    """
    try:
        user_id = message.from_user.id
        balance = get_balance(user_id)
...
def handle_bowling(message):
    try:
        ...
        bot.send_message(message.chat.id, "💀 Ошибка в игре. Повторите почкику.")
# КОМАНДЫ ДЛЯ РАБОТЫ С БАЗОЙ ДАННЫХ
# Добавь эти команды в свой bot.py (желательно рядом с другими админ-командами)
