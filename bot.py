...
def handle_mine_dig_text(message):
    ...
    if _mine_captcha_expired(uid):
        _mine_captcha_start(uid)
    if _mine_captcha_required(uid):
        q = _mine_captcha_start(uid)
        bot.send_message(message.chat.id,
            f"🔐 <b>Подтверди что ты человек</b>\n\nРеши пример: <b>{q} = ?</b>\n\nНапиши ответ числом в чат. Есть 2 минуты.",
            parse_mode="HTML")
        return
...
# ─── Антибот для шахты ────────────────────────────────────────────────────────
# Каждые 10 копаний требуем капчу. Скрипт не знает ответ — застрянет навсегда.
MINE_CAPTCHA_EVERY = 10   # копаний между капчами
_mine_captcha_pending = {}  # uid -> {'answer': int, 'expires': float, 'digs': int}
_mine_captcha_lock   = threading.Lock()

def _mine_dig_count(uid):
    """Сколько копаний сделал игрок за сессию (сбрасывается при рестарте)."""
    with _mine_captcha_lock:
        return _mine_captcha_pending.get(uid, {}).get('digs', 0)

def _mine_captcha_required(uid):
    """True если нужна капча перед следующим копанием."""
    with _mine_captcha_lock:
        entry = _mine_captcha_pending.get(uid, {})
        if 'answer' in entry:  # уже ждём ответа
            return True
        digs = entry.get('digs', 0)
        return digs > 0 and digs % MINE_CAPTCHA_EVERY == 0

def _mine_captcha_start(uid):
    """Генерирует капчу и сохраняет ответ."""
    a, b = random.randint(2, 12), random.randint(2, 12)
    ops = [(a + b, f"{a} + {b}"), (a * b, f"{a} × {b}"), (max(a,b) - min(a,b), f"{max(a,b)} − {min(a,b)}")]
    answer, question = random.choice(ops)
    with _mine_captcha_lock:
        digs = _mine_captcha_pending.get(uid, {}).get('digs', 0)
        _mine_captcha_pending[uid] = {'answer': str(answer), 'question': question,
                                       'expires': time.time() + 120, 'digs': digs}
    return question

def _mine_captcha_check(uid, user_text):
    """Проверяет ответ. Возвращает True если верно, сбрасывает pending."""
    with _mine_captcha_lock:
        entry = _mine_captcha_pending.get(uid, {})
        if 'answer' not in entry:
            return False
        if time.time() > entry.get('expires', 0):
            del _mine_captcha_pending[uid]
            return False
        ok = user_text.strip() == entry['answer']
        if ok:
            digs = entry.get('digs', 0)
            _mine_captcha_pending[uid] = {'digs': digs}  # сбрасываем pending, счётчик копаний сохраняем
        return ok

def _mine_captcha_increment(uid):
    """Увеличивает счётчик копаний после успешного копа."""
    with _mine_captcha_lock:
        entry = _mine_captcha_pending.get(uid, {})
        _mine_captcha_pending[uid] = {**entry, 'digs': entry.get('digs', 0) + 1}

def _mine_captcha_expired(uid):
    """True если капча висит но время вышло."""
    with _mine_captcha_lock:
        entry = _mine_captcha_pending.get(uid, {})
        return 'answer' in entry and time.time() > entry.get('expires', 0)

@bot.message_handler(func=lambda m: m.chat.type == 'private' and m.text and m.text.strip().lstrip('-').isdigit()
                     and m.from_user.id in _mine_captcha_pending
                     and 'answer' in _mine_captcha_pending.get(m.from_user.id, {}))
def handle_mine_captcha_answer(message):
    uid = message.from_user.id
    if _mine_captcha_check(uid, message.text.strip()):
        bot.send_message(message.chat.id, "✅ Верно! Можешь копать дальше ⛏️", parse_mode="HTML")
    else:
        with _mine_captcha_lock:
            q = _mine_captcha_pending.get(uid, {}).get('question', '?')
        bot.send_message(message.chat.id, f"❌ Неверно. Попробуй ещё раз.\n\n🔐 <b>{q} = ?</b>", parse_mode="HTML")
# ──────────────────────────────────────────────────────────────────────────────
...
def cb_mine_dig(call):
    ...
    if _mine_captcha_required(uid):
        q = _mine_captcha_start(uid)
        bot.answer_callback_query(call.id, "🔐 Нужна проверка!", show_alert=False)
        bot.send_message(uid,
            f"🔐 <b>Подтверди что ты человек</b>\n\nРеши пример: <b>{q} = ?</b>\n\nНапиши ответ числом в чат. Есть 2 минуты.",
            parse_mode="HTML")
        return
    ...
    try:
        ...
        pass
    _mine_captcha_increment(uid)  # считаем копание для антибота
    lvl  = _pickaxe_get(uid)
    info = PICKAXE_LEVELS[lvl]
    cd_m          = info["cooldown"] // 60
    digs_per_hour = 60 // cd_m
    sample_base   = 450
    sample_earn   = int(sample_base * info["bonus"] * digs_per_hour)

    lines = [
        f"⛏️ <b>Кирка</b>  —  {info['label']}\n",
        f"💥 Бонус: <b>×{info['bonus']}</b>   📦 Ресурсов/коп: <b>{info['res_qty']} шт.</b>",
        f"⏱ Кулдаун: <b>{cd_m} мин</b>  →  <b>{digs_per_hour}×/час</b>",
        f"💎 Бонус к редким: <b>+{info['rare_boost']*10}%</b>" if info['rare_boost'] > 0 else "💎 Бонус к редким: нет",
        f"📊 Примерный доход/час: <b>≈{sample_earn} 🌸</b>",
        "",
        "<b>Уровни:</b>",
    ]
    for lv, d in PICKAXE_LEVELS.items():
        mark  = "▶️" if lv == lvl else ("✅" if lv < lvl else "◻️")
        cd    = d["cooldown"] // 60
        digs  = 60 // cd
        earn  = int(sample_base * d["bonus"] * digs)
        cost  = f"{d['stars']} ⭐" if lv > 1 else "бесплатно"
        rare  = f" редк.+{d['rare_boost']*10}%" if d["rare_boost"] > 0 else ""
        lines.append(
            f"{mark} <b>Лвл {lv}</b> {d['label']}  ×{d['bonus']}  {d['res_qty']}шт  {cd}м  ≈{earn}/ч{rare}  [{cost}]"
        )
    bl = _boost_left(uid)
    if bl > 0:
        bm, bs = divmod(bl, 60)
        lines.append(f"\n⚡ <b>Буст ×2 активен</b> — ещё {bm}м {bs}с")
    else:
        lines.append("\n⚡ <b>Буст ×2 ресурсов</b> на 30 мин — <b>10 ⭐</b>")
    lines.append("")
    lines.append("💡 <i>Кирка множит награду, даёт больше ресурсов и открывает редкие руды.</i>")
    return "\n".join(lines)
