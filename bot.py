...
MINE_RESOURCES = [
    {"id": "coal",      "name": "Уголь",        "emoji_id": 5456408804541340493, "chance": 32,   "price": 1200},
    {"id": "earth",     "name": "Земля",         "emoji_id": 5458822138075028493, "chance": 18,   "price": 800},
    {"id": "stone",     "name": "Камень",        "emoji_id": 5458781211331665562, "chance": 14,   "price": 1000},
    {"id": "copper",    "name": "Медь",          "emoji_id": 5458520300658368161, "chance": 10,   "price": 2800},
    {"id": "iron",      "name": "Железо",        "emoji_id": 5458833133191306560, "chance": 9,    "price": 4500},
    {"id": "wood",      "name": "Дерево",        "emoji_id": 5458693503804513909, "chance": 5,    "price": 7000},
    {"id": "planks",    "name": "Доски",         "emoji_id": 5456151226762665376, "chance": 4,    "price": 9500},
    {"id": "redstone",  "name": "Редстоун",      "emoji_id": 5458448020653743095, "chance": 3,    "price": 16000},
    ...
    {"id": "diamond",   "name": "Алмаз",         "emoji_id": 5458420940884942467, "chance": 0.4,  "price": 140000},
    {"id": "gold_ore",  "name": "Золотая руда",  "emoji_id": 5458617543012918174, "chance": 0.25, "price": 250000},
    {"id": "netherite", "name": "Незерит",       "emoji_id": 6280349132968168995, "chance": 0.05, "price": 1000000},
]
...
def cb_mkt_sell_pick(call):
    ...
    for rid, amt in inv.items():
        ...
        if not r:
            continue
        has_order = "✅ " if _sell_order_get(uid, rid) else ""
        rows.append([{"text": f"{has_order}{r['name']} ({amt} шт.)", "callback_data": f"mkt_sell_res:{rid}"}])
...
def cb_mkt_buy_pick(call):
    ...
    for r in MINE_RESOURCES:
        ...
        if cnt:
            label = f"{r['name']} — от {plain_balance(minp)}  ({cnt})"
        else:
            label = f"{r['name']} — нет заявок"
        rows.append([{"text": label, "callback_data": f"mkt_buy_res:{r['id']}"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_main"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
...
def cb_mkt_browse_res(call):
    ...
    for r in MINE_RESOURCES:
        with get_db_cursor() as cur:
            ...
            bc = cur.fetchone()[0]
        if sc or bc:
            rows.append([{"text": f"{r['name']}  продают: {sc}  покупают: {bc}",
