...
def _eq_calc_stats(user_id):
    """Считает итоговые боевые характеристики игрока с учётом снаряжения."""
    ...
    for slot, set_id in equipped.items():
        ...
        if not s:
            continue
        if slot == "sword":
            base_dmg += s["stats"]["damage"]
        else:
            base_def += s["stats"]["defense"] // 4

    if len(equipped) == 5 and len(set(equipped.values())) == 1:
        set_id = list(equipped.values())[0]
        base_dmg = int(base_dmg * (1 + FULL_SET_BONUS))
        base_def = int(base_def * (1 + FULL_SET_BONUS))

    return {"damage": base_dmg, "defense": base_def, "equipped": equipped}
...
def cb_pvp_action(call):
    ...
    if action_type == "guard":
        ...
        armor_slot = bp_info["armor_slot"]
        opp_equipped = stats_def["equipped"]

        # Защита слота брони — поглощает реальный процент урона.
        # Каждый слот берёт своё значение defense напрямую (не // 4).
        # Железо ~8, Золото ~11, Алмаз ~18, Незерит ~26 — это % поглощения (cap 60%).
        slot_defense_pct = 0.0
        if armor_slot in opp_equipped:
            s = EQUIPMENT_SETS.get(opp_equipped[armor_slot])
            if s:
                slot_defense_pct = min(s["stats"]["defense"] / 100.0, 0.60)

        # Бонус полного сета защитника работает и в бою
        full_set_bonus = 0.0
        if len(opp_equipped) == 5 and len(set(opp_equipped.values())) == 1:
            full_set_bonus = FULL_SET_BONUS  # +15%

        base = random.randint(PVP_BASE_DMG_MIN, PVP_BASE_DMG_MAX)
        raw_dmg = int((base + stats_atk["damage"]) * bp_info["dmg_mult"])
        # Сначала снимаем броню слота, потом бонус полного сета
        absorbed = int(raw_dmg * (slot_defense_pct + full_set_bonus))
        reduced = max(1, raw_dmg - absorbed)
        if is_blocked:
        ...
        armor_note = f" · 🛡{int((slot_defense_pct + full_set_bonus)*100)}% погл." if (slot_defense_pct + full_set_bonus) > 0 and not is_blocked else ""
