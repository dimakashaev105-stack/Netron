...
''')
        ...
        """)
        cursor.execute("SELECT COUNT(*) FROM donate_packages")
        if cursor.fetchone()[0] == 0:
            ...
            cursor.executemany(
                "INSERT INTO donate_packages (key, stars, amount, emoji) VALUES (?,?,?,?)",
                defaults
            )
...
def collect_taxes():
    ...
    try:
        # Сначала получаем всех пользователей отдельным запросом
        with get_db_cursor() as cursor:
            cursor.execute('''
        ...
        # Теперь обрабатываем и обновляем в отдельном соединении
        with get_db_cursor() as cursor:
...
def handle_collect_tax(message):
    _do_collect_tax(message)

@bot.message_handler(commands=['налог'])
def handle_collect_tax_cmd(message):
    _do_collect_tax(message)

def _do_collect_tax(message):
...
@bot.message_handler(func=lambda message: message.text and message.text.lower() == 'стата налогов' and is_admin(message.from_user.id))
