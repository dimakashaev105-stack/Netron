...
def _eq_main_text(user_id):
    ...
    for slot in slot_order:
        ...
        if set_id:
            ...
            lines_out.append(f"  {emoji}  <b>{SLOT_NAMES[slot]}</b>\n      <i>{s['emoji']} {s['name']} сет</i>")
        else:
            lines_out.append(f"  {fallbacks[slot]}  <b>{SLOT_NAMES[slot]}</b>\n      <i>— не надет —</i>")

    lines_out.append("\n╠══ ⚔️ <b>ХАРАКТЕРИСТИКИ</b> ══╣")
    dmg = stats["damage"]
    dfn = stats["defense"]
    dmg_bar = "▰" * min(dmg // 3, 10) + "▱" * max(0, 10 - dmg // 3)
    def_bar = "▰" * min(dfn // 3, 10) + "▱" * max(0, 10 - dfn // 3)
    lines_out.append(f"  ⚔️  Урон     {dmg_bar}  <b>{dmg}</b>")
    lines_out.append(f"  🛡️  Защита   {def_bar}  <b>{dfn}</b>")

    if len(equipped) == 5 and len(set(equipped.values())) == 1:
        set_name = EQUIPMENT_SETS[list(equipped.values())[0]]["name"]
        set_emoji = EQUIPMENT_SETS[list(equipped.values())[0]]["emoji"]
        lines_out.append(f"\n✨ <b>Полный {set_emoji} {set_name} сет!</b>  +15% ко всему")
...
def _pvp_battle_text(b, last_event=None, viewer_id=None):
    ...
    def self_guard_hint(uid):
        is_attacker = (uid == a_id)
        pg = b.get("attacker_pending_guard" if is_attacker else "defender_pending_guard", "")
        g  = b.get("attacker_guard"         if is_attacker else "defender_guard",         "")
        part = pg or g
    ...
    if b["status"] == "active":
        if viewer_id:
            is_attacker = (viewer_id == a_id)
            pg = b.get("attacker_pending_guard" if is_attacker else "defender_pending_guard", "")
            if viewer_id == b["whose_turn"]:
                if pg:
                    lines_out.append(f"\n🛡 Защита выбрана — теперь куда бьёшь?")
                else:
                    lines_out.append("\n🛡 Выбери защиту")
            else:
                lines_out.append("\n⏳ Ход соперника...")
...
def handle_pvp_challenge(message):
    ...
    text = (
        f"⚔️ <b>— В Ы З О В   Н А   Б О Й —</b>\n\n"
        f"🔴 <b>{a_name}</b>\n"
        f"    ⚔️ {a_stats['damage']} атк  •  🛡️ {a_stats['defense']} защ\n\n"
        f"🔵 <b>{d_name}</b>\n"
        f"    ⚔️ {d_stats['damage']} атк  •  🛡️ {d_stats['defense']} защ"
        f"{bet_line}\n\n"
        f"📬 Бой пройдёт в личных сообщениях\n"
        f"<b>{d_name}</b>, принимаешь вызов?"
    )
    ...
    try:
        ...
        _pvp_set_msg(battle_id, sent.message_id)
    except Exception as e:
        print(f"[pvp] не удалось отправить вызов: {e}")
