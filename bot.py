...
TAX_CONFIG = {
    ...
    "high_wealth_threshold": 100000,
    "medium_wealth_tax": 0.05,
    "medium_wealth_threshold": 10000,
    "general_tax": 0.03,
    "min_tax_amount": 1
}
...
def handle_admin_stats(message):
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            cursor.execute("SELECT SUM(balance), AVG(balance), MAX(balance) FROM users")
            row = cursor.fetchone()
            ...
            cursor.execute("SELECT SUM(bank_deposit) FROM users")
            total_bank = int(cursor.fetchone()[0] or 0)
            cursor.execute("SELECT SUM(loan_amount) FROM loans WHERE status='active'")
            total_loans = int(cursor.fetchone()[0] or 0)

            cursor.execute("""
                SELECT COALESCE(custom_name, username, first_name, CAST(user_id AS TEXT)), balance
            ...
            row = cursor.fetchone(); games_day = row[0]; games_payout = int(row[1])
        ...
        rich_lines = "\n".join(f"  {i+1}. {nm} — {f(bal)} 🌸" for i,(nm,bal) in enumerate(top_rich)) or "  нет данных"
        ...
        text = (
            ...
            f"  Активны сегодня: <b>{f(active_day)}</b>\n"
            f"  Активны за неделю: <b>{f(active_week)}</b>\n\n"
            f"💰 <b>Экономика</b>\n"
            f"  В обороте: <b>{f(total_balance)} 🌸</b>\n"
            f"  Средний баланс: <b>{f(avg_balance)} 🌸</b>\n"
            f"  Максимальный: <b>{f(max_balance)} 🌸</b>\n"
            f"  В банке: <b>{f(total_bank)} 🌸</b>\n"
            ...
            f"  Топ игр за неделю:\n{game_lines}\n\n"
            f"🚗 <b>Такси</b>\n"
            f"  Водителей: <b>{f(taxi_drivers)}</b>\n"
            f"  Поездок: <b>{f(taxi_trips)}</b>\n"
            f"  Заработано: <b>{f(taxi_earned)} 🌸</b>\n\n"
            f"⛏️ <b>Майнинг</b>\n"
            f"  Майнеров: <b>{f(miners_count)}</b>\n"
            f"  Видеокарт: <b>{f(total_gpus)}</b>\n\n"
            f"🛍️ <b>Магазин</b>\n"
