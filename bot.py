...
def update_balance(user_id, amount):
    """Обновляет баланс пользователя БЕЗ кэширования"""
    with get_db_cursor() as cursor:
...
def handle_db_stats(message):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            # Количество забаненных (если есть поле ban_status или banned)
            cursor.execute("SELECT COUNT(*) FROM users WHERE balance < 0")  # Пример условия бана
        ...
        stats_text = (
            ...
            f"🏠 Домов: <b>{total_businesses}</b>\n\n"
            f"💰 Общий баланс: <b>❄️{format_balance(total_balance)}</b>\n"
            f"🏦 Всего на вкладах: <b>❄️{format_balance(total_deposits)}</b>\n\n"
    ...
    try:
        ...
        with open(db_path, 'rb') as db_file:
            bot.send_document(
                ...
                visible_file_name=f"game_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
            )
...
def handle_upload_db_request(message):
    ...
    bot.send_message(
        ...
        "⚠️ <b>ВНИМАНИЕ:</b> Текущая база будет полностью заменена!",
        parse_mode='HTML'
    )


# ===== ОБРАБОТЧИК ПОЛУЧЕНИЯ ФАЙЛА БАЗЫ =====
...
def handle_document_upload(message):
    ...
    try:
        ...
        # Проверяем, что это файл .db
        if not file_name.endswith('.db'):
            bot.send_message(message.chat.id, "💀 Ошибка: отправьте файл с расширением .db")
            return
        ...
        old_db_path = 'game.db'
        backup_path = f'game_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.db'
        ...
        with open(old_db_path, 'wb') as new_file:
            new_file.write(downloaded_file)
