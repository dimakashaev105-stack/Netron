...
def show_class_info(chat_id, user_id):
    ...
    if progress_info['has_next']:
        ...
        message_text += f"⚔️ Осталось: {progress_info['rides_needed']} поездок\n"
...
def generate_taxi_order(chat_id, user_id):
    ...
    # Фильтруем заказы по минимальному времени (3+м для высоких классов)
    available_orders = []
    for order in TAXI_SYSTEM["orders"]:
        # Для VIP класса - только заказы от 4м
        if taxi_class == "vip" and order["min_time"] >= 4:
            available_orders.append(order)
        # Для Business класса - от 3м
    ...
    # Добавляем бонус очкиа за класс
    ...
    message_text = (
        ...
        f"⚔️ <b>Опыт:</b> +{order_data['experience']}\n\n"
        f"<b>Принять заказ?</b>"
    )
...
def handle_reward_taxi(message):
    ...
    try:
            ...
            if reward_amount:
                ...
                try:
                    notification_text = (
                        ...
                        f"🎁 Твоя награда: {format_balance(reward_amount)}\n\n"
                        f"Так держать! 🚗"
                    )
        ...
        report_text = (
            f"🏆 <b>Награждение топа таксистов окончено!</b>\n\n"
            f"🏆 <b>Награжденные:</b>\n"
        )
...
def accept_taxi_order(call):
    ...
    message_text = (
        ...
        f"⏰ <b>Завершится через {time_minutes}м...</b>"
    )
    ...
    # Запускаем таймер окончения поездки
...
def complete_taxi_ride(user_id, chat_id, message_id, order_data):
    ...
    # Проверяем, что заказ еще работает в БД
    active_order = get_active_taxi_order(user_id)
    track_taxi_complete(user_id)
    if not active_order:
        try:
            bot.send_message(chat_id, "🏆 Поездка уже была окончена ранее.")
        except:
            pass
        return
    
    try:
        # Начисляем деньги и очки
        ...
        # Формируем сообщение о окончении
        ...
        message_text += f"⚔️ <b>Опыт:</b> +{order_data['experience']}\n"
        message_text += f"⭐ <b>Класс:</b> {class_info['name']}\n"
        message_text += f"💳 <b>Средства:</b> {format_balance(new_balance)}\n"
        ...
        print(f"💀 Ошибка при окончении поездки: {e}")
        try:
            bot.send_message(chat_id, "💀 Произошла ошибка при окончении поездки. Обратитесь к админу.")
        except:
            pass
...
def handle_clear_taxi(message):
    ...
    if cleared_count > 0:
        ...
        bot.reply_to(message, "ℹ️ Активных заказов такси отсутствуето")

def cleanup_stuck_taxi_orders():
    """Очистить зависшие заказы такси при запуске бота"""
    try:
        with get_db_cursor() as cursor:
            # Находим заказы старше 2ч (7200с)
            ...
            if stuck_orders:
                ...
                print("🏆 Зависших заказов такси отсутствуето")
    
    except Exception as e:
        print(f"💀 Ошибка при очистке зависших заказов такси: {e}")

# Запускаем обновление топа каждые 5м и очистку при старте
...
# Добавьте этот код в твой файл после функции get_balance
...
def get_balance(user_id):
    now = time.time()
    if user_id in _cache_ttl and (now - _cache_ttl[user_id]) < CACHE_TIME:
        if user_id in _user_cache and 'balance' in _user_cache[user_id]:
            return _user_cache[user_id]['balance']
    with get_db_cursor() as cursor:
        ...
        if user_id not in _user_cache:
            _user_cache[user_id] = {}
        _user_cache[user_id]['balance'] = balance
...
def handle_stars_selection(call):
    ...
    packages = {
        ...
        "stars_30kkk": {"amount": 60000, "stars": 10, "title": "60k"},
        "stars_75kkk": {"amount": 150000, "stars": 15, "title": "150k"},
        "stars_100kkk": {"amount": 200000, "stars": 25, "title": "200k"},
        "stars_500kkk": {"amount": 5000000, "stars": 100, "title": "500k"}
    }
...
def handle_darts(message):
    try:
        ...
        dice_message = bot.send_dice(message.chat.id, emoji='⚔️')
        time.sleep(4)
...
def handle_experience(message):
    try:
        ...
        message_text += f"📊 Накоплено очкиа: {experience:,}\n".replace(",", " ")
        message_text += f"📈 До следующего уровня: {exp_needed:,} очкиа\n".replace(",", " ")
