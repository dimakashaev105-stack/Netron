...
def handle_mines(message):
    try:
        ...
        if bet_amount is None or bet_amount < 1000000:
            bot.send_message(message.chat.id, f"💀 Мин. бет: {format_balance(1000000)}")
            return
                ...
                if check_data:
                    ...
                    if ref_user and ref_user[0] != user_id:
                        ...
                        if not current_ref or not current_ref[0]:
                            ...
                            bot.send_message(message.chat.id, "🎉 Ты получил 1 000 000 000🌸 за приглашение друга!")
            ...
            if check_data:
                ...
                if ref_user and ref_user[0] != user_id:
                    ...
                    if not current_ref or not current_ref[0]:
                        ...
                        cursor.execute('UPDATE users SET referred_by = ? WHERE user_id = ?', (referred_by, user_id))
                        cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (user_bonus, user_id))
                        bot.send_message(user_id, "🎉 Ты получил 1 000 000 000🌸 за регистрацию по реферальной ссылке!")
...
def handle_clan_upgrade(message):
    ...
    upgrade_cost = user_clan['level'] * 5000000000  # 5B за ранг
...
# ДУЭЛИ КОНФИГУРАЦИЯ
DUEL_CONFIG = {
    "min_bet": 1000000,  # 1M минимальная бет
