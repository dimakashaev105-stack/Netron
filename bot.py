...
def _eq_emoji_btn(set_id, slot):
    s = EQUIPMENT_SETS.get(set_id, {}).get("slots", {}).get(slot)
    if s:
        return f'<tg-emoji emoji-id="{s["emoji_id"]}">{s["fallback"]}</tg-emoji>'
    return {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "⚔️"}.get(slot, "❓")

def _eq_main_kb(user_id):
    equipped = _eq_get_all(user_id)
    slot_order = ["helmet", "chestplate", "leggings", "boots", "sword"]
    slot_names_short = {"helmet": "Шлем", "chestplate": "Нагрудник", "leggings": "Поножи", "boots": "Сапоги", "sword": "Меч"}
    rows = []
    row = []
    for slot in slot_order:
        if equipped.get(slot):
            set_id = equipped[slot]
            mat_emoji = EQUIPMENT_SETS[set_id]["emoji"]
            item_emoji = _eq_emoji_btn(set_id, slot)
            label = f"{item_emoji} {mat_emoji} {slot_names_short[slot]}"
        else:
            base = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "⚔️"}[slot]
            label = f"{base} {slot_names_short[slot]}"
...
def _pvp_battle_text(b, last_event=None, viewer_id=None):
    ...
    a_hp = b["attacker_hp"]
    d_hp = b["defender_hp"]
    a_guard = b.get("attacker_guard", "")
    d_guard = b.get("defender_guard", "")

    def my_guard(guard):
        if guard and guard in BODY_PARTS:
            bp = BODY_PARTS[guard]
            return f" · 🛡 {bp['name']}"
        return ""

    a_guard_hint = my_guard(a_guard) if viewer_id == a_id else ""
    d_guard_hint = my_guard(d_guard) if viewer_id == d_id else ""

    lines_out = [
        "⚔️ <b>БОЙ</b>",
        "",
        f"🔴 <b>{a_name}</b>{a_guard_hint}",
        _pvp_hp_bar(a_hp),
        f"<i>{a_stats['damage']} ⚔️  {a_stats['defense']} 🛡</i>",
        "",
        f"🔵 <b>{d_name}</b>{d_guard_hint}",
        _pvp_hp_bar(d_hp),
    ...
    if last_event:
        lines_out.append(f"\n{last_event}")
    if b["status"] == "active":
        is_my_turn = (viewer_id == b["whose_turn"]) if viewer_id else False
        if viewer_id:
            lines_out.append("\n▸ <b>Твой ход</b>" if is_my_turn else "\n⏳ Ход соперника...")
        else:
            turn_name = a_name if b["whose_turn"] == a_id else d_name
            lines_out.append(f"\n▸ Ход <b>{turn_name}</b>")
    return "\n".join(lines_out)

def _pvp_tgemoji(emoji_id, fallback):
    return f'<tg-emoji emoji-id="{emoji_id}">{fallback}</tg-emoji>'

def _pvp_action_kb(battle_id, user_id):
    equipped = _eq_get_all(user_id)

    part_slot = {"head": "helmet", "body": "chestplate", "legs": "leggings"}

    def slot_icon(part_id):
        slot = part_slot[part_id]
        set_id = equipped.get(slot)
        if set_id:
            s = EQUIPMENT_SETS[set_id]["slots"].get(slot)
            if s:
                return _pvp_tgemoji(s["emoji_id"], s["fallback"])
        return BODY_PARTS[part_id]["emoji"]

    sword_set = equipped.get("sword")
    if sword_set:
        sw = EQUIPMENT_SETS[sword_set]["slots"].get("sword")
        sword_icon = _pvp_tgemoji(sw["emoji_id"], sw["fallback"]) if sw else "⚔️"
    else:
        sword_icon = "⚔️"

    atk_row = []
    for part_id in ["head", "body", "legs"]:
        icon = slot_icon(part_id)
        name = BODY_PARTS[part_id]["name"]
        atk_row.append({"text": f"{icon} {name}", "callback_data": f"pvp_action_{battle_id}_{user_id}_atk_{part_id}"})

    grd_row = []
    for part_id in ["head", "body", "legs"]:
        icon = slot_icon(part_id)
        name = BODY_PARTS[part_id]["name"]
        grd_row.append({"text": f"🛡 {name}", "callback_data": f"pvp_action_{battle_id}_{user_id}_guard_{part_id}"})

    return {
        "inline_keyboard": [
            [{"text": f"{sword_icon} Удар:", "callback_data": "pvp_noop"}],
            atk_row,
            [{"text": "🛡 Защита:", "callback_data": "pvp_noop"}],
            grd_row,
        ]
    }
...
def cb_pvp_action(call):
    ...
    attacker_hp = b["attacker_hp"]
    defender_hp = b["defender_hp"]
    bp_info = BODY_PARTS[body_part]
    event = ""

    if action_type == "guard":
        if is_attacker:
            _pvp_update(battle_id, attacker_guard=body_part, whose_turn=opponent_id, last_action=int(time.time()))
        else:
            _pvp_update(battle_id, defender_guard=body_part, whose_turn=opponent_id, last_action=int(time.time()))
        actor_name = _mkt_user_name_by_id(uid)
        event = f"🛡️ <b>{actor_name}</b> защищает {bp_info['emoji']} <b>{bp_info['name']}</b>!"
        ...
        actor_name = _mkt_user_name_by_id(uid)
        block_note = " (🛡️ заблокировано!)" if is_blocked else ""
        crit_note = " 💥 КРИТ!" if bp_info["dmg_mult"] > 1.0 else ""
        event = f"⚔️ <b>{actor_name}</b> бьёт по {bp_info['emoji']} <b>{bp_info['name']}</b> — <b>{reduced}</b> урона{block_note}{crit_note}"
        if is_attacker:
            defender_hp = max(0, defender_hp - reduced)
            _pvp_update(battle_id, defender_hp=defender_hp, defender_guard="", whose_turn=opponent_id, last_action=int(time.time()))
        else:
            attacker_hp = max(0, attacker_hp - reduced)
            _pvp_update(battle_id, attacker_hp=attacker_hp, attacker_guard="", whose_turn=opponent_id, last_action=int(time.time()))
    else:
        bot.answer_callback_query(call.id, "❌")
        return
