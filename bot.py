...
def cb_pvp_action(call):
    ...
    if action_type == "guard":
        ...
        # Урон меча: base ± 3 (как разброс урона в MC)
        base = stats_atk["damage"] + random.randint(-3, 3)
        raw_dmg = max(1, int(base * bp_info["dmg_mult"]))

        # Официальная формула Minecraft для снижения урона:
        # eff_ap = min(20, max(AP/5, AP - raw/(2 + toughness/4)))
        # reduction = eff_ap / 25
        ...
        # Износ брони защищающегося в атакованном слоте
        if armor_slot:
