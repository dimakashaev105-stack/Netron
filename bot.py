...
def cb_eq_slot(call):
    ...
    if set_id:
        ...
        kb = {"inline_keyboard": _eq_set_pick_rows(slot) + [[{"text": "◀️ Назад", "callback_data": "eq_back"}]]}
...
def _eq_set_pick_rows(slot):
    """Список кнопок для выбора сета под слот (только если есть хотя бы 1 крафченный)."""
    rows = []
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        row.append({"text": f"{s['emoji']} {s['name']}", "callback_data": f"eq_equip_{slot}_{set_id}"})
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
...
def cb_eq_pick_set(call):
    ...
    kb = {"inline_keyboard": _eq_set_pick_rows(slot) + [[{"text": "◀️ Назад", "callback_data": f"eq_slot_{slot}"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
