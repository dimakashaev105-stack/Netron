...
def handle_buy_stock(message):
    ...
    # Атомарное списание и запись портфеля
    buy_ok = False
    with get_db_cursor() as cur:
        cur.execute(
            "UPDATE users SET balance=balance-? WHERE user_id=? AND balance>=?",
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Цена: {format_balance(price)}/шт.\n"
        ...
        f"⏳ Следующая сделка через 1 минуту",
        parse_mode="HTML"
    )
...
def handle_sell_stock(message):
    ...
    raw_profit = (price - avg) * qty if avg > 0 else 0
    ...
    with get_db_cursor() as cur:
        ...
        cur.execute(
            "INSERT INTO stock_trades (user_id,ticker,action,amount,price,fee,total,created_at) VALUES (?,?,?,?,?,?,?,?)",
            (user_id, STOCK_TICKER, "sell", qty, price, fee + tax, net, now)
        )
        sell_ok = True
    ...
    price_change = (new_price - price) / price * 100
    tax_line     = f"\n💼 Налог с прибыли (10%): -{format_balance(tax)}" if tax > 0 else ""

    bot.send_message(message.chat.id,
        f"✅ <b>Продано {qty} акций {STOCK_TICKER}</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Цена продажи: {format_balance(price)}/шт.\n"
        f"💸 Комиссия (2%): -{format_balance(fee)}"
        f"{tax_line}\n"
    ...
    threading.Thread(target=_log_stock_trade, args=(user_id, "sell", qty, price, net, new_price), daemon=True).start()
