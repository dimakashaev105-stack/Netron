...
def handle_bank(message):
    ...
    text = (
        ...
        f"<i>Депозит приносит 0.01% в час (0.015% для Premium)</i>"
    )
    markup = InlineKeyboardMarkup()
    markup.row(
...
# Заглушки для обратной совместимости (loan был удалён)
def handle_take_loan(message):
    bot.send_message(message.chat.id, "ℹ️ Система займов отключена.", parse_mode='HTML')
...
def calculate_interest():
    ...
    with get_db_cursor() as cursor:
        cursor.execute(
            'SELECT user_id, bank_deposit FROM users WHERE bank_deposit > 0 AND last_interest_calc <= ? - 3600',
            (current_time,)
        )
        rows = cursor.fetchall()
        for uid, deposit in rows:
            rate = 0.00015 if is_premium(uid) else 0.0001  # премиум 0.015%/ч vs 0.01%/ч
            new_deposit = int(deposit + deposit * rate)
            cursor.execute(
    ...
    try:
        ...
        if last_bonus > 0:
            ...
            if time_passed < cooldown:
                ...
                mins = time_left // 60
                secs = time_left % 60
                bot.answer_callback_query(call.id, f"⏳ Еще {mins}м {secs}с", show_alert=True)
                return
...
def handle_transfer(message):
    try:
        ...
        if amount > user_balance:
            bot.send_message(message.chat.id, f"❌ Недостаточно монет. Баланс: {format_balance(user_balance)}", parse_mode='HTML')
            return
