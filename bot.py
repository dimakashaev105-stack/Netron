    ...
    try:
        ...
        if symbol == "🏆":
            with get_db_cursor() as cursor:
                ...
                # Бонус за ранг (небольшой)
                if new_level > old_level:
                    level_up_bonus = new_level * 200  # скромный бонус: ур.5 = 1000, ур.25 = 5000
                    if level_up_bonus > 0:
...
MYTHIC_E = "<tg-emoji emoji-id='5258208871423425369'>🔮</tg-emoji>"
LEGEND_E = "<tg-emoji emoji-id='5447584416274595624'>👑</tg-emoji>"
...
def handle_me(message):
    try:
        ...
        message_text = f"{emoji} {time_greeting} <b>{display_name}</b>\n"
