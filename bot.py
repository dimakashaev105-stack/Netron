...
''')
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS donate_packages (
                ...
                emoji   TEXT NOT NULL DEFAULT '<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji>'
            )
        """)
        defaults = [
            ...
            if len(items) > 1:
                ...
                for i, (item_id, name, price, item_type) in enumerate(items[:10]):
                    ...
                    markup.add(InlineKeyboardButton(
                        f"<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> {name}", 
    ...
    if active:
        kb = {"inline_keyboard": [
            [{"text": "📊 Статистика", "callback_data": "taxi_stats"},
             {"text": "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Рейтинг",   "callback_data": "taxi_rating"}],
        ]}
    else:
        kb = {"inline_keyboard": [
            ...
            [{"text": "📊 Статистика",       "callback_data": "taxi_stats"},
             {"text": "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Рейтинг",          "callback_data": "taxi_rating"}],
            [{"text": "<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> Мой уровень",      "callback_data": "taxi_my_level"}],
        ]}
...
@bot.message_handler(func=lambda message: message.text == "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Рейтинг водителей")
...
@bot.message_handler(func=lambda message: message.text == "<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> Мой уровень")
...
def create_main_menu():
    ...
    buttons = [
        "🏙️ Город", "📞 Помощь", "🏛 Банк",
        "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ", "💎 Донат", "<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> Бонус",
        "📈 Биржа", "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> Клан", "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> ПВП",
    ]
...
def create_business_menu():
    ...
    markup.add(
        ...
        KeyboardButton("<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Собрать доход"),
...
def create_mining_keyboard():
    ...
    markup.row_width = 2
    markup.add(
        InlineKeyboardButton("<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Собрать", callback_data="mining_collect"),
        InlineKeyboardButton("<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Купить видеокарту", callback_data="mining_buy")
    )
    return markup
...
def create_top_menu(top_type="balance", page=0):
    ...
    type_buttons = [
        InlineKeyboardButton("<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Баланс", callback_data=f"top_type_balance_{page}"),
...
def _build_donate_markup():
    ...
    for i in range(0, len(keys), 2):
        ...
        for k in row_keys:
            ...
            amt_str = f"{pkg['amount']:,}".replace(',', ' ')
            row.append({
                "text": f"<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> {pkg['stars']} зв  —  {amt_str}",
                "callback_data": k
            })
...
def send_discount_message(user_id, percent=50):
    ...
    try:
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(
            "<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> Использовать скидку",
...
def handle_stars_selection(call):
    ...
    if package in packages:
        ...
        try:
            bot.send_invoice(
                chat_id=call.message.chat.id,
                title=f"<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> {actual_stars} зв. -> {clean_amount}{discount_line}",
...
@bot.message_handler(func=lambda message: message.text in ["<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ", "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji>", "Топ", "топ", "/top"])
...
def handle_lottery(message):
    ...
    markup.row(*buttons)
    markup.row(InlineKeyboardButton("<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> ОТКРЫТЬ ВСЕ", callback_data=f"lot_{user_id}_all"))
...
def create_mines_keyboard(game_data):
    ...
    for i in range(25):
        if i in game_data['revealed_safe']:
            row.append(InlineKeyboardButton("<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji>", callback_data=f"mines_already_{i}"))
    ...
    markup.row(
        InlineKeyboardButton("<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Забрать", callback_data="mines_cashout"),
...
def create_final_mines_keyboard(game_data):
    ...
    for i in range(25):
        if i in game_data['revealed_mines']:
            ...
            row.append(InlineKeyboardButton("<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji>", callback_data="mines_final"))
...
def create_tower_keyboard(game_id, level, left_state, right_state, multipliers, show_mines=False):
    ...
    if show_mines and left_state == 'mine':
        ...
        left_button = InlineKeyboardButton("<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Левая", callback_data=f"tower_{game_id}_{level}_left_c")
    else:
    ...
    if show_mines and right_state == 'mine':
        ...
        right_button = InlineKeyboardButton("<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Правая", callback_data=f"tower_{game_id}_{level}_right_c")
    else:
    ...
    if level > 1 or show_mines:
        markup.add(InlineKeyboardButton(f"<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Забрать {plain_balance(win_amount)}", callback_data=f"tower_{game_id}_x"))
...
@bot.message_handler(func=lambda message: message.text in ["<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> Бонус", "Бонус"])
    ...
    try:
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> ЗАБРАТЬ",
        ...
        with get_db_cursor() as cursor:
            ...
            for i, (user_id,) in enumerate(users, 1):
                try:
                    ...
                    if is_subscribed:
                        markup = InlineKeyboardMarkup()
                        markup.add(InlineKeyboardButton("<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> Забрать", callback_data="bonus_broadcast_claim"))
                        ...
                        markup = InlineKeyboardMarkup()
                        markup.add(InlineKeyboardButton("<tg-emoji emoji-id="5203996991054432397">🎁</tg-emoji> Получить бонус", callback_data="bonus_broadcast_claim"))
                        markup.add(InlineKeyboardButton("📢 Подписаться", url=f"https://t.me/FECTIZ"))
...
def event_get_markup(count):
    return {
        "inline_keyboard": [[
            {"text": f"<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Участвовать ({count}/{EVENT_REQUIRED})", "callback_data": "event_join"}
        ]]
    }
...
def _clan_main_markup(user_id):
    ...
    if not clan:
        ...
        mk.add(InlineKeyboardButton("<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ кланов", callback_data="clan_top_0"))
        return mk
    ...
    mk.add(
        InlineKeyboardButton("<tg-emoji emoji-id="5217871612321216821">💰</tg-emoji> Взнос в казну", callback_data="clan_deposit_prompt"),
        InlineKeyboardButton("<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> Война",          callback_data="clan_war_menu"),
    )
    mk.add(InlineKeyboardButton("<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ кланов", callback_data="clan_top_0"))
...
def handle_declare_war(message):
    ...
    mk = InlineKeyboardMarkup()
    mk.add(
        InlineKeyboardButton(f"<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> Начать войну с [{enemy_tag}]",
...
def _war_menu_text_markup(user_id):
    ...
    if cd > 0:
        ...
        mk = InlineKeyboardMarkup(row_width=2)
        mk.add(
            InlineKeyboardButton(f"<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> Атака (-{format_balance(CLAN_WAR_ATTACK_COST)})",
...
def _mine_main_kb(uid):
    ...
    kb["inline_keyboard"].append([
        ...
        {"text": "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ майнеров", "callback_data": "mine_top"},
    ])

    if my_mine:
        kb["inline_keyboard"].append([{"text": "🏔️ Моя шахта", "callback_data": "mine_mymine"}])
    else:
        kb["inline_keyboard"].append([{"text": "<tg-emoji emoji-id="5217871612321216821">💰</tg-emoji> Купить шахту", "callback_data": "mine_buy_prompt"}])
...
MINE_EVENTS = [
    ...
    {"mult": 2.0, "text": "<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Удачный удар! Двойная добыча!", "weight": 5},
...
def _pickaxe_kb(uid):
    ...
    if lvl < PICKAXE_MAX:
        ...
        rows.append([{
            "text": f"⬆️ Улучшить до Лвл {next_lvl} — {next_info['stars']} <tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji>",
            "callback_data": f"mine_pickaxe_upgrade:{next_lvl}"
        }])
    else:
    ...
    if bl > 0:
        bm, bs = divmod(bl, 60)
        rows.append([{"text": f"<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Буст активен ({bm}м {bs}с)", "callback_data": "mine_pickaxe"}])
    else:
        rows.append([{"text": "<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Купить буст ×2 · 10 <tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji>", "callback_data": "mine_boost_buy"}])
...
def _mymine_kb(mine):
    ...
    kb = {"inline_keyboard": []}
    if mine['balance'] > 0:
        kb["inline_keyboard"].append([{"text": f"<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Вывести {plain_balance(mine["balance"])}", "callback_data": "mine_withdraw"}])
...
def cb_mkt_sell_res(call):
    ...
    if buy_orders and inv_amt > 0:
        rows.append([{"text": "<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Продать в заявку", "callback_data": f"mkt_sell_to_buy:{rid}"}])
    if existing:
...
def cb_mkt_buy_res(call):
    ...
    if any(o["seller_id"] != uid for o in sell_orders):
        rows.append([{"text": "<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Купить из заявок", "callback_data": f"mkt_buy_from_sell:{rid}"}])
    if existing_buy:
...
def cb_mkt_browse_detail(call):
    ...
    if any(True for o in sell if o.get("seller_id") != call.from_user.id):
        rows.append([{"text": "<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji> Купить сейчас", "callback_data": f"mkt_buy_from_sell:{rid}"}])
...
def _eq_main_kb(user_id):
    ...
    # базовые emoji_id (железный сет — дефолт)
    slot_fallbacks = {
        "helmet": "🪖", "chestplate": "🦺", "leggings": "👖",
        "boots": "👟", "sword": "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji>"
    }
    rows = []
    row = []
    for slot in slot_order:
        ...
        if set_id:
            s = EQUIPMENT_SETS[set_id]
            btn_text = f"✅ {fb} {slot_names[slot]}"
        else:
            btn_text = f"◻️ {fb} {slot_names[slot]}"
        row.append({
            "text": btn_text,
            "callback_data": f"eq_slot_{slot}",
        })
        if len(row) == 2:
            rows.append(row)
            row = []
...
def cb_craft_set(call):
    ...
    for slot, cost in CRAFT_COST.items():
        ...
        slot_fb = {"helmet":"🪖","chestplate":"🦺","leggings":"👖","boots":"👟","sword":"<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji>"}.get(slot,"")
        btn_text = f"{slot_fb} {SLOT_NAMES[slot]}  ({cost} ед.)"
        rows.append([{
            "text": btn_text,
            "callback_data": f"craft_item_{set_id}_{slot}",
        }])
...
@bot.message_handler(func=lambda m: m.text and m.text.strip() == "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> ПВП")
def handle_pvp_menu(message):
    ...
    kb = {"inline_keyboard": [
        [{"text": "🛡️ Экипировка", "callback_data": "pvp_menu_eq"},
         {"text": "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> ПВП стат",   "callback_data": "pvp_menu_stat"}],
...
def _arena_main_kb(uid):
    ...
    rows = []
    if in_battle:
        rows.append([{"text": "<tg-emoji emoji-id="5415965335192883624">⚔️</tg-emoji> Ты уже в обычном бою!", "callback_data": "arena_noop"}])
    elif in_q:
        rows.append([{"text": "❌ Выйти из поиска", "callback_data": "arena_leave_queue"}])
    else:
        rows.append([{"text": "🔍 Найти бой", "callback_data": "arena_search"}])
    rows.append([{"text": "<tg-emoji emoji-id="5213326931331659859">🏆</tg-emoji> Топ арены", "callback_data": "arena_top"}])
