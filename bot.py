...
def handle_mine_dig_text(message):
    """Текстовая команда копать — открывает меню шахты"""
    uid = message.from_user.id
    bot.send_message(message.chat.id, _mine_main_text(uid),
                     reply_markup=_mine_main_kb(uid), parse_mode="HTML")
...
def cb_mine_dig(call):
    ...
    # tg-emoji смайл ресурса
    res_emoji = "<tg-emoji emoji-id='" + str(resource['emoji_id']) + "'>" + resource['name'][0] + "</tg-emoji>"

    # Составляем уведомление (answer_callback не поддерживает HTML, используем edit)
    if event["mult"] == 0.0:
        notif     = "⛏️ " + where + "\n" + event["text"]
        mine_line = event["text"]
    elif event["mult"] >= 2.0:
        notif     = "⛏️ " + where + "\n" + event["text"] + "\n" + resource["name"] + " · +" + format_balance(reward) + " 🌸"
        mine_line = event["text"] + "\n" + res_emoji + " " + resource["name"] + " · <b>+" + format_balance(reward) + " 🌸</b>"
    else:
        ...
        mine_line   = (event["text"] + "\n" if event["text"] else "") + res_emoji + " " + resource["name"] + " · <b>+" + format_balance(reward) + " 🌸</b>" + streak_tag

    bot.answer_callback_query(call.id, notif, show_alert=False)
    try:
        main_text = _mine_main_text(uid)
        # Добавляем последний результат копания в текст
        if event["mult"] == 0.0:
            result_line = "\n\n💥 " + event["text"]
        else:
            result_line = "\n\n" + mine_line
        bot.edit_message_text(main_text + result_line,
                              call.message.chat.id, call.message.message_id,
                              reply_markup=_mine_main_kb(uid), parse_mode="HTML")
    except Exception:
        pass
...
def cb_mine_list(call):
    ...
    for row in rows:
        ...
        btn     = f"🏔️ #{mid} {lv_info['label']} x{lv_info['bonus']} · {format_balance(rent_price)} · {name}"
...
def cb_mine_view(call):
    ...
    if mine['owner_id'] == uid:
        ...
        kb["inline_keyboard"].append([{
            "text": f"⛏️ Арендовать · {format_balance(mine['rent_price'])}",
...
def cb_mine_buy_prompt(call):
    ...
    if _mine_by_owner(uid):
        ...
        return
    kb = {"inline_keyboard": [
        [{"text": f"✅ Купить · {format_balance(MINE_BUY_PRICE)}", "callback_data": "mine_buy_confirm"}],
