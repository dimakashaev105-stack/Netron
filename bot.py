...
def _build_donate_markup():
    ...
    for i in range(0, len(keys), 2):
        ...
        for k in row_keys:
            ...
            amt_str = f"{pkg['amount']:,}".replace(',', ' ')
            row.append(
                _btn(f'<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> {pkg["stars"]} зв  —  {amt_str}', callback_data=k)
            )
        inline_kb.append(row)
    inline_kb.append([_btn(
        '<tg-emoji emoji-id="5204065856560045377">⭐</tg-emoji> Любое кол-во звёзд',
...
def handle_buy(message):
    ...
    text = (
        ...
        f"За звёзды Telegram ты получаешь внутриигровую валюту 🌸\n\n"
        f"<b>Пакеты:</b>\n"
...
def handle_football(message):
    try:
        ...
        # ⚽ Значения Telegram dice (по документации python-telegram-bot):
        # 1, 2, 3 — мимо/штанга/сейв    4, 5 — ГОЛ
        win = False
        multiplier = 2
        ...
        football_results = {
            1: "⚽ Мимо!", 2: "⚽ Мимо!", 3: "🧤 Вратарь поймал!",
        ...
        if win:
            ...
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: <b>+{format_balance(win_amount)}</b> (x{multiplier})\n"
