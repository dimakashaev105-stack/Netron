...
STOCK_COOLDOWN    = 3 * 60        # 3 минуты между сделками
...
STOCK_LIQUIDITY   = 200           # "глубина стакана" (базовый объём за 1%)

# Режимы волатильности рынка (переключаются автоматически)
STOCK_VOL_NORMAL  = 0.015         # обычный шум ±1.5% за период
STOCK_VOL_HIGH    = 0.04          # повышенная волатильность ±4%
STOCK_VOL_CRISIS  = 0.08          # кризис ±8%
STOCK_DRIFT_BASE  = 0.003         # небольшой upward bias (компания "растёт")
...
def _apply_trade_impact(qty, is_buy):
    ...
    impact_pct = qty / (qty + STOCK_LIQUIDITY) * 0.20  # max ~20% при огромных объёмах
