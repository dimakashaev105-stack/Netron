...
def _do_mine_dig(uid, chat_id, reply_to_msg_id=None, call=None):
    """Общая логика копания — используется и из message, и из callback."""
    if not _mine_try_dig(uid):
        cd = _mine_cd_left(uid)
        msg = f"⏳ Подожди ещё {_mine_fmt_cd(cd)}"
        if call:
    ...
    if call:
        try:
            bot.edit_message_text(_mine_main_text(uid), chat_id, call.message.message_id,
                                  reply_markup=_mine_main_kb(uid), parse_mode="HTML")
        except Exception:
            pass
    else:
        bot.send_message(chat_id, _mine_main_text(uid),
                         reply_markup=_mine_main_kb(uid), parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "копать")
def handle_mine_dig_text(message):
    _do_mine_dig(message.from_user.id, message.chat.id)

@bot.callback_query_handler(func=lambda c: c.data == "mine_dig")
def cb_mine_dig(call):
    _do_mine_dig(call.from_user.id, call.message.chat.id, call=call)
