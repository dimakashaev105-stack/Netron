...
shop_pages = {}
wardrobe_pages = {}
_invoice_cooldown = {}  # user_id -> timestamp последнего инвойса
_callback_cooldown = {}  # user_id -> timestamp последнего callback
...
# ── Глобальный антиспам для ВСЕХ callback кнопок ────────────────────────────
def _check_callback_spam(user_id, cooldown=0.5):
    """Возвращает True если можно обработать, False если спам"""
    now = time.time()
    last = _callback_cooldown.get(user_id, 0)
    if now - last < cooldown:
        return False
    _callback_cooldown[user_id] = now
    return True

# Monkey-patch: перехватываем ВСЕ callback через process_new_updates
...
def _patched_process_new_updates(updates):
    ...
    for update in updates:
        if update.callback_query:
            ...
            # Кликер и майнинг пропускаем без антиспама — у них своя логика
            if data.startswith('clicker_') or data.startswith('mining_'):
                filtered.append(update)
                continue
            uid = update.callback_query.from_user.id
            now = time.time()
            last = _callback_cooldown.get(uid, 0)
            if now - last < 0.4:
                # Спам — тихо отвечаем и пропускаем
                try:
        ...
        filtered.append(update)
    if filtered:
        _orig_process_new_updates(filtered)
...
def handle_loan_info(message):
    ...
    bot.send_message(
        message.chat.id,
        message_text,
        reply_markup=repay_admin_keyboard,
        parse_mode='HTML'
    )
...
def handle_stars_selection(call):
    ...
    # Антиспам
    if not _check_callback_spam(user_id, cooldown=1.5):
    ...
    if package in packages:
        ...
        # Антиспам: не более 1 инвойса в 3 секунды
        now = time.time()
        last = _invoice_cooldown.get(user_id, 0)
        if now - last < 3:
            bot.answer_callback_query(call.id, "⏳ Подождите секунду...", show_alert=False)
            return
...
def clicker_callback_handler(call):
    ...
    # Антиспам: не чаще раза в 0.3 сек
    if not _check_callback_spam(user_id, cooldown=0.3):
        bot.answer_callback_query(call.id)
        return
...
def handle_transfer(message):
    try:
        ...
        # Разбиваем текст: команда в lower, но сохраняем оригинал для юзернейма
        parts_lower = message.text.lower().split()
        parts_orig = message.text.split()
        parts = parts_lower
        ...
        if message.reply_to_message:
            ...
            if len(parts) < 3:
                bot.send_message(message.chat.id, 
                               ...
                               parse_mode='Markdown')
                return
            # Берём юзернейм из оригинального текста (без lower), чтобы сохранить регистр
            target_identifier = parts_orig[1]
            amount_text = ' '.join(parts[2:])
