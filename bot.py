...
STOCK_MAX_PER_USER = 100          # макс акций на игрока
...
STOCK_DRIFT_MAX   = 0.03          # макс случайный drift ±3% за период
...
# ── Обновление цены (drift каждые 15 минут) ──────────────────

def _drift_stock_price():
    """Случайный drift ±3% каждые 15 минут — имитирует рыночные колебания."""
    price, _ = _get_stock_price()
    drift = (random.random() - 0.5) * 2 * STOCK_DRIFT_MAX
    new_price = _set_stock_price(price * (1 + drift))
    print(f"[stocks] drift: {price:,} → {new_price:,} ({drift*100:+.2f}%)")
    return price, new_price

# ── Влияние сделок на цену ────────────────────────────────────

def _apply_trade_impact(qty, is_buy):
    """Покупка поднимает цену, продажа опускает. Пропорционально объёму."""
    price, _ = _get_stock_price()
    impact = qty * STOCK_BUY_IMPACT if is_buy else -qty * STOCK_SELL_IMPACT
    # Ограничиваем impact одной сделки максимум ±5%
...
@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() in ("акции", "акция", "биржа", "рынок"))
...
def handle_buy_stock(message):
    ...
    if qty <= 0 or qty > 100:
        bot.send_message(message.chat.id, "❌ От 1 до 100 акций за раз.", parse_mode="HTML")
        return

    owned, avg, _ = _get_portfolio(user_id)
    if owned + qty > STOCK_MAX_PER_USER:
        bot.send_message(message.chat.id,
            f"❌ Максимум {STOCK_MAX_PER_USER} акций.\n"
            f"У тебя: {owned}, можно купить: {STOCK_MAX_PER_USER - owned}",
            parse_mode="HTML")
        return
...
def _stock_price_scheduler():
    print(f"[stocks] планировщик запущен, drift каждые {STOCK_UPDATE_SEC//60} мин")
    while True:
        time.sleep(STOCK_UPDATE_SEC)
        try:
            old_price, new_price = _drift_stock_price()
