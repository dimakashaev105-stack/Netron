...
PREMIUM_PRICES = [
    {"stars": 15,  "days": 7,   "label": "7 дней — 15 ⭐ (~22₽)"},
    {"stars": 50,  "days": 30,  "label": "30 дней — 50 ⭐ (~75₽)"},
    {"stars": 100, "days": 90,  "label": "90 дней — 100 ⭐ (~150₽)"},
]
...
def _premium_main_text(user_id):
    ...
    return (
        ...
        f"Статус: {status}\n\n"
        f"<blockquote>"
        f"💰 Бонус каждые 10 мин вместо 15\n"
        f"💳 Комиссия переводов 5% вместо 10%\n"
        f"🏦 Вклад 0.015%/ч вместо 0.01%\n"
...
@bot.message_handler(func=lambda m: m.text and re.match(r"^прем\s+", m.text.lower()) and is_admin(m.from_user.id))
...
@bot.message_handler(func=lambda m: m.text and re.match(r"^прем всем\s+\d+", m.text.lower()) and is_admin(m.from_user.id))
def handle_admin_grant_premium_all(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Формат: <code>прем всем дней</code>", parse_mode="HTML")
    except Exception as e:
