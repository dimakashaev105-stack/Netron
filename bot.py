...
def handle_download_db(message):
    ...
    try:
        ...
        # Сначала сливаем WAL чтобы все транзакции были в основном файле
        _db_checkpoint()
...
def handle_broadcast_confirmation(call):
    ...
    def _do_broadcast():
        ...
        # Если в тексте есть ссылка — отключаем превью
        has_link = bool(re.search(r'https?://', text))
        link_preview_options = {"is_disabled": True} if has_link else {}
...
def _pickaxe_kb(uid):
    ...
    if lvl < PICKAXE_MAX:
        ...
        rows.append([{
            "text": f"⬆️ Улучшить до Лвл {next_lvl} — {next_info['stars']} ⭐",
    ...
    if bl > 0:
        bm, bs = divmod(bl, 60)
        rows.append([{"text": f"⚡ Буст активен ({bm}м {bs}с)", "callback_data": "mine_pickaxe"}])
    else:
...
def _mymine_kb(mine):
    ...
    kb = {"inline_keyboard": []}
    if mine['balance'] > 0:
        kb["inline_keyboard"].append([{"text": f"💵 Вывести {plain_balance(mine["balance"])}", "callback_data": "mine_withdraw"}])
