...
class _PgRow:
    """Строка результата — поддерживает и row[0] (по индексу) и row['col'] (по имени)."""
...
class _PgCursor:
    """Обёртка над psycopg2 cursor: поддерживает ? и возвращает _PgRow."""
        ...
        for column in required_columns:
            if column not in columns:
                if column in ['bank_deposit', 'click_streak', 'total_clicks', 'daily_streak', 
                    ...
                    cursor.execute(f'ALTER TABLE users ADD COLUMN {column} BIGINT DEFAULT 0')
                else:
                    cursor.execute(f'ALTER TABLE users ADD COLUMN {column} INTEGER DEFAULT 0')
        
        shop_columns = []  # pg: skip PRAGMA
    ...
    try:
        ...
        if file_ext == 'db':
            ...
            try:
                ...
                test_conn.cursor().execute("SELECT COUNT(*) FROM users")
                test_conn.close()
