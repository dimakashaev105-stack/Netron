...
def handle_accept_app(message):
    ...
    with get_db_cursor() as cur:
        ...
        # Бонус в казну за нового участника
        cur.execute("UPDATE clans SET balance=balance+? WHERE id=?", (100_000, cid))
    # Прогресс квеста на новых участников
    ...
    bot.send_message(message.chat.id,
        ...
        f"💰 +100 000 🌸 в казну клана!",
        parse_mode="HTML")

    # Пост в канал клана о новом участнике
    threading.Thread(target=_clan_post, args=(cid, f"👋 <b>Новый участник!</b>\n➕ {name_str} вступил в клан [{clan['tag']}]\n💰 +100 000 🌸 в казну!"), daemon=True).start()
        ...
        try:
            if mid == target_uid:
                bot.send_message(mid,
                    ...
                    f"💰 Казна клана пополнилась на 100 000 🌸 за твой вход!\n"
...
# ── Кнопка квеста в меню клана ───────────────────────────────
# Добавляем в _clan_main_markup (патч)

_orig_clan_main_markup = _clan_main_markup

def _clan_main_markup(user_id):
    mk   = _orig_clan_main_markup(user_id)
    clan = get_user_clan(user_id)
    if clan:
        # Добавляем кнопку квеста
        mk.add(InlineKeyboardButton("🎯 Квест", callback_data="clan_quest_view"))
    return mk

@bot.callback_query_handler(func=lambda c: c.data == "clan_quest_view")
def cb_clan_quest(call):
    ...
    mk = InlineKeyboardMarkup()
    mk.add(InlineKeyboardButton("🔄 Обновить", callback_data="clan_quest_view"))
    mk.add(InlineKeyboardButton("◀️ Назад",    callback_data="clan_back"))

    if not quest:
        bot.edit_message_text("🎯 Квест генерируется, попробуй через минуту.",
                              call.message.chat.id, call.message.message_id,
                              reply_markup=mk, parse_mode="HTML")
        bot.answer_callback_query(call.id)
        return
