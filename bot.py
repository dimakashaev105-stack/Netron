...
def _protected_process(messages):
    ...
    for message in messages:
        try:
            ...
            # Учитываем только ЛС бота (private) как признак активного пользователя
            try:
                ...
                if is_private:
                    ...
                    # В группах тоже обновляем, но только если уже есть запись
                    with get_db_cursor() as _cur:
                        _cur.execute(
...
def show_active_stats(chat_id, user_id, message_id=None):
    ...
    try:
        now = int(time.time())
        with get_db_cursor() as cursor:
            # Активные за 24 часа (писали боту)
            cursor.execute('SELECT COUNT(*) FROM users WHERE last_activity > ?', (now - 86400,))
            active_users_24h = cursor.fetchone()[0] or 0

            # Активные за 7 дней
            cursor.execute('SELECT COUNT(*) FROM users WHERE last_activity > ?', (now - 604800,))
            active_users_7d = cursor.fetchone()[0] or 0

            # Активные за 30 дней
            cursor.execute('SELECT COUNT(*) FROM users WHERE last_activity > ?', (now - 2592000,))
            ...
            # Новые за сегодня (зарегистрировались — last_activity в пределах суток и не было до этого)
            cursor.execute('SELECT COUNT(*) FROM users WHERE last_activity > ?', (now - 86400,))
...
TAX_CONFIG = {
    ...
    "high_wealth_threshold": 1_000_000,   # >1 млн 🌸 — богачи (10%)
    "medium_wealth_tax": 0.05,
    "medium_wealth_threshold": 100_000,   # >100к 🌸 — состоятельные (5%)
    "general_tax": 0.03,                  # все остальные с деньгами (3%)
    "min_tax_amount": 100                 # минимальный налог 100 🌸
}

def collect_taxes():
    """Собрать налоги со всех пользователей (с баланса И с банка)"""
    try:
        with get_db_cursor() as cursor:
            cursor.execute('''
            ...
            for user in users:
                ...
                # Определяем ставку по суммарному состоянию
                if total_wealth >= TAX_CONFIG["high_wealth_threshold"]:
                    tax_rate   = TAX_CONFIG["high_wealth_tax"]
                    tax_reason = f"10% (богачи >{TAX_CONFIG['high_wealth_threshold']//1000}к)"
                elif total_wealth >= TAX_CONFIG["medium_wealth_threshold"]:
                    tax_rate   = TAX_CONFIG["medium_wealth_tax"]
                    tax_reason = f"5% (состоятельные >{TAX_CONFIG['medium_wealth_threshold']//1000}к)"
                ...
                # Считаем налог со всего состояния
                total_tax = int(total_wealth * tax_rate)
                if total_tax < TAX_CONFIG["min_tax_amount"]:
                    continue

                # Сначала списываем с баланса, остаток — с банка
                ...
                # Уведомление пользователю — красивое и стильное
                breakdown_parts = []
                if tax_from_balance > 0:
                    breakdown_parts.append(
                        f"👛 Кошелёк:  <b>-{format_balance(tax_from_balance)} 🌸</b>\n"
                        f"   <code>{format_balance(balance)}</code> ➜ <code>{format_balance(new_balance)}</code>"
                    )
                if tax_from_bank > 0:
                    breakdown_parts.append(
                        f"🏦 Банк:      <b>-{format_balance(tax_from_bank)} 🌸</b>\n"
                        f"   <code>{format_balance(bank_deposit)}</code> ➜ <code>{format_balance(new_bank)}</code>"
                    )

                # Определяем «уровень» налогоплательщика
                if total_wealth >= TAX_CONFIG["high_wealth_threshold"]:
                    tier_icon = "🏆"
                    tier_name = "Элита"
                elif total_wealth >= TAX_CONFIG["medium_wealth_threshold"]:
                ...
                remaining = total_wealth - actual_tax
                ...
                notify_text = (
                    ...
                    f"╚══════════════════════╝\n\n"
                    f"{tier_icon} Статус: <b>{tier_name}</b>\n"
                    f"📋 Налоговая ставка: <b>{tax_percent}%</b>\n\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n"
                    f"💸 <b>Списано налога:</b>  <b>{format_balance(actual_tax)} 🌸</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━━\n\n"
                    + ("\n".join(breakdown_parts)) +
                    f"\n\n💰 Состояние после: <b>{format_balance(remaining)} 🌸</b>\n\n"
                    f"<i>Налоги — основа экономики государства.\nСпасибо за участие в жизни страны! 🌸</i>"
                )
...
@bot.message_handler(func=lambda message: message.text and message.text.lower().strip() == 'собрать налог' and is_admin(message.from_user.id))
def handle_collect_tax(message):
    ...
    try:
        ...
        message_text = (
            f"╔══════════════════════╗\n"
            f"   🏛️  <b>СБОР НАЛОГОВ</b>\n"
            f"╚══════════════════════╝\n\n"
        )
        message_text += (
            f"<blockquote>"
            f"📊 <b>Экономика:</b>\n"
            f"👥 Граждан:         {stats['total_users']}\n"
            f"💵 Всего в обороте: {format_balance(stats['total_wealth'])} 🌸\n"
            f"👛 На балансах:     {format_balance(stats['total_balance'])} 🌸\n"
            f"🏦 На вкладах:      {format_balance(stats['total_deposits'])} 🌸"
            f"</blockquote>\n\n"
        )
        message_text += f"📈 <b>Категории налогообложения:</b>\n"
        message_text += f"🏆 Элита (&gt;1млн):        {stats['high_wealth']['count']} чел. — <b>10%</b>\n"
        message_text += f"💎 Состоятельные (&gt;100к): {stats['medium_wealth']['count']} чел. — <b>5%</b>\n"
        message_text += f"👤 Граждане:                {stats['low_wealth']['count']} чел. — <b>3%</b>\n\n"
        message_text += f"⚠️ <b>Подтвердите запуск сбора налогов!</b>"
...
def handle_confirm_tax_collection(call):
    ...
    try:
        ...
        if result['success']:
            message_text = (
                f"╔══════════════════════╗\n"
                f"   🏛️  <b>НАЛОГИ СОБРАНЫ!</b>\n"
                f"╚══════════════════════╝\n\n"
                f"💰 Итого в казну:  <b>{format_balance(result['total_collected'])} 🌸</b>\n"
                f"👥 Затронуто:  <b>{result['affected_users']}</b> граждан\n\n"
            )
            
            if result['tax_report']:
                # Сортируем по сумме налога
                sorted_report = sorted(result['tax_report'], key=lambda x: x['tax'], reverse=True)
                message_text += "🏆 <b>Крупнейшие налогоплательщики:</b>\n"
                medals = ["🥇", "🥈", "🥉"] + ["🔹"] * 10
                for i, taxpayer in enumerate(sorted_report[:10]):
                    medal = medals[i] if i < len(medals) else "▪️"
                    message_text += (
                        f"{medal} {taxpayer['user']}\n"
                        f"   └ <b>{format_balance(taxpayer['tax'])} 🌸</b>  <i>({taxpayer['rate']})</i>\n"
                    )
...
def handle_tax_stats(message):
    ...
    try:
        ...
        message_text += f"• 🏆 Богачи (>1млн): {stats['high_wealth']['count']} чел. ({format_balance(stats['high_wealth']['total'])})\n"
        message_text += f"• 💵 Состоятельные (>100к): {stats['medium_wealth']['count']} чел. ({format_balance(stats['medium_wealth']['total'])})\n"
...
📋 <b>Ставки налогов:</b>
• 10% - для богачей (>1 000 000 🌸 общего состояния)
• 5% - для состоятельных (>100 000 🌸 общего состояния)
