...
# Абсолютный путь — чтобы bot.py и app.py читали одну и ту же базу
# независимо от рабочей директории процесса
_DB_PATH = os.getenv("DB_PATH", os.path.join(os.path.dirname(os.path.abspath(__file__)), "game.db"))
...
def handle_broadcast_confirmation(call):
    ...
    # Определяем есть ли ссылка в тексте — если есть, отключаем превью
    import re as _re
    has_link = bool(_re_url := _re.search(r'https?://', broadcast_text))
    ...
    pending_broadcast = None  # сбрасываем до цикла чтобы не заблокировать
        ...
        for user_id in all_users:
            try:
                bot.send_message(
                    ...
                    broadcast_text,
                    parse_mode='HTML',
                    disable_web_page_preview=has_link
                )
                sent += 1
            except Exception:
                failed += 1
            time.sleep(0.05)  # ~20 msg/s — в пределах лимита Telegram
    ...
    try:
        if call.data == "mining_collect":
            ...
            with get_db_cursor() as cursor:
                ...
                # Максимум 24 часа накопления — защита от «накопил за месяц и забрал»
                MAX_HOURS = 24
                time_passed = current_time - last_collect if last_collect > 0 else 3600
                time_passed = min(time_passed, MAX_HOURS * 3600)

                income_per_hour = calculate_mining_income(video_cards)
                income = int(income_per_hour * (time_passed / 3600))

                if income > 0:
                    # Атомарное обновление: сбрасываем last_mining_collect ТОЛЬКО если он не изменился
                    # (защита от двойного сбора при двойном нажатии)
                    rows = cursor.execute(
                        'UPDATE users SET balance = balance + ?, last_mining_collect = ? '
                        'WHERE user_id = ? AND last_mining_collect = ?',
                        (income, current_time, user_id, last_collect if last_collect else 0)
                    ).rowcount
                    if rows == 0:
