...
STOCK_LOG_CHANNEL = -1003777645780
...
FLOOD_MAX_MESSAGES = 12
FLOOD_WINDOW = 5
FLOOD_BAN_DURATION = 300
    ...
    for message in messages:
        try:
            ...
            try:
                ...
                is_private = True
            if is_private and check_flood(uid):
                continue
            try:
                if is_private:
...
def _btn(text_with_emoji, **kwargs):
    """
    Создать inline-кнопку с поддержкой премиум-смайлов через text_entities.
    Парсит <tg-emoji emoji-id="...">FALLBACK</tg-emoji> → plain text + entities.
    kwargs: callback_data, url, etc.
    """
    import re as _re
    pattern = _re.compile(r'<tg-emoji emoji-id=["\'](\d+)["\'\"]>([^<]*)</tg-emoji>')
    entities = []
    result_chars = []
    i = 0
    s = text_with_emoji
    while i < len(s):
        ...
        if not m:
            ...
            break
        result_chars.append(s[i:m.start()])
        emoji_id = m.group(1)
        fallback = m.group(2)
        offset = len(''.join(result_chars))
        ...
        i = m.end()
    plain = ''.join(result_chars)
    btn = {"text": plain}
    if entities:
        btn["text_entities"] = entities
    btn.update(kwargs)
    return btn
...
class DatabasePool:
    def __init__(self, db_name=_DB_PATH, pool_size=20):
...
_db_maintenance = threading.Event()
_db_swap_lock = threading.Lock()

@contextmanager
def get_db_connection():
    if _db_maintenance.is_set():
        waited = 0.0
        while _db_maintenance.is_set() and waited < 30:
            time.sleep(0.2)
            waited += 0.2
        if _db_maintenance.is_set():
            raise sqlite3.OperationalError("database maintenance in progress")
...
def _db_checkpoint():
    """Принудительно сливает WAL в основной файл. Вызывать ПЕРЕД копированием базы."""
    try:
        with get_db_connection() as conn:
            conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
        print("[db] WAL checkpoint выполнен — база готова к копированию")
    except Exception as e:
        print(f"[db] checkpoint ошибка: {e}")

def _db_shutdown(signum, frame):
    """При SIGTERM/SIGINT — checkpoint + закрыть все соединения из пула."""
    print("[db] завершение — выполняю checkpoint...")
    _db_checkpoint()
    with db_pool._lock:
...
import signal
signal.signal(signal.SIGTERM, _db_shutdown)
signal.signal(signal.SIGINT,  _db_shutdown)


def _db_keepalive():
    """Раз в 4 минуты делаем лёгкий SELECT — не даём SQLite/Render закрыть соединение при простое."""
    while True:
        time.sleep(240)
        try:
...
''')
        ...
        CREATE TABLE IF NOT EXISTS invoice_payments (
            ...
            PRIMARY KEY (user_id, invoice_code)
        )
        ''')
        ...
        # Миграция pickaxe.dig_mode
        try:
            cursor.execute("PRAGMA table_info(pickaxe)")
            pk_cols = [r[1] for r in cursor.fetchall()]
            if 'dig_mode' not in pk_cols:
                cursor.execute("ALTER TABLE pickaxe ADD COLUMN dig_mode TEXT DEFAULT 'single'")
                print("Добавлена колонка dig_mode в таблицу pickaxe")
        except Exception:
            pass
        ...
        """)
        defaults = [
            ("stars_1",   1,   10000,   "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>"),
            ("stars_5",   5,   66000,   "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>"),
            ("stars_15",  15,  266000,  "<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>"),
            ("stars_50",  50,  1000000, "<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>"),
            ("stars_150", 150, 4000000, "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>️"),
            ("stars_250", 250, 8000000, "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>️"),
        ]
        cursor.executemany(
...
# Миграция БД: добавить колонки бана если их нет
...
def _migrate_bio_cached():
    try:
        with get_db_cursor() as cur:
            cur.execute("ALTER TABLE users ADD COLUMN bio_cached TEXT DEFAULT NULL")
    except Exception:
        pass
_migrate_bio_cached()

def _migrate_weekly_tax():
    """Добавляет колонки для еженедельного налога."""
    cols = [
        ("tax_debt",         "INTEGER DEFAULT 0"),
        ("tax_due_at",       "INTEGER DEFAULT 0"),
        ("tax_paid_at",      "INTEGER DEFAULT 0"),
        ("tax_restricted",   "INTEGER DEFAULT 0"),
        ("tax_amount_due",   "INTEGER DEFAULT 0"),
    ]
    for col, definition in cols:
        try:
            ...
            pass
    # Таблица истории налоговых платежей
    try:
        with get_db_cursor() as cur:
            cur.execute("""
...
# ══════════════════════════════════════════════════════════════
#   🏛️  ЕЖЕНЕДЕЛЬНЫЕ НАЛОГИ
# ══════════════════════════════════════════════════════════════

WEEKLY_TAX_DAY      = 0          # 0=понедельник — день выставления счёта
WEEKLY_TAX_GRACE    = 48 * 3600  # 48 часов на оплату до просрочки
WEEKLY_TAX_RATES    = [          # (порог, ставка%)
    (10_000_000, 0.12),
    (1_000_000,  0.08),
    (100_000,    0.05),
    (10_000,     0.03),
    (0,          0.00),          # у кого меньше 10к — освобождены
]
WEEKLY_TAX_MIN      = 1_000      # минимальная сумма налога

def _tax_rate_for(wealth: int) -> float:
    for threshold, rate in WEEKLY_TAX_RATES:
        if wealth >= threshold:
            return rate
    return 0.0

def _tax_calc_for_user(user_id: int) -> dict:
    """Рассчитывает текущий налог для пользователя."""
    with get_db_cursor() as cur:
        cur.execute("SELECT balance, bank_deposit FROM users WHERE user_id=?", (user_id,))
        row = cur.fetchone()
    if not row:
        return {"amount": 0, "rate": 0.0, "wealth": 0}
    wealth = int(row[0] or 0) + int(row[1] or 0)
    rate   = _tax_rate_for(wealth)
    amount = int(wealth * rate)
    if amount < WEEKLY_TAX_MIN:
        amount = 0
    return {"amount": amount, "rate": rate, "wealth": wealth}

def _tax_is_restricted(user_id: int) -> bool:
    """Есть ли у пользователя налоговое ограничение."""
    with get_db_cursor() as cur:
        cur.execute("SELECT tax_restricted FROM users WHERE user_id=?", (user_id,))
        row = cur.fetchone()
    return bool(row and row[0])

def _tax_get_status(user_id: int) -> dict:
    """Полный налоговый статус пользователя."""
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT tax_debt, tax_due_at, tax_paid_at, tax_restricted, tax_amount_due "
            "FROM users WHERE user_id=?",
            (user_id,)
        )
        row = cur.fetchone()
    if not row:
        return {"debt": 0, "due_at": 0, "paid_at": 0, "restricted": False, "amount_due": 0}
    return {
        "debt":       int(row[0] or 0),
        "due_at":     int(row[1] or 0),
        "paid_at":    int(row[2] or 0),
        "restricted": bool(row[3]),
        "amount_due": int(row[4] or 0),
    }

def _tax_issue_weekly():
    """
    Выставляет еженедельный налоговый счёт всем пользователям.
    Запускается каждый понедельник в 10:00.
    """
    import datetime as _dt2
    now    = int(time.time())
    due_at = now + WEEKLY_TAX_GRACE
    period = _dt2.date.today().strftime("%Y-W%W")

    with get_db_cursor() as cur:
        cur.execute(
            "SELECT user_id, balance, bank_deposit, tax_paid_at FROM users WHERE is_banned=0"
        )
        rows = cur.fetchall()

    issued = 0
    for user_id, balance, bank_deposit, tax_paid_at in rows:
        wealth = int(balance or 0) + int(bank_deposit or 0)
        rate   = _tax_rate_for(wealth)
        amount = int(wealth * rate)
        if amount < WEEKLY_TAX_MIN:
            continue

        # Если в этом периоде уже платил — пропускаем
        if tax_paid_at and (now - tax_paid_at) < 6 * 24 * 3600:
            continue

        with get_db_cursor() as cur:
            cur.execute(
                "UPDATE users SET tax_amount_due=?, tax_due_at=?, tax_debt=1, tax_restricted=0 WHERE user_id=?",
                (amount, due_at, user_id)
            )
        issued += 1

        tier = next(
            (name for threshold, _, name in [
                (10_000_000, 0.12, "💎 Элита"),
                (1_000_000,  0.08, "🏆 Состоятельный"),
                (100_000,    0.05, "👤 Средний класс"),
                (10_000,     0.03, "🌱 Начинающий"),
            ] if wealth >= threshold),
            "🌱 Начинающий"
        )
        ...
        try:
            bot.send_message(
                user_id,
                f"🏛️ <b>НАЛОГОВАЯ СЛУЖБА</b>\n\n"
                f"📋 Тебе выставлен еженедельный налог!\n\n"
                f"{tier} — <b>{int(rate*100)}%</b>\n"
                f"💰 Сумма: <b>{format_balance(amount)}</b>\n"
                f"⏰ Срок оплаты: <b>48 часов</b>\n\n"
                f"⚠️ При просрочке — ограничения на переводы, чеки, КНБ и дуэли.\n\n"
                f"Напиши <code>налог</code> чтобы оплатить.",
                parse_mode="HTML"
            )
        except Exception:
            pass

    print(f"[weekly_tax] выставлено счетов: {issued}")
    return issued

def _tax_apply_overdue():
    """
    Проверяет просрочки и включает ограничения.
    Запускается каждые 30 минут.
    """
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT user_id, tax_amount_due FROM users "
            "WHERE tax_debt=1 AND tax_restricted=0 AND tax_due_at > 0 AND tax_due_at <= ?",
            (now,)
        )
        overdue = cur.fetchall()

    for user_id, amount_due in overdue:
        with get_db_cursor() as cur:
            cur.execute(
                "UPDATE users SET tax_restricted=1 WHERE user_id=?",
                (user_id,)
            )
        try:
            bot.send_message(
                user_id,
                f"🚫 <b>НАЛОГОВОЕ ОГРАНИЧЕНИЕ ВВЕДЕНО</b>\n\n"
                f"Срок оплаты налога истёк.\n"
                f"💸 Долг: <b>{format_balance(amount_due)}</b>\n\n"
                f"Ограничены: переводы, чеки, КНБ, дуэли, PVP-ставки.\n"
                f"Напиши <code>налог</code> чтобы оплатить и снять ограничения.",
                parse_mode="HTML"
            )
        except Exception:
            pass

def _tax_pay(user_id: int) -> tuple:
    """
    Пользователь оплачивает налог. Возвращает (ok, message).
    """
    status = _tax_get_status(user_id)
    if not status["debt"] and not status["restricted"]:
        return False, "✅ У тебя нет налоговых долгов!"
    ...
    if amount <= 0:
        ...
        with get_db_cursor() as cur:
            cur.execute(
                "UPDATE users SET tax_debt=0, tax_restricted=0, tax_amount_due=0 WHERE user_id=?",
                (user_id,)
            )
        return True, "✅ Налоговый долг погашен."

    balance = get_balance(user_id)
    if balance < amount:
        return False, (
            f"❌ Недостаточно средств для оплаты налога!\n\n"
            f"💸 Долг: <b>{format_balance(amount)}</b>\n"
            f"💰 Баланс: <b>{format_balance(balance)}</b>\n\n"
            f"Не хватает: <b>{format_balance(amount - balance)}</b>"
        )

    now  = int(time.time())
    late = status["restricted"] or (status["due_at"] > 0 and now > status["due_at"])
    import datetime as _dt2
    period = _dt2.date.today().strftime("%Y-W%W")
    ...
    with get_db_cursor() as cur:
        ...
        cur.execute(
            "UPDATE users SET tax_debt=0, tax_restricted=0, tax_amount_due=0, "
            "tax_due_at=0, tax_paid_at=? WHERE user_id=?",
            (now, user_id)
        )
        cur.execute(
            "INSERT INTO tax_payments (user_id, amount, period, paid_at, was_late) VALUES (?,?,?,?,?)",
            (user_id, amount, period, now, 1 if late else 0)
        )

    late_text = " (с просрочкой)" if late else ""
    return True, (
        f"✅ <b>Налог оплачен{late_text}!</b>\n\n"
        f"💸 Уплачено: <b>{format_balance(amount)}</b>\n"
        f"💰 Баланс: <b>{format_balance(get_balance(user_id))}</b>\n\n"
        f"Все ограничения сняты. 🎉"
    )

def _tax_scheduler():
    """Планировщик: выставляет счёт по понедельникам, проверяет просрочки."""
    import datetime as _dt2
    print("[weekly_tax] планировщик запущен")
    last_issue_week = -1
    while True:
        try:
            now  = _dt2.datetime.now()
            week = now.isocalendar()[1]
            # Выставляем счёт в понедельник в 10:00–11:00
            if now.weekday() == WEEKLY_TAX_DAY and now.hour == 10 and week != last_issue_week:
                _tax_issue_weekly()
                last_issue_week = week
            # Проверяем просрочки каждые 30 минут
            _tax_apply_overdue()
        except Exception as _e:
            print(f"[weekly_tax] error: {_e}")
        time.sleep(30 * 60)

threading.Thread(target=_tax_scheduler, daemon=True).start()
...
PREMIUM_EMOJI = "<tg-emoji emoji-id='5323651219393095687'><tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji></tg-emoji>"
MONEY_EMOJI   = "<tg-emoji emoji-id='5435999124245729290'><tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji></tg-emoji>"
SAD_EMOJI     = "<tg-emoji emoji-id='5335037736040545052'>😢</tg-emoji>"
SALUTE_EMOJI  = "<tg-emoji emoji-id='5334792918609705827'>🎉</tg-emoji>"

PREMIUM_PRICES = [
    {"stars": 7,  "days": 7,   "label": "7 дней — 7 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> (~22₽)"},
    {"stars": 25,  "days": 30,  "label": "30 дней — 25 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> (~75₽)"},
    {"stars": 50, "days": 90,  "label": "90 дней — 50 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> (~150₽)"},
]
...
def _premium_main_text(user_id):
    ...
    return (
        ...
        f"Статус: {status}\n\n"
        f"<blockquote>"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Бонус каждые 10 мин вместо 20\n"
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id,
                           ...
                           f"<blockquote><tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Вещь: {item_name}\n"
                           f"🆔 ID: {item_id}\n"
                           f"📁 Файл: {image_name}\n"
...
def confirm_delete_item(call):
    try:
        ...
        result_message = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Вещь полностью удалена!\n\n"
        result_message += f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Название: {item_name}\n"
...
def cancel_delete_item(call):
    bot.edit_message_text(
        "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Удаление вещи сброшено",
        call.message.chat.id,
        call.message.message_id
    , parse_mode='HTML')
...
def handle_item_info(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            info_text += f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Название: {item['name']}\n"
            info_text += f"🆔 ID: {item['id']}\n"
            info_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Цена: {format_balance(item['price'])}\n"
            ...
            if owners:
                ...
                for owner in owners[:10]:
                    ...
                    status = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Надето" if equipped else "👕 В инвентаре"
        ...
        with get_db_cursor() as cursor:
            ...
            if admin_chat_id:
                bot.send_message(admin_chat_id, 
                               f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Вещь выдана!\n\n"
                               f"🎴 Пользователь: {user_name}\n"
                               f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Вещь: {item_name}\n"
                               f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Стоимость: {format_balance(item_price)}", parse_mode='HTML')
            
            try:
                bot.send_message(user_id,
                               f"🎉 Вам выдана вещь!\n\n"
                               f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> {item_name}\n"
                               f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Стоимость: {format_balance(item_price)}\n\n"
...
def ban_user(user_id, reason="Нарушение правил", banned_by=None):
    """Забанить пользователя — сохраняется в БД навсегда."""
    if is_admin(user_id):
        return False
    with get_db_cursor() as cur:
        cur.execute(
            "UPDATE users SET is_banned=1, ban_reason=?, banned_by=?, banned_at=? WHERE user_id=?",
            (reason, banned_by, int(time.time()), user_id)
        )
...
def unban_user(user_id):
    ...
    with get_db_cursor() as cur:
        cur.execute(
            "UPDATE users SET is_banned=0, ban_reason=NULL, banned_by=NULL, banned_at=NULL WHERE user_id=?",
            (user_id,)
        )
    print(f"🔓 Пользователь {user_id} разбанен")

def is_user_banned(user_id):
    """Проверить бан через БД."""
    if is_admin(user_id):
        return False
    try:
        with get_db_cursor() as cur:
...
def get_ban_info(user_id):
    ...
    try:
        with get_db_cursor() as cur:
            cur.execute("SELECT is_banned, ban_reason, banned_by, banned_at FROM users WHERE user_id=?", (user_id,))
            row = cur.fetchone()
            if row and row[0]:
                return {"banned": True, "reason": row[1] or "Нарушение правил", "by": row[2], "at": row[3]}
    except Exception:
        pass
    return {"banned": False}
...
def handle_ban_user(message):
    try:
        ...
        success = ban_user(target_user_id, reason, banned_by=message.from_user.id)
        
        if success:
            from datetime import datetime as _dt
            ban_time = _dt.now().strftime('%d.%m.%Y %H:%M')
            bot.send_message(
                message.chat.id,
                f"🔨 <b>Пользователь {target_user_id} забанен!</b>\n"
    ...
    try:
        ...
        try:
            bot.send_message(
                ...
                "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Теперь вы снова можете использовать все функции бота.",
                parse_mode='HTML'
            )
        except:
            pass
...
def init_taxi_database():
    with get_db_cursor() as cursor:
        ...
        try:
            cursor.execute('ALTER TABLE active_trips ADD COLUMN status TEXT DEFAULT "pending"')
        except:
            pass
...
def get_active_trip(user_id):
    """Получить принятую активную поездку (status=accepted)"""
    with get_db_cursor() as cursor:
        cursor.execute(
            "SELECT trip_data FROM active_trips WHERE user_id = ? AND finish_time IS NULL AND status = 'accepted'",
            (user_id,)
        )
        result = cursor.fetchone()
        if result:
...
def get_pending_trip(user_id):
    """Получить заказ ожидающий принятия"""
    with get_db_cursor() as cursor:
        cursor.execute(
            "SELECT trip_data FROM active_trips WHERE user_id = ? AND finish_time IS NULL AND status = 'pending'",
            (user_id,)
        )
        result = cursor.fetchone()
        if result:
...
def save_active_trip(user_id, trip_data, chat_id=None, message_id=None, status="pending"):
    """Сохранить поездку. status='pending' — ожидает принятия, 'accepted' — в пути"""
    ...
    with get_db_cursor() as cursor:
        ...
        cursor.execute(
            'INSERT INTO active_trips (user_id, trip_data, chat_id, message_id, start_time, finish_time, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
            (user_id, trip_json, chat_id, message_id, int(time.time()), None, status)
        )

def accept_trip(user_id):
    """Отметить поездку как принятую"""
    with get_db_cursor() as cursor:
        cursor.execute(
            "UPDATE active_trips SET status='accepted' WHERE user_id=? AND finish_time IS NULL",
            (user_id,)
        )
...
def show_driver_class(chat_id, user_id):
    ...
    msg += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Уровень:</b> {class_info['name']}\n"
    ...
    if progress['has_next']:
        ...
        msg += f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Осталось: {progress['trips_needed']} поездок\n"
    else:
...
def show_taxi_menu(chat_id, user_id):
    """Главное меню такси — inline-сообщение"""
    driver_class  = get_driver_class_info(user_id)
    class_info    = driver_class["info"]
    stats         = get_driver_stats(user_id)
    progress      = get_next_class_progress(user_id)
    ...
    if active:
        status_line = f"🚗 <b>В пути:</b> {active['name']} · {active['time']}"
    else:
        status_line = "🟢 <b>Свободен</b> · готов к выезду"

    if progress["has_next"]:
        nc = progress["next_class"]
        bar_filled = int(progress["progress"] / 10)
        bar = "█" * bar_filled + "░" * (10 - bar_filled)
        progress_line = f"\n📈 До {nc['name']}: [{bar}] {progress['current_trips']}/{progress['required_trips']}"
    else:
        progress_line = "\n<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Максимальный класс!"

    text = (
        f"{class_info['emoji']} <b>ТАКСИ</b>\n\n"
        f"👤 Класс: <b>{class_info['name']}</b>\n"
        f"📊 Поездок: <b>{trips}</b>   <tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Заработано: <b>{format_balance(earned)}</b>\n"
        f"{status_line}"
        f"{progress_line}"
    )
    ...
    if active:
        kb = {"inline_keyboard": [
            [{"text": "📊 Статистика", "callback_data": "taxi_stats"},
             {"text": "🏆 Рейтинг",   "callback_data": "taxi_rating"}],
        ]}
    else:
        kb = {"inline_keyboard": [
            [{"text": "🚕 Выехать на линию", "callback_data": "taxi_start_shift"}],
            [{"text": "📊 Статистика",       "callback_data": "taxi_stats"},
             {"text": "🏆 Рейтинг",          "callback_data": "taxi_rating"}],
            [{"text": "⭐ Мой уровень",      "callback_data": "taxi_my_level"}],
        ]}

    _tg_api("sendMessage", chat_id=chat_id, text=text, parse_mode="HTML", reply_markup=kb)
...
def handle_start_shift(message):
    ...
    active = get_active_trip(user_id)
    if active:
        class_info = DRIVER_CLASSES.get(active.get("class", "economy"), DRIVER_CLASSES["economy"])
        bot.send_message(
            message.chat.id,
            f"🚦 <b>Ты уже в пути!</b>\n\n"
            f"📍 {active['name']}\n"
            f"⏱ {active['time']}\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(active['price'])}\n"
            f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> {class_info['name']}\n\n"
            f"Дождись завершения поездки.",
            parse_mode='HTML'
        )
        return
    ...
    with get_db_cursor() as _cur:
        _cur.execute(
            "UPDATE active_trips SET finish_time=? WHERE user_id=? AND status='pending' AND finish_time IS NULL",
            (int(time.time()), user_id)
        )
...
def create_new_order(chat_id, user_id):
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Оплата:</b> {format_balance(order['price'])}\n"
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> <b>Класс:</b> {class_info['name']} (x{order['class_multiplier']})\n"
        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Опыт:</b> +{order['experience']}\n\n"
        f"<b>Берешь заказ?</b>"
    )
...
def show_driver_rating(chat_id):
    ...
    text = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>ТОП ВОДИТЕЛЕЙ</b>\n\n"
...
def handle_my_stats(message):
    ...
    if active:
        ...
        text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Оплата:</b> {format_balance(active['price'])}\n\n"
    
    text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Класс:</b> {class_info['name']}\n"
    text += f"📊 <b>Поездок:</b> {stats['trips_completed']}\n"
    text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Заработано:</b> {format_balance(stats['total_earned'])}\n"
    text += f"🏅 <b>Уровень:</b> {level}\n"
    
    if stats['trips_completed'] > 0:
        avg = stats['total_earned'] // stats['trips_completed']
        text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Средний чек:</b> {format_balance(avg)}\n"
...
def handle_taxi_actions(call):
    ...
    if call.data == "taxi_start_shift":
        active = get_active_trip(user_id)
        if active:
            class_info = DRIVER_CLASSES.get(active.get("class","economy"), DRIVER_CLASSES["economy"])
            bot.answer_callback_query(call.id, f"Ты уже в пути: {active['name']}", show_alert=True)
            return
        with get_db_cursor() as _cur:
            _cur.execute(
                "UPDATE active_trips SET finish_time=? WHERE user_id=? AND status='pending' AND finish_time IS NULL",
    ...
    elif call.data == "taxi_stats":
        bot.answer_callback_query(call.id)
        _show_taxi_stats_inline(call.message.chat.id, user_id)

    elif call.data == "taxi_rating":
        bot.answer_callback_query(call.id)
        show_driver_rating(call.message.chat.id)

    elif call.data == "taxi_my_level":
        bot.answer_callback_query(call.id)
        show_driver_class(call.message.chat.id, user_id)

    elif call.data == "taxi_accept":
        accept_order(call)
...
def accept_order(call):
    ...
    order = get_pending_trip(user_id)
    if not order:
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Стоимость:</b> {format_balance(order['price'])}\n"
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> <b>Класс:</b> {class_info['name']}\n\n"
...
def finish_ride(user_id, chat_id, message_id, order):
    ...
    try:
        ...
        text = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>ПОЕЗДКА ЗАВЕРШЕНА!</b>\n\n"
        text += f"📍 <b>Маршрут:</b> {order['name']}\n"
        text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Заработано:</b> {format_balance(order['price'])}\n"
        text += f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Опыт:</b> +{order['experience']}\n"
        text += f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> <b>Класс:</b> {class_info['name']}\n"
...
def _show_taxi_stats_inline(chat_id, user_id):
    """Статистика водителя — отдельным сообщением"""
    stats      = get_driver_stats(user_id)
    class_info = get_driver_class_info(user_id)["info"]
    active     = get_active_trip(user_id)

    avg = (stats["total_earned"] // stats["trips_completed"]) if stats["trips_completed"] > 0 else 0

    text = (
        f"{class_info['emoji']} <b>Статистика водителя</b>\n\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Класс: <b>{class_info['name']}</b>\n"
        f"📊 Поездок: <b>{stats['trips_completed']}</b>\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Заработано: <b>{format_balance(stats['total_earned'])}</b>\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Средний чек: <b>{format_balance(avg)}</b>\n"
    )
    if active:
        text += f"\n🚗 <b>Сейчас в пути:</b> {active['name']}"

    _tg_api("sendMessage", chat_id=chat_id, text=text, parse_mode="HTML",
            reply_markup={"inline_keyboard": [[{"text": "◀️ Назад", "callback_data": "taxi_back_menu"}]]})
...
@bot.callback_query_handler(func=lambda c: c.data == "taxi_back_menu")
def cb_taxi_back_menu(call):
    uid = call.from_user.id
    bot.answer_callback_query(call.id)
    show_taxi_menu(call.message.chat.id, uid)
...
def calculate_interest():
    ...
    for uid, earned, new_deposit, premium in users_to_notify:
        try:
            ...
            premium_tag = " <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Premium" if premium else ""
            bot.send_message(
                uid,
                f"🏦 <b>Проценты зачислены!</b>{premium_tag}\n\n"
                f"📈 Ставка: <b>{rate_text}</b> каждые 3 часа\n"
                f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Начислено: <b>+{format_balance(earned)}</b>\n"
...
def handle_bank_deposit_cmd(message):
    ...
    if is_user_banned(user_id):
        bot.send_message(message.chat.id, "🚫 Вы забанены — банк недоступен.", parse_mode='HTML')
        return
    parts = message.text.strip().split()
    if len(parts) < 2:
    ...
    if amount > balance:
        bot.send_message(
            message.chat.id,
            f"❌ Недостаточно монет.\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Баланс: {format_balance(balance)}",
            parse_mode='HTML'
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        f"🏦 <b>Вклад внесён</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Внесено: <b>{format_balance(amount)}</b>\n"
...
def handle_bank_withdraw_cmd(message):
    ...
    bot.send_message(
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Получено: <b>{format_balance(amount)}</b>\n"
...
def handle_bank(message):
    ...
    # Налоговый блок
    tax_st = _tax_get_status(user_id)
    now    = int(time.time())
    if tax_st["debt"] or tax_st["restricted"]:
        amount_due = tax_st["amount_due"]
        due_at     = tax_st["due_at"]
        if tax_st["restricted"]:
            tax_block = (
                f"\n🚫 <b>НАЛОГОВОЕ ОГРАНИЧЕНИЕ</b>\n"
                f"💸 Долг: <b>{format_balance(amount_due)}</b>\n"
                f"Напиши <code>налог</code> чтобы оплатить и снять блок.\n"
            )
        else:
            left   = max(0, due_at - now)
            h, m   = divmod(left // 60, 60)
            tax_block = (
                f"\n⚠️ <b>Ожидает налоговый платёж</b>\n"
                f"💸 Сумма: <b>{format_balance(amount_due)}</b>\n"
                f"⏰ До просрочки: <b>{h}ч {m}м</b>\n"
                f"Напиши <code>налог</code> чтобы оплатить.\n"
            )
    else:
        # Показываем прогноз следующего налога
        tax_info = _tax_calc_for_user(user_id)
        if tax_info["amount"] > 0:
            tax_block = (
                f"\n🏛 <b>Следующий налог</b> (~{int(tax_info['rate']*100)}%): "
                f"<b>{format_balance(tax_info['amount'])}</b>\n"
                f"Выставляется каждый понедельник в 10:00\n"
            )
        else:
            tax_block = "\n✅ <b>Налогов нет</b> — состояние ниже порога\n"

    text = (
        f"🏛 <b>Банк</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Баланс: {format_balance(balance)}\n"
        f"🏦 Депозит: {format_balance(deposit)}\n\n"
        f"<blockquote>"
        f"📈 Ставка: <b>{rate}/3ч</b>\n\n"
        f"💳 <b>Команды:</b>\n"
        f"<code>вклад [сумма]</code> — положить на депозит\n"
        f"<code>снять [сумма]</code> — снять с депозита\n"
        f"<code>налог</code> — оплатить налог / статус"
        f"</blockquote>"
        f"{tax_block}"
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def handle_deposit_money(message):
    ...
    text = (
        f"🏦 <b>Вклад в банк</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Доступно: {format_balance(balance)}\n"
...
def process_deposit_amount(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Сумма: {format_balance(amount)}\n"
...
def handle_ban_list(message):
    with get_db_cursor() as cur:
        cur.execute("SELECT user_id, ban_reason, banned_at FROM users WHERE is_banned=1 ORDER BY banned_at DESC")
        rows = cur.fetchall()
    if not rows:
        bot.send_message(message.chat.id, "✅ Забаненных пользователей нет.", parse_mode='HTML')
        return
    from datetime import datetime as _dt
    lines = ["🚫 <b>Забаненные пользователи:</b>\n"]
    for uid_r, reason, banned_at in rows:
        ts = _dt.fromtimestamp(banned_at).strftime('%d.%m.%Y') if banned_at else '?'
        lines.append(f"• <code>{uid_r}</code> — {reason or '?'} ({ts})")
...
def handle_unwarn_user(message):
    ...
    try:
        ...
        if success:
            try:
                bot.send_message(target_user_id, "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Твой варн снят! Ограничения сняты.", parse_mode='HTML')
            except:
                pass
            
            bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Варн снят с пользователя ID: {target_user_id}", parse_mode='HTML')
        else:
...
def handle_check_warn(message):
    ...
    try:
        ...
        if warn_info:
            ...
            bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Пользователь ID: {target_user_id} не имеет активных варнов", parse_mode='HTML')
...
def handle_db_stats(message):
    ...
    try:
        ...
        stats_text = (
            ...
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Общий баланс: ❄️<b>{format_balance(total_balance)}</b>\n"
...
def handle_document_upload(message):
    ...
    try:
        ...
        if file_ext == 'db':
            ...
            if not _db_swap_lock.acquire(blocking=False):
                bot.send_message(message.chat.id, "⏳ Замена базы уже выполняется, подождите.")
                return
            ...
            try:
                ...
                try:
                    ...
                    global active_rides, active_mines_games, active_captchas
                    ...
                    with get_db_cursor() as cursor:
                        ...
                        if user_count == 0:
                            ...
                            bot.send_message(
                                ...
                                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> С ненулевым балансом: {users_with_balance}\n\n"
...
def transfer_money(from_user_id, to_user_id, amount):
    ...
    if _tax_is_restricted(from_user_id):
        return False, "🚫 Переводы заблокированы из-за налогового долга! Напишите <code>налог</code> чтобы оплатить."
    ...
    with get_db_cursor() as cursor:
        ...
        return True, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Транзакция окончена!\n💸 Сумма: {format_balance(net_amount)}\n📊 Комиссия: {format_balance(fee)}"
import time
import json
import os
...
_clicker_tokens = {}  # uid -> (token, expires_at)
_clicker_tokens_lock = threading.Lock()

def _clicker_gen_token(user_id):
    """Генерирует одноразовый токен для клика. Без него нажатие не засчитается."""
    token = secrets.token_hex(8)
    with _clicker_tokens_lock:
        _clicker_tokens[user_id] = (token, time.time() + 30)  # 30 сек на нажатие
    return token

def _clicker_check_token(user_id, token):
    """Проверяет токен и сразу удаляет — одноразовый."""
    with _clicker_tokens_lock:
        entry = _clicker_tokens.get(user_id)
        if not entry:
            return False
        stored_token, expires_at = entry
        del _clicker_tokens[user_id]
        if time.time() > expires_at:
            return False
        return stored_token == token

def create_clicker_keyboard(user_id=None):
    """Создает клавиатуру для кликера. Правильная кнопка имеет одноразовый токен."""
    token = _clicker_gen_token(user_id) if user_id else secrets.token_hex(8)
    symbols = ["❌", "❌", "❌", "❌", "✅"]
    ...
    for symbol in symbols:
        if symbol == "✅":
            button = {"text": symbol, "callback_data": f"clicker_ok_{token}", "style": "success"}
        else:
            wrong_token = secrets.token_hex(4)
            button = {"text": symbol, "callback_data": f"clicker_no_{wrong_token}", "style": "danger"}
        row.append(button)
        if len(row) == 3:
...
def safe_cleanup_old_data():
    ...
    for user_id in expired_mines:
        try:
            ...
            try:
                bot.send_message(
                    user_id, 
                    f"🕒 Игра в 'Мины' автоматически окончена\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Возвращено: {format_balance(game_data['bet_amount'])}"
                , parse_mode='HTML')
            except:
                pass
    ...
    logger.info(f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Безопасная очистка окончена: {cleaned_count} объектов")
...
def show_top_balance(chat_id, user_id, page=0, message_id=None):
    try:
        ...
        err_s = str(e)
        if "message is not modified" in err_s or ("Bad Request" in err_s and "not modified" in err_s):
            pass  # контент не изменился — не ошибка
        else:
        ...
        if new_level > old_level:
            ...
            try:
                bot.send_message(
                    ...
                    f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Бонус к кликам: +{int(get_level_click_bonus(new_level)*100-100)}%\n"
                    f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Бонус к ежедневке: +{int(get_level_daily_bonus(new_level)*100-100)}%",
                    parse_mode='HTML'
                )
            except Exception:
                pass
...
def handle_broadcast_confirmation(call):
    ...
    broadcast_data = pending_broadcast
    pending_broadcast = None
    ...
    def _do_broadcast():
        ...
        # Определяем есть ли ссылка в тексте — если да, отключаем превью
        has_link = bool(re.search(r'https?://', text))
        link_preview_options = {"is_disabled": True} if has_link else {"is_disabled": True}
        ...
        result_text = (
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>РАССЫЛКА ЗАВЕРШЕНА</b>\n\n"
            f"📊 Результаты:\n"
            f"✅ Успешно: {sent}\n"
            f"❌ Ошибок: {failed}\n"
            f"👥 Всего: {total}\n\n"
...
def handle_remind_all(message):
    ...
    bot.reply_to(message,
                ...
                f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Отправлено: {sent}\n"
                f"❌ Ошибок: {failed}",
                parse_mode='HTML')
...
def _build_donate_text():
    lines = []
    tier_icons = ["🥉", "🥈", "🥇", "💎", "👑", "🌟"]
    for idx, (key, pkg) in enumerate(DONATE_PACKAGES.items()):
        amt_str = f"{pkg['amount']:,}".replace(',', ' ')
        icon = tier_icons[idx] if idx < len(tier_icons) else "⭐"
        lines.append(f"{icon} <b>{pkg['stars']} зв</b>  →  <code>{amt_str}</code> 🌸")
    return "\n".join(lines)
...
def _build_donate_markup():
    ...
    for i in range(0, len(keys), 2):
        ...
        for k in row_keys:
            ...
            amt_str = f"{pkg['amount']:,}".replace(',', ' ')
            row.append(
                _btn(f'⭐ {pkg["stars"]}', callback_data=k)
            )
        inline_kb.append(row)
    inline_kb.append([_btn('✏️ Своё кол-во ⭐', callback_data="donate_custom")])
...
def handle_buy(message):
    ...
    disc_line = (
        f"\n\n<blockquote>"
        f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> "
        f"У тебя скидка <b>{discount}%</b> на следующую покупку!"
        f"</blockquote>"
    ) if discount else ""

    text = (
        f"<tg-emoji emoji-id='5204065856560045377'>⭐</tg-emoji> <b>Пополнение баланса</b>\n\n"
        f"<b>Пакеты:</b>\n"
...
def handle_setdonat(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> {stars_count} звёзд: <s>{old_str}</s> → <b>{amt_str} 🌸</b>\n\n"
        f"Посмотреть все пакеты: /донат",
        parse_mode='HTML')

@bot.message_handler(commands=['донат'])
def handle_donate_admin_cmd(message):
    """Только для админов: /донат — показывает пакеты"""
    if not is_admin(message.from_user.id):
        handle_buy(message)
        return
    lines = []
    ...
    bot.send_message(message.chat.id,
        "<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Текущие пакеты доната:</b>\n\n" + "\n".join(lines) +
...
def handle_donate_view(message):
    """Текст 'донат' — любой пользователь видит экран покупки"""
    handle_buy(message)
...
@bot.message_handler(func=lambda m: m.text and re.match(r'^скидка\s+всем\b', m.text.strip().lower()) and is_admin(m.from_user.id))
def handle_discount_all(message):
    """Выдать одноразовую скидку всем пользователям. Формат: скидка всем [процент]"""
    parts = message.text.strip().split()
    percent = 50
    if len(parts) >= 3:
        try:
            percent = int(parts[2])
            if not (1 <= percent <= 99):
                bot.send_message(message.chat.id, "❌ Процент должен быть от 1 до 99", parse_mode='HTML')
                return
        except ValueError:
            bot.send_message(message.chat.id, "❌ Процент должен быть числом, например: <code>скидка всем 30</code>", parse_mode='HTML')
            return

    bot.send_message(message.chat.id,
        f"⏳ Выдаю скидку <b>{percent}%</b> всем пользователям...",
        parse_mode='HTML')
    ...
    def _give_all():
        ...
        for uid in all_users:
            try:
                set_discount(uid, percent=percent, admin=True)
                given += 1
                try:
                    send_discount_message(uid, percent=percent)
                    notified += 1
                except Exception:
                    pass
                time.sleep(0.05)
                ...
                pass
        bot.send_message(message.chat.id,
            f"✅ <b>Скидка {percent}% выдана!</b>\n\n"
            f"👥 Пользователей: {total}\n"
            f"🎁 Скидок выдано: {given}\n"
            f"📨 Уведомлено: {notified}",
            parse_mode='HTML')

    threading.Thread(target=_give_all, daemon=True).start()
...
def send_discount_message(user_id, percent=50):
    ...
    try:
        ...
        kb = {
            "inline_keyboard": [[
                {
                    _btn("<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Использовать скидку", url=f"https://t.me/{bot_username}?start=discount")
                }
            ]]
        }
        _tg_api("sendMessage",
                chat_id=user_id,
                text=(f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> <b>Специальное предложение</b>\n\n"
...
@bot.callback_query_handler(func=lambda c: c.data == "donate_custom")
def cb_donate_custom(call):
    user_id = call.from_user.id
    if call.message.chat.type != 'private':
        bot.answer_callback_query(call.id, "💳 Покупки доступны только в личных сообщениях!", show_alert=True)
        return
    per_star = DONATE_PACKAGES.get("stars_1", {}).get("amount", 0)
    if not per_star:
        bot.answer_callback_query(call.id, "❌ Курс не настроен", show_alert=True)
        return
    bot.answer_callback_query(call.id)
    msg = bot.send_message(
        call.message.chat.id,
        f"<tg-emoji emoji-id='5204065856560045377'>⭐</tg-emoji> <b>Введи количество звёзд</b>\n\n"
        f"Курс: <b>1 ⭐ = {per_star:,}</b> 🌸\n"
        f"Напиши целое число от 1 до 10 000:",
        parse_mode="HTML"
    )
    bot.register_next_step_handler(msg, _process_custom_stars_input)

def _process_custom_stars_input(message):
    user_id = message.from_user.id
    ...
    try:
        ...
        bot.send_message(message.chat.id, "❌ Введи целое число, например <b>17</b>", parse_mode="HTML")
        return
    if stars < 1:
        bot.send_message(message.chat.id, "❌ Минимум 1 звезда")
        return
    if stars > 10000:
        bot.send_message(message.chat.id, "❌ Максимум 10 000 звёзд за раз")
        return

    per_star = DONATE_PACKAGES.get("stars_1", {}).get("amount", 0)
    if not per_star:
        bot.send_message(message.chat.id, "❌ Курс не настроен")
        return

    # Бонус: находим лучший пакет не превышающий кол-во звёзд
    bonus_mult = 1.0
    best_pkg_stars = 1
    for key, pkg in sorted(DONATE_PACKAGES.items(), key=lambda x: x[1]['stars']):
        if pkg['stars'] <= stars:
            mult = pkg['amount'] / pkg['stars'] / per_star
            if mult > bonus_mult:
                bonus_mult = mult
                best_pkg_stars = pkg['stars']

    final_amount = int(per_star * stars * bonus_mult)
    discount_pct = get_discount(user_id)
    actual_stars = stars
    discount_line = ''
    if discount_pct:
        actual_stars = max(1, int(stars * (1 - discount_pct / 100)))
        discount_line = f" (-{discount_pct}%)"

    clean_amount = f"{final_amount:,}".replace(',', ' ')
    bonus_line = f"\n🎁 Бонус за объём: <b>×{bonus_mult:.2f}</b>" if bonus_mult > 1.0 else ""

    try:
        bot.send_invoice(
            chat_id=message.chat.id,
            title=f"⭐ {actual_stars} зв. → {clean_amount}{discount_line}",
            description=f"Пополнение баланса на {clean_amount} 🌸" + (f". Бонус ×{bonus_mult:.2f}" if bonus_mult > 1.0 else ""),
            invoice_payload=f"stars_{user_id}_{final_amount}_{discount_pct or 0}",
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(label=f"{actual_stars} звёзд → {clean_amount} 🌸", amount=actual_stars)],
        ...
        bot.send_message(
            message.chat.id,
            f"💡 <b>Итого за {actual_stars} ⭐:</b>\n"
            f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> <code>{clean_amount}</code>{bonus_line}",
            parse_mode="HTML"
        )
    except Exception as e:
        print(f"[custom_stars] {e}")
...
def process_pre_checkout(pre_checkout_query):
    try:
        ...
        # Проверяем актуальность цены для stars-платежей
        if len(payload_parts) >= 3 and payload_parts[0] == 'stars':
            invoice_amount = int(payload_parts[2])  # сумма 🌸 зашитая в инвойс
            invoice_stars  = pre_checkout_query.total_amount  # звёзды которые платит юзер
            
            # Находим текущую цену пакета по кол-ву звёзд
            current_amount = None
            for key, pkg in DONATE_PACKAGES.items():
                if pkg['stars'] == invoice_stars:
                    current_amount = pkg['amount']
                    break
            
            # Если пакет найден и цена изменилась — отклоняем
            if current_amount is not None and invoice_amount != current_amount:
                bot.answer_pre_checkout_query(
                    pre_checkout_query.id,
                    ok=False,
                    error_message="Цена пакета изменилась. Пожалуйста, создайте новый счёт."
                )
                print(f"[pre_checkout] Отклонён устаревший инвойс: stars={invoice_stars}, old_amount={invoice_amount}, current={current_amount}")
                return
...
def handle_successful_payment(message):
    try:
        user_id = message.from_user.id
        if is_user_banned(user_id):
            print(f"[ban] successful_payment заблокирован для user_id={user_id}")
            return
        ...
        if payload_parts[0] == "premium" and len(payload_parts) >= 3:
        ...
        elif payload_parts[0] == "pickaxe" and len(payload_parts) >= 3:
            target_uid = int(payload_parts[1])
            target_lvl = int(payload_parts[2])
            cur_lvl    = _pickaxe_get(target_uid)
            if target_lvl == cur_lvl + 1 and target_lvl <= PICKAXE_MAX:
                _pickaxe_set(target_uid, target_lvl)
                info = PICKAXE_LEVELS[target_lvl]
                bonus_msg = ""
                if target_lvl == 6:
                    update_balance(target_uid, 1_488_000)
                    bonus_msg = "\n🎁 <b>+1 488 000 🌸 бонус за Незеритовую кирку!</b>"
                bot.send_message(
                    message.chat.id,
                    f"⛏️ <b>Кирка улучшена!</b>\n\n"
                    f"{info['label']}  ·  x{info['bonus']}  ·  КД {info['cooldown']//60}м{bonus_msg}",
                    parse_mode="HTML"
                )
            else:
                bot.send_message(message.chat.id, "❌ Ошибка апгрейда кирки — уже улучшена или неверный уровень.")

        elif payload.startswith("mine_boost_") and len(payload_parts) >= 3:
            target_uid = int(payload_parts[2])
            _boost_set(target_uid, minutes=30)
            bot.send_message(
                message.chat.id,
                "<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Буст ×2 активирован!</b>\n\n"
                "Ресурсов за копание вдвое больше — <b>30 минут</b>.",
                parse_mode="HTML"
            )
            ...
            # Защита от двойного начисления: используем charge_id как уникальный ключ
            charge_id = payment_info.telegram_payment_charge_id
            with get_db_cursor() as _cur:
                _cur.execute(
                    "SELECT 1 FROM invoice_payments WHERE user_id=? AND invoice_code=?",
                    (user_id, charge_id)
                )
                already_paid = _cur.fetchone()
            
            if already_paid:
                print(f"[donate] Дубль платежа заблокирован: user_id={user_id}, charge_id={charge_id}")
                bot.send_message(message.chat.id, "⚠️ Этот платёж уже был обработан.", parse_mode='HTML')
                return
            
            # Записываем charge_id до начисления
            with get_db_cursor() as _cur:
                _cur.execute(
                    "INSERT OR IGNORE INTO invoice_payments (user_id, invoice_code) VALUES (?,?)",
                    (user_id, charge_id)
                )
            ...
            print(f"Успешный платеж: user_id={user_id}, stars={payment_info.total_amount}, amount={amount}, charge_id={charge_id}")
...
def top_callback_handler(call):
    ...
    try:
        ...
        err_str = str(e)
        # Telegram кидает исключение если сообщение не изменилось — это не ошибка
        if "message is not modified" in err_str or "Bad Request" in err_str and "not modified" in err_str:
...
def process_roulette_photo(message, number):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Фото для числа {number} успешно сохранено!\n\n📁 Путь: {photo_path}", parse_mode='HTML')
...
def confirm_delete_roulette_photo(call):
    try:
        ...
        if os.path.exists(photo_path):
            ...
            bot.edit_message_text(
                f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Фото для числа {number} удалено!",
                call.message.chat.id,
                call.message.message_id
            , parse_mode='HTML')
...
def cancel_delete_roulette_photo(call):
    bot.edit_message_text(
        "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Удаление сброшено",
        call.message.chat.id,
        call.message.message_id
    , parse_mode='HTML')
    bot.answer_callback_query(call.id, "Отменено")
...
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> ДОСТУПНЫЕ СТАВКИ:
...
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> ЧИСЛА:
...
def check_game_allowed(func):
    ...
    def wrapper(message):
        ...
        if is_user_banned(uid):
            bot.reply_to(message, "🚫 Вы забанены и не можете играть.", parse_mode='HTML')
            return
        now = time.time()
...
def handle_roulette(message):
    ...
    try:
        ...
        if is_winner:
            ...
            message_text += f"<blockquote><tg-emoji emoji-id='5375296873982604963'><tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji></tg-emoji> — {_fmt_num(new_balance)}</blockquote>"
        else:
            new_balance = get_balance(user_id)
        ...
        try:
            ...
            print(f"[roulette] send error: {send_err}")
            try:
                bot.send_message(message.chat.id, message_text, parse_mode='HTML')
            except Exception:
                pass
        ...
        if 'bet_amount' in dir() and bet_amount:
...
def handle_football(message):
    try:
        ...
        football_results = {
            1: "⚽ Мимо!", 2: "⚽ Мимо!", 3: "⚽ ГОЛ!",
            4: "⚽ ГОЛ!", 5: "⚽ ГОЛ — в девятку!"
        }
        result_desc = football_results.get(result, f"Результат: {result}")
        if win:
            ...
            bot.send_message(
                ...
                f"👤 {display_name}\n"
                f"🎲 {result_desc}\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: <b>{format_balance(bet_amount)}</b>\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: <b>+{format_balance(win_amount)}</b> (x{multiplier:.1f})\n"
    ...
    try:
        ...
        if required_balance > balance:
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> У вас: {format_balance(balance)}\n"
        ...
        if result == 6:
            ...
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: <b>{format_balance(bet_amount)}</b>\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: <b>+{format_balance(win_amount)}</b> (x5)\n"
                f"📊 Баланс: <b>{format_balance(new_balance)}</b>",
                parse_mode='HTML'
            )
            ...
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка возвращена: <b>{format_balance(bet_amount)}</b>\n"
                f"📊 Баланс: <b>{format_balance(new_balance)}</b>",
                parse_mode='HTML'
            )
...
def handle_tower_game(message):
    ...
    try:
        ...
        message_text += f"<blockquote><tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {format_balance(bet_amount)}\n"
        message_text += f"📈 Уровень: 1/5\n"
        message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель: x{multipliers[1]}</blockquote>\n\n"
...
def handle_lottery(message):
    ...
    msg = bot.send_message(
        ...
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {format_balance(bet)}\n"
        f"📊 Открыто: 0/3\n\n"
...
def show_mines_game(chat_id, user_id, message_id=None):
    ...
    info_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {format_balance(game_data['bet_amount'])}\n"
    info_text += f"💣 Мин: {game_data['mines_count']}\n"
    info_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Открыто: {game_data['opened_cells']}/25\n"
    ...
    if game_data['opened_cells'] > 0:
        ...
        info_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Выигрыш: <b>{format_balance(potential_win)}</b>\n\n"
    else:
        info_text += "\n"
    
    info_text += "❇️ - закрытая\n<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> - безопасная\n💣 - мина"
...
def end_mines_game(user_id, won=False, win_amount=0, exited=False):
    ...
    if won:
        ...
        result_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: <b>{format_balance(win_amount)}</b>\n"
        result_text += f"📈 Множитель: <b>{multiplier:.2f}x</b>\n"
        result_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Открыто: {game_data['opened_cells']} клеток"
    elif exited:
        result_text = f"🏁 <b>Игра окончена</b>\n\n"
        result_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Возвращено: <b>{format_balance(game_data['bet_amount'])}</b>\n"
        result_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Открыто: {game_data['opened_cells']} клеток"
    else:
...
<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Как играть:</b>
• Открывайте безопасные клетки (<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji>)
• Избегайте мин (💣)
...
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Мин. бет: 1.000"""
    ...
    for user_id in expired_games:
        ...
        try:
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Возвращено: {format_balance(bet_amount)}"
            , parse_mode='HTML')
        except Exception as e:
    ...
    for item_type, items in equipped.items():
        if isinstance(items, list):
            for item in items:
                ...
                exists = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji>" if os.path.exists(file_path) else "❌"
...
def restore_gucci_type(message):
    ...
    with get_db_cursor() as cursor:
        ...
        bot.send_message(message.chat.id, "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Тип Gucci Pepe возвращен на 'accessories'", parse_mode='HTML')
...
def fix_gucci_type(message):
    ...
    with get_db_cursor() as cursor:
        ...
        bot.send_message(message.chat.id, "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Тип Gucci Pepe изменен на 'body'. Проверьте отображение.", parse_mode='HTML')
...
def change_item_type(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Тип вещи '{item[0]}' изменен на '{new_type}'", parse_mode='HTML')
...
def handle_add_supply(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Установлен саплай для {item_name}!\n"
...
def buy_clothes(user_id, item_id):
    ...
    with get_db_cursor() as cursor:
        ...
        return True, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> {name} куплен!"
...
def equip_clothes(user_id, item_id):
    ...
    with get_db_cursor() as cursor:
        ...
        return True, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> {name} надет!"
def unequip_clothes(user_id, item_id):
    ...
    with get_db_cursor() as cursor:
        ...
        return True, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> {name} снят!"
def get_equipment_limits_info(user_id):
    ...
    with get_db_cursor() as cursor:
        ...
        type_limits = {
            ...
            'Ноги': (1, "👖 Ноги"), 
            'Слева': (1, "💍 Слева"),
            'Справа': (1, "<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Справа")
        }
        ...
        for item_type, (limit, display_name) in type_limits.items():
            ...
            status = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji>" if current_count < limit else "❌"
    ...
    with get_db_cursor() as cursor:
        ...
        if item:
            ...
            if os.path.exists(file_path):
                message_text += f"\n<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Файл существует: {file_path}"
...
def handle_restore_pepe(message):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            if affected > 0:
                bot.send_message(message.chat.id, 
                               "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Pepe Gucci возвращен!\n"
...
def show_shop_page(chat_id, user_id, page=0, category=None):
    ...
    try:
        ...
        caption = f"👕 {item['name']}\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(item['price'])}{category_info}{supply_info}\n\n📄 Страница {page + 1} из {total_pages}"
...
def show_wardrobe_page(chat_id, user_id, page=0):
    ...
    try:
        ...
        item = page_items[0]
        status = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Надето" if item['equipped'] else "👕 Надеть"
        caption = f"👕 {item['name']}\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(item['price'])}\n📦 {item['type']}\n{status}\n\n📄 Страница {page + 1} из {total_pages}"
...
def start(message):
    try:
        ...
        # Кешируем bio в фоне (для воздушного груза)
        def _bg_cache_bio(uid):
            try:
                info = bot.get_chat(uid)
                _cache_user_bio(uid, getattr(info, 'bio', None) or '')
            except Exception:
                pass
        threading.Thread(target=_bg_cache_bio, args=(user_id,), daemon=True).start()
...
def handle_existing_user(message):
    ...
    try:
        ...
        if len(message.text.split()) > 1:
            ...
            with get_db_cursor() as cursor:
                # Сначала проверяем счёт (invoice)
                cursor.execute('SELECT amount, max_payments, current_payments, created_by, comment FROM invoices WHERE code=?', (ref_code,))
                inv_data = cursor.fetchone()
                if inv_data:
                    inv_amount, inv_max, inv_cur, inv_creator, inv_comment = inv_data
                    cursor.execute('SELECT 1 FROM invoice_payments WHERE user_id=? AND invoice_code=?', (user_id, ref_code))
                    already_paid = cursor.fetchone()
                    if already_paid:
                        bot.send_message(message.chat.id, "❌ Вы уже оплатили этот счёт!")
                    elif inv_cur >= inv_max:
                        bot.send_message(message.chat.id, "❌ Счёт уже полностью оплачен!")
                    else:
                        bal = get_balance(user_id)
                        if bal < inv_amount:
                            bot.send_message(message.chat.id,
                                f"❌ Недостаточно средств!\n\n"
                                f"Нужно: {format_balance(inv_amount)}\n"
                                f"У вас: {format_balance(bal)}", parse_mode='HTML')
                        else:
                            # Показываем подтверждение вместо прямой оплаты
                            kb = {"inline_keyboard": [[
                                _btn(f"<tg-emoji emoji-id=\'5440354006335495210\'>🌸</tg-emoji> Оплатить {format_balance(inv_amount)}", callback_data=f"inv_pay:{ref_code}"),
                                {"text": "❌ Отмена", "callback_data": "inv_cancel"}
                            ]]}
                            conf_text = (
                                f"💳 <b>Подтверждение оплаты</b>\n\n"
                                f"<tg-emoji emoji-id=\'5440354006335495210\'>🌸</tg-emoji> Сумма: {format_balance(inv_amount)}\n"
                                f"💰 Ваш баланс: {format_balance(bal)}\n"
                                + (f"📝 Комментарий: <i>{inv_comment}</i>\n" if inv_comment else "") +
                                f"\n❓ Подтвердить оплату?"
                            )
                            _tg_api("sendMessage", chat_id=message.chat.id,
                                    text=conf_text, parse_mode="HTML", reply_markup=kb)
                    return
                ...
                if check_data:
                    ...
                    if already_activated:
                        ...
                        if password:
                            ...
                            success, result_message, _check_creator_id = activate_check(user_id, ref_code, amount, max_activations, current_activations, password, target_username)
                            bot.send_message(message.chat.id, result_message, parse_mode='HTML')
                            if success:
                                # Уведомляем создателя чека
                                _notify_check_creator(ref_code, user_id, amount)
                        return
                    else:
        ...
        if '_ref_notify_id' in dir() and _ref_notify_id:
            ...
            try:
                bot.send_message(_ref_notify_id, f"👥 Друг принял твоё приглашение!\n<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> +{_ref_notify_exp} опыта", parse_mode='HTML')
            except Exception:
                pass
            _ref_notify_id = None
...
• <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Вступай в <b>Гильдию</b>"""
...
def handle_referral_code(user_id, ref_code):
    ...
    _notify_check = None
    _notify_check_password = None
    _notify_ref_bonus = None

    try:
        with get_db_cursor() as cursor:
            # Проверка счёта (invoice) — для новых пользователей после регистрации
            cursor.execute('SELECT amount, max_payments, current_payments, created_by, comment FROM invoices WHERE code=?', (ref_code,))
            inv_data = cursor.fetchone()
            if inv_data:
                inv_amount, inv_max, inv_cur, inv_creator, inv_comment = inv_data
                ...
                already_paid = cursor.fetchone()
                if already_paid:
                    _notify_check = ("❌ Вы уже оплатили этот счёт!", 'HTML')
                elif inv_cur >= inv_max:
                    _notify_check = ("❌ Счёт уже полностью оплачен!", 'HTML')
                else:
                    bal = get_balance(user_id)
                    if bal < inv_amount:
                        _notify_check = (
                            f"❌ Недостаточно средств!\nНужно: {format_balance(inv_amount)}\nУ вас: {format_balance(bal)}", 'HTML')
                    else:
                        bal = get_balance(user_id)
                        if bal < inv_amount:
                            _notify_check = (
                            ...
                            _inv_confirm = (ref_code, inv_amount, inv_comment, user_id, inv_creator)

            cursor.execute('SELECT amount, max_activations, current_activations, password, target_username FROM checks WHERE code = ?', (ref_code,))
            check_data = cursor.fetchone()

            if inv_data:
                pass  # уже обработали выше
            elif check_data:
                amount, max_activations, current_activations, password, target_username = check_data
                ...
                if ref_user and ref_user[0] != user_id:
                    ...
                    if not current_ref or not current_ref[0]:
                        ...
                        cursor.execute('UPDATE users SET referred_by = ? WHERE user_id = ?', (referred_by, user_id))
                        cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (user_bonus, user_id))
                        # Получаем имя нового пользователя для уведомления
                        cursor.execute('SELECT username, first_name, custom_name FROM users WHERE user_id = ?', (user_id,))
                        new_row = cursor.fetchone()
                        if new_row:
                            new_name = new_row[2] or (f"@{new_row[0]}" if new_row[0] else new_row[1] or str(user_id))
                        else:
                            new_name = str(user_id)
                        _notify_ref_bonus = (referred_by, new_name, referrer_bonus, user_bonus)
    ...
    if '_inv_confirm' in dir() and _inv_confirm:
        rc, inv_amt, inv_cmt, payer_id, creator_id = _inv_confirm
        bal = get_balance(payer_id)
        kb = {"inline_keyboard": [[
            _btn(f"<tg-emoji emoji-id=\'5440354006335495210\'>🌸</tg-emoji> Оплатить {format_balance(inv_amt)}", callback_data=f"inv_pay:{rc}"),
            {"text": "❌ Отмена", "callback_data": "inv_cancel"}
        ]]}
        conf_text = (
            f"💳 <b>Подтверждение оплаты</b>\n\n"
            f"<tg-emoji emoji-id=\'5440354006335495210\'>🌸</tg-emoji> Сумма: {format_balance(inv_amt)}\n"
            f"💰 Ваш баланс: {format_balance(bal)}\n"
            + (f"📝 Комментарий: <i>{inv_cmt}</i>\n" if inv_cmt else "") +
            f"\n❓ Подтвердить оплату?"
        )
        _tg_api("sendMessage", chat_id=payer_id, text=conf_text, parse_mode="HTML", reply_markup=kb)
    ...
    if _notify_ref_bonus:
        referred_by_id, new_name, ref_bonus, usr_bonus = _notify_ref_bonus
        try:
            bot.send_message(
                user_id,
                f"🎉 Ты зарегистрировался по реферальной ссылке!\n"
                f"💰 Бонус: <b>+{format_balance(usr_bonus)} 🌸</b>",
                parse_mode='HTML'
            )
        except Exception:
            pass
        try:
            bot.send_message(
                referred_by_id,
                f"👥 <b>По твоей ссылке зарегистрировался {new_name}!</b>\n"
                f"💰 Тебе начислено: <b>+{format_balance(ref_bonus)} 🌸</b>",
                parse_mode='HTML'
            )
        except Exception:
            pass
    ...
    with get_db_cursor() as cursor:
        if target_username:
            ...
            if (current_username or '').lower() != (target_username or '').lower():
                return False, f"❌ Этот чек предназначен для {target_username}!", None
        ...
        if real_current >= real_max:
            return False, "❌ Чек уже использован максимальное количество раз!", None
        ...
        cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (amount, user_id))
        cursor.execute('SELECT created_by FROM checks WHERE code = ?', (ref_code,))
        cb_row = cursor.fetchone()
        created_by = cb_row[0] if cb_row else None

    return True, f"✅ <b>Чек активирован!</b>\n\n<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> Получено: {format_balance(amount)}", created_by
...
def handle_name_change(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Твое имя изменено на: {new_name}", parse_mode='HTML')
...
def _notify_check_creator(check_code, activator_id, amount):
    """Уведомить создателя чека об активации."""
    try:
        with get_db_cursor() as cur:
            cur.execute("SELECT created_by FROM checks WHERE code=?", (check_code,))
            row = cur.fetchone()
            if not row:
                return
            creator_id = row[0]
        if not creator_id or creator_id == activator_id:
            return
        # Получаем имя активировавшего
        with get_db_cursor() as cur:
            cur.execute("SELECT first_name, username, custom_name FROM users WHERE user_id=?", (activator_id,))
            u = cur.fetchone()
        if u:
            disp = u[2] or u[0] or (f"@{u[1]}" if u[1] else f"#{activator_id}")
        else:
            disp = f"#{activator_id}"
        try:
            bot.send_message(
                ...
                f"👤 Кто активировал: <b>{disp}</b>\n"
                f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> Сумма: {format_balance(amount)}",
                parse_mode="HTML"
            )
        except Exception:
            pass
    except Exception as e:
        print(f"[notify_check_creator] {e}")

def _notify_invoice_payer(invoice_code, payer_id, amount):
    """Уведомить создателя счёта об оплате."""
    try:
        with get_db_cursor() as cur:
            cur.execute("SELECT created_by FROM invoices WHERE code=?", (invoice_code,))
            row = cur.fetchone()
            if not row:
                return
            creator_id = row[0]
        if not creator_id or creator_id == payer_id:
            return
        with get_db_cursor() as cur:
            cur.execute("SELECT first_name, username, custom_name FROM users WHERE user_id=?", (payer_id,))
            u = cur.fetchone()
        if u:
            disp = u[2] or u[0] or (f"@{u[1]}" if u[1] else f"#{payer_id}")
        else:
            disp = f"#{payer_id}"
        try:
            bot.send_message(
        ...
        print(f"[notify_invoice_payer] {e}")



# ─── Лимит активных счётов на пользователя ─────────────────────────────────
INVOICE_LIMIT = 5  # Макс активных счётов у обычного пользователя

def _count_active_invoices(user_id):
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT COUNT(*) FROM invoices WHERE created_by=? AND current_payments < max_payments",
            (user_id,)
        )
        return cur.fetchone()[0]

# ─── Callback: подтвердить оплату счёта ────────────────────────────────────
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("inv_pay:"))
def cb_invoice_pay(call):
    user_id = call.from_user.id
    inv_code = call.data.split(":", 1)[1]

    with get_db_cursor() as cur:
        cur.execute(
            "SELECT amount, max_payments, current_payments, created_by, comment FROM invoices WHERE code=?",
            (inv_code,)
        )
        row = cur.fetchone()
        if not row:
        ...
        inv_amount, inv_max, inv_cur, inv_creator, inv_comment = row

        cur.execute("SELECT 1 FROM invoice_payments WHERE user_id=? AND invoice_code=?", (user_id, inv_code))
        if cur.fetchone():
            bot.answer_callback_query(call.id, "❌ Вы уже оплатили этот счёт!", show_alert=True)
            try:
                bot.delete_message(call.message.chat.id, call.message.message_id)
            except Exception:
                pass
        ...
        if inv_cur >= inv_max:
            bot.answer_callback_query(call.id, "❌ Счёт уже полностью оплачен!", show_alert=True)
            try:
                bot.delete_message(call.message.chat.id, call.message.message_id)
            except Exception:
                pass
        ...
        if bal < inv_amount:
            bot.answer_callback_query(
                call.id,
                f"❌ Недостаточно средств!\nНужно: {plain_balance(inv_amount)}\nУ вас: {plain_balance(bal)}",
                show_alert=True
            )
            return

        # Проводим оплату
        update_balance(user_id, -inv_amount)
        update_balance(inv_creator, inv_amount)
        cur.execute("INSERT OR IGNORE INTO invoice_payments (user_id, invoice_code) VALUES (?,?)", (user_id, inv_code))
        cur.execute("UPDATE invoices SET current_payments=current_payments+1 WHERE code=?", (inv_code,))
        new_cur = inv_cur + 1
    ...
    paid_text = (
        f"✅ <b>Счёт оплачен!</b>\n\n"
        f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> Списано: {format_balance(inv_amount)}\n"
        + (f"📝 Комментарий: <i>{inv_comment}</i>\n" if inv_comment else "") +
        f"\n💳 Оплат по счёту: {new_cur}/{inv_max}"
    )
    try:
        bot.edit_message_text(paid_text, call.message.chat.id, call.message.message_id, parse_mode="HTML")
    except Exception:
        bot.send_message(call.message.chat.id, paid_text, parse_mode="HTML")

    _notify_invoice_payer(inv_code, user_id, inv_amount)

# ─── Callback: отмена оплаты ────────────────────────────────────────────────
@bot.callback_query_handler(func=lambda c: c.data == "inv_cancel")
def cb_invoice_cancel(call):
    bot.answer_callback_query(call.id, "Отменено")
    try:
        bot.delete_message(call.message.chat.id, call.message.message_id)
    except Exception:
...
# ─── Команда "счета" — список активных счётов + удаление ───────────────────
@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("счета", "счёта", "мои счета", "мои счёта"))
def handle_my_invoices(message):
    user_id = message.from_user.id
    if not is_registered(user_id):
        return
    ...
    with get_db_cursor() as cur:
        cur.execute(
            "SELECT code, amount, max_payments, current_payments, comment, created_at "
            "FROM invoices WHERE created_by=? AND current_payments < max_payments "
    ...
    if not rows:
        bot.send_message(message.chat.id, "📋 У вас нет активных счётов.")
        return

    bot_username = bot.get_me().username
    kb = {"inline_keyboard": []}
    text = f"📋 <b>Ваши активные счета ({len(rows)}):</b>\n\n"

    for code, amount, mx, cur_p, comment, created_at in rows:
        link = f"https://t.me/{bot_username}?start={code}"
        text += (
            f"💳 {plain_balance(amount)} · {cur_p}/{mx} оплат"
            + (f" · <i>{comment}</i>" if comment else "") +
            f"\n🔗 <a href=\'{link}\'>Ссылка</a>\n\n"
        )
        kb["inline_keyboard"].append([
            {"text": f"🗑 Удалить {plain_balance(amount)} ({cur_p}/{mx})", "callback_data": f"inv_del:{code}"}
        ])

    _tg_api("sendMessage", chat_id=message.chat.id, text=text,
            parse_mode="HTML", reply_markup=kb,
            link_preview_options={"is_disabled": True})

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("inv_del:"))
def cb_invoice_delete(call):
    user_id = call.from_user.id
    inv_code = call.data.split(":", 1)[1]

    with get_db_cursor() as cur:
        cur.execute("SELECT created_by, current_payments FROM invoices WHERE code=?", (inv_code,))
        row = cur.fetchone()
        if not row:
            bot.answer_callback_query(call.id, "❌ Счёт не найден")
            return
        creator_id, cur_p = row
        if creator_id != user_id and not is_admin(user_id):
            bot.answer_callback_query(call.id, "❌ Это не ваш счёт")
            return
        if cur_p > 0:
            bot.answer_callback_query(call.id, f"⚠️ По счёту уже {cur_p} оплат — всё равно удаляем", show_alert=False)
        # Удаляем — ставим max_payments = current_payments (закрываем)
        cur.execute("UPDATE invoices SET max_payments=current_payments WHERE code=?", (inv_code,))

    bot.answer_callback_query(call.id, "✅ Счёт закрыт")
    # Обновляем список
    try:
        # Убираем строку этого счёта из клавиатуры
        old_kb = call.message.reply_markup
        new_rows = [row for row in old_kb.inline_keyboard
                    if not any(btn.callback_data == f"inv_del:{inv_code}" for btn in row if hasattr(btn, 'callback_data'))]
        if new_rows:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id,
                                           reply_markup=types.InlineKeyboardMarkup(new_rows))
        else:
            bot.edit_message_text("📋 Активных счётов нет.",
                                   call.message.chat.id, call.message.message_id)
    except Exception:
        pass

@bot.message_handler(func=lambda m: m.text and m.text.lower().startswith('счёт ') or
                      m.text and m.text.lower().startswith('счет '))
def handle_invoice_command(message):
    """
    счёт [сумма] [кол-во оплат] (комментарий)
    Создаёт счёт — ссылку, по которой другие пользователи платят указанную сумму.
    """
    user_id = message.from_user.id
    if not is_registered(user_id):
        bot.send_message(message.chat.id, "❌ <b>Ты не зарегистрирован!</b>\n\nНапиши /start чтобы начать.", parse_mode='HTML')
    ...
    if is_user_warned(user_id):
        bot.send_message(message.chat.id, "❌ Вы не можете создавать счета, так как у вас активный варн!")
        return

    # Лимит активных счётов
    if not is_admin(user_id):
        active_count = _count_active_invoices(user_id)
        if active_count >= INVOICE_LIMIT:
            bot.send_message(message.chat.id,
                f"❌ У вас уже {active_count} активных счётов (лимит {INVOICE_LIMIT}).\n\n"
                f"Закройте старые командой <b>счета</b> или дождитесь их оплаты.",
                parse_mode='HTML')
            return
    ...
    if len(parts) < 3:
        bot.send_message(message.chat.id,
            "❌ Используйте: <b>счёт [сумма] [кол-во оплат] (комментарий)</b>\n\n"
            "Примеры:\n"
            "• счёт 50000 1 — счёт на 50к, 1 оплата\n"
            "• счёт 1000000 5 За услугу — счёт на 1М, 5 оплат с комментарием",
            parse_mode='HTML')
        return

    balance = get_balance(user_id)
    amount = parse_bet_amount(parts[1], balance * 100)  # сумма с других — не с баланса создателя
    if amount is None or amount <= 0:
        bot.send_message(message.chat.id, "❌ Неверная сумма!")
        return

    try:
        max_payments = int(parts[2])
        if max_payments <= 0 or max_payments > 1000:
            bot.send_message(message.chat.id, "❌ Количество оплат: от 1 до 1000!")
            return
    except ValueError:
        bot.send_message(message.chat.id, "❌ Неверное количество оплат!")
        return

    comment = parts[3] if len(parts) > 3 else None

    inv_code = f"inv{user_id}{random.randint(10000, 99999)}"
    with get_db_cursor() as cur:
        cur.execute(
            "INSERT INTO invoices (code, amount, max_payments, created_by, comment) VALUES (?,?,?,?,?)",
            (inv_code, amount, max_payments, user_id, comment)
        )

    bot_username = bot.get_me().username
    inv_link = f"https://t.me/{bot_username}?start={inv_code}"

    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("💳 Оплатить счёт", url=inv_link))

    text = (
        f"💳 <b>Счёт создан!</b>\n\n"
        f"<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji> Сумма оплаты: {format_balance(amount)}\n"
        f"🔢 Оплат: {max_payments}\n"
        + (f"📝 Комментарий: {comment}\n" if comment else "") +
        f"\n🔗 Поделитесь ссылкой с плательщиками."
    )
...
def handle_user_check_command(message):
    ...
    if _tax_is_restricted(user_id):
        bot.send_message(message.chat.id,
            "🚫 <b>Создание чеков заблокировано</b> — есть налоговый долг!\n"
            "Напиши <code>налог</code> чтобы оплатить.", parse_mode="HTML")
        return
    ...
    try:
        ...
        message_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Сумма: {format_balance(amount)}\n"
...
def handle_cheque_command(message):
    try:
        ...
        total_amount = amount * max_activations
        message_text = f"🎫 <b>Ваучер создан!</b>\n\n"
        message_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Сумма за активацию: {format_balance(amount)}\n"
        message_text += f"🔢 Активаций: {max_activations}\n"
        message_text += f"🔐 Пароль: {'есть' if password else 'нет'}"
            ...
            help_result = InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    message_text="💡 **Создание чека:**\n\n"
                               ...
                               "<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> **С вашего баланса спишется: сумма × активации**",
                    parse_mode='Markdown'
                )
            )
            ...
            error_result = InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    message_text=f"❌ Не хватает монет!\n\n"
                               f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Нужно: {format_balance(total_cost)}\n"
...
def create_inline_cheque_result(amount_str, activations_str, password, description):
    ...
    return InlineQueryResultArticle(
        id=check_code,
        title=f'<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Чек на {format_balance(amount)}',
...
@bot.message_handler(func=lambda message: message.text and message.text.lower().strip() == 'актив' and (is_admin(message.from_user.id) or is_premium(message.from_user.id)))
...
def show_active_stats(chat_id, user_id, message_id=None):
    ...
    try:
        ...
        message_text += "<b><tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> ЭКОНОМИКА</b>\n"
        message_text += (
            f"<blockquote>"
...
def handle_create_check(message):
    ...
    try:
        ...
        message_text = f"🎫 <b>Ваучер создан!</b>\n\n"
        message_text += f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Сумма: {format_balance(amount)}\n"
...
def handle_profile(message):
    ...
    try:
        ...
        if lvl_need > 0:
            ...
            message_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Максимальный уровень!\n\n"
        message_text += f"📅 В игре: {age_days} дн.\n"
        message_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Баланс: {format_balance(user_info['balance'])}\n"
        ...
        user_clan = get_user_clan(user_id)
        if user_clan:
            message_text += f"\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Гильдия: {user_clan['name']} [{user_clan['tag']}]\n"
...
def handle_reset_stats(message):
    ...
    try:
        ...
        bot.reply_to(message, 
                   f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Статистика игр сброшена!\n\n"
    ...
    with get_db_cursor() as cursor:
        ...
        cursor.execute('''
            UPDATE users SET
                ...
                experience = 0
        ''')
    bot.send_message(message.chat.id, f"♻️ <b>Обулизм выполнен!</b>\n\nОбнулено у {total} игроков:\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Баланс · 🏛 Вклад · ⛏️ Майнинг · <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Опыт", parse_mode='HTML')
...
def confirm_reset_all_stats(call):
    try:
        ...
        bot.edit_message_text(
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Вся статистика игр сброшена!</b>\n\n"
...
def cancel_reset_stats(call):
    bot.edit_message_text(
        "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Сброс статистики сброшен",
        call.message.chat.id,
        call.message.message_id
    , parse_mode='HTML')
    bot.answer_callback_query(call.id, "Отменено")
...
def handle_clicker(message):
    ...
    bot.send_message(message.chat.id, "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Найди правильную кнопку:", reply_markup=create_clicker_keyboard(message.from_user.id), parse_mode='HTML')
...
def handle_scam(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            message_text += f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Заработано: {format_balance(earned)}\n\n"
...
def handle_air_cargo(message):
    text = (
        ...
        "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Награда победителя: <b>300 000 🌸</b>"
        "</blockquote>\n\n"
...
def handle_balance(message):
    try:
        user_id = message.from_user.id
        balance = get_balance(user_id)
        bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Твой баланс: {format_balance(balance)}", parse_mode='HTML')
    except Exception as e:
...
SLOT_CONFIG = {
    ...
    "symbols": [
        ...
        {"emoji": "<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji>", "name": "Бриллиант", "multiplier": 10, "weight": 2},
        {"emoji": "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", "name": "Звезда", "multiplier": 7, "weight": 3},
        {"emoji": "🍀", "name": "Клевер", "multiplier": 3, "weight": 8},
        {"emoji": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>", "name": "Джекпот", "multiplier": 25, "weight": 1}
    ],
    "special_combinations": {
        "jackpot": {"symbols": ["<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>", "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>", "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"], "multiplier": 100, "name": "ДЖЕКПОТ!!!", "chance": 0.001},
        "three_diamonds": {"symbols": ["<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji>", "<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji>", "<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji>"], "multiplier": 50, "name": "ТРИ БРИЛЛИАНТА", "chance": 0.005},
        "three_bells": {"symbols": ["🔔", "🔔", "🔔"], "multiplier": 20, "name": "ТРИ КОЛОКОЛЬЧИКА", "chance": 0.01},
        "three_stars": {"symbols": ["<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>"], "multiplier": 15, "name": "ТРИ ЗВЕЗДЫ", "chance": 0.008},
...
def create_slots_display(reels, bet_amount=None, win_amount=None):
    ...
    if bet_amount is not None:
        slot_display += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {format_balance(bet_amount)}\n"
...
🔔 Колокольчик (x5) <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Звезда (x7)
🍀 Клевер (x4)     <tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Бриллиант (x10)
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Джекпот (x25)
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            if result:
                ...
                message_text += f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Доход: {format_balance(income_per_hour)}/час\n"
    ...
    try:
        if call.data == "mining_collect":
            ...
            with get_db_cursor() as cursor:
                ...
                time_passed = min(time_passed, 24 * 3600)
                ...
                # Атомарное обновление: WHERE last_mining_collect = ? гарантирует
                # что между SELECT и UPDATE никто другой уже не собрал
                rows_updated = cursor.execute(
                    'UPDATE users SET balance = balance + ?, last_mining_collect = ? '
                    'WHERE user_id = ? AND last_mining_collect = ?',
                    (income, current_time, user_id, last_collect)
                ).rowcount

                if rows_updated == 0:
                    # Другой поток уже успел собрать — игнорируем
            ...
            with get_db_cursor() as cursor:
                ...
                if balance >= card_price:
                    ...
                    message_text += f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Доход: {format_balance(new_income)}/час\n\n"
...
def handle_clicker_boost(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,
                       f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Буст кликера активирован!\n\n"
...
def get_boost_info_text():
    ...
    return (f"\n🎉 АКЦИЯ! 🎉\n"
            f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Буст: x{boost['multiplier']}\n"
...
def clicker_callback_handler(call):
    ...
    if not check_click_limit(user_id):
        try:
            ...
            bot.edit_message_text(rest_text, call.message.chat.id, call.message.message_id)
        except: pass
        bot.answer_callback_query(call.id)
        return

    parts = call.data.split('_', 2)  # clicker_ok_TOKEN или clicker_no_TOKEN
    if len(parts) < 3:
        bot.answer_callback_query(call.id)
        return
    ...
    try:
        ...
        if action == "ok":
            # Проверяем одноразовый токен — автокликер не знает токен заранее
            if not _clicker_check_token(user_id, token):
                bot.answer_callback_query(call.id, "⚠️ Устаревшая кнопка, попробуй ещё раз")
                try:
                    bot.edit_message_text(
                        "🔄 Кнопка устарела — получи новую:",
                        call.message.chat.id, call.message.message_id,
                        reply_markup=create_clicker_keyboard(user_id)
                    )
                except: pass
                return
            ...
            if new_level > old_level:
                try:
                    ...
                    bot.send_message(user_id,
                        f"🎉 <b>НОВЫЙ УРОВЕНЬ!</b>\n\n{em} <b>{new_level} — {ttl}</b>\n\n"
                        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Бонус: +{format_balance(level_up_bonus)}\n"
                        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Бонус к кликам: +{click_b}%\n"
                        f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Бонус к ежедневке: +{daily_b}%", parse_mode='HTML')
                except Exception as e:
            ...
            display_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(new_balance)}\n"
            display_text += f"{lv_emoji} {lv_title} · Ур.{level_now}\n"
            display_text += f"[{bar}] +{EXP_PER_CLICK} exp\n\n"
            display_text += f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> Серия: {new_streak}  ·  👆 {new_total_clicks} кликов\n"
            if bonus > 0:
                display_text += f"\n<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Бонус за 100 кликов! +{format_balance(bonus)}"
            else:
                display_text += f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> До бонуса: {next_bonus} кликов"

            bot.edit_message_text(
                display_text, call.message.chat.id, call.message.message_id,
                reply_markup=create_clicker_keyboard(user_id), parse_mode='HTML'
            )

        else:  # action == "no" — неверная кнопка
            with get_db_cursor() as cursor:
                cursor.execute('UPDATE users SET click_streak = 0 WHERE user_id = ?', (user_id,))
            # Сбрасываем токен — новая клавиатура, новый токен
            boost_info = get_boost_info_text()
            message_text = (f"{boost_info}\n❌ Неверный выбор! Серия сброшена.\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Найди правильную кнопку:"
                            if boost_info else "❌ Неверный выбор! Серия сброшена.\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Найди правильную кнопку:")
            bot.edit_message_text(
                message_text, call.message.chat.id, call.message.message_id,
                reply_markup=create_clicker_keyboard(user_id)
            )
...
def confirm_convert_balances(call):
    try:
        ...
        result_message = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> **БАЛАНСЫ И ЦЕНЫ КОНВЕРТИРОВАНЫ!**\n\n"
        ...
        result_message += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> **Общий баланс ДО:** {format_balance(old_total_balance)}\n"
        result_message += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> **Общий баланс ПОСЛЕ:** {format_balance(new_total_balance)}\n\n"
...
def cancel_convert(call):
    bot.edit_message_text(
        "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Конвертация сброшена",
        call.message.chat.id,
        call.message.message_id
    , parse_mode='HTML')
    bot.answer_callback_query(call.id, "Отменено")
    ...
    for game_id in expired_games:
        ...
        if game_data.get('chat_id') and game_data.get('message_id'):
            try:
                ...
                message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель: x{multipliers[current_level]}</blockquote>\n\n"
                message_text += f"🕒 <b>ВРЕМЯ ИГРЫ ИСТЕКЛО!</b>\n"
                message_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Возвращено: {format_balance(bet_amount)}\n\n"
...
def handle_tower_safe(game_id, level, tower, call):
    ...
    if level < 5:
        ...
        if next_level:
            ...
            message_text = f"🏰 <b>БАШНЯ</b>\n\n"
            message_text += f"<blockquote><tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {format_balance(game['bet_amount'])}\n"
            message_text += f"📈 Уровень: {next_level}/5\n"
            message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель: x{next_multiplier}</blockquote>\n\n"
            message_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Уровень {level} пройден!\n"
            message_text += f"💣 В одной башне мина!\n\n"
            message_text += f"🎲 Выбери безопасную башню:"
...
def handle_tower_mine(game_id, level, tower, call):
    ...
    message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель: x{multipliers[level]}</blockquote>\n\n"
...
def handle_tower_win(game_id, call):
    ...
    message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель: x{multipliers[5]}</blockquote>\n\n"
    message_text += f"🎉 <b>ПОБЕДА!</b> Ты достиг вершины башни!\n"
    message_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: {format_balance(win_amount)}"
...
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Цель:</b> Дойти до вершины башни, избегая мины
...
<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> <b>Множители:</b>
Уровень 1 • x1.5
Уровень 2 • x2.0  
Уровень 3 • x3.0
Уровень 4 • x5.0
...
def handle_boost_status(message):
    ...
    bot.send_message(message.chat.id,
                   ...
                   f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Скорее жмите 'Кликер' пока действует буст!",
                   parse_mode='Markdown')
...
def handle_daily_bonus(message):
    try:
        ...
        bonus_text = (
            f"<tg-emoji emoji-id='5442939099906325301'><tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji></tg-emoji> <b>Бонус готов</b>\n\n"
            f"<blockquote>"
            f"<tg-emoji emoji-id='5435999124245729290'><tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji></tg-emoji> +{format_balance(bonus_amount)}\n"
            f"<tg-emoji emoji-id='5438496463044752972'><tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji></tg-emoji> +500 опыта"
            f"</blockquote>\n\n"
...
def handle_bonus_broadcast_confirmation(call):
    ...
    try:
            ...
            for i, (user_id,) in enumerate(users, 1):
                try:
                    ...
                    if is_subscribed:
                        ...
                        bonus_text = f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> <b>Бонус от администрации</b>\n\n"
                        bonus_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(bonus_amount)}\n"
                        bonus_text += f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> +500 опыта"
        ...
        result_text = (
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>РАССЫЛКА БОНУСОВ ЗАВЕРШЕНА</b>\n\n"
            f"📊 Результаты:\n"
            f"👥 Всего пользователей: {total_users}\n"
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Успешно: {sent}\n"
            f"❌ Ошибок: {failed}\n\n"
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Теперь пользователи получат тот же бонус что и при нажатии 'Бонус'!"
        )
...
def handle_broadcast_bonus_claim(call):
    ...
    try:
        ...
        bot.edit_message_text(
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Бонус получен\n\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> +{format_balance(bonus_amount)}\n"
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> +500 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> опыта",
            call.message.chat.id,
            call.message.message_id
        , parse_mode='HTML')
...
def add_experience(user_id, exp_amount):
    ...
    if new_level > old_level:
        ...
        try:
            bonus_text = ""
            if new_level % 5 == 0:
                bonus_text = f"\n\n<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> <b>Каждый 5-й уровень — особые привилегии!</b>"
            bot.send_message(
                user_id,
                f"🎉 <b>НОВЫЙ УРОВЕНЬ!</b>\n\n"
                f"{emoji} <b>{new_level} — {title}</b>\n\n"
    ...
    try:
        ...
        if not user_clothes:
            ...
            for i, item in enumerate(user_clothes, 1):
                in_trade = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> В обмене" if item['item_id'] in (trade.user1_items if user_id == trade.user1_id else trade.user2_items) else ""
...
def execute_trade(trade):
    try:
        with get_db_cursor() as cursor:
            ...
            trade_result = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> ОБМЕН ЗАВЕРШЕН!\n\n"
...
def handle_add_clothing(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id,
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Одежда добавлена в магазин!\n\n"
                           f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Название: {name}\n"
                           f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Цена: {format_balance(price)}\n"
                           f"📁 Тип: {item_type}\n"
                           f"🖼️ Файл: {image_file}\n\n"
...
def handle_add_multiple_clothing(message):
    ...
    try:
        ...
        result_text = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Добавлено {added_count} вещей\n"
        if errors:
            result_text += f"\n❌ Ошибки ({len(errors)}):\n" + "\n".join(errors[:10])
...
def create_shop_command(message):
    ...
    ("BMB M5 f90 karabasa", 50000000000, "Справа", "BMB M5 f90 karabasa.png"),
        ...
        bot.send_message(message.chat.id, f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> 🛒 Шоп создан! Добавлено {added_count} айтемов.", parse_mode='HTML')
...
def handle_migration_types(message):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Миграция окончена!\n\n"
...
def handle_remove_item_from_shop(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Вещь снята с продажи!\n\n"
...
def handle_return_item_to_shop(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Вещь возвращена в продажу!\n\n"
                           f"🆔 Название: {item_name}\n"
                           f"🆔 ID: {item_id}\n"
                           f"📦 Статус: 🟢 Доступна для покупки\n"
                           f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Режим: {supply_text}\n\n"
                           f"💡 Счетчик продаж сброшен.", parse_mode='HTML')
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            for item_id, name, item_type, price, supply, sold_count in items:
                ...
                message_text += f"  <tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(price)} | {status}\n"
...
def start_add_clothes(message):
    ...
    bot.send_message(
        ...
        "1. 📸 Отправь фото вещи\n"
        "2. <tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Укажи цену\n" 
        "3. 🏷️ Укажи название\n"
...
def fix_shoes_filename(message):
    ...
    try:
        with get_db_cursor() as cursor:
            ...
            result = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> ИСПРАВЛЕНО:\n\n"
            for name, image_name in shoes:
                result += f"👟 {name}: {image_name}\n"
                exists_icon = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> ДА" if os.path.exists(f'images/{image_name}') else "❌ НЕТ"
...
def handle_clothes_price(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Цена установлена!\n\n"
...
def handle_dice_bet(message):
    ...
    try:
        ...
        dice_keyboard = {
            "inline_keyboard": [[
                {"text": "Принять", "callback_data": f"dice_accept_{user_id}_{target_user_id}_{bet_amount}", "icon_custom_emoji_id": "5440539497382038629"},
                {"text": "Отказаться", "callback_data": f"dice_decline_{user_id}", "icon_custom_emoji_id": "5440660857294101863"}
            ]]
        }
        ...
        challenge_text += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: <b>{format_balance(bet_amount)}</b>"
...
def handle_dice_response(call):
    try:
        ...
        result_message = f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Результаты:\n\n"
        ...
        if win_amount > 0:
            result_message += f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: {format_balance(win_amount)}"
...
def update_balance(user_id, amount):
    # Забаненным — никаких транзакций
    if is_user_banned(user_id):
        print(f"[ban] update_balance заблокирован для user_id={user_id}, amount={amount}")
        return
    with get_db_cursor() as cursor:
        cursor.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
...
def handle_give_money(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, 
                       f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Успешно выдано {format_balance(amount)} пользователю {target_first_name} (@{target_username})", parse_mode='HTML')
...
def handle_take_money(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, 
                       f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Успешно снято {format_balance(amount)} у пользователя {target_first_name} (@{target_username})", parse_mode='HTML')
...
def handle_set_balance(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, 
                       f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Баланс пользователя {target_first_name} (@{target_username}) установлен:\n"
                       f"📊 Было: {format_balance(current_balance)}\n"
                       f"📈 Стало: {format_balance(amount)}", parse_mode='HTML')
        
        try:
            bot.send_message(target_user_id, 
                           f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Администратор установил твой баланс: {format_balance(amount)}!", parse_mode='HTML')
        except:
            pass
...
def handle_check_balance(message):
    ...
    try:
        ...
        message_text += f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Общий капитал: {format_balance(balance + bank_deposit)}\n"
        message_text += f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Майнеров: {video_cards}\n"
        ...
        if user_clan:
            message_text += f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Гильдия: {user_clan['name']} [{user_clan['tag']}]\n"
            message_text += f"🎖 Статус: {user_clan['role']}\n"
    ...
    for row in rows:
        try:
            ...
            if total_wealth >= TAX_CONFIG["high_wealth_threshold"]:
                ...
                tax_reason = "10% (элита)"
                tier_icon  = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji>"
                tier_name  = "Элита"
            ...
            notify = (
                ...
                + "\n".join(parts)
                + "\n\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Осталось: <b>" + format_balance(total_wealth - actual_tax) + " 🌸</b>"
            )
            try:
...
@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() in ("налог", "/налог"))
def handle_tax_command(message):
    uid = message.from_user.id
    status   = _tax_get_status(uid)
    now      = int(time.time())
    tax_info = _tax_calc_for_user(uid)

    if not status["debt"] and not status["restricted"]:
        # Нет долгов — показываем статус и прогноз
        last_paid_text = ""
        if status["paid_at"]:
            import datetime as _dt2
            lp = _dt2.datetime.fromtimestamp(status["paid_at"]).strftime("%d.%m.%Y")
            last_paid_text = f"\n✅ Последняя оплата: {lp}"

        if tax_info["amount"] > 0:
            forecast = (
                f"\n\n📊 <b>Прогноз следующего налога:</b>\n"
                f"💰 Состояние: <b>{format_balance(tax_info['wealth'])}</b>\n"
                f"📈 Ставка: <b>{int(tax_info['rate']*100)}%</b>\n"
                f"💸 Сумма: <b>~{format_balance(tax_info['amount'])}</b>"
            )
        else:
            forecast = "\n\n✅ Состояние ниже налогового порога (10 000) — налогов нет."

        bot.send_message(message.chat.id,
            f"🏛️ <b>Налоговая служба</b>\n\n"
            f"✅ Долгов нет!{last_paid_text}"
            f"{forecast}\n\n"
            f"Налог выставляется каждый <b>понедельник в 10:00</b>.\n"
            f"На оплату даётся <b>48 часов</b>.",
            parse_mode="HTML")
        return

    # Есть долг — пытаемся оплатить
    ok, text = _tax_pay(uid)
    bot.send_message(message.chat.id, text, parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() == "собрать налог")
@bot.message_handler(commands=["collect_tax"])
    ...
    try:
        ...
        if result["success"]:
            text = (
                ...
                "<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> В казну: <b>"
                + format_balance(result["total_collected"]) + " 🌸</b>\n"
                "👥 Затронуто: <b>" + str(result["affected_users"]) + "</b> чел.\n\n"
            )
            if result.get("tax_report"):
                top = sorted(result["tax_report"], key=lambda x: x["tax"], reverse=True)[:10]
                text += "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Крупнейшие налогоплательщики:</b>\n"
                medals = ["🥇","🥈","🥉"] + ["🔹"]*7
                for i, tp in enumerate(top):
                    text += medals[i] + " " + tp["user"] + " — <b>" + format_balance(tp["tax"]) + " 🌸</b> <i>(" + tp["rate"] + ")</i>\n"
        else:
            text = "❌ Ошибка: " + result.get("error","?")
...
@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() == "выставить налог")
def handle_issue_tax(message):
    if not is_admin(message.from_user.id):
        return
    msg   = bot.send_message(message.chat.id, "⏳ Выставляем налоги...", parse_mode="HTML")
    count = _tax_issue_weekly()
    bot.edit_message_text(
        f"🏛️ <b>Налоговые счета выставлены!</b>\n\n👥 Получили счёт: <b>{count}</b> чел.",
...
DONATE_MIN_PER_STAR = 15_000
DONATE_MAX_PER_STAR = 100_000
DONATE_MAX_CHANGE   = 0.20
...
def handle_recalc_donate(message):
    ...
    try:
        ...
        text = (
            "<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Цены доната обновлены!</b>\n\n"
            "1 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> было: <b>" + format_balance(old_price) + " 🌸</b>\n"
            "1 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> стало: <b>" + format_balance(new_price) + " 🌸</b>\n\n"
            + "\n".join(lines)
        )
        bot.send_message(message.chat.id, text, parse_mode="HTML")
    except Exception as e:
    ...
    try:
        ...
        if success:
            ...
            try:
                bot.send_message(
                    target_user_id, 
                    f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Вам перевели {format_balance(receive_amount)}", 
                    parse_mode='HTML'
                )
            except:
                pass
...
def handle_change_price(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Цена изменена для {item_name}!\n"
                           f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Было: {format_balance(old_price)}\n"
                           f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Стало: {format_balance(new_price)}", parse_mode='HTML')
...
def handle_change_price_by_id(message):
    ...
    try:
        ...
        with get_db_cursor() as cursor:
            ...
            bot.send_message(message.chat.id, 
                           f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Цена изменена для {item_name} (ID: {item_id})!\n"
                           f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Было: {format_balance(old_price)}\n"
                           f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Стало: {format_balance(new_price)}", parse_mode='HTML')
    ...
    for user_id, data in victims:
        ...
        try:
            ...
            bot.send_message(
                ...
                f"💥 *ВЫ ПРОИГРАЛИ*\n\n"
                f"🎰 Игра: *Краш*\n"
                f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Твой множитель: *{target_multiplier:.2f}x*\n"
                f"📊 Результат краша: *{result_multiplier:.2f}x*\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Потеряно: *{format_balance(bet)}*\n\n"
...
def process_crash_game_ls_only(user_id, bet_amount, target_multiplier):
    ...
    decision_text = (
        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> *БЫСТРОЕ РЕШЕНИЕ*\n\n"
        f"👤 Игрок: {user_name} (ID: {user_id})\n"
        f"🎰 Игра: Краш\n"
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель ставки: {target_multiplier:.2f}x\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {int(bet_amount)} 🌸\n\n"
    ...
    try:
        if won:
            bot.send_message(
                ...
                f"🎉 *ВЫ ВЫИГРАЛИ!*\n\n"
                f"🎰 Игра: Краш\n"
                f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Твой множитель: {target_mult:.2f}x\n"
                f"📊 Результат краша: {result_multiplier:.2f}x\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {fmt(bet)} 🌸\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: {fmt(win_amount)} 🌸\n"
            ...
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Твой множитель: {target_mult:.2f}x\n"
                f"📊 Результат краша: {result_multiplier:.2f}x\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Потеряно: {fmt(bet)} 🌸\n\n"
    ...
    if admin_msg_id:
        try:
            ...
            update_text = (
                ...
                f"🎰 Игра: Краш\n"
                f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Множитель ставки: {target_mult:.2f}x\n"
                f"📊 Результат краша: {result_multiplier:.2f}x\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: {int(bet)} 🌸"
            )
            
            if won:
                update_text += f"\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Выигрыш: {int(win_amount)} 🌸"
...
<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> *Как играть:*
...
<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> *Обозначения сумм:*
• `100к` = 100.000$
...
<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> *Результат через:* 5с
📩 *Результат придет:* в личные сообщения
<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> *Мин. бет:* 50.000$"""
    ...
    try:
        ...
        if success:
            ...
            for clan_name, data in result.items():
                report_text += f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> {data['position']} место: {clan_name}\n"
                report_text += f"   👥 Участников: {data['members']}\n"
                report_text += f"   <tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Награда: {data['reward']}\n\n"
            
            report_text += "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Награды получили только кланы с 3+ боецами"
...
def handle_knb_challenge(message):
    ...
    if _tax_is_restricted(challenger_id):
        bot.send_message(message.chat.id,
            "🚫 <b>КНБ заблокирован</b> — есть налоговый долг!\n"
            "Напиши <code>налог</code> чтобы оплатить.", parse_mode="HTML")
        return
    ...
    bot.send_message(
        message.chat.id,
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Вызов на КНБ!</b>\n\n"
        f"<blockquote>👤 {challenger_name} вызывает {opponent_name}\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: <b>{format_balance(bet_amount)}</b></blockquote>\n\n"
...
def handle_knb_response(call):
    ...
    bot.edit_message_text(
        ...
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> {game['challenger_name']} vs {game['opponent_name']}\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: *{format_balance(bet)}*\n\n"
    ...
    try:
        msg1 = bot.send_message(
            challenger_id,
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> *КНБ против {game['opponent_name']}*\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: *{format_balance(bet)}*\n\nВыберите ваш ход:",
            parse_mode='HTML',
    ...
    try:
        msg2 = bot.send_message(
            opponent_id,
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> *КНБ против {game['challenger_name']}*\n<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставка: *{format_balance(bet)}*\n\nВыберите ваш ход:",
            parse_mode='HTML',
            reply_markup=choices_markup
        )
...
def process_knb_result(game_key):
    ...
    if winner == 0:
        result_text = (
            ...
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Ставки остаются у игроков"
        )
    elif winner == 1:
        ...
        result_text = (
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>ПОБЕДИЛ {challenger_name.upper()}!</b>\n\n"
            f"👤 {challenger_name}: {c_choice}\n"
            f"👤 {opponent_name}: {o_choice}\n\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {challenger_name} получил <b>+{format_balance(bet)}</b>\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Новый баланс: <b>{format_balance(new_balance)}</b>"
        )
    else:
        update_balance(challenger_id, -bet)
        update_balance(opponent_id, bet)
        ...
        result_text = (
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>ПОБЕДИЛ {opponent_name.upper()}!</b>\n\n"
            f"👤 {challenger_name}: {c_choice}\n"
            f"👤 {opponent_name}: {o_choice}\n\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {opponent_name} получил <b>+{format_balance(bet)}</b>\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Новый баланс: <b>{format_balance(new_balance)}</b>"
        )
...
def handle_use_promo(message):
    ...
    if ok:
        bot.send_message(
            ...
            f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Код: <code>{code}</code>\n"
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: {format_balance(result)}",
            parse_mode='HTML'
        )
    else:
...
def handle_create_promo(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Код: <code>{code}</code>\n"
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: {format_balance(reward)}\n"
...
def handle_list_promos(message):
    ...
    if not promos:
        ...
        return
    text = "<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> <b>Промокоды</b>\n\n"
    now = time.time()
...
def handle_level_command(message):
    ...
    if lvl_need > 0:
        ...
        progress_bar = "██████████"
        progress_text = f"[{progress_bar}] <tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Максимальный уровень!"
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Опыт: <b>{experience:,}</b>\n\n"
        f"{progress_text}"
        f"{next_info}\n\n"
        f"<b>Твои бонусы:</b>\n"
        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Клики: +{click_bonus}%\n"
        f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Ежедневка: +{daily_bonus}%\n\n"
        f"<b>Как получить опыт:</b>\n"
        f"🎲 Игры (краш, рулетка, слоты...)\n"
        f"👆 Клики\n"
        f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> Ежедневный бонус"
    )
    bot.send_message(message.chat.id, text, parse_mode='HTML')
...
def event_get_markup(count):
    return {
        "inline_keyboard": [[
            _btn(f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Участвовать ({count}/{EVENT_REQUIRED})", callback_data="event_join")
        ]]
    }

EVENT_DICE_EMOJI   = "<tg-emoji emoji-id='6129959611853180053'>🎲</tg-emoji>"
EVENT_TROPHY_EMOJI = "<tg-emoji emoji-id='5422546251587527850'><tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji></tg-emoji>"
...
def event_finish():
    ...
    notify_text = (
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Ивент завершён!</b>\n\n"
...
def event_launch():
    ...
    start_notify = (
        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Новый ивент в чате!</b>\n\n"
...
METEOR_CHAT_ID = int(os.getenv("EVENT_CHAT_ID", "0"))
...
METEOR_REWARDS = [
    ...
    {"reward": 500_000, "max": 1,  "label": "<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> МЕГА-МЕТЕОРИТ"},
]
...
def meteor_launch():
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: <b>{reward_fmt} 🌸</b> каждому поймавшему\n"
        f"👥 Успеют поймать: <b>{chosen['max']} игроков</b>\n\n"
        f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Напиши <b>словить</b> чтобы поймать метеорит!"
    )
...
def meteor_finish(cancelled=False):
    ...
    if cancelled and caught_count == 0:
        ...
        finish_text = (
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: <b>{reward_fmt} 🌸</b>\n"
...
def _do_catch_meteor(uid, name, username=None, chat_id=None):
    ...
    try:
        ...
        if remaining <= 0:
            ...
            new_text = (
                f"🌠 <b>Метеоритный дождь!</b>\n\n"
                f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: <b>{reward_fmt} 🌸</b> каждому\n"
                f"👥 Поймали: <b>{caught_count}/{max_c}</b>\n\n"
                f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Напиши <b>словить</b> чтобы поймать!"
            )
        _tg_api(
            "editMessageCaption",
            chat_id=METEOR_CHAT_ID,
...
BIO_DROP_BOT_USERNAME = "fectiz_bot"
BIO_DROP_HOUR        = int(os.getenv("BIO_DROP_HOUR", "20"))
...
def _cache_user_bio(user_id: int, bio: str):
    """Кешировать bio пользователя в БД."""
    try:
        with get_db_cursor() as cur:
            cur.execute("UPDATE users SET bio_cached=? WHERE user_id=?", (bio[:500] if bio else '', user_id))
    except Exception:
        pass

def _check_bio_has_bot(user_id: int) -> bool:
    """Проверяет bio из кэша в БД — без блокирующего API-вызова."""
    try:
        with get_db_cursor() as cur:
            cur.execute("SELECT bio_cached FROM users WHERE user_id=?", (user_id,))
            row = cur.fetchone()
        if row and row[0]:
            return BIO_DROP_BOT_USERNAME in row[0].lower()
    except Exception:
        pass
    return False
...
def run_bio_drop():
    ...
    try:
        bot.send_message(
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Начислено: <b>{reward_fmt} 🌸</b>\n\n"
    ...
    for admin_id in ADMIN_IDS:
        try:
            bot.send_message(
                admin_id,
                f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> <b>Дроп разыгран!</b>\n\n"
                f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Победитель: <b>{name}</b> (id: {winner_id})\n"
                f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: <b>{reward_fmt} 🌸</b>\n"
...
def handle_admin_stats(message):
    try:
        ...
        text = (
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Экономика</b>\n"
            ...
            f"  <tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Состоятельные (>{TAX_CONFIG['medium_wealth_threshold']//1000}к): <b>{mid_count}</b> чел.\n"
            f"  👤 Обычные: <b>{low_count}</b> чел.\n"
            f"  Топ-5 держат: <b>{top5_share:.1f}%</b> всех денег\n\n"

            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Топ 5 богачей (баланс + банк)</b>\n{rich_lines}\n\n"
            ...
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Кланы</b>\n"
            ...
            f"<tg-emoji emoji-id=\"5203996991054432397\">🎁</tg-emoji> <b>Био-дроп</b>\n"
...
init_db()
_load_donate_packages()
_start_donate_scheduler()
init_dice_tables()

# ─── Гарантируем все таблицы при каждом старте ───────────────────────────────
def ensure_all_tables():
    """Безопасная миграция — создаёт недостающие таблицы и колонки в users."""
    USERS_COLS = [
        ('custom_name',           'TEXT'),
        ('referral_code',         'TEXT UNIQUE'),
        ('referred_by',           'INTEGER'),
            ...
            for col, typedef in USERS_COLS:
                if col not in existing:
                    try:
                        ...
                        print(f'✅ migrate users: +{col}')
                    except Exception as e:
                        print(f'⚠️ migrate users {col}: {e}')
        print('✅ ensure_all_tables: users OK')
    except Exception as e:
        print(f'❌ ensure_all_tables error: {e}')
...
_GEMINI_COOLDOWNS = {}
GEMINI_COOLDOWN_SEC = 1800
GEMINI_NO_LIMIT_ID = 8139807344
...
<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> ДЕНЬГИ И ПЕРЕВОДЫ:
...
if __name__ == "__main__":
    ...
    print("Бот запущен...")
    try:
        with get_db_cursor() as _qc: _qc.execute("DELETE FROM arena_queue")
        print("[arena] очередь сброшена")
    except Exception: pass
...
def start_deposit_scheduler():
    ...
    while True:
        try:
            ...
            bot.infinity_polling(timeout=25, long_polling_timeout=20, allowed_updates=['message','callback_query','pre_checkout_query','successful_payment','inline_query'])
        except Exception as e:
...
def handle_term_deposit(message):
    ...
    if active:
        ...
        bot.send_message(message.chat.id,
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Сумма: <b>{format_balance(active['amount'])}</b>\n"
    ...
    bot.send_message(message.chat.id,
        f"✅ <b>Вклад открыт!</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Сумма: <b>{format_balance(amount)}</b>\n"
        f"⏱ Срок: <b>{days_h}</b>\n"
        f"📈 Ставка: <b>{rate*100:.2f}%/час</b>\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Прибыль: <b>+{format_balance(profit)}</b>{prem_line}\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Получишь: <b>{format_balance(payout)}</b>\n\n"
...
def handle_deposit_info(message):
    ...
    if done:
        ...
        bot.send_message(message.chat.id,
            ...
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Вложено: {format_balance(active['amount'])}\n"
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Прибыль: +{format_balance(active['profit'])}\n"
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Выплачено: <b>{format_balance(active['payout'])}</b>",
            parse_mode="HTML"
        )
        return
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Итого прибыль: <b>+{format_balance(active['profit'])}</b>\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Получишь: <b>{format_balance(active['payout'])}</b>\n\n"
        f"⏱ Осталось: <b>{h}ч {m_}м</b>\n"
...
def handle_deposit_claim(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Вложено: {format_balance(active['amount'])}\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Прибыль: +{format_balance(active['profit'])}\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Итого: <b>{format_balance(active['payout'])}</b> → на баланс",
        parse_mode="HTML"
    )
    ...
    for row in rows:
        ...
        try:
            bot.send_message(
                ...
                f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Вложено: {format_balance(dep['amount'])}\n"
                f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Прибыль: +{format_balance(dep['profit'])}\n"
                f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Итого: <b>{format_balance(dep['payout'])}</b> → баланс",
                parse_mode="HTML"
            )
        except:
            pass
...
ALERT_CONFIG = {
    "game_payout_1h":      5_000_000,
    "single_win":          2_000_000,
    "winrate_threshold":   0.80,
    "winrate_min_games":   20,
    "transfer_single":     10_000_000,
    "balance_jump":        5_000_000,
}
        ...
        try:
            ...
            for uid, game, win in big_wins:
                ...
                if _alerted_users.get(key, 0) < now - 3600:
                    ...
                    send_alert(
                        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Крупная победа!\n"
...
STOCK_INIT_PRICE  = 10_000
STOCK_MIN_PRICE   = 1_000
STOCK_MAX_PRICE   = 10_000_000
STOCK_MAX_PER_USER  = 5000
STOCK_MAX_PER_TRADE = 200
STOCK_SELL_FEE    = 0.03
STOCK_PROFIT_TAX  = 0.15
STOCK_COOLDOWN     = 10 * 60
STOCK_UPDATE_SEC  = 15 * 60
STOCK_HISTORY_LEN = 96

STOCK_LIQUIDITY   = 150

STOCK_VOL_NORMAL  = 0.008
STOCK_VOL_HIGH    = 0.018
STOCK_VOL_CRISIS  = 0.035
STOCK_DRIFT_BASE  = 0.0   # отключён — акции не растут сами по себе

# ── AMM Liquidity Pool ──
STOCK_POOL_EMISSION   = 1_000_000   # стартовая эмиссия акций в пуле
STOCK_POOL_MONEY_MULT = 100         # money_reserve = shares * price * MULT
...
def _init_stocks():
    with get_db_cursor() as cur:
        ...
        ''')
        # ── Таблица пула ликвидности (AMM x*y=k) ──
        cur.execute('''
            CREATE TABLE IF NOT EXISTS stock_pool (
                ticker         TEXT PRIMARY KEY,
                shares_reserve INTEGER NOT NULL,
                money_reserve  INTEGER NOT NULL,
                updated_at     INTEGER NOT NULL
            )
        ''')
        cur.execute(
        ...
        # Инициализация пула если пусто
        cur.execute("SELECT 1 FROM stock_pool WHERE ticker=?", (STOCK_TICKER,))
        if not cur.fetchone():
            shares_r = STOCK_POOL_EMISSION
            money_r  = shares_r * STOCK_INIT_PRICE * STOCK_POOL_MONEY_MULT
            cur.execute(
                "INSERT INTO stock_pool (ticker,shares_reserve,money_reserve,updated_at) VALUES (?,?,?,?)",
                (STOCK_TICKER, shares_r, money_r, int(time.time()))
            )
            print(f"[stocks] pool initialized: shares={shares_r:,}, money={money_r:,}")
...
# ══════════════════════════════════════════════════════════════
#   AMM LIQUIDITY POOL  (x * y = k)
# ══════════════════════════════════════════════════════════════

def _get_pool():
    """Вернуть (shares_reserve, money_reserve) из пула."""
    with get_db_cursor() as cur:
        cur.execute("SELECT shares_reserve, money_reserve FROM stock_pool WHERE ticker=?", (STOCK_TICKER,))
        row = cur.fetchone()
        if row:
            return row[0], row[1]
        # Fallback — создаём пул
        shares_r = STOCK_POOL_EMISSION
        money_r = shares_r * _get_stock_price()[0] * STOCK_POOL_MONEY_MULT
        cur.execute(
            "INSERT INTO stock_pool (ticker,shares_reserve,money_reserve,updated_at) VALUES (?,?,?,?)",
            (STOCK_TICKER, shares_r, money_r, int(time.time()))
        )
        return shares_r, money_r

def _set_pool(shares_r, money_r):
    """Атомарно обновить резервы пула и цену акции."""
    shares_r = max(1, int(shares_r))
    money_r  = max(1, int(money_r))
    new_price = money_r // shares_r
    new_price = max(STOCK_MIN_PRICE, min(STOCK_MAX_PRICE, new_price))
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute(
            "UPDATE stock_pool SET shares_reserve=?, money_reserve=?, updated_at=? WHERE ticker=?",
            (shares_r, money_r, now, STOCK_TICKER)
        )
        # Обновляем цену в stocks
        cur.execute(
...
def _amm_buy_shares(qty, money_in):
    """
    Пользователь вносит money_in денег, хочет купить qty акций.
    По формуле x*y=k:
      new_money = money_reserve + money_in
      new_shares = k / new_money
      shares_out = shares_reserve - new_shares
    Возвращает (фактически_куплено_акций, цена_до, цена_после).
    """
    shares_r, money_r = _get_pool()
    k = shares_r * money_r

    new_money = money_r + money_in
    new_shares = k // new_money
    shares_out = shares_r - new_shares

    price_before = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE

    # Обновляем пул
    new_price = _set_pool(new_shares, new_money)

    return shares_out, price_before, new_price

def _amm_sell_shares(qty):
    """
    Пользователь продаёт qty акций.
    По формуле x*y=k:
      new_shares = shares_reserve + qty
      new_money = k / new_shares
      money_out = money_reserve - new_money
    Возвращает (получено_денег, цена_до, цена_после).
    """
    shares_r, money_r = _get_pool()
    k = shares_r * money_r

    new_shares = shares_r + qty
    new_money = k // new_shares
    money_out = money_r - new_money
    ...
    return money_out, price_before, new_price

def _pool_price():
    """Текущая цена из пула."""
    shares_r, money_r = _get_pool()
    return money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE

def _pool_market_cap():
    """Текущая рыночная капитализация = цена * эмиссия."""
    shares_r, money_r = _get_pool()
    price = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE
    return price * STOCK_POOL_EMISSION
...
def _get_market_volatility():
    ...
    if avg_change > 0.06:
        return STOCK_VOL_CRISIS
    elif avg_change > 0.025:
        return STOCK_VOL_HIGH
    else:
        return STOCK_VOL_NORMAL
...
def _get_momentum():
    ...
    Возвращает значение от -0.015 до +0.015 (±1.5% максимум).
    """
    ...
    momentum = streak * 0.003  # было 0.007, стало 0.003
    return max(-0.015, min(0.015, momentum))

def _market_update():
    """
    Реалистичная рыночная модель через AMM-пул:
    1. Торговое давление покупателей/продавцов (±1.5% макс)
    2. Случайный шум с низкой волатильностью (±1-2%)
    3. Momentum — слабое усиление тренда (±0.5%)
    4. Mean reversion — мягкий возврат к среднему (±1%)
    5. Без upward bias — акции НЕ растут сами по себе

    Итого: ±0.5-3% за обновление, без системного роста.
    """
    shares_r, money_r = _get_pool()
    price = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE
    ...
    # Уменьшены в 2-3 раза чтобы не было +13% за раз
    trade_impact = pressure * 0.015        # было 0.04, стало 0.015 (макс ±1.5%)
    noise = random.gauss(0, vol * 0.6)     # уменьшили волатильность шума
    mom_impact = momentum * 0.2            # было 0.5, стало 0.2 (макс ±0.6%)

    # Mean reversion — мягкий, максимум ±1%
    reversion = 0.0
    history = _stock_history(limit=96)
    if len(history) >= 6:
        avg_price = sum(p for p, _ in history) / len(history)
        deviation = (price - avg_price) / avg_price
        if abs(deviation) > 0.20:          # порог снижен с 30% до 20%
            reversion = -deviation * 0.025  # было 0.08, стало 0.025 (макс ±1%)

    # БИАС УБРАН — акции не растут сами по себе
    bias = 0.0

    total_impact = trade_impact + noise + mom_impact + reversion + bias

    # Простое обновление: меняем только money_reserve пропорционально
    # shares_reserve не трогаем — так цена меняется честно
    new_money = int(money_r * (1 + total_impact))
    new_money = max(money_r // 10, new_money)  # не даём money упасть в 10 раз
    new_price = _set_pool(shares_r, new_money)

    label = (
        f"pressure={pressure:+.2f}({trade_impact*100:+.2f}%) "
        f"noise={noise*100:+.2f}% mom={mom_impact*100:+.2f}% "
        f"rev={reversion*100:+.2f}% vol={vol*100:.2f}% "
        f"total={total_impact*100:+.2f}%"
    )
...
def _apply_trade_impact(qty, is_buy):
    """
    Только для логирования — пул уже обновлён в handle_buy_stock/handle_sell_stock.
    Возвращает текущую цену после сделки.
    """
    new_price, _ = _get_stock_price()
    print(f"[stocks] trade logged: qty={qty} {'BUY' if is_buy else 'SELL'} "
          f"price→{format_balance(new_price)}")
    return new_price
...
def _log_stock_trade(user_id, action, qty, price, total, new_price):
    ...
    try:
        ...
        text = (
            ...
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> {format_balance(price)}/шт · {format_balance(total)}\n"
...
def handle_stock_market(message):
    ...
    total       = _total_shares()
    shares_r, money_r = _get_pool()
    mcap        = price * STOCK_POOL_EMISSION
    ...
    bot.send_message(message.chat.id,
        ...
        f"💧 Ликвидность: {format_balance(money_r)}\n"
        f"📦 Акций у игроков: <b>{total}</b>\n"
        f"🏦 Капитализация: {format_balance(mcap)}\n"
...
def handle_buy_stock(message):
    ...
    if len(parts) < 3:
        bot.send_message(message.chat.id, "❌ Формат: <code>купить акции 100</code>", parse_mode="HTML")
        return

    # Проверка налогового долга
    if _tax_is_restricted(user_id):
        bot.send_message(message.chat.id,
            "🏛️ <b>Налоговое ограничение!</b>\n\n"
            "У тебя просроченный налоговый платёж.\n"
            "Оплати налог командой <code>налог</code> чтобы снять ограничения.",
            parse_mode="HTML")
        return

    # Кулдаун — проверяем ДО списания денег
    ...
    if qty <= 0:
        bot.send_message(message.chat.id, "❌ Укажи положительное число.", parse_mode="HTML")
        return

    if qty > STOCK_MAX_PER_TRADE:
        bot.send_message(message.chat.id, f"❌ Максимум {STOCK_MAX_PER_TRADE} акций за сделку.", parse_mode="HTML")
        return

    owned, avg, _ = _get_portfolio(user_id)
    if owned + qty > STOCK_MAX_PER_USER:
        bot.send_message(message.chat.id, f"❌ Максимум {STOCK_MAX_PER_USER} акций на игрока. У тебя {owned}.", parse_mode="HTML")
        return

    # ── AMM: считаем сколько денег нужно для покупки qty акций ──
    shares_r, money_r = _get_pool()
    k = shares_r * money_r
    price_before = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE

    # Чтобы купить qty акций, новый shares_reserve = shares_r - qty
    new_shares_target = shares_r - qty
    if new_shares_target <= 0:
        bot.send_message(message.chat.id, "❌ Недостаточно ликвидности в пуле.", parse_mode="HTML")
        return
    new_money_needed = k // new_shares_target
    money_in = new_money_needed - money_r
    # Slippage + комиссия пула 1%
    money_in = int(money_in * 1.01) + 1

    now = int(time.time())
    balance = get_balance(user_id)
    if balance < money_in:
        bot.send_message(message.chat.id,
            f"❌ Не хватает средств.\nНужно: <b>{format_balance(money_in)}</b>\n"
            f"Есть: <b>{format_balance(balance)}</b>", parse_mode="HTML")
        return

    # Атомарно: списать деньги → внести в пул → выдать акции
    buy_ok = False
    with get_db_cursor() as cur:
        cur.execute(
            "UPDATE users SET balance=balance-? WHERE user_id=? AND balance>=?",
            (money_in, user_id, money_in)
        )
        if cur.rowcount > 0:
            # Обновляем пул: деньги пришли, акции ушли
            final_shares = shares_r - qty
            final_money = money_r + money_in
            cur.execute(
                "UPDATE stock_pool SET shares_reserve=?, money_reserve=?, updated_at=? WHERE ticker=?",
                (final_shares, final_money, now, STOCK_TICKER)
            )
            # Пересчитываем цену
            new_price = final_money // final_shares if final_shares > 0 else price_before
            new_price = max(STOCK_MIN_PRICE, min(STOCK_MAX_PRICE, new_price))
            cur.execute(
                "UPDATE stocks SET prev_price=price, price=?, updated_at=? WHERE ticker=?",
                (new_price, now, STOCK_TICKER)
            ...
            # Обновляем портфель
            new_avg = (owned * avg + qty * price_before) // (owned + qty) if owned > 0 else price_before
            cur.execute(
            ...
            cur.execute(
                ...
                (user_id, STOCK_TICKER, "buy", qty, price_before, 0, money_in, now)
            )
            buy_ok = True

    if not buy_ok:
        bot.send_message(message.chat.id, "❌ Не удалось провести сделку. Попробуй ещё раз.", parse_mode="HTML")
        return

    # Получаем актуальную цену после обновления
    new_price, _ = _get_stock_price()
    price_change = (new_price - price_before) / price_before * 100 if price_before else 0

    bot.send_message(message.chat.id,
        f"✅ <b>Куплено {qty} акций {STOCK_TICKER}</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Цена: {format_balance(price_before)}/шт.\n"
        f"💸 Списано: <b>{format_balance(money_in)}</b>\n"
        f"📂 В портфеле: {owned + qty} шт.\n"
        f"📈 Цена выросла до {format_balance(new_price)} ({price_change:+.2f}%)\n\n"
        f"⏳ Следующая сделка через {STOCK_COOLDOWN//60} минут",
        parse_mode="HTML"
    )
    threading.Thread(target=_log_stock_trade, args=(user_id, "buy", qty, price_before, money_in, new_price), daemon=True).start()
...
def handle_sell_stock(message):
    ...
    if len(parts) < 3:
        bot.send_message(message.chat.id, "❌ Формат: <code>продать акции 100</code>", parse_mode="HTML")
        return

    # Кулдаун — СНАЧАЛА, до любых операций
    cd = _get_cooldown_left(user_id)
    if cd > 0:
        bot.send_message(message.chat.id, f"⏳ Кулдаун: <b>{cd//60}м {cd%60}с</b>", parse_mode="HTML")
        return
    ...
    # ── AMM: считаем сколько денег получим за продажу qty акций ──
    shares_r, money_r = _get_pool()
    k = shares_r * money_r
    price_before = money_r // shares_r if shares_r > 0 else STOCK_INIT_PRICE

    # Продажа qty акций: shares↑ money↓
    new_shares = shares_r + qty
    new_money = k // new_shares
    money_out = money_r - new_money
    # Slippage 1%
    money_out = int(money_out * 0.99)
    if money_out <= 0:
        bot.send_message(message.chat.id, "❌ Ошибка ликвидности. Попробуй меньшее количество.", parse_mode="HTML")
        return

    now       = int(time.time())

    # Комиссия и налог
    fee        = int(money_out * STOCK_SELL_FEE)
    raw_profit = (price_before - avg) * qty if avg > 0 else 0
    tax        = int(max(0, raw_profit) * STOCK_PROFIT_TAX)
    net        = money_out - fee - tax
    ...
    sell_ok    = False
    with get_db_cursor() as cur:
        # Обновляем пул: акции вернулись, деньги ушли пользователю
        final_shares = shares_r + qty
        final_money = money_r - money_out
        # Комиссия остаётся в пуле (увеличивает цену для всех)
        final_money += fee
        cur.execute(
            "UPDATE stock_pool SET shares_reserve=?, money_reserve=?, updated_at=? WHERE ticker=?",
            (final_shares, final_money, now, STOCK_TICKER)
        ...
        # Пересчитываем цену
        pool_price = final_money // final_shares if final_shares > 0 else price_before
        pool_price = max(STOCK_MIN_PRICE, min(STOCK_MAX_PRICE, pool_price))
        cur.execute(
            "UPDATE stocks SET prev_price=price, price=?, updated_at=? WHERE ticker=?",
            (pool_price, now, STOCK_TICKER)
        )
        cur.execute(
            "INSERT INTO stock_history (ticker,price,recorded_at) VALUES (?,?,?)",
            (STOCK_TICKER, pool_price, now)
        )
        cur.execute(
            "DELETE FROM stock_history WHERE ticker=? AND id NOT IN "
            "(SELECT id FROM stock_history WHERE ticker=? ORDER BY recorded_at DESC LIMIT ?)",
        ...
        # Обновляем портфель
        if new_amount == 0:
            cur.execute("DELETE FROM stock_portfolio WHERE user_id=? AND ticker=?",
                        (user_id, STOCK_TICKER))
        else:
            cur.execute(
        ...
        cur.execute(
            "INSERT INTO stock_trades (user_id,ticker,action,amount,price,fee,total,created_at) VALUES (?,?,?,?,?,?,?,?)",
            (user_id, STOCK_TICKER, "sell", qty, price_before, fee + tax, net, now)
        )
        sell_ok = True
    ...
    new_price, _ = _get_stock_price()
    price_change = (new_price - price_before) / price_before * 100 if price_before else 0
    tax_line     = f"\n💼 Налог с прибыли ({int(STOCK_PROFIT_TAX*100)}%): -{format_balance(tax)}" if tax > 0 else ""

    bot.send_message(message.chat.id,
        f"✅ <b>Продано {qty} акций {STOCK_TICKER}</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Цена продажи: {format_balance(price_before)}/шт.\n"
        f"💸 Комиссия ({int(STOCK_SELL_FEE*100)}%): -{format_balance(fee)}"
        f"{tax_line}\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Получено: <b>{format_balance(net)}</b>\n"
    ...
    threading.Thread(target=_log_stock_trade, args=(user_id, "sell", qty, price_before, net, new_price), daemon=True).start()
...
def handle_stock_history(message):
    ...
    for action, amt, p, fee, total, ts in my_trades:
        dt   = _dt.datetime.utcfromtimestamp(ts).strftime("%d.%m %H:%M")
        icon = "🛒" if action == "buy" else "<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji>"
        ...
        for user_id, shares in holders:
            ...
            try:
                ...
                try:
                    bot.send_message(
                        user_id,
                        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Дивиденды по акциям {STOCK_TICKER}!</b>\n\n"
                        f"📂 У тебя: <b>{shares} акций</b>\n"
                        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> Дивиденд: <b>{format_balance(amount_per_share)}</b>/шт.\n"
...
def handle_pay_dividends(message):
    ...
    msg = bot.send_message(message.chat.id,
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Выплата дивидендов</b>\n\n"
        f"📊 Держателей: <b>{holders_count}</b> чел.\n"
        f"📦 Акций всего: <b>{total_shares_held}</b> шт.\n"
        f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> За акцию: <b>{format_balance(amount_per_share)}</b>\n"
    ...
    if result["success"]:
        ...
        bot.edit_message_text(
            ...
            f"<tg-emoji emoji-id=\"5368741375204415686\">💵</tg-emoji> За акцию: <b>{format_balance(amount_per_share)}</b>\n\n"
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Топ получателей:</b>\n{top_lines}",
            chat_id=message.chat.id,
...
def _role_emoji(role):
    return {"owner": "👑", "officer": "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", "member": "👤"}.get(role, "👤")
...
def _clan_main_text(user_id):
    ...
    if not clan:
        return (
            "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Кланы</b>\n\n"
    ...
    if war:
        ...
        war_line = f"\n<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> <b>Война с [{enemy['tag']}]</b> — до {ends} UTC\n"

    return (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>[{tag}] {name}</b>  Ур.{lvl}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна: <b>{format_balance(balance)}</b>\n"
        f"👥 Участников: <b>{members}/{CLAN_MAX_MEMBERS}</b>\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Войны: <b>{wars_w}W / {wars_l}L</b>  |  Очки: <b>{score}</b>\n"
        f"{war_line}"
        f"━━━━━━━━━━━━━━━\n"
...
@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("клан", "⚔️ клан", "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> клан"))
...
def handle_accept_app(message):
    ...
    bonus_line = "\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> +100 000 🌸 в казну клана!" if is_first_join else ""
    bot.send_message(message.chat.id,
    ...
    if is_first_join:
        channel_msg += "\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> +100 000 🌸 в казну!"
    ...
    notif = (
        ...
        + (f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна пополнилась на <b>100 000 🌸</b>\n" if is_first_join else "")
        ...
        try:
            if mid == target_uid:
                welcome = (
                    ...
                    + (f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна пополнилась на 100 000 🌸 за твой первый вход!\n" if is_first_join else "")
...
def handle_set_officer(message):
    ...
    name_str = _user_display(target_uid)
    action = "назначен офицером <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>" if new_role == "officer" else "разжалован до участника"
    bot.send_message(message.chat.id,
...
def handle_clan_deposit(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна теперь: <b>{format_balance(new_clan_balance)}</b>",
        parse_mode="HTML")
...
def handle_transfer(message):
    if is_user_banned(message.from_user.id):
        bot.send_message(message.chat.id, "🚫 Вы забанены — переводы недоступны.", parse_mode='HTML')
        return
    uid  = message.from_user.id
    clan = get_user_clan(uid)
    if not clan or clan['role'] != 'owner':
...
def handle_declare_war(message):
    ...
    bot.send_message(message.chat.id,
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Объявление войны</b>\n\n"
        f"🔵 <b>[{clan['tag']}] {clan['name']}</b>\n"
        f"   ❤️ HP: <b>{att_hp}</b>\n\n"
        f"🔴 <b>[{enemy['tag']}] {enemy['name']}</b>\n"
        f"   ❤️ HP: <b>{def_hp}</b>\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Взнос из казны: <b>{format_balance(CLAN_WAR_COST)}</b>\n"
        f"⏱ До 24 часов или пока у кого-то HP = 0\n\n"
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Атака: <b>{format_balance(CLAN_WAR_ATTACK_COST)}</b> → -{CLAN_WAR_ATTACK_DMG} HP врагу\n"
...
def cb_war_declare(call):
    ...
    bot.edit_message_text(
        f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> <b>ВОЙНА НАЧАЛАСЬ!</b>\n\n"
        f"🔵 <b>[{attacker['tag']}]</b>  ❤️ {att_hp} HP\n"
        f"🔴 <b>[{defender['tag']}]</b>  ❤️ {def_hp} HP\n\n"
        f"⏱ Конец: <b>{ends_str}</b>\n\n"
        f"Нажми <b><tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Война</b> в меню клана чтобы атаковать!",
        call.message.chat.id, call.message.message_id, parse_mode="HTML"
    )
    bot.answer_callback_query(call.id)
    _clan_post(att_id, f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> <b>ВОЙНА ОБЪЯВЛЕНА!</b>\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> [{attacker['tag']}] vs [{defender['tag']}]\n⏱ Конец: <b>{ends_str}</b>")
    _clan_post(def_id, f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Клан [{attacker['tag']}] объявил нам войну!</b>\n⏱ Конец: <b>{ends_str}</b>")
    ...
    for mid in def_members:
        try:
            bot.send_message(mid,
                f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Клан [{attacker['tag']}] объявил вам войну!</b>\n\n"
                f"🔵 Их HP: <b>{att_hp}</b>\n"
                f"🔴 Ваш HP: <b>{def_hp}</b>\n\n"
                f"⏱ Конец: <b>{ends_str}</b>\n"
                f"Открой меню клана → <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Война!",
                parse_mode="HTML")
        except Exception:
            pass
...
def _war_action(uid, action):
    ...
    if action == 'attack':
        action_txt = f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{name}</b> атаковал [{enemy['tag']}]! -{CLAN_WAR_ATTACK_DMG} HP"
    else:
...
def _war_menu_text_markup(user_id):
    ...
    if not war:
        ...
        mk.add(InlineKeyboardButton("◀️ Назад", callback_data="clan_back"))
        return (
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Клановые войны</b>\n\n"
            f"Клан сейчас не воюет.\n\n"
            f"Объяви войну: <code>клан война ТЕГ</code>\n\n"
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Атака: <b>{format_balance(CLAN_WAR_ATTACK_COST)}</b> → -{CLAN_WAR_ATTACK_DMG} HP\n"
    ...
    for act, val, ts, fn, cn in recent:
        ...
        icon = "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>" if act == 'attack' else "🛡"
    ...
    text = (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Война: [{clan['tag']}] vs [{enemy['tag']}]</b>\n"
        f"⏱ <b>{ends_str}</b>\n\n"
...
def _finish_war(war):
    ...
    if is_draw:
        result_text = (
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Война завершена!</b>\n\n"
        ...
        result_text = (
            ...
            f"🔵 [{attacker['tag']}] ❤️ {att_hp} HP\n"
            f"🔴 [{defender['tag']}] ❤️ {def_hp} HP\n\n"
            f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Победитель: <b>[{winner['tag']}] {winner['name']}</b>\n"
            f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Приз: <b>{format_balance(prize)}</b> из казны проигравшего"
        )
...
def cb_clan_top(call):
    ...
    for i, row in enumerate(rows):
        ...
        lines.append(
            ...
            f"   👥{members} | <tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji>{format_balance(bal)} | <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>{ww}W/{wl}L | <tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji>{ws}pts"
        )
    ...
    bot.edit_message_text(
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Топ кланов</b>\n\n" + "\n\n".join(lines),
        call.message.chat.id, call.message.message_id,
        reply_markup=mk, parse_mode="HTML"
    )
    bot.answer_callback_query(call.id)
...
def handle_clan_info(message):
    ...
    if war:
        ...
        enemy     = _get_clan_by_id(enemy_id)
        ends_str  = _cdt.datetime.utcfromtimestamp(war['ends_at']).strftime("%d.%m %H:%M UTC")
        war_txt   = f"\n<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> <b>Война с [{enemy['tag']}]</b> до {ends_str}\n"
    ...
    bot.send_message(message.chat.id,
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>[{clan['tag']}] {clan['name']}</b>  Ур.{clan['level']}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна: <b>{format_balance(clan['balance'])}</b>\n"
        f"👥 Участников: <b>{len(members)}/{CLAN_MAX_MEMBERS}</b>\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Войны: <b>{clan.get('war_wins',0)}W / {clan.get('war_losses',0)}L</b>\n"
...
def cb_clan_deposit_prompt(call):
    ...
    bot.edit_message_text(
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Взнос в казну [{clan['tag']}]</b>\n\n"
...
def cb_clan_manage(call):
    ...
    if next_info:
        upgrade_line = (
            ...
            + f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Стоимость: {format_balance(cur_info['upgrade_cost'])} из казны"
        )
    else:
        upgrade_line = "\n\n<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Максимальный уровень!"
...
CLAN_LEVELS = {
    ...
    5: {"max_members": 50,  "hp_bonus": 150, "quest_bonus": 0.30, "badge": "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", "upgrade_cost": 0,         "label": "Ур.5 <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>"},
}
...
def _complete_quest(clan_id, quest):
    ...
    text = (
        ...
        f"✅ {desc}\n\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Казна клана <b>[{tag}]</b> пополнилась на <b>{format_balance(reward)}</b>!"
    )
...
def handle_clan_quest(message):
    ...
    bot.send_message(message.chat.id,
        ...
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Награда: <b>{format_balance(quest['reward'])}</b> в казну\n"
        f"⏱ До конца: <b>{ends}</b>",
        parse_mode="HTML"
    )
...
def handle_clan_upgrade(message):
    ...
    if next_level not in CLAN_LEVELS:
        bot.send_message(message.chat.id,
            f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Клан уже на максимальном уровне ({cur_info['label']})!",
            parse_mode="HTML")
        return
    ...
    bot.send_message(message.chat.id,
        ...
        + (f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Уникальный значок клана\n" if next_info['badge'] else "")
        + f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Стоимость: <b>{format_balance(cost)}</b> из казны",
        reply_markup=mk, parse_mode="HTML")
...
def cb_clan_upgrade(call):
    ...
    text = (
        ...
        + (f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> <b>Уникальный значок получен!</b>\n" if next_info['badge'] else "")
...
MINE_RESOURCES = [
    {"id": "coal",      "name": "Уголь",        "fallback": "🪨", "emoji_id": 5456408804541340493, "chance": 28,  "price": 1200},
    {"id": "earth",     "name": "Земля",         "fallback": "🟫", "emoji_id": 5458822138075028493, "chance": 16,  "price": 800},
    {"id": "stone",     "name": "Камень",        "fallback": "🪨", "emoji_id": 5458781211331665562, "chance": 12,  "price": 1000},
    {"id": "copper",    "name": "Медь",          "fallback": "🔶", "emoji_id": 5458520300658368161, "chance": 9,   "price": 2800},
    {"id": "iron",      "name": "Железо",        "fallback": "⚙️", "emoji_id": 5458833133191306560, "chance": 8,   "price": 4500},
    {"id": "wood",      "name": "Дерево",        "fallback": "🪵", "emoji_id": 5458693503804513909, "chance": 5,   "price": 7000},
    {"id": "planks",    "name": "Доски",         "fallback": "📋", "emoji_id": 5456151226762665376, "chance": 4,   "price": 9500},
    {"id": "redstone",  "name": "Редстоун",      "fallback": "🔴", "emoji_id": 5458448020653743095, "chance": 3,   "price": 16000},
    {"id": "lapis",     "name": "Лазурит",       "fallback": "🔵", "emoji_id": 5458825788797231419, "chance": 2,   "price": 30000},
    {"id": "emerald",   "name": "Изумруд",       "fallback": "💚", "emoji_id": 5458517053663093203, "chance": 1.2, "price": 70000},
    {"id": "diamond",   "name": "Алмаз",         "fallback": "💎", "emoji_id": 5458420940884942467, "chance": 1.0, "price": 140000},
    {"id": "gold_ore",  "name": "Золотая руда",  "fallback": "🟡", "emoji_id": 5458617543012918174, "chance": 1.5, "price": 250000},
    {"id": "netherite", "name": "Незерит",       "fallback": "🌑", "emoji_id": 6280349132968168995, "chance": 0.5, "price": 1000000},
]

MINE_COOLDOWN         = 10 * 60
MINE_COOLDOWN_PRIVATE_BY_LEVEL = {
    1: 9 * 60,
    2: 8 * 60,
    3: 7 * 60,
    4: 6 * 60,
...
MINE_RENT_DURATION = 12 * 60 * 60  # 12 часов
MINE_BUY_PRICE     = 500_000
...
MINE_DAILY_DIG_LIMITS = {
    0: 15,   # публичная шахта
    1: 20,
    2: 35,
    3: 55,
    4: 75,
...
def _mine_init():
    with get_db_cursor() as cur:
        ...
        """)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS pickaxe (
                user_id  INTEGER PRIMARY KEY,
                level    INTEGER DEFAULT 1
            )
        """)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS mine_boosts (
                user_id    INTEGER PRIMARY KEY,
                boost_x2   INTEGER DEFAULT 0,
                boost_until INTEGER DEFAULT 0
            )
        """)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS mine_daily_digs (
                user_id    INTEGER NOT NULL,
...
def _boost_active(uid):
    """Возвращает множитель ресурсов от буста (1 или 2)."""
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute("SELECT boost_x2, boost_until FROM mine_boosts WHERE user_id=?", (uid,))
        row = cur.fetchone()
        if row and row[0] and row[1] > now:
            return 2
    return 1

def _boost_set(uid, minutes=30):
    until = int(time.time()) + minutes * 60
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO mine_boosts (user_id, boost_x2, boost_until)
            VALUES (?,1,?)
            ON CONFLICT(user_id) DO UPDATE SET boost_x2=1, boost_until=?
        """, (uid, until, until))

def _boost_left(uid):
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute("SELECT boost_until FROM mine_boosts WHERE user_id=?", (uid,))
        row = cur.fetchone()
...
PICKAXE_LEVELS = {
    ...
    3: {"stars": 20, "bonus": 1.6, "cooldown": 3*60,  "label": "Железная",   "emoji_id": 6280750854144266316, "rare_boost": 0.1, "res_qty": 2},
    4: {"stars": 40, "bonus": 1.8, "cooldown": 2*60,  "label": "Золотая",    "emoji_id": 6278187557532472415, "rare_boost": 0.2, "res_qty": 2},
    5: {"stars": 80, "bonus": 2.0, "cooldown": 90,    "label": "Алмазная",   "emoji_id": 6280722322676519252, "rare_boost": 0.3, "res_qty": 3},
    6: {"stars": 160, "bonus": 2.3, "cooldown": 60,   "label": "Незеритовая", "emoji_id": 6280318969412849349, "rare_boost": 0.5, "res_qty": 4, "multi_res": True},
}
PICKAXE_MAX = 6


def _dig_mode_get(uid):
    """Получить режим добычи: 'mixed' или 'single'. По умолчанию 'single'."""
    try:
        with get_db_cursor() as cur:
            cur.execute("SELECT dig_mode FROM pickaxe WHERE user_id=?", (uid,))
            row = cur.fetchone()
            return (row[0] if row and row[0] else "single")
    except Exception:
        return "single"

def _dig_mode_set(uid, mode):
    with get_db_cursor() as cur:
        cur.execute("UPDATE pickaxe SET dig_mode=? WHERE user_id=?", (mode, uid))

def _pickaxe_get(uid):
    with get_db_cursor() as cur:
        cur.execute("SELECT level FROM pickaxe WHERE user_id=?", (uid,))
        row = cur.fetchone()
        return row[0] if row else 1

def _pickaxe_set(uid, level):
    with get_db_cursor() as cur:
        cur.execute(
            "INSERT INTO pickaxe (user_id, level) VALUES (?,?) "
            "ON CONFLICT(user_id) DO UPDATE SET level=?",
            (uid, level, level)
        )

def _mine_resource(bonus=1.0, rare_boost=0.0):
    """
    rare_boost — множитель шанса редких руд (0.3 = +30% к шансу каждой руды
    начиная с copper и выше).  Для макс. кирки (Лвл 5) rare_boost=0.3 → x2
    шанс редкого, что соответствует описанию в кирке.
    """
    # Порог «редкости» — руды начиная с copper считаются редкими
    RARE_FROM = {"copper", "iron", "wood", "planks", "redstone", "lapis",
                 "emerald", "diamond", "gold_ore", "netherite"}

    chances = []
    for r in MINE_RESOURCES:
        c = r["chance"]
        if rare_boost > 0 and r["id"] in RARE_FROM:
            c = c * (1.0 + rare_boost)   # +30% → ×1.3; +100% → ×2.0
        chances.append(c)

    total = sum(chances)
    roll  = random.uniform(0, total)
    cum   = 0
    for r, c in zip(MINE_RESOURCES, chances):
        cum += c
        if roll <= cum:
            reward = int(r["price"] * bonus * random.uniform(0.85, 1.15))
            return r, reward
    return MINE_RESOURCES[-1], int(MINE_RESOURCES[-1]["price"] * bonus)

def _get_cooldown(user_id):
    """Кулдаун = min(кирка, шахта). Кирка всегда учитывается."""
    pk_lvl   = _pickaxe_get(user_id)
    pk_cd    = PICKAXE_LEVELS[pk_lvl]["cooldown"]
    rental   = _mine_rental(user_id)
    if rental:
        mine     = _mine_by_id(rental[0])
        mine_lvl = mine["level"] if mine else 1
        mine_cd  = MINE_COOLDOWN_PRIVATE_BY_LEVEL.get(mine_lvl, 9 * 60)
        return min(pk_cd, mine_cd)
    return pk_cd

def _mine_cd_left(user_id, cooldown=None):
    if cooldown is None:
        cooldown = _get_cooldown(user_id)
    with get_db_cursor() as cur:
        cur.execute("SELECT last_dig FROM mine_cooldowns WHERE user_id=?", (user_id,))
        row = cur.fetchone()
        if not row:
            return 0
        return max(0, row[0] + cooldown - int(time.time()))
...
def _mine_main_text(uid):
    ...
    if streak >= 10:
        ...
        lines.append(f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> Стрик: <b>{streak}</b> · бонус +{bonus_pct}%")
    elif streak > 0:
        lines.append(f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> Стрик: <b>{streak}</b>  (×10 = бонус)")

    pk_lvl  = _pickaxe_get(uid)
    pk_info = PICKAXE_LEVELS[pk_lvl]
    lines.append(f"⛏️ Кирка: <b>{pk_info['label']}</b>  ·  x{pk_info['bonus']}")
    ...
    mine_lv_for_limit = 0
    if rental:
        _m = _mine_by_id(rental[0])
        mine_lv_for_limit = _m['level'] if _m else 1
    elif my_mine:
        mine_lv_for_limit = my_mine['level']
    dig_limit = MINE_DAILY_DIG_LIMITS.get(mine_lv_for_limit, 15)
    with get_db_cursor() as _dlimit_cur:
        _dlimit_cur.execute("SELECT digs FROM mine_daily_digs WHERE user_id=? AND date_str=?", (uid, today_s))
        _dlimit_row = _dlimit_cur.fetchone()
    digs_done = _dlimit_row[0] if _dlimit_row else 0
    lines.append(f"📊 Сегодня: <b>{digs_done}/{dig_limit}</b> копаний")
...
def _mine_main_kb(uid):
    ...
    kb["inline_keyboard"].append([{"text": "⛏️ Кирка", "callback_data": "mine_pickaxe"}])
    return kb
...
MINE_EVENTS = [
    {"mult": 1.0, "text": "",                                 "weight": 70},
    {"mult": 1.0, "text": "",                                 "weight": 10},
    {"mult": 2.0, "text": "💎 Богатая жила! Двойная добыча!", "weight": 8},
    {"mult": 2.0, "text": "⚡ Удачный удар! Двойная добыча!", "weight": 5},
    {"mult": 0.0, "text": "💥 Обвал! Ничего не нашёл.",      "weight": 5},
    {"mult": 0.0, "text": "🌑 Пустая порода. Ничего нет.",   "weight": 2},
]
...
def _mine_pick_event():
    ...
    for ev in MINE_EVENTS:
        ...
        if roll <= cum:
            return ev
    return MINE_EVENTS[0]

_MINE_CAPTCHA_FRUITS = ["🍎", "🍋", "🍇", "🍊", "🍓", "🍉", "🍑", "🥝", "🍒"]
_mine_captcha_store = {}  # uid -> {"correct": emoji, "chat_id": int, "msg_id": int}

def _mine_reset_cd(user_id):
    with get_db_cursor() as cur:
        cur.execute("DELETE FROM mine_cooldowns WHERE user_id=?", (user_id,))

def _mine_send_captcha(uid, chat_id):
    import random as _r
    fruits = _r.sample(_MINE_CAPTCHA_FRUITS, 3)
    correct = _r.choice(fruits)
    _mine_captcha_store[uid] = {"correct": correct, "chat_id": chat_id}
    kb = {"inline_keyboard": [[
        {"text": f, "callback_data": f"mine_captcha:{uid}:{f}"} for f in fruits
    ]]}
    sent = bot.send_message(chat_id,
        f"🔒 <b>Нажми:</b> {correct}",
        parse_mode="HTML", reply_markup=kb)
    _mine_captcha_store[uid]["msg_id"] = sent.message_id

@bot.callback_query_handler(func=lambda c: c.data.startswith("mine_captcha:"))
def cb_mine_captcha(call):
    parts = call.data.split(":")
    uid = int(parts[1])
    chosen = parts[2]
    if call.from_user.id != uid:
        bot.answer_callback_query(call.id, "❌ Это не твоя капча!", show_alert=False)
        return
    data = _mine_captcha_store.pop(uid, None)
    if not data:
        bot.answer_callback_query(call.id, "⏳ Капча устарела", show_alert=False)
        return
    try:
    ...
    if chosen != data["correct"]:
        _mine_reset_cd(uid)
        bot.send_message(data["chat_id"], "❌ Неверно! Попробуй ещё раз.")
        return
    _mine_do_dig(uid, data["chat_id"])
...
def handle_mine_dig_text(message):
    ...
    if not _mine_try_dig(uid):
        ...
        bot.send_message(message.chat.id, f"⏳ {_mine_fmt_cd(cd)}")
        return
    _mine_send_captcha(uid, message.chat.id)

def _mine_do_dig(uid, chat_id):
    rental  = _mine_rental(uid)
    my_mine = _mine_by_owner(uid)
    pk_lvl  = _pickaxe_get(uid)
    pk_info = PICKAXE_LEVELS[pk_lvl]
    pk_bonus      = pk_info["bonus"]
    pk_rare_boost = pk_info["rare_boost"]
    pk_res_qty    = pk_info["res_qty"]

    # Определяем шахту: сначала аренда, потом своя, иначе — стоп
    if rental:
        mine  = _mine_by_id(rental[0])
        mine_level_t = mine['level'] if mine else 1
        bonus = (MINE_LEVELS[mine_level_t]['bonus'] if mine else 1.0) * pk_bonus
        where = f"шахта #{rental[0]}"
    elif my_mine:
        mine_level_t = my_mine['level']
        bonus = MINE_LEVELS[mine_level_t]['bonus'] * pk_bonus
        where = f"моя шахта #{my_mine['id']}"
    else:
        bot.send_message(
            chat_id,
            "⛏️ <b>Нет доступной шахты!</b>\n\n"
            "Арендуй шахту в разделе <b>Шахта → 🏪 Шахты</b>\n"
            "или купи свою шахту.",
            parse_mode="HTML"
        )
        return

    # Дневной лимит копания
    from datetime import date as _date_t
    today_str_t = _date_t.today().isoformat()
    daily_limit_t = MINE_DAILY_DIG_LIMITS.get(mine_level_t, 15)
    with get_db_cursor() as _dlc_t:
        _dlc_t.execute(
            "INSERT INTO mine_daily_digs (user_id, date_str, digs) VALUES (?,?,1) "
            "ON CONFLICT(user_id, date_str) DO UPDATE SET digs=digs+1",
            (uid, today_str_t)
        )
        _dlc_t.execute("SELECT digs FROM mine_daily_digs WHERE user_id=? AND date_str=?", (uid, today_str_t))
        digs_today_t = _dlc_t.fetchone()[0]
    if digs_today_t > daily_limit_t:
        with get_db_cursor() as _dlr_t:
            _dlr_t.execute("UPDATE mine_daily_digs SET digs=digs-1 WHERE user_id=? AND date_str=?", (uid, today_str_t))
        bot.send_message(
            chat_id,
            f"⛔ <b>Дневной лимит {daily_limit_t} копаний исчерпан.</b> Возвращайся завтра!",
            parse_mode="HTML"
        )
        return

    resource, base_reward = _mine_resource(bonus=bonus, rare_boost=pk_rare_boost)
    # Нерф: кирка Лвл 5 — x0.5 для редких ресурсов
    RARE_HV_T = {"gold_ore", "diamond", "emerald", "netherite", "lapis", "redstone"}
    if pk_lvl >= 5 and resource["id"] in RARE_HV_T:
    ...
    if reward > 0:
        update_balance(uid, reward // 2)
    boost_mult = _boost_active(uid)
    multi_res = pk_info.get("multi_res", False) and _dig_mode_get(uid) == "mixed"
    if event["mult"] > 0:
        event_qty = 2 if event["mult"] >= 2.0 else 1
        if multi_res and pk_res_qty > 1:
            # Лвл 6: добываем pk_res_qty разных ресурсов
            dug_resources = []
            used_ids = set()
            for _ in range(pk_res_qty * event_qty * boost_mult):
                r2, _ = _mine_resource(bonus=bonus, rare_boost=pk_rare_boost)
                _inv_add(uid, r2["id"], 1)
                # группируем для отображения
                found = next((x for x in dug_resources if x["id"] == r2["id"]), None)
                if found:
                    found["qty"] += 1
                else:
                    dug_resources.append({"id": r2["id"], "name": r2["name"],
                                          "fallback": r2["fallback"], "emoji_id": r2["emoji_id"], "qty": 1})
            total_qty = pk_res_qty * event_qty * boost_mult
            resource = dug_resources[0]  # для совместимости
        else:
            total_qty = pk_res_qty * event_qty * boost_mult
            _inv_add(uid, resource["id"], total_qty)
            dug_resources = [{"id": resource["id"], "name": resource["name"],
                               "fallback": resource["fallback"], "emoji_id": resource["emoji_id"], "qty": total_qty}]
    else:
        total_qty = 0
        dug_resources = []
    ...
    if event["mult"] == 0.0:
        ...
        streak_tag = f" <tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>×{new_streak}" if new_streak >= 10 else ""
        boost_tag  = " <tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji>×2" if boost_mult == 2 else ""
        ev_tag     = f"\n<i>{event['text']}</i>" if event["text"] else ""
        if multi_res and len(dug_resources) > 0:
            res_lines = " ".join(
                f"<tg-emoji emoji-id='{r['emoji_id']}'>{r['fallback']}</tg-emoji>" +
                (f"×{r['qty']}" if r['qty'] > 1 else "")
                for r in dug_resources
            )
            text = (
                f"⛏️ <b>{where}</b>{ev_tag}\n"
                f"<blockquote>{res_lines}{boost_tag}  +{_fmt_num(reward // 2)} 🌸{streak_tag}</blockquote>"
            )
        else:
            qty_tag   = f" ×{total_qty}" if total_qty > 1 else ""
            res_emoji = f"<tg-emoji emoji-id='{resource['emoji_id']}'>{resource['fallback']}</tg-emoji>"
            text = (
                f"⛏️ <b>{where}</b>{ev_tag}\n"
                f"<blockquote>{res_emoji}{qty_tag}{boost_tag}  +{_fmt_num(reward // 2)} 🌸{streak_tag}</blockquote>"
            )
    try:
        bot.send_message(chat_id, text, parse_mode="HTML")
    except Exception as e:
        print(f"[mine_dig] HTML send failed: {e}")
        res_fb = " ".join(f"{r['fallback']}×{r['qty']}" if r['qty']>1 else r['fallback'] for r in dug_resources) if dug_resources else resource.get('fallback','⬛')
        st_tag = f"🔥×{new_streak}" if new_streak >= 10 else ""
        fb_text = f"⛏️ {where}\n{res_fb}  +{_fmt_num(reward // 2)} 🌸{st_tag}"
        bot.send_message(chat_id, fb_text)
...
def cb_mine_dig(call):
    ...
    if not _mine_try_dig(uid):
        ...
        return
    bot.answer_callback_query(call.id)
    _mine_send_captcha(uid, call.message.chat.id)

def _cb_mine_dig_unused(call):
    uid = call.from_user.id
    rental  = _mine_rental(uid)
    my_mine = _mine_by_owner(uid)
    pk_lvl  = _pickaxe_get(uid)
    ...
    if rental:
        mine  = _mine_by_id(rental[0])
        mine_level = mine['level'] if mine else 1
        bonus = (MINE_LEVELS[mine_level]['bonus'] if mine else 1.0) * pk_bonus
        where = f"шахта #{rental[0]}"
    elif my_mine:
        mine_level = my_mine['level']
        bonus = MINE_LEVELS[mine_level]['bonus'] * pk_bonus
        where = f"моя шахта #{my_mine['id']}"
    else:
        bot.answer_callback_query(call.id, "⛏️ Нет шахты! Арендуй в разделе Шахта → 🏪 Шахты", show_alert=True)
        return

    # Дневной лимит копания
    from datetime import date as _date
    today_str = _date.today().isoformat()
    daily_limit = MINE_DAILY_DIG_LIMITS.get(mine_level if (rental or my_mine) else 0, 15)
    with get_db_cursor() as _dlc:
        _dlc.execute(
            "INSERT INTO mine_daily_digs (user_id, date_str, digs) VALUES (?,?,1) "
            "ON CONFLICT(user_id, date_str) DO UPDATE SET digs=digs+1",
            (uid, today_str)
        )
        _dlc.execute("SELECT digs FROM mine_daily_digs WHERE user_id=? AND date_str=?", (uid, today_str))
        digs_today = _dlc.fetchone()[0]
    if digs_today > daily_limit:
        # Откатываем инкремент
        with get_db_cursor() as _dlr:
            _dlr.execute("UPDATE mine_daily_digs SET digs=digs-1 WHERE user_id=? AND date_str=?", (uid, today_str))
        bot.answer_callback_query(
            call.id,
            f"⛔ Дневной лимит {daily_limit} копаний исчерпан. Возвращайся завтра!",
            show_alert=True
        )
        return

    resource, base_reward = _mine_resource(bonus=bonus, rare_boost=pk_rare_boost)
    # Нерф: кирка Лвл 5 даёт x0.5 от цены редких ресурсов
    RARE_HIGH_VALUE = {"gold_ore", "diamond", "emerald", "netherite", "lapis", "redstone"}
    if pk_lvl >= 5 and resource["id"] in RARE_HIGH_VALUE:
        base_reward = int(base_reward * 0.5)
    event   = _mine_pick_event()
    reward  = int(base_reward * event["mult"])
    ...
    flower   = "<tg-emoji emoji-id='5440354006335495210'>🌸</tg-emoji>"
    pk_eid   = pk_info['emoji_id']
    pk_emoji = f"<tg-emoji emoji-id='{pk_eid}'>⛏</tg-emoji>"
    if event["mult"] == 0.0:
        text = f"{pk_emoji} <b>{where}</b>\n<blockquote>{event['text']}</blockquote>"
    else:
        qty_tag    = f" ×{total_qty}" if total_qty > 1 else ""
        streak_tag = f" <tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>×{new_streak}" if new_streak >= 10 else ""
        boost_tag  = " <tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji>×2" if boost_mult == 2 else ""
        ev_tag     = f"\n<i>{event['text']}</i>" if event["text"] else ""
        res_eid   = resource['emoji_id']
        res_emoji  = f"<tg-emoji emoji-id='{res_eid}'>⬛</tg-emoji>"
        text = (
    ...
    bot.answer_callback_query(call.id)
    # Кнопка быстрой продажи скупщику под результатом
    dig_kb = None
    if reward > 0 and total_qty > 0:
        buyer_each = _buyer_price(resource)
        inv_after  = _inv_get(uid, resource["id"])
        if inv_after > 0:
            sell_val = buyer_each * inv_after
            dig_kb = {"inline_keyboard": [[
                {"text": f"🏷 Сдать {resource['name']} ×{inv_after} → {plain_balance(sell_val)}",
                 "callback_data": f"buyer_autosell:{resource['id']}"},
            ]]}
    try:
        bot.send_message(call.message.chat.id, text, parse_mode="HTML", reply_markup=dig_kb)
    except Exception:
        bot.send_message(call.message.chat.id,
            f"⛏️ {where}\n{resource['name']}" +
            (f" ×{total_qty}" if total_qty > 1 else "") +
            f"\n+{_fmt_num(reward)} 🌸")
    try:
...
def _pickaxe_kb(uid):
    lvl = _pickaxe_get(uid)
    rows = []
    if lvl < PICKAXE_MAX:
        next_lvl  = lvl + 1
        next_info = PICKAXE_LEVELS[next_lvl]
        rows.append([
            _btn(f"⬆️ Улучшить до Лвл {next_lvl} — {next_info['stars']} <tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji>", callback_data=f"mine_pickaxe_upgrade:{next_lvl}")
        ])
    else:
        rows.append([{"text": "✅ Максимальный уровень", "callback_data": "mine_pickaxe"}])
    if lvl >= 6:
        mode = _dig_mode_get(uid)
        mode_label = "🔀 Режим: Смешанный" if mode == "mixed" else "📦 Режим: Одинаковый"
        rows.append([{"text": mode_label, "callback_data": "mine_toggle_dig_mode"}])
    bl = _boost_left(uid)
    if bl > 0:
        bm, bs = divmod(bl, 60)
        rows.append([_btn(f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Буст активен ({bm}м {bs}с)", callback_data="mine_pickaxe")])
    else:
        rows.append([{"text": "⚡ Купить буст ×2 · 10 ⭐", "callback_data": "mine_boost_buy"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mine_main"}])
    return {"inline_keyboard": rows}

@bot.callback_query_handler(func=lambda c: c.data == "mine_toggle_dig_mode")
def cb_toggle_dig_mode(call):
    uid = call.from_user.id
    if _pickaxe_get(uid) < 6:
        bot.answer_callback_query(call.id, "❌ Только для Незеритовой кирки", show_alert=True)
        return
    cur_mode = _dig_mode_get(uid)
    new_mode = "mixed" if cur_mode == "single" else "single"
    _dig_mode_set(uid, new_mode)
    label = "🔀 Смешанный" if new_mode == "mixed" else "📦 Одинаковый"
    bot.answer_callback_query(call.id, f"✅ Режим: {label}", show_alert=False)
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_pickaxe_text(uid), parse_mode="HTML",
            reply_markup=_pickaxe_kb(uid))

def _pickaxe_text(uid):
    lvl = _pickaxe_get(uid)
    info = PICKAXE_LEVELS[lvl]
    cd_m = info["cooldown"] // 60
    cd_s = info["cooldown"] % 60
    cd_str = f"{cd_m}м" if cd_s == 0 else f"{cd_m}м {cd_s}с"
    bl = _boost_left(uid)
    boost_line = ""
    if bl > 0:
        bm, bs = divmod(bl, 60)
        boost_line = f"\n<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> <b>Буст ×2</b> активен: {bm}м {bs}с"
    return (
        f"⛏️ <b>Кирка — {info['label']}</b> (Лвл {lvl}/{PICKAXE_MAX})\n\n"
        f"🔸 Бонус к добыче: ×{info['bonus']}\n"
        f"⏱ Кулдаун: {cd_str}\n"
        f"💎 Редкие ресурсы: +{int(info['rare_boost']*100)}%\n"
        f"📦 Ресурсов за копание: {info['res_qty']}\n"
        f"{boost_line}"
    )


@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "кирка")
def handle_krika_cmd(message):
    uid = message.from_user.id
    _tg_api("sendMessage", chat_id=message.chat.id,
            text=_pickaxe_text(uid), parse_mode="HTML",
            reply_markup=_pickaxe_kb(uid))

@bot.callback_query_handler(func=lambda c: c.data == "mine_pickaxe")
def cb_mine_pickaxe(call):
    uid = call.from_user.id
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_pickaxe_text(uid), parse_mode="HTML",
...
@bot.callback_query_handler(func=lambda c: c.data in ("mine_main", "mine_back"))
def cb_mine_main(call):
    uid = call.from_user.id
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_mine_main_text(uid), parse_mode="HTML",
            reply_markup=_mine_main_kb(uid))
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mine_pickaxe_upgrade:"))
def cb_mine_pickaxe_upgrade(call):
    uid      = call.from_user.id
    next_lvl = int(call.data.split(":")[1])
    cur_lvl  = _pickaxe_get(uid)

    if next_lvl != cur_lvl + 1 or next_lvl > PICKAXE_MAX:
        bot.answer_callback_query(call.id, "❌ Неверный уровень", show_alert=True); return

    info = PICKAXE_LEVELS[next_lvl]
    stars = info["stars"]

    try:
        bot.send_invoice(
            chat_id=call.message.chat.id,
            title=f"Кирка Лвл {next_lvl}",
            description=f"{info['label']} · x{info['bonus']} · КД {info['cooldown']//60}м",
            invoice_payload=f"pickaxe_{uid}_{next_lvl}",
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(label=f"Кирка Лвл {next_lvl}", amount=stars)],
...
@bot.callback_query_handler(func=lambda c: c.data == "mine_boost_buy")
def cb_mine_boost_buy(call):
    uid = call.from_user.id
    bl  = _boost_left(uid)
    if bl > 0:
        bot.answer_callback_query(call.id, "Буст уже активен!", show_alert=True)
        return
    try:
        bot.send_invoice(
            chat_id=call.message.chat.id,
            title="<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> Буст ×2 ресурсов",
            description="Удваивает количество ресурсов за каждое копание на 30 минут",
            invoice_payload=f"mine_boost_{uid}",
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(label="Буст ×2 на 30 мин", amount=10)],
            protect_content=True,
        )
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Ошибка: {e}", show_alert=True)
...
def cb_mine_list(call):
    ...
    for row in rows:
        ...
        name    = (cname or fname or (f"@{uname}" if uname else f"#{owner_id}"))[:10]
        lv_info = MINE_LEVELS[level]
        btn_text = (
            f'<tg-emoji emoji-id="5217871612321216821">💰</tg-emoji>'
            f" #{mid} {lv_info['label']} x{lv_info['bonus']}"
            f"\n{plain_balance(rent_price)} · {name}"
        )
        row_buf.append(_btn(btn_text, callback_data=f"mine_view_{mid}"))
        if len(row_buf) == 2:
            kb["inline_keyboard"].append(row_buf)
            row_buf = []
    if row_buf:
        kb["inline_keyboard"].append(row_buf)
...
def cb_mine_view(call):
    ...
    text = (
        ...
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> {lv_info['label']} · Бонус <b>x{lv_info['bonus']}</b>\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Аренда: <b>{format_balance(mine['rent_price'])}</b> / 10 мин\n\n"
    ...
    if mine['owner_id'] == uid:
        ...
        kb["inline_keyboard"].append([{
            "text": f"⛏ Арендовать за {plain_balance(mine['rent_price'])}",
...
def cb_mine_rent(call):
    ...
    bot.answer_callback_query(call.id, "✅ Арендовал на 1 час! Жми Копать.", show_alert=False)
    try:
        bot.edit_message_text(_mine_main_text(uid), call.message.chat.id, call.message.message_id,
                              reply_markup=_mine_main_kb(uid), parse_mode="HTML")
    except Exception:
...
def cb_mine_buy_prompt(call):
    ...
    if _mine_by_owner(uid):
        ...
        return
    kb = {"inline_keyboard": [
        [{"text": f"✅ Купить за {plain_balance(MINE_BUY_PRICE)}", "callback_data": "mine_buy_confirm"}],
        [{"text": "◀️ Назад", "callback_data": "mine_main"}],
    ]}
    bot.edit_message_text(
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>Купить шахту</b>\n\n"
...
def _mymine_text(mine):
    ...
    upgrade = (f"⬆️ До Ур.{next_lv}: <b>{format_balance(lv_info['upgrade_cost'])}</b>"
               if next_lv in MINE_LEVELS else "<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Максимальный уровень")
    return (
        f"🏔️ <b>Моя шахта #{mine['id']}</b>\n\n"
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> {lv_info['label']} · Бонус <b>x{lv_info['bonus']}</b>\n"
        f"<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Баланс: <b>{format_balance(mine['balance'])}</b>\n"
...
def _mymine_kb(mine):
    ...
    kb = {"inline_keyboard": []}
    if mine['balance'] > 0:
        kb["inline_keyboard"].append([_btn(f'<tg-emoji emoji-id="5368741375204415686">💵</tg-emoji> Вывести {plain_balance(mine["balance"])}', callback_data="mine_withdraw")])
    if next_lv in MINE_LEVELS:
        kb["inline_keyboard"].append([{"text": f"⬆️ Улучшить · {plain_balance(lv_info['upgrade_cost'])}", "callback_data": "mine_upgrade"}])
...
def cb_mine_top(call):
    ...
    text   = "<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>Лучшие майнеры</b>\n\n"
    if not rows:
    ...
    for i, row in enumerate(rows):
        ...
        streak_tag = f" <tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji>{bs}" if bs >= 10 else ""
...
def _inv_get(user_id, resource_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT amount FROM resource_inventory WHERE user_id=? AND resource_id=?",
                    (user_id, resource_id))
        row = cur.fetchone()
        return row[0] if row else 0

def _inv_add(user_id, resource_id, amount):
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO resource_inventory (user_id, resource_id, amount)
            VALUES (?,?,?)
            ON CONFLICT(user_id, resource_id) DO UPDATE SET amount=MAX(0, amount+?)
        """, (user_id, resource_id, max(0, amount), amount))

def _inv_all(user_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT resource_id, amount FROM resource_inventory WHERE user_id=? AND amount>0",
                    (user_id,))
...
def _res_emoji(r):
    return f'<tg-emoji emoji-id="{r["emoji_id"]}">⛏</tg-emoji>'

def _sell_order_get(user_id, resource_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE seller_id=? AND resource_id=?",
                    (user_id, resource_id))
        row = cur.fetchone()
        return dict(row) if row else None

def _buy_order_get(user_id, resource_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_buy_orders WHERE buyer_id=? AND resource_id=?",
                    (user_id, resource_id))
        row = cur.fetchone()
        return dict(row) if row else None

def _sell_orders_list(resource_id=None, limit=8):
    with get_db_cursor() as cur:
        if resource_id:
            cur.execute("""
                SELECT o.*, u.first_name, u.custom_name, u.username
                FROM market_sell_orders o JOIN users u ON u.user_id=o.seller_id
                WHERE o.resource_id=? ORDER BY o.price_each ASC LIMIT ?
            """, (resource_id, limit))
        else:
            cur.execute("""
                SELECT o.*, u.first_name, u.custom_name, u.username
...
def _buy_orders_list(resource_id=None, limit=8):
    with get_db_cursor() as cur:
        if resource_id:
            cur.execute("""
                SELECT o.*, u.first_name, u.custom_name, u.username
                FROM market_buy_orders o JOIN users u ON u.user_id=o.buyer_id
                WHERE o.resource_id=? ORDER BY o.price_each DESC LIMIT ?
            """, (resource_id, limit))
        else:
            cur.execute("""
                SELECT o.*, u.first_name, u.custom_name, u.username
...
def _mkt_user_name(row):
    return (row.get("custom_name") or row.get("first_name") or
            (f'@{row["username"]}' if row.get("username") else
             f'#{row.get("seller_id") or row.get("buyer_id")}'))

def _mkt_user_name_by_id(uid):
    with get_db_cursor() as cur:
        cur.execute("SELECT first_name, custom_name, username FROM users WHERE user_id=?", (uid,))
        r = cur.fetchone()
        if not r:
            return f"#{uid}"
        return r[1] or r[0] or (f"@{r[2]}" if r[2] else f"#{uid}")

def _market_main_text():
    with get_db_cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM market_sell_orders")
        sc = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM market_buy_orders")
        bc = cur.fetchone()[0]
        cur.execute("""
            SELECT resource_id, COUNT(*) as cnt
            FROM (
                SELECT resource_id FROM market_sell_orders
                UNION ALL
                SELECT resource_id FROM market_buy_orders
            ) GROUP BY resource_id ORDER BY cnt DESC LIMIT 3
        """)
        top = cur.fetchall()

    lines = ["🏪 <b>Рынок ресурсов</b>\n"]

    if sc == 0 and bc == 0:
        lines.append("Пока тихо — заявок нет.\nБудь первым!")
    else:
        lines.append(f"<code>Продают  {sc:>3}   Покупают  {bc:>3}</code>")
        if top:
            hot = []
            for (rid, _) in top:
                r = _res_by_id(rid)
                if r:
                    hot.append(r["name"])
            if hot:
                lines.append(f"\n<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> В топе: {' · '.join(hot)}")

    lines.append(f"\n<i>Комиссия продавца — 5%</i>")
    return "\n".join(lines)

def _market_main_kb():
    return {"inline_keyboard": [
        [
            {"text": "📦 Продать", "callback_data": "mkt_sell_pick"},
            {"text": "🛒 Купить",  "callback_data": "mkt_buy_pick"},
        ],
        [
            {"text": "📋 Мои заявки", "callback_data": "mkt_my_orders"},
            {"text": "🔍 Все заявки", "callback_data": "mkt_browse_res"},
        ],
        [
            {"text": "🎒 Инвентарь", "callback_data": "mkt_inventory"},
            {"text": "🏷 Скупщик",   "callback_data": "buyer_main"},
        ],
    ]}


# ─── СКУПЩИК ───────────────────────────────────────────────
BUYER_RATE = 5  # продаём за price * 3 // 5 базовой цены

def _buyer_price(resource):
    """Цена скупщика за 1 шт."""
    return max(1, resource["price"] * 3 // 5)

def _buyer_sell_all(user_id):
    """Продать весь инвентарь скупщику. Возвращает (total_gold, sold_list)."""
    inv = _inv_all(user_id)
    if not inv:
        return 0, []
    total = 0
    sold = []
    for rid, amt in inv.items():
        r = _res_by_id(rid)
        if not r or amt <= 0:
            continue
        earned = _buyer_price(r) * amt
        _inv_add(user_id, rid, -amt)
        update_balance(user_id, earned)
        total += earned
        sold.append((r, amt, earned))
    return total, sold

def _buyer_sell_one(user_id, resource_id, amount=None):
    """Продать конкретный ресурс скупщику (amount=None → всё)."""
    r = _res_by_id(resource_id)
    if not r:
        return 0, 0
    inv_amt = _inv_get(user_id, resource_id)
    if inv_amt <= 0:
        return 0, 0
    amt = min(amount, inv_amt) if amount else inv_amt
    earned = _buyer_price(r) * amt
    _inv_add(user_id, resource_id, -amt)
    update_balance(user_id, earned)
    return amt, earned

def _buyer_orders_text():
    """Строка с заявками скупщика — показывается в меню рынка."""
    lines = ["🏷 <b>Скупщик</b> (постоянно)\n"]
    for r in MINE_RESOURCES:
        price = _buyer_price(r)
        lines.append(f"  {_res_emoji(r)} {r['name']}: {plain_balance(price)}/шт.")
    return "\n".join(lines)

# ─── CALLBACKS ─────────────────────────────────────────────

@bot.callback_query_handler(func=lambda c: c.data == "buyer_main")
def cb_buyer_main(call):
    uid = call.from_user.id
    inv = _inv_all(uid)
    lines = ["🏷 <b>Скупщик ресурсов</b>\n",
             "Скупает всё — быстро и без заявок.\n",
             f"<i>Цена: 1/5 от базовой</i>\n"]
    rows = []
    if inv:
        total_potential = 0
        for rid, amt in inv.items():
            r = _res_by_id(rid)
            if not r: continue
            val = _buyer_price(r) * amt
            total_potential += val
            lines.append(f"{_res_emoji(r)} {r['name']}: {amt} шт. → {plain_balance(val)}")
        lines.append(f"\n💼 Итого: <b>{plain_balance(total_potential)}</b>")
        rows.append([{"text": f"💸 Сдать всё ({plain_balance(total_potential)})",
                      "callback_data": "buyer_sell_all"}])
        # По одному ресурсу
        pair = []
        for rid, amt in inv.items():
            r = _res_by_id(rid)
            if not r: continue
            pair.append({"text": f"{r['name']} ×{amt}",
                         "callback_data": f"buyer_sell_one:{rid}"})
            if len(pair) == 2:
                rows.append(pair); pair = []
        if pair: rows.append(pair)
    else:
        lines.append("Инвентарь пуст — нечего сдавать.")
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_main"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text="\n".join(lines), parse_mode="HTML",
...
@bot.callback_query_handler(func=lambda c: c.data == "buyer_sell_all")
def cb_buyer_sell_all(call):
    uid = call.from_user.id
    total, sold = _buyer_sell_all(uid)
    if not sold:
        bot.answer_callback_query(call.id, "🎒 Нечего продавать", show_alert=True)
        return
    lines = ["💸 <b>Продано скупщику</b>\n"]
    for r, amt, earned in sold:
        lines.append(f"{_res_emoji(r)} {r['name']} ×{amt} → {plain_balance(earned)}")
    lines.append(f"\n✅ Итого: <b>{plain_balance(total)}</b>")
    new_bal = get_balance(uid)
    lines.append(f"💼 Баланс: <b>{format_balance(new_bal)}</b>")
    kb = {"inline_keyboard": [
        [{"text": "🏷 Скупщик", "callback_data": "buyer_main"}],
        [{"text": "◀️ Рынок",   "callback_data": "mkt_main"}],
    ]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("buyer_sell_one:"))
def cb_buyer_sell_one(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    amt, earned = _buyer_sell_one(uid, rid)
    if not amt:
        bot.answer_callback_query(call.id, "❌ Нет ресурса", show_alert=True)
        return
    r = _res_by_id(rid)
    bot.answer_callback_query(
        call.id,
        f"✅ {r['name']} ×{amt} → +{plain_balance(earned)}",
        show_alert=False
    )
    # Обновить панель скупщика
    cb_buyer_main(call)

# ─── AUTO-SELL после копания ────────────────────────────────
@bot.callback_query_handler(func=lambda c: c.data.startswith("buyer_autosell:"))
def cb_buyer_autosell(call):
    """Продать конкретный ресурс сразу после копания — inline кнопка под результатом."""
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    amt, earned = _buyer_sell_one(uid, rid)
    if not amt:
        bot.answer_callback_query(call.id, "❌ Ресурс уже продан", show_alert=False)
        return
    r = _res_by_id(rid)
    bot.answer_callback_query(
        call.id,
        f"💸 Продано: {r['name']} ×{amt} → +{plain_balance(earned)}",
        show_alert=False
    )
# ────────────────────────────────────────────────────────────

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("рынок", "🏪 рынок"))
def handle_market(message):
    if message.chat.type != "private":
        bot_username = bot.get_me().username
        kb = {"inline_keyboard": [[
            {"text": "🏪 Открыть рынок", "url": f"https://t.me/{bot_username}?start=market"}
        ]]}
        _tg_api("sendMessage", chat_id=message.chat.id,
                text="🏪 <b>Рынок ресурсов</b> — открой в личке:",
                parse_mode="HTML", reply_markup=kb)
        return
    _tg_api("sendMessage", chat_id=message.chat.id,
            text=_market_main_text(), parse_mode="HTML", reply_markup=_market_main_kb())

@bot.callback_query_handler(func=lambda c: c.data == "mkt_main")
def cb_mkt_main(call):
    _market_fsm.pop(call.from_user.id, None)
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_market_main_text(), parse_mode="HTML", reply_markup=_market_main_kb())
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "mkt_inventory")
def cb_mkt_inventory(call):
    uid = call.from_user.id
    inv = _inv_all(uid)
    if not inv:
        text = "🎒 <b>Инвентарь пуст</b>\n\nИди копать — пиши <code>копать</code>!"
    else:
        text = "🎒 <b>Инвентарь</b>\n\n"
        total_val = 0
        for rid, amt in inv.items():
            ...
            if r:
                val = amt * r["price"]
                total_val += val
                text += f"{_res_emoji(r)} {r['name']}: <b>{amt} шт.</b>  ≈ {plain_balance(val)}\n"
        text += f"\n💼 Всего: ≈ {plain_balance(total_val)}"
    if inv:
        kb = {"inline_keyboard": [
            [{"text": "🏷 Сдать всё скупщику", "callback_data": "buyer_sell_all"}],
            [{"text": "◀️ Назад", "callback_data": "mkt_main"}],
        ]}
    else:
        kb = {"inline_keyboard": [[{"text": "◀️ Назад", "callback_data": "mkt_main"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "mkt_sell_pick")
def cb_mkt_sell_pick(call):
    uid = call.from_user.id
    inv = _inv_all(uid)
    if not inv:
        bot.answer_callback_query(call.id, "🎒 Инвентарь пуст — нечего продавать", show_alert=True)
        return
    pair = []
    rows = []
    for rid, amt in inv.items():
        ...
        if not r:
            continue
        mark = "✅ " if _sell_order_get(uid, rid) else ""
        pair.append({
            "text": f"{mark}{r['name']} ·{amt}",
            "callback_data": f"mkt_sell_res:{rid}",
            "icon_custom_emoji_id": str(r['emoji_id'])
        })
        if len(pair) == 2:
            rows.append(pair)
            pair = []
    ...
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text="📦 <b>Продать — выбери ресурс</b>\n\n✅ = активная заявка",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_res:"))
def cb_mkt_sell_res(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    if not r:
        bot.answer_callback_query(call.id, "❌ Ресурс не найден"); return
    inv_amt  = _inv_get(uid, rid)
    existing = _sell_order_get(uid, rid)
    buy_orders = [o for o in _buy_orders_list(rid, limit=5) if o["buyer_id"] != uid]
    text = (f"📦 {_res_emoji(r)} <b>{r['name']}</b>\n\n"
            f"В инвентаре: <b>{inv_amt} шт.</b>\n"
            f"Базовая цена: <b>{plain_balance(r['price'])}/шт.</b>\n\n")
    if buy_orders:
        text += "🛒 <b>Хотят купить:</b>\n"
        for o in buy_orders:
            text += f"  • {o['amount']} шт. по {plain_balance(o['price_each'])} — {_mkt_user_name(o)}\n"
        text += "\n"
    rows = []
    if buy_orders and inv_amt > 0:
        rows.append([{"text": "⚡ Продать в заявку", "callback_data": f"mkt_sell_to_buy:{rid}"}])
    if existing:
        text += (f"⚠️ <b>Активная заявка:</b>\n"
                 f"  {existing['amount']} шт. по {plain_balance(existing['price_each'])}\n"
                 f"  Итого: {plain_balance(existing['amount'] * existing['price_each'])}\n\n"
                 f"Отмени чтобы выставить новую.")
        rows.append([{"text": "❌ Отменить заявку", "callback_data": f"mkt_sell_cancel:{rid}"}])
    else:
        if inv_amt <= 0:
            text += "Нет ресурса в инвентаре."
        else:
            rows.append([{"text": "➕ Выставить заявку", "callback_data": f"mkt_sell_start:{rid}"}])
    # Кнопка скупщика
    if inv_amt > 0:
        buyer_p = _buyer_price(r)
        rows.append([{"text": f"🏷 Сдать скупщику ({plain_balance(buyer_p * inv_amt)})",
                      "callback_data": f"buyer_sell_one:{rid}"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_sell_pick"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_to_buy:"))
def cb_mkt_sell_to_buy(call):
    """Показать список чужих заявок на покупку — продавец выбирает в какую продать."""
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    buy_orders = [o for o in _buy_orders_list(rid, limit=8) if o["buyer_id"] != uid]
    if not buy_orders:
        bot.answer_callback_query(call.id, "Нет чужих заявок на покупку", show_alert=True); return
    rows = []
    for o in buy_orders:
        total = o["amount"] * o["price_each"]
        rows.append([{
            "text": f"{o['amount']} шт. × {plain_balance(o['price_each'])} = {plain_balance(total)}  ({_mkt_user_name(o)})",
            "callback_data": f"mkt_sell_to_order:{o['id']}"
        }])
    rows.append([{"text": "◀️ Назад", "callback_data": f"mkt_sell_res:{rid}"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> {_res_emoji(r)} <b>{r['name']}</b> — выбери заявку покупателя:",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_to_order:"))
def cb_mkt_sell_to_order(call):
    """Подтверждение продажи в конкретную заявку покупателя."""
    uid      = call.from_user.id
    order_id = int(call.data.split(":", 1)[1])
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_buy_orders WHERE id=?", (order_id,))
        row = cur.fetchone()
        order = dict(row) if row else None
    if not order:
        bot.answer_callback_query(call.id, "❌ Заявка уже закрыта", show_alert=True); return
    if order["buyer_id"] == uid:
        bot.answer_callback_query(call.id, "❌ Нельзя продать самому себе", show_alert=True); return
    r       = _res_by_id(order["resource_id"])
    inv_amt = _inv_get(uid, order["resource_id"])
    max_amt = min(order["amount"], inv_amt)
    total   = max_amt * order["price_each"]
    kb = {"inline_keyboard": [
        [{"text": f"Всё ({max_amt} шт.) — {plain_balance(total)}", "callback_data": f"mkt_sell_exec:{order_id}:{max_amt}"}],
        [{"text": "Указать количество", "callback_data": f"mkt_sell_partial:{order_id}"}],
        [{"text": "❌ Отмена", "callback_data": f"mkt_sell_to_buy:{order['resource_id']}"}],
    ]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=(f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> {_res_emoji(r)} <b>{r['name']}</b>\n\n"
                  f"Покупатель: {_mkt_user_name_by_id(order['buyer_id'])}\n"
                  f"Хочет: <b>{order['amount']} шт.</b>\n"
                  f"Цена: <b>{plain_balance(order['price_each'])}/шт.</b>\n"
                  f"У тебя в инвентаре: <b>{inv_amt} шт.</b>\n"
                  f"Сколько продаёшь?"),
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_exec:"))
def cb_mkt_sell_exec(call):
    uid   = call.from_user.id
    parts = call.data.split(":")
    order_id = int(parts[1])
    amount   = int(parts[2])
    _execute_market_sell(call.message, uid, order_id, amount)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_partial:"))
def cb_mkt_sell_partial(call):
    uid      = call.from_user.id
    order_id = int(call.data.split(":", 1)[1])
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_buy_orders WHERE id=?", (order_id,))
        ...
        order = dict(row) if row else None
    if not order:
        bot.answer_callback_query(call.id, "❌ Заявка закрыта"); return
    inv_amt = _inv_get(uid, order["resource_id"])
    max_amt = min(order["amount"], inv_amt)
    _market_fsm[uid] = {
        "step": "sell_to_order_amount",
        "order_id": order_id,
        "rid": order["resource_id"],
        "max_amt": max_amt,
        "price_each": order["price_each"],
    }
    kb = {"inline_keyboard": [[{"text": "❌ Отмена", "callback_data": f"mkt_sell_to_buy:{order['resource_id']}"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"Сколько штук продать?\nМакс. {max_amt} шт. по {plain_balance(order['price_each'])}/шт.\nНапиши число:",
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

def _execute_market_sell(message, uid, order_id, amount):
    """Атомарное исполнение сделки продажи в заявку покупателя."""
    _market_fsm.pop(uid, None)
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_buy_orders WHERE id=?", (order_id,))
        row = cur.fetchone()
        order = dict(row) if row else None
    if not order:
        bot.send_message(message.chat.id, "❌ Заявка уже закрыта"); return
    if order["buyer_id"] == uid:
        bot.send_message(message.chat.id, "❌ Нельзя продать самому себе"); return
    inv_amt = _inv_get(uid, order["resource_id"])
    if amount <= 0 or amount > min(order["amount"], inv_amt):
        bot.send_message(message.chat.id, f"❌ От 1 до {min(order['amount'], inv_amt)} шт."); return
    total       = amount * order["price_each"]
    commission  = int(total * MARKET_COMMISSION)
    seller_gets = total - commission
    _inv_add(uid, order["resource_id"], -amount)
    update_balance(uid, seller_gets)
    _inv_add(order["buyer_id"], order["resource_id"], amount)
    remaining = order["amount"] - amount
    refund    = remaining * order["price_each"]
    with get_db_cursor() as cur:
        if remaining <= 0:
            cur.execute("DELETE FROM market_buy_orders WHERE id=?", (order_id,))
        else:
            cur.execute("UPDATE market_buy_orders SET amount=?, frozen_gold=? WHERE id=?",
                        (remaining, refund, order_id))
    spent = amount * order["price_each"]
    leftover = order["frozen_gold"] - spent
    if leftover > 0 and remaining <= 0:
        update_balance(order["buyer_id"], leftover)
    r = _res_by_id(order["resource_id"])
    kb = {"inline_keyboard": [[{"text": "🏪 На рынок", "callback_data": "mkt_main"}]]}
    bot.send_message(message.chat.id,
        f"✅ <b>Продажа совершена!</b>\n\n"
        f"{_res_emoji(r)} {r['name']} × {amount}\n"
        f"Получено: <b>{plain_balance(seller_gets)}</b> (−5% комиссия)",
        parse_mode="HTML", reply_markup=kb)
    try:
        seller_name = _mkt_user_name_by_id(uid)
        bot.send_message(order["buyer_id"],
            f"🔔 <b>Твою заявку исполнили!</b>\n\n"
            f"{_res_emoji(r)} {r['name']} × {amount}\n"
            f"Продавец: {seller_name}\n"
            f"Списано: <b>{plain_balance(spent)}</b>",
            parse_mode="HTML")
    except Exception:
        pass

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_start:"))
def cb_mkt_sell_start(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    inv_amt = _inv_get(uid, rid)
    if inv_amt <= 0:
        bot.answer_callback_query(call.id, "❌ Нет ресурса в инвентаре", show_alert=True); return
    if _sell_order_get(uid, rid):
        bot.answer_callback_query(call.id, "Заявка уже есть — отмени её", show_alert=True); return
    _market_fsm[uid] = {"step": "sell_amount", "rid": rid}
    kb = {"inline_keyboard": [
        [{"text": f"Всё ({inv_amt} шт.)", "callback_data": f"mkt_fsm_val:{inv_amt}"}],
        [{"text": "❌ Отмена", "callback_data": "mkt_sell_pick"}],
    ]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"📦 {_res_emoji(r)} <b>{r['name']}</b>\n\nСколько продаёшь? (макс. {inv_amt})\nНапиши число:",
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_sell_cancel:"))
def cb_mkt_sell_cancel(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE seller_id=? AND resource_id=?", (uid, rid))
        row = cur.fetchone()
        if not row:
            bot.answer_callback_query(call.id, "Заявка не найдена"); return
        order = dict(row)
        cur.execute("DELETE FROM market_sell_orders WHERE seller_id=? AND resource_id=?", (uid, rid))
    _inv_add(uid, rid, order["amount"])
    r = _res_by_id(rid)
    bot.answer_callback_query(call.id, f"Заявка отменена, {r['name']} возвращён")
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_market_main_text(), parse_mode="HTML", reply_markup=_market_main_kb())

@bot.callback_query_handler(func=lambda c: c.data == "mkt_buy_pick")
def cb_mkt_buy_pick(call):
    pair = []
    rows = []
    for r in MINE_RESOURCES:
        with get_db_cursor() as cur:
            cur.execute("SELECT COUNT(*), MIN(price_each) FROM market_sell_orders WHERE resource_id=?", (r["id"],))
            cnt, minp = cur.fetchone()
        label = f"{r['name']} ·{cnt}" if cnt else r['name']
        pair.append({
            "text": label,
            "callback_data": f"mkt_buy_res:{r['id']}",
            "icon_custom_emoji_id": str(r['emoji_id'])
        })
        if len(pair) == 2:
            rows.append(pair)
    ...
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text="🛒 <b>Купить ресурс</b>\n\nВыбери что хочешь:",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_res:"))
def cb_mkt_buy_res(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    if not r:
        bot.answer_callback_query(call.id, "❌ Ресурс не найден"); return
    sell_orders  = _sell_orders_list(rid, limit=5)
    existing_buy = _buy_order_get(uid, rid)
    text = f"🛒 {_res_emoji(r)} <b>{r['name']}</b>\n\n"
    if sell_orders:
        text += "📦 <b>Заявки на продажу:</b>\n"
        for o in sell_orders:
            if o["seller_id"] == uid:
                text += f"  • {o['amount']} шт. по {plain_balance(o['price_each'])} — <i>твоя</i>\n"
            else:
                text += f"  • {o['amount']} шт. по {plain_balance(o['price_each'])} — {_mkt_user_name(o)}\n"
        text += "\n"
    else:
        text += "📦 Заявок на продажу нет.\n\n"
    rows = []
    if any(o["seller_id"] != uid for o in sell_orders):
        rows.append([{"text": "⚡ Купить из заявок", "callback_data": f"mkt_buy_from_sell:{rid}"}])
    if existing_buy:
        text += (f"⚠️ <b>Твоя заявка на покупку:</b>\n"
                 f"  {existing_buy['amount']} шт. по {plain_balance(existing_buy['price_each'])}\n"
                 f"  Заморожено: {plain_balance(existing_buy['frozen_gold'])}")
        rows.append([{"text": "❌ Отменить мою заявку", "callback_data": f"mkt_buy_cancel:{rid}"}])
    else:
        rows.append([{"text": "➕ Выставить заявку на покупку", "callback_data": f"mkt_buy_start:{rid}"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_buy_pick"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_from_sell:"))
def cb_mkt_buy_from_sell(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    sell_orders = [o for o in _sell_orders_list(rid, limit=8) if o["seller_id"] != uid]
    if not sell_orders:
        bot.answer_callback_query(call.id, "Нет чужих заявок", show_alert=True); return
    rows = []
    for o in sell_orders:
        total = o["amount"] * o["price_each"]
        rows.append([{
            "text": f"{o['amount']} шт. × {plain_balance(o['price_each'])} = {plain_balance(total)}  ({_mkt_user_name(o)})",
            "callback_data": f"mkt_buy_order:{o['id']}"
        }])
    rows.append([{"text": "◀️ Назад", "callback_data": f"mkt_buy_res:{rid}"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"<tg-emoji emoji-id=\"5456140674028019486\">⚡</tg-emoji> {_res_emoji(r)} <b>{r['name']}</b> — выбери заявку:",
            parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_order:"))
def cb_mkt_buy_order(call):
    uid      = call.from_user.id
    order_id = int(call.data.split(":", 1)[1])
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE id=?", (order_id,))
        row = cur.fetchone()
        order = dict(row) if row else None
    if not order:
        bot.answer_callback_query(call.id, "❌ Заявка уже закрыта", show_alert=True); return
    if order["seller_id"] == uid:
        bot.answer_callback_query(call.id, "❌ Нельзя купить у себя", show_alert=True); return
    r     = _res_by_id(order["resource_id"])
    total = order["amount"] * order["price_each"]
    _market_fsm[uid] = {"step": "buy_confirm", "order_id": order_id, "rid": order["resource_id"]}
    kb = {"inline_keyboard": [
        [{"text": f"Всё ({order['amount']} шт.) — {plain_balance(total)}", "callback_data": f"mkt_buy_exec:{order_id}:{order['amount']}"}],
        [{"text": "Указать количество", "callback_data": f"mkt_buy_partial:{order_id}"}],
        [{"text": "❌ Отмена", "callback_data": f"mkt_buy_res:{order['resource_id']}"}],
    ]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=(f"🛒 {_res_emoji(r)} <b>{r['name']}</b>\n\n"
                  f"Продавец: {_mkt_user_name_by_id(order['seller_id'])}\n"
                  f"Количество: <b>{order['amount']} шт.</b>\n"
                  f"Цена: <b>{plain_balance(order['price_each'])}/шт.</b>\n"
                  f"Итого: <b>{plain_balance(total)}</b>\n\nСколько берёшь?"),
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_partial:"))
def cb_mkt_buy_partial(call):
    uid      = call.from_user.id
    order_id = int(call.data.split(":", 1)[1])
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE id=?", (order_id,))
    ...
    if not order:
        bot.answer_callback_query(call.id, "❌ Заявка закрыта"); return
    _market_fsm[uid] = {
        "step": "buy_partial_amount",
        "order_id": order_id,
        "rid": order["resource_id"],
        "max_amt": order["amount"],
        "price_each": order["price_each"],
    }
    kb = {"inline_keyboard": [[{"text": "❌ Отмена", "callback_data": f"mkt_buy_res:{order['resource_id']}"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"Сколько штук купить?\nМакс. {order['amount']} шт. по {plain_balance(order['price_each'])}/шт.\nНапиши число:",
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_exec:"))
def cb_mkt_buy_exec(call):
    uid   = call.from_user.id
    parts = call.data.split(":")
    order_id = int(parts[1])
    amount   = int(parts[2])
    _execute_market_buy(call.message, uid, order_id, amount)
    bot.answer_callback_query(call.id)

def _execute_market_buy(message, uid, order_id, amount):
    """Атомарное исполнение сделки покупки."""
    _market_fsm.pop(uid, None)
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE id=?", (order_id,))
        row = cur.fetchone()
        order = dict(row) if row else None
    if not order:
        bot.send_message(message.chat.id, "❌ Заявка уже закрыта")
        return
    if order["seller_id"] == uid:
        bot.send_message(message.chat.id, "❌ Нельзя купить у себя")
        return
    if amount <= 0 or amount > order["amount"]:
        bot.send_message(message.chat.id, f"❌ От 1 до {order['amount']} шт.")
        return
    total    = amount * order["price_each"]
    balance  = get_balance(uid)
    if balance < total:
        bot.send_message(message.chat.id,
            f"❌ Недостаточно средств.\nНужно: {plain_balance(total)}, у тебя: {plain_balance(balance)}")
        return
    commission  = int(total * MARKET_COMMISSION)
    seller_gets = total - commission
    update_balance(uid, -total)
    update_balance(order["seller_id"], seller_gets)
    _inv_add(uid, order["resource_id"], amount)
    remaining = order["amount"] - amount
    with get_db_cursor() as cur:
        if remaining <= 0:
            cur.execute("DELETE FROM market_sell_orders WHERE id=?", (order_id,))
        else:
            cur.execute("UPDATE market_sell_orders SET amount=? WHERE id=?", (remaining, order_id))
    r = _res_by_id(order["resource_id"])
    kb = {"inline_keyboard": [[{"text": "🏪 На рынок", "callback_data": "mkt_main"}]]}
    bot.send_message(message.chat.id,
        f"✅ <b>Покупка совершена!</b>\n\n"
        f"{_res_emoji(r)} {r['name']} × {amount}\n"
        f"Заплачено: <b>{plain_balance(total)}</b>",
        parse_mode="HTML", reply_markup=kb)
    try:
        buyer_name = _mkt_user_name_by_id(uid)
        bot.send_message(order["seller_id"],
            f"🔔 <b>Твою заявку купили!</b>\n\n"
            f"{_res_emoji(r)} {r['name']} × {amount}\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_cancel:"))
def cb_mkt_buy_cancel(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    order = _buy_order_get(uid, rid)
    if not order:
        bot.answer_callback_query(call.id, "Заявка не найдена"); return
    update_balance(uid, order["frozen_gold"])
    with get_db_cursor() as cur:
        cur.execute("DELETE FROM market_buy_orders WHERE buyer_id=? AND resource_id=?", (uid, rid))
    bot.answer_callback_query(call.id, f"Заявка отменена, {plain_balance(order['frozen_gold'])} возвращены")
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_market_main_text(), parse_mode="HTML", reply_markup=_market_main_kb())

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_buy_start:"))
def cb_mkt_buy_start(call):
    uid = call.from_user.id
    rid = call.data.split(":", 1)[1]
    r   = _res_by_id(rid)
    if _buy_order_get(uid, rid):
        bot.answer_callback_query(call.id, "У тебя уже есть заявка на этот ресурс", show_alert=True); return
    _market_fsm[uid] = {"step": "buy_amount", "rid": rid}
    kb = {"inline_keyboard": [[{"text": "❌ Отмена", "callback_data": f"mkt_buy_res:{rid}"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"🛒 {_res_emoji(r)} <b>{r['name']}</b>\n\nСколько хочешь купить? Напиши число:",
            parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.message_handler(func=lambda m: m.text and m.from_user.id in _market_fsm and m.chat.type == "private")
def handle_market_fsm(message):
    uid  = message.from_user.id
    fsm  = _market_fsm.get(uid)
    if not fsm:
        return
    step = fsm["step"]
    ...
    if step == "sell_amount":
        try:
            ...
            _bad("❌ Введи целое число"); return
        rid     = fsm["rid"]
        r       = _res_by_id(rid)
        inv_amt = _inv_get(uid, rid)
        if amount <= 0 or amount > inv_amt:
            _bad(f"❌ От 1 до {inv_amt}"); return
        _market_fsm[uid]["step"]   = "sell_price"
        _market_fsm[uid]["amount"] = amount
        kb = {"inline_keyboard": [
            [{"text": f"По базовой ({plain_balance(r['price'])})", "callback_data": f"mkt_fsm_val:{r['price']}"}],
            [{"text": "❌ Отмена", "callback_data": "mkt_sell_pick"}],
        ]}
        bot.send_message(message.chat.id,
            f"📦 {_res_emoji(r)} <b>{r['name']}</b> × {amount}\n\nЗа сколько за штуку? (в 🌸)\nНапиши или выбери:",
            parse_mode="HTML", reply_markup=kb)
        ...
        try:
            ...
            _bad("❌ Введи целое число"); return
        if price <= 0:
            _bad("❌ Цена должна быть > 0"); return
        rid    = fsm["rid"]
        amount = fsm["amount"]
        r      = _res_by_id(rid)
        inv_amt = _inv_get(uid, rid)
        if inv_amt < amount:
            _bad("❌ Ресурс уже использован")
            _market_fsm.pop(uid, None); return
        _inv_add(uid, rid, -amount)
        with get_db_cursor() as cur:
            cur.execute("""
                INSERT INTO market_sell_orders (seller_id, resource_id, amount, price_each, created_at)
                VALUES (?,?,?,?,?)
            """, (uid, rid, amount, price, int(time.time())))
        _market_fsm.pop(uid, None)
        commission = int(amount * price * MARKET_COMMISSION)
        kb = {"inline_keyboard": [[{"text": "🏪 На рынок", "callback_data": "mkt_main"}]]}
        bot.send_message(message.chat.id,
            f"✅ <b>Заявка выставлена!</b>\n\n"
            f"{_res_emoji(r)} {r['name']} × {amount}\n"
            f"Цена: <b>{plain_balance(price)}/шт.</b>\n"
            f"Итого: <b>{plain_balance(amount * price)}</b>\n"
            f"Комиссия при продаже: <b>{plain_balance(commission)}</b>",
            parse_mode="HTML", reply_markup=kb)

    elif step == "buy_amount":
        try:
            amount = int(message.text.strip())
        except ValueError:
            _bad("❌ Введи целое число"); return
        if amount <= 0:
            _bad("❌ Количество должно быть > 0"); return
        _market_fsm[uid]["step"]   = "buy_price"
        _market_fsm[uid]["amount"] = amount
        rid = fsm["rid"]
        r   = _res_by_id(rid)
        kb = {"inline_keyboard": [
            [{"text": f"По базовой ({plain_balance(r['price'])})", "callback_data": f"mkt_fsm_val:{r['price']}"}],
            [{"text": "❌ Отмена", "callback_data": f"mkt_buy_res:{rid}"}],
        ]}
        bot.send_message(message.chat.id,
            f"🛒 {_res_emoji(r)} <b>{r['name']}</b> × {amount}\n\nПо какой цене за штуку? (в 🌸)",
            parse_mode="HTML", reply_markup=kb)
        ...
        balance = get_balance(uid)
        if balance < total:
            _bad(f"❌ Нужно заморозить {plain_balance(total)}, у тебя только {plain_balance(balance)}")
            _market_fsm.pop(uid, None); return
        update_balance(uid, -total)
        with get_db_cursor() as cur:
            cur.execute("""
                INSERT INTO market_buy_orders (buyer_id, resource_id, amount, price_each, frozen_gold, created_at)
                VALUES (?,?,?,?,?,?)
            """, (uid, rid, amount, price, total, int(time.time())))
        _market_fsm.pop(uid, None)
        kb = {"inline_keyboard": [[{"text": "🏪 На рынок", "callback_data": "mkt_main"}]]}
        bot.send_message(message.chat.id,
            f"✅ <b>Заявка на покупку выставлена!</b>\n\n"
            f"{_res_emoji(r)} {r['name']} × {amount}\n"
            f"Цена: <b>{plain_balance(price)}/шт.</b>\n"
            f"Заморожено: <b>{plain_balance(total)}</b>",
            parse_mode="HTML", reply_markup=kb)

    elif step == "buy_partial_amount":
        try:
            amount = int(message.text.strip())
        except ValueError:
            _bad("❌ Введи целое число"); return
        ...
        if amount <= 0 or amount > max_amt:
            _bad(f"❌ От 1 до {max_amt}"); return
        _execute_market_buy(message, uid, order_id, amount)

    elif step == "sell_to_order_amount":
        try:
            amount = int(message.text.strip())
        except ValueError:
            _bad("❌ Введи целое число"); return
        ...
        if amount <= 0 or amount > max_amt:
            _bad(f"❌ От 1 до {max_amt}"); return
        _execute_market_sell(message, uid, order_id, amount)

@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_fsm_val:"))
def cb_mkt_fsm_val(call):
    uid = call.from_user.id
    fsm = _market_fsm.get(uid)
    if not fsm:
        bot.answer_callback_query(call.id, "❌ Сессия истекла"); return
    ...
    class _FakeMsg:
        def __init__(self):
            ...
            self.text = str(val)
    handle_market_fsm(_FakeMsg())

@bot.callback_query_handler(func=lambda c: c.data == "mkt_my_orders")
def cb_mkt_my_orders(call):
    uid = call.from_user.id
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM market_sell_orders WHERE seller_id=?", (uid,))
        sell = [dict(r) for r in cur.fetchall()]
        cur.execute("SELECT * FROM market_buy_orders WHERE buyer_id=?", (uid,))
        buy  = [dict(r) for r in cur.fetchall()]
    text = "📋 <b>Мои заявки</b>\n\n"
    if sell:
        text += "📦 <b>Продаю:</b>\n"
        for o in sell:
            r = _res_by_id(o["resource_id"])
            text += f"  {_res_emoji(r)} {r['name']} × {o['amount']} по {plain_balance(o['price_each'])}\n"
    else:
        text += "📦 Нет заявок на продажу\n"
    text += "\n"
    if buy:
        text += "🛒 <b>Покупаю:</b>\n"
        for o in buy:
            r = _res_by_id(o["resource_id"])
            text += f"  {_res_emoji(r)} {r['name']} × {o['amount']} по {plain_balance(o['price_each'])}  заморожено: {plain_balance(o['frozen_gold'])}\n"
    else:
        text += "🛒 Нет заявок на покупку\n"
    rows = []
    for o in sell:
        r = _res_by_id(o["resource_id"])
        rows.append([{"text": f"❌ Снять продажу {r['name']}", "callback_data": f"mkt_sell_cancel:{o['resource_id']}"}])
    for o in buy:
        r = _res_by_id(o["resource_id"])
        rows.append([{"text": f"❌ Снять покупку {r['name']}", "callback_data": f"mkt_buy_cancel:{o['resource_id']}"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_main"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup={"inline_keyboard": rows})
...
@bot.callback_query_handler(func=lambda c: c.data == "mkt_browse_res")
def cb_mkt_browse_res(call):
    pair = []
    rows = []
    for r in MINE_RESOURCES:
        with get_db_cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM market_sell_orders WHERE resource_id=?", (r["id"],))
            sc = cur.fetchone()[0]
            cur.execute("SELECT COUNT(*) FROM market_buy_orders WHERE resource_id=?", (r["id"],))
            bc = cur.fetchone()[0]
        if sc or bc:
            icon = r.get("fallback", "📦")
            pair.append({"text": f"{icon} {r['name']}  📦{sc} 🛒{bc}",
                         "callback_data": f"mkt_browse_detail:{r['id']}"})
            if len(pair) == 2:
                rows.append(pair)
                pair = []
    if pair:
        rows.append(pair)
    if not rows:
        rows.append([{"text": "Заявок нет", "callback_data": "mkt_main"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_main"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text="🔍 <b>Все заявки</b>\n\nВыбери ресурс:",
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("mkt_browse_detail:"))
def cb_mkt_browse_detail(call):
    rid  = call.data.split(":", 1)[1]
    r    = _res_by_id(rid)
    sell = _sell_orders_list(rid)
    buy  = _buy_orders_list(rid)
    text = f"🔍 {_res_emoji(r)} <b>{r['name']}</b>\n\n"
    if sell:
        text += "📦 <b>Продают:</b>\n"
        for o in sell:
    ...
    if buy:
        ...
        text += "🛒 Заявок на покупку нет\n"
    rows = []
    if any(True for o in sell if o.get("seller_id") != call.from_user.id):
        rows.append([{"text": "⚡ Купить сейчас", "callback_data": f"mkt_buy_from_sell:{rid}"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "mkt_browse_res"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup={"inline_keyboard": rows})
    bot.answer_callback_query(call.id)
...
EQUIPMENT_SETS = {
    "iron": {
        ...
        "emoji": "⚙️",
        "slots": {
            "helmet":     {"emoji_id": 6280459251634672193, "fallback": "🪖"},
            "chestplate": {"emoji_id": 6280373850504958136, "fallback": "🦺"},
            "leggings":   {"emoji_id": 6280310409543028056, "fallback": "👖"},
            "boots":      {"emoji_id": 6278320826072699252, "fallback": "👟"},
            "sword":      {"emoji_id": 6278389670103491005, "fallback": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"},
        },
        # damage = base урон меча (±3 рандом в бою, как в MC)
        # ap = armor points за ОДИН слот (helmet/chest/legs/boots) — точные значения MC
        # toughness = toughness за ОДИН слот (diamond=2, netherite=3, остальные=0)
        "stats": {"damage": 15, "ap_per_slot": {"helmet":2,"chestplate":6,"leggings":5,"boots":2}, "toughness_per_slot": 0},
    },
    "gold": {
        "name": "Золотой",
        "resource": "gold_ore",
        "emoji": "🟡",
        "slots": {
            "helmet":     {"emoji_id": 6280607844618211753, "fallback": "🪖"},
            "chestplate": {"emoji_id": 6278429501630190066, "fallback": "🦺"},
            "leggings":   {"emoji_id": 6278403701761642906, "fallback": "👖"},
            "boots":      {"emoji_id": 6278210638686720158, "fallback": "👟"},
            "sword":      {"emoji_id": 6280626858438432759, "fallback": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"},
        },
        "stats": {"damage": 14, "ap_per_slot": {"helmet":2,"chestplate":5,"leggings":3,"boots":1}, "toughness_per_slot": 0},
    },
    "diamond": {
        "name": "Алмазный",
        "resource": "diamond",
        "emoji": "💎",
        "slots": {
            "helmet":     {"emoji_id": 6278271090351411403, "fallback": "🪖"},
            "chestplate": {"emoji_id": 6278550001232647489, "fallback": "🦺"},
            "leggings":   {"emoji_id": 6278091698157392724, "fallback": "👖"},
            "boots":      {"emoji_id": 6278020947161124766, "fallback": "👟"},
            "sword":      {"emoji_id": 6278347003898368427, "fallback": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"},
        },
        "stats": {"damage": 18, "ap_per_slot": {"helmet":3,"chestplate":8,"leggings":6,"boots":3}, "toughness_per_slot": 2},
    },
    "netherite": {
        "name": "Незеритовый",
        "resource": "netherite",
        "emoji": "🌑",
        "slots": {
            "helmet":     {"emoji_id": 6278466403989197908, "fallback": "🪖"},
            "chestplate": {"emoji_id": 6280757262235471883, "fallback": "🦺"},
            "leggings":   {"emoji_id": 6278543837954577099, "fallback": "👖"},
            "boots":      {"emoji_id": 6278134321412838039, "fallback": "👟"},
            "sword":      {"emoji_id": 6278398831268729149, "fallback": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"},
        },
        "stats": {"damage": 22, "ap_per_slot": {"helmet":3,"chestplate":8,"leggings":6,"boots":3}, "toughness_per_slot": 3},
    },
}
...
def _eq_emoji(set_id, slot):
    s = EQUIPMENT_SETS.get(set_id, {}).get("slots", {}).get(slot)
    if not s:
        return {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖",
                "boots": "👟", "sword": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"}.get(slot, "❓")
    return f'<tg-emoji emoji-id="{s["emoji_id"]}">{s["fallback"]}</tg-emoji>'
...
def _eq_init():
    with get_db_cursor() as cur:
        ...
        """)
        # Хранит прочность предметов в инвентаре (как в Minecraft — каждый предмет со своей прочностью)
        cur.execute("""
            CREATE TABLE IF NOT EXISTS eq_stored_durability (
                user_id    INTEGER NOT NULL,
                set_id     TEXT    NOT NULL,
                slot       TEXT    NOT NULL,
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS pvp_battles (
                ...
                attacker_blocked INTEGER DEFAULT 0,
                defender_blocked INTEGER DEFAULT 0,
                chat_id      INTEGER NOT NULL,
                message_id   INTEGER,
                created_at   INTEGER NOT NULL,
...
def _pvp_migrate():
    with get_db_cursor() as cur:
        for col, typ in [
            ("attacker_dm_msg_id",    "INTEGER DEFAULT 0"),
            ("defender_dm_msg_id",    "INTEGER DEFAULT 0"),
            ("attacker_guard",        "TEXT DEFAULT ''"),
            ("defender_guard",        "TEXT DEFAULT ''"),
            ("attacker_pending_guard","TEXT DEFAULT ''"),
            ("defender_pending_guard","TEXT DEFAULT ''"),
        ]:
            try:
                cur.execute(f"ALTER TABLE pvp_battles ADD COLUMN {col} {typ}")
            except Exception:
                pass
_pvp_migrate()

def _eq_get_all(user_id):
    """Возвращает dict slot -> set_id для игрока."""
    with get_db_cursor() as cur:
        cur.execute("SELECT slot, set_id FROM equipment WHERE user_id=?", (user_id,))
        return {row[0]: row[1] for row in cur.fetchall()}

def _eq_set(user_id, slot, set_id):
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO equipment (user_id, slot, set_id) VALUES (?,?,?)
            ON CONFLICT(user_id, slot) DO UPDATE SET set_id=?
        """, (user_id, slot, set_id, set_id))

def _eq_remove(user_id, slot):
    with get_db_cursor() as cur:
        cur.execute("DELETE FROM equipment WHERE user_id=? AND slot=?", (user_id, slot))

def _eq_get_durability(user_id, slot):
    with get_db_cursor() as cur:
        cur.execute("SELECT durability FROM equipment WHERE user_id=? AND slot=?", (user_id, slot))
        row = cur.fetchone()
        return row[0] if row else 0

def _eq_damage(user_id, slot, amount=1):
    with get_db_cursor() as cur:
        cur.execute("SELECT set_id, durability FROM equipment WHERE user_id=? AND slot=?", (user_id, slot))
        row = cur.fetchone()
        if not row:
            return None
        set_id, dur = row
        ...
        if new_dur <= 0:
            cur.execute("DELETE FROM equipment WHERE user_id=? AND slot=?", (user_id, slot))
            return "broken"
        cur.execute("UPDATE equipment SET durability=? WHERE user_id=? AND slot=?", (new_dur, user_id, slot))
        return new_dur

def _eq_set_with_durability(user_id, slot, set_id):
    """Надеть предмет: восстанавливает сохранённую прочность из инвентаря (как в Minecraft)."""
    full_dur = DURABILITY.get(set_id, 60)
    with get_db_cursor() as cur:
        # Проверяем, есть ли сохранённая прочность для этого предмета в инвентаре
        cur.execute(
            "SELECT durability FROM eq_stored_durability WHERE user_id=? AND set_id=? AND slot=?",
            (user_id, set_id, slot)
        )
        row = cur.fetchone()
        restore_dur = row[0] if row else full_dur
        # Удаляем запись о сохранённой прочности (предмет теперь надет)
        cur.execute(
            "DELETE FROM eq_stored_durability WHERE user_id=? AND set_id=? AND slot=?",
            (user_id, set_id, slot)
        )
        cur.execute(
            "INSERT INTO equipment (user_id, slot, set_id, durability) VALUES (?,?,?,?)"
            " ON CONFLICT(user_id, slot) DO UPDATE SET set_id=excluded.set_id, durability=excluded.durability",
            (user_id, slot, set_id, restore_dur)
        )

def _eq_migrate_durability():
    with get_db_cursor() as cur:
        try:
            cur.execute("ALTER TABLE equipment ADD COLUMN durability INTEGER NOT NULL DEFAULT 100")
        except Exception:
            pass
        cur.execute("SELECT user_id, slot, set_id, durability FROM equipment WHERE durability=100")
        rows = cur.fetchall()
        for uid, slot, set_id, dur in rows:
            full = DURABILITY.get(set_id, 60)
            cur.execute("UPDATE equipment SET durability=? WHERE user_id=? AND slot=?", (full, uid, slot))

_eq_migrate_durability()

def _eq_calc_stats(user_id):
    """Считает боевые характеристики по системе Minecraft (AP + toughness)."""
    equipped = _eq_get_all(user_id)
    base_dmg = 0
    total_ap = 0
    total_toughness = 0

    for slot, set_id in equipped.items():
        s = EQUIPMENT_SETS.get(set_id)
        if not s:
            continue
        stats = s["stats"]
        if slot == "sword":
            ...
            # Суммируем AP конкретного слота
            ap_map = stats.get("ap_per_slot", {})
            total_ap += ap_map.get(slot, 0)
            total_toughness += stats.get("toughness_per_slot", 0)

    # Для отображения: конвертируем AP в % (каждый AP = 4%, max 80%)
    display_def = min(int(total_ap * 4), 80)

    return {
        "damage": base_dmg,
        "defense": display_def,   # только для показа в интерфейсе
        "total_ap": total_ap,
        "total_toughness": total_toughness,
        "equipped": equipped,
    }

def _eq_main_text(user_id):
    equipped = _eq_get_all(user_id)
    stats = _eq_calc_stats(user_id)
    slot_order = ["helmet", "chestplate", "leggings", "boots", "sword"]
    fallbacks = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"}

    lines_out = ["🛡️ <b>Снаряжение</b>\n"]
    for slot in slot_order:
        set_id = equipped.get(slot)
        if set_id:
            s = EQUIPMENT_SETS[set_id]
            sl = s["slots"].get(slot, {})
            em_id = sl.get("emoji_id")
            fb = sl.get("fallback", fallbacks[slot])
            if em_id:
                tge = f'<tg-emoji emoji-id="{em_id}">{fb}</tg-emoji>'
            else:
                tge = fb
            dur = _eq_get_durability(user_id, slot)
            max_dur = DURABILITY.get(set_id, 60)
            dur_pct = dur / max_dur
            dur_icon = "🟢" if dur_pct > 0.5 else ("🟡" if dur_pct > 0.2 else "🔴")
            lines_out.append(f"{tge} <b>{SLOT_NAMES[slot]}</b>  <i>{s['name']}</i>  {dur_icon}{dur}/{max_dur}")
        else:
            lines_out.append(f"{fallbacks[slot]} <b>{SLOT_NAMES[slot]}</b>  <i>не надет</i>")

    dmg = stats["damage"]
    dfn = stats["defense"]
    lines_out.append(f"\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{dmg}</b> атк   🛡️ <b>{dfn}</b> защ")

    if len(equipped) == 5 and len(set(equipped.values())) == 1:
        sid = list(equipped.values())[0]
        lines_out.append(f"\n✨ <b>Полный {EQUIPMENT_SETS[sid]['name']} сет</b> · +15% ко всему")

    return "\n".join(lines_out)

def _eq_main_kb(user_id):
    equipped = _eq_get_all(user_id)
    slot_order = ["helmet", "chestplate", "leggings", "boots", "sword"]
    slot_names = {"helmet": "Шлем", "chestplate": "Нагрудник", "leggings": "Поножи", "boots": "Сапоги", "sword": "Меч"}
    slot_fallbacks = {
        "helmet": "🪖", "chestplate": "🦺", "leggings": "👖",
        "boots": "👟", "sword": "⚔️"
    }
    # emoji_id дефолтного (железного) сета для каждого слота
    default_emoji_ids = {
        "helmet":     EQUIPMENT_SETS["iron"]["slots"]["helmet"]["emoji_id"],
        "chestplate": EQUIPMENT_SETS["iron"]["slots"]["chestplate"]["emoji_id"],
        "leggings":   EQUIPMENT_SETS["iron"]["slots"]["leggings"]["emoji_id"],
        "boots":      EQUIPMENT_SETS["iron"]["slots"]["boots"]["emoji_id"],
        "sword":      EQUIPMENT_SETS["iron"]["slots"]["sword"]["emoji_id"],
    }
    rows = []
    row = []
    for slot in slot_order:
        set_id = equipped.get(slot)
        fb = slot_fallbacks[slot]
        # Берём emoji_id надетого сета или дефолт железного
        if set_id and set_id in EQUIPMENT_SETS:
            eid = EQUIPMENT_SETS[set_id]["slots"][slot]["emoji_id"]
            fb  = EQUIPMENT_SETS[set_id]["slots"][slot].get("fallback", fb)
            # fallback может быть строкой с tg-emoji — берём чистый символ
            if "<tg-emoji" in str(fb):
                fb = "⚔️"
        else:
            eid = default_emoji_ids[slot]
        tge_tag = f'<tg-emoji emoji-id="{eid}">{fb}</tg-emoji>'
        prefix = "✅" if set_id else "◻️"
        btn_text = f"{prefix} {tge_tag} {slot_names[slot]}"
        row.append(_btn(btn_text, callback_data=f"eq_slot_{slot}"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    rows.append([{"text": "🔨 Крафт снаряжения", "callback_data": "eq_craft_menu"}])
    return {"inline_keyboard": rows}

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("снаряжение", "🛡️ снаряжение", "экипировка"))
def handle_eq_menu(message):
    uid = message.from_user.id
    if not is_registered(uid):
        return
    _tg_api("sendMessage", chat_id=message.chat.id,
            text=_eq_main_text(uid),
            reply_markup=_eq_main_kb(uid),
            parse_mode="HTML")

@bot.callback_query_handler(func=lambda c: c.data == "eq_back")
def cb_eq_back(call):
    uid = call.from_user.id
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("eq_slot_"))
def cb_eq_slot(call):
    uid = call.from_user.id
    slot = call.data[len("eq_slot_"):]
    if slot not in SLOT_NAMES:
        bot.answer_callback_query(call.id, "❌")
        return
    equipped = _eq_get_all(uid)
    set_id = equipped.get(slot)
    slot_label = SLOT_NAMES[slot]

    if set_id:
        s = EQUIPMENT_SETS[set_id]
        emoji = _eq_emoji(set_id, slot)
        if slot == "sword":
            slot_dmg = s["stats"]["damage"]
            stat_line = f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> +{slot_dmg} урон"
        else:
            ap = s["stats"].get("ap_per_slot", {}).get(slot, 0)
            slot_def = ap * 4  # 1 AP = 4% защиты (как в MC)
            stat_line = f"🛡️ +{slot_def}% защита"
        text = (
            f"{emoji}  <b>{SLOT_NAMES[slot]}</b>\n\n"
            f"  Материал:  {s['emoji']} <b>{s['name']}</b>\n"
            f"  Бонус:     {stat_line}\n\n"
            f"Заменить или снять?"
        )
        kb = {"inline_keyboard": [
            [{"text": "🔄 Заменить", "callback_data": f"eq_pick_set_{slot}"},
             {"text": "❌ Снять",    "callback_data": f"eq_unequip_{slot}"}],
            [{"text": "◀️ Назад",   "callback_data": "eq_back"}],
        ]}
    else:
        fallback = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖",
                   "boots": "👟", "sword": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"}[slot]
        text = (
            f"{fallback}  <b>{SLOT_NAMES[slot]}</b>\n\n"
            f"  <i>Слот пуст.</i>\n\n"
            f"Выбери материал:"
        )
        kb = {"inline_keyboard": _eq_set_pick_rows(slot, uid) + [[{"text": "◀️ Назад", "callback_data": "eq_back"}]]}
...
def _eq_set_pick_rows(slot, user_id=None):
    """Список кнопок для выбора сета под слот с индикатором наличия."""
    rows = []
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        if user_id is not None:
            owned = _inv_get_eq(user_id, set_id, slot)
            status = "✅" if owned > 0 else "❌"
        else:
            status = ""
        btn = {
            "text": f"{status} {s['emoji']} {s['name']}",
            "callback_data": f"eq_equip_{slot}_{set_id}"
        }
        row.append(btn)
        if len(row) == 2:
            rows.append(row)
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("eq_pick_set_"))
def cb_eq_pick_set(call):
    uid = call.from_user.id
    slot = call.data[len("eq_pick_set_"):]
    kb = {"inline_keyboard": _eq_set_pick_rows(slot, uid) + [[{"text": "◀️ Назад", "callback_data": f"eq_slot_{slot}"}]]}
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"🔄 Выбери сет для <b>{SLOT_NAMES[slot]}</b>:",
            reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("eq_equip_"))
def cb_eq_equip(call):
    uid = call.from_user.id
    parts = call.data[len("eq_equip_"):].split("_", 1)
    if len(parts) != 2:
        bot.answer_callback_query(call.id, "❌")
        return
    slot, set_id = parts[0], parts[1]
    if slot not in SLOT_NAMES or set_id not in EQUIPMENT_SETS:
        bot.answer_callback_query(call.id, "❌")
        return

    inv = _inv_get_eq(uid, set_id, slot)
    if inv < 1:
        bot.answer_callback_query(call.id, "❌ Предмет не скрафчен!", show_alert=True)
        return

    old_equipped = _eq_get_all(uid)
    if old_equipped.get(slot):
        old_set_id = old_equipped[slot]
        # Сохраняем прочность снимаемого предмета (как в Minecraft — предмет помнит свою прочность)
        old_dur = _eq_get_durability(uid, slot)
        with get_db_cursor() as cur:
            cur.execute(
                "INSERT INTO eq_stored_durability (user_id, set_id, slot, durability) VALUES (?,?,?,?)"
                " ON CONFLICT(user_id, set_id, slot) DO UPDATE SET durability=excluded.durability",
                (uid, old_set_id, slot, old_dur)
            )
        _inv_add_eq(uid, old_set_id, slot, 1)

    _eq_set_with_durability(uid, slot, set_id)
    _inv_add_eq(uid, set_id, slot, -1)

    s = EQUIPMENT_SETS[set_id]
    bot.answer_callback_query(call.id, f"✅ {s['name']} {SLOT_NAMES[slot]} надет!", show_alert=False)
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=_eq_main_text(uid),
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("eq_unequip_"))
def cb_eq_unequip(call):
    uid = call.from_user.id
    slot = call.data[len("eq_unequip_"):]
    equipped = _eq_get_all(uid)
    set_id = equipped.get(slot)
    if not set_id:
        bot.answer_callback_query(call.id, "Слот уже пуст")
        return
    # Сохраняем текущую прочность предмета перед возвратом в инвентарь
    cur_dur = _eq_get_durability(uid, slot)
    with get_db_cursor() as cur:
        cur.execute(
            "INSERT INTO eq_stored_durability (user_id, set_id, slot, durability) VALUES (?,?,?,?)"
            " ON CONFLICT(user_id, set_id, slot) DO UPDATE SET durability=excluded.durability",
            (uid, set_id, slot, cur_dur)
        )
    _inv_add_eq(uid, set_id, slot, 1)
    _eq_remove(uid, slot)
    bot.answer_callback_query(call.id, f"✅ {SLOT_NAMES[slot]} снят")
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=_eq_main_text(uid),
...
def _eq_item_key(set_id, slot):
    return f"eq_{set_id}_{slot}"

def _inv_get_eq(user_id, set_id, slot):
    return _inv_get(user_id, _eq_item_key(set_id, slot))

def _inv_add_eq(user_id, set_id, slot, amount):
    _inv_add(user_id, _eq_item_key(set_id, slot), amount)

def _craft_menu_text(uid):
    lines_out = ["╔══ 🔨 <b>КРАФТ СНАРЯЖЕНИЯ</b> ══╗\n",
                 "Выбери материал для крафта:\n"]
    for set_id, s in EQUIPMENT_SETS.items():
        res = _res_by_id(s["resource"])
        inv_count = _inv_get(uid, s["resource"]) if res else 0
        res_emoji = _res_emoji(res) if res else s["emoji"]
        total_cost = sum(CRAFT_COST.values())
        inv_bar = min(inv_count, total_cost)
        lines_out.append(
            f"  {s['emoji']} <b>{s['name']}</b>\n"
            f"      {res_emoji} В инвентаре: <b>{inv_count}</b>  (полный сет: {total_cost})"
        )
    lines_out.append("\n╚══════════════════════╝")
    return "\n".join(lines_out)
...
def _craft_menu_kb():
    ...
    row = []
    for set_id, s in EQUIPMENT_SETS.items():
        btn = {"text": f"{s['emoji']} {s['name']}", "callback_data": f"craft_set_{set_id}"}
        row.append(btn)
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    rows.append([{"text": "◀️ Назад", "callback_data": "eq_back"}])
    return {"inline_keyboard": rows}

@bot.callback_query_handler(func=lambda c: c.data == "eq_craft_menu")
def cb_craft_menu(call):
    uid = call.from_user.id
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=_craft_menu_text(uid),
            reply_markup=_craft_menu_kb(),
            parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("craft_set_"))
def cb_craft_set(call):
    uid = call.from_user.id
    set_id = call.data[len("craft_set_"):]
    ...
    lines_out = [
        f"╔══ {s['emoji']} <b>{s['name'].upper()} СЕТ</b> ══╗\n",
        f"  {res_emoji} Ресурс: <b>{res['name'] if res else s['resource']}</b>",
        f"  📦 В инвентаре: <b>{inv_count}</b>\n",
        "  <b>Предметы:</b>"
    ]

    rows = []
    for slot, cost in CRAFT_COST.items():
        in_inv = _inv_get_eq(uid, set_id, slot)
        equipped_sets = _eq_get_all(uid)
        is_eq = equipped_sets.get(slot) == set_id
        emoji = _eq_emoji(set_id, slot)
        has_enough = inv_count >= cost
        if is_eq:
            status = "✅ надет"
        elif in_inv > 0:
            status = f"📦 ×{in_inv} в инвентаре"
        elif has_enough:
            status = f"🔨 можно крафтить"
        else:
            status = f"❌ мало ({inv_count}/{cost})"
        lines_out.append(f"  {emoji} <b>{SLOT_NAMES[slot]}</b> — {cost} ед.   {status}")
        slot_eid = EQUIPMENT_SETS[set_id]["slots"][slot]["emoji_id"]
        slot_fb2 = {"helmet":"🪖","chestplate":"🦺","leggings":"👖","boots":"👟","sword":"⚔️"}.get(slot,"⚔️")
        btn_text = f'<tg-emoji emoji-id="{slot_eid}">{slot_fb2}</tg-emoji> {SLOT_NAMES[slot]}  ({cost} ед.)'
        rows.append([_btn(btn_text, callback_data=f"craft_item_{set_id}_{slot}")])

    lines_out.append("\n╚══════════════════════╝")

    rows.append([{"text": "◀️ Назад", "callback_data": "eq_craft_menu"}])
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="\n".join(lines_out),
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("craft_item_"))
def cb_craft_item(call):
    uid = call.from_user.id
    parts = call.data[len("craft_item_"):].split("_", 1)
    if len(parts) != 2:
        bot.answer_callback_query(call.id, "❌")
        return
    set_id, slot = parts[0], parts[1]
    if set_id not in EQUIPMENT_SETS or slot not in CRAFT_COST:
        bot.answer_callback_query(call.id, "❌")
        return

    s = EQUIPMENT_SETS[set_id]
    cost = CRAFT_COST[slot]
    inv_count = _inv_get(uid, s["resource"])

    if inv_count < cost:
        res = _res_by_id(s["resource"])
        res_name = res["name"] if res else s["resource"]
        bot.answer_callback_query(
            call.id,
            f"❌ Недостаточно! Нужно {cost} {res_name}, у тебя {inv_count}",
            show_alert=True
        )
        return

    _inv_add(uid, s["resource"], -cost)
    _inv_add_eq(uid, set_id, slot, 1)

    emoji = _eq_emoji(set_id, slot)
    bot.answer_callback_query(
        call.id,
        f"✅ {emoji} {s['name']} {SLOT_NAMES[slot]} скрафчен!",
        show_alert=True
    )
    cb_craft_set(call)
...
PVP_BASE_DMG_MIN = 10
PVP_BASE_DMG_MAX = 20
PVP_BLOCK_REDUCTION = 0.50
PVP_TURN_TIMEOUT = 60
PVP_MAX_HP = 100

BODY_PARTS = {
    "head":  {"name": "Голова",  "emoji": "🪖", "dmg_mult": 1.5, "armor_slot": "helmet"},
    "body":  {"name": "Корпус",  "emoji": "🦺", "dmg_mult": 1.0, "armor_slot": "chestplate"},
    "legs":  {"name": "Ноги",    "emoji": "👖", "dmg_mult": 0.8, "armor_slot": "leggings"},
}
PVP_BODYBLOCK_REDUCTION = 0.80

_pvp_timeout_timers = {}

def _pvp_get(battle_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM pvp_battles WHERE id=?", (battle_id,))
        row = cur.fetchone()
        return dict(row) if row else None

def _pvp_active_for(user_id):
    with get_db_cursor() as cur:
        cur.execute("""
...
def _pvp_create(attacker_id, defender_id, bet, chat_id):
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO pvp_battles
            (attacker_id, defender_id, bet, status,
             attacker_hp, defender_hp, whose_turn,
             attacker_blocked, defender_blocked,
             chat_id, created_at, last_action,
             attacker_dm_msg_id, defender_dm_msg_id,
             attacker_guard, defender_guard)
            VALUES (?,?,?,'pending',100,100,?,0,0,?,?,?,0,0,'','')
        """, (attacker_id, defender_id, bet, attacker_id, chat_id, now, now))
        return cur.lastrowid

def _pvp_set_msg(battle_id, message_id):
    with get_db_cursor() as cur:
        cur.execute("UPDATE pvp_battles SET message_id=? WHERE id=?", (message_id, battle_id))

def _pvp_update(battle_id, **kwargs):
    if not kwargs:
        return
    sets = ", ".join(f"{k}=?" for k in kwargs)
    vals = list(kwargs.values()) + [battle_id]
    with get_db_cursor() as cur:
        cur.execute(f"UPDATE pvp_battles SET {sets} WHERE id=?", vals)

def _pvp_record_stat(winner_id, loser_id):
    with get_db_cursor() as cur:
        for uid, col in [(winner_id, "wins"), (loser_id, "losses")]:
            cur.execute("""
                INSERT INTO pvp_stats (user_id, wins, losses) VALUES (?,0,0)
                ON CONFLICT(user_id) DO NOTHING
            """, (uid,))
            cur.execute(f"UPDATE pvp_stats SET {col}={col}+1 WHERE user_id=?", (uid,))

def _pvp_get_stats(user_id):
    with get_db_cursor() as cur:
        cur.execute("SELECT wins, losses FROM pvp_stats WHERE user_id=?", (user_id,))
        row = cur.fetchone()
        return (row[0], row[1]) if row else (0, 0)

def _pvp_hp_bar(hp, max_hp=100):
    filled = round(hp / max_hp * 8)
    filled = max(0, min(8, filled))
    empty = 8 - filled
    bar = "█" * filled + "░" * empty
    pct = int(hp / max_hp * 100)
    return f"<code>[{bar}]</code> {hp}"

def _pvp_eq_line(user_id):
    """Строка экипировки игрока с tg-emoji для текста сообщения."""
    equipped = _eq_get_all(user_id)
    slot_order = ["helmet", "chestplate", "leggings", "boots", "sword"]
    fallbacks = {"helmet": "🪖", "chestplate": "🦺", "leggings": "👖", "boots": "👟", "sword": "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"}
    parts = []
    for slot in slot_order:
        set_id = equipped.get(slot)
        if set_id:
            sl = EQUIPMENT_SETS[set_id]["slots"].get(slot, {})
            em_id = sl.get("emoji_id")
            fb = sl.get("fallback", fallbacks[slot])
            parts.append(f'<tg-emoji emoji-id="{em_id}">{fb}</tg-emoji>' if em_id else fb)
        else:
            parts.append(f'<i>{fallbacks[slot]}</i>')
    return " ".join(parts)

def _pvp_battle_text(b, last_event=None, viewer_id=None):
    a_id = b["attacker_id"]
    d_id = b["defender_id"]
    a_name = _mkt_user_name_by_id(a_id)
    d_name = _mkt_user_name_by_id(d_id)
    a_stats = _eq_calc_stats(a_id)
    d_stats = _eq_calc_stats(d_id)
    a_hp = b["attacker_hp"]
    d_hp = b["defender_hp"]

    def self_guard_hint(uid):
        is_att = (uid == a_id)
        pg = b.get("attacker_pending_guard" if is_att else "defender_pending_guard", "")
        g  = b.get("attacker_guard"         if is_att else "defender_guard",         "")
        part = pg or g
        if part and part in BODY_PARTS:
            return f"  🛡 {BODY_PARTS[part]['name']}"
        return ""

    a_hint = self_guard_hint(a_id) if viewer_id == a_id else ""
    d_hint = self_guard_hint(d_id) if viewer_id == d_id else ""

    lines_out = [
        "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>БОЙ</b>",
        "",
        f"🔴 <b>{a_name}</b>{a_hint}",
        _pvp_eq_line(a_id),
        _pvp_hp_bar(a_hp),
        f"<i>{a_stats['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {a_stats['defense']} 🛡</i>",
        "",
        f"🔵 <b>{d_name}</b>{d_hint}",
        _pvp_eq_line(d_id),
        _pvp_hp_bar(d_hp),
        f"<i>{d_stats['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {d_stats['defense']} 🛡</i>",
    ]
    if b["bet"] > 0:
        lines_out.append(f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> {format_balance(b['bet'] * 2)}")
    if last_event:
        lines_out.append(f"\n{last_event}")
    if b["status"] == "active":
        if viewer_id:
            is_att = (viewer_id == a_id)
            pg = b.get("attacker_pending_guard" if is_att else "defender_pending_guard", "")
            if viewer_id == b["whose_turn"]:
                lines_out.append("\n🛡 Защита выбрана — куда бьёшь?" if pg else "\n🛡 Выбери защиту")
            else:
                lines_out.append("\n⏳ Ход соперника...")
        else:
            turn_name = a_name if b["whose_turn"] == a_id else d_name
            lines_out.append(f"\n▸ Ход <b>{turn_name}</b>")
    return "\n".join(lines_out)

def _pvp_slot_fallback(equipped, part_id):
    """Unicode-fallback иконка из снаряжения (для текста кнопок)."""
    slot = {"head": "helmet", "body": "chestplate", "legs": "leggings"}[part_id]
    set_id = equipped.get(slot)
    if set_id:
        s = EQUIPMENT_SETS[set_id]["slots"].get(slot)
        if s:
            return s["fallback"]
    return BODY_PARTS[part_id]["emoji"]

def _pvp_sword_fallback(equipped):
    set_id = equipped.get("sword")
    if set_id:
        s = EQUIPMENT_SETS[set_id]["slots"].get("sword")
        if s:
            return s["fallback"]
    return "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>"

def _pvp_guard_kb(battle_id, user_id):
    eq = _eq_get_all(user_id)
    row = []
    for part_id in ["head", "body", "legs"]:
        icon = _pvp_slot_fallback(eq, part_id)
        name = BODY_PARTS[part_id]["name"]
        row.append({"text": f"{icon} {name}", "callback_data": f"pvp_action_{battle_id}_{user_id}_guard_{part_id}"})
    return {
        "inline_keyboard": [
            [{"text": "🛡 Что защищаешь?", "callback_data": "pvp_noop"}],
            row,
        ]
    }

def _pvp_atk_kb(battle_id, user_id, opponent_id):
    eq_opp = _eq_get_all(opponent_id)
    eq_self = _eq_get_all(user_id)
    sword = _pvp_sword_fallback(eq_self)
    row = []
    for part_id in ["head", "body", "legs"]:
        icon = _pvp_slot_fallback(eq_opp, part_id)
        name = BODY_PARTS[part_id]["name"]
        row.append({"text": f"{icon} {name}", "callback_data": f"pvp_action_{battle_id}_{user_id}_atk_{part_id}"})
    return {
        "inline_keyboard": [
            [{"text": f"{sword} Куда бьёшь?", "callback_data": "pvp_noop"}],
            row,
        ]
    }

def _pvp_action_kb(battle_id, user_id, b=None):
    if b is None:
        b = _pvp_get(battle_id)
    if not b:
        return {"inline_keyboard": []}
    is_attacker = (user_id == b["attacker_id"])
    pending = b.get("attacker_pending_guard" if is_attacker else "defender_pending_guard", "")
    if pending:
        opponent_id = b["defender_id"] if is_attacker else b["attacker_id"]
        return _pvp_atk_kb(battle_id, user_id, opponent_id)
    return _pvp_guard_kb(battle_id, user_id)

def _pvp_send_dm(battle_id, b, last_event=None):
    a_id = b["attacker_id"]
    d_id = b["defender_id"]
    whose_turn = b.get("whose_turn")

    def _send_one(uid):
        text = _pvp_battle_text(b, last_event=last_event, viewer_id=uid)
        is_turn = (whose_turn == uid)
        kb = _pvp_action_kb(battle_id, uid) if is_turn else {"inline_keyboard": []}
        try:
            result = _tg_api("sendMessage", chat_id=uid, text=text, parse_mode="HTML", reply_markup=kb)
            if result.get("ok"):
                return result["result"]["message_id"]
        except Exception as e:
            print(f"[pvp_dm] send error uid={uid}: {e}")
        return 0

    a_mid = _send_one(a_id)
    d_mid = _send_one(d_id)
    _pvp_update(battle_id, attacker_dm_msg_id=a_mid, defender_dm_msg_id=d_mid)
    return a_mid, d_mid

def _pvp_edit_dm(b, last_event=None, battle_id=None):
    a_id = b["attacker_id"]
    d_id = b["defender_id"]
    whose_turn = b.get("whose_turn")
    a_mid = b.get("attacker_dm_msg_id", 0)
    d_mid = b.get("defender_dm_msg_id", 0)
    bid = battle_id or b.get("id")

    def _edit(uid, msg_id):
        if not msg_id:
            return
        text = _pvp_battle_text(b, last_event=last_event, viewer_id=uid)
        is_turn = (whose_turn == uid)
        kb = _pvp_action_kb(bid, uid) if (is_turn and bid) else {"inline_keyboard": []}
        try:
            _tg_api("editMessageText", chat_id=uid, message_id=msg_id, text=text, parse_mode="HTML", reply_markup=kb)
        except Exception as e:
            print(f"[pvp_dm] edit error uid={uid}: {e}")

    _edit(a_id, a_mid)
    _edit(d_id, d_mid)

def _pvp_edit(b, last_event=None, battle_id=None):
    a_mid = b.get("attacker_dm_msg_id", 0)
    d_mid = b.get("defender_dm_msg_id", 0)
    if not a_mid and not d_mid:
        try:
            text = _pvp_battle_text(b, last_event=last_event)
            bot.edit_message_text(text, chat_id=b["chat_id"], message_id=b["message_id"], parse_mode="HTML")
        except Exception as e:
            print(f"[pvp] edit fallback error: {e}")
        return
    _pvp_edit_dm(b, last_event=last_event, battle_id=battle_id)


@bot.message_handler(func=lambda m: m.text and m.text.strip() == "⚔️ ПВП")
def handle_pvp_menu(message):
    uid = message.from_user.id
    if not is_registered(uid):
        return
    wins, losses = _pvp_get_stats(uid)
    total = wins + losses
    winrate = round(wins / total * 100) if total > 0 else 0
    stats = _eq_calc_stats(uid)
    text = (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>ПВП</b>\n\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Побед: <b>{wins}</b>   💀 Поражений: <b>{losses}</b>\n"
        f"📊 Винрейт: <b>{winrate}%</b>\n\n"
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> {stats['damage']} атк   🛡️ {stats['defense']} защ\n\n"
        f"Вызов: <code>пвп @игрок</code> или <code>/pvp @игрок</code>"
    )
    kb = {"inline_keyboard": [
        [{"text": "🛡️ Экипировка", "callback_data": "pvp_menu_eq"},
         {"text": "⚔️ ПВП стат",   "callback_data": "pvp_menu_stat"}],
        [{"text": "🏟 Арена — поиск боёв", "callback_data": "arena_main"}],
    ]}
    _tg_api("sendMessage", chat_id=message.chat.id, text=text,
            parse_mode="HTML", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data == "pvp_menu_eq")
def cb_pvp_menu_eq(call):
    uid = call.from_user.id
    _tg_api("editMessageText",
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
...
@bot.callback_query_handler(func=lambda c: c.data == "pvp_menu_stat")
def cb_pvp_menu_stat(call):
    uid = call.from_user.id
    wins, losses = _pvp_get_stats(uid)
    total = wins + losses
    winrate = round(wins / total * 100) if total > 0 else 0
    ...
    for slot in slot_order:
        ...
        if set_id:
            ...
            em_id = sl.get("emoji_id")
            fb = sl.get("fallback", fallbacks[slot])
            tge = f'<tg-emoji emoji-id="{em_id}">{fb}</tg-emoji>' if em_id else fb
            eq_lines.append(f"{tge} {SLOT_NAMES[slot]}: <b>{s['name']}</b>")
        else:
            eq_lines.append(f"{fallbacks[slot]} {SLOT_NAMES[slot]}: <i>пусто</i>")
    bar = "🟩" * (winrate // 10) + "⬜" * (10 - winrate // 10)
    text = (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>ПВП Статистика</b>\n\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Побед: <b>{wins}</b>\n"
        f"💀 Поражений: <b>{losses}</b>\n"
        f"📊 {bar} <b>{winrate}%</b>\n\n"
        f"🛡️ <b>Снаряжение</b>\n" + "\n".join(eq_lines) +
        f"\n\n<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{stats['damage']}</b> атк   🛡️ <b>{stats['defense']}</b> защ"
    )
    kb = {"inline_keyboard": [[{"text": "◀️ Назад", "callback_data": "pvp_menu_back"}]]}
    _tg_api("editMessageText", chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=text, parse_mode="HTML", reply_markup=kb)
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "pvp_menu_back")
def cb_pvp_menu_back(call):
    uid = call.from_user.id
    wins, losses = _pvp_get_stats(uid)
    total = wins + losses
    winrate = round(wins / total * 100) if total > 0 else 0
...
@bot.message_handler(commands=["pvp", "PVP"])
@bot.message_handler(func=lambda m: m.text and not m.text.startswith("/") and m.text.strip().lower().startswith("пвп") and m.chat.type in ("group", "supergroup"))
def handle_pvp_challenge(message):
    uid = message.from_user.id
    if not is_registered(uid):
        return
    if check_flood(uid):
    ...
    if len(parts) >= 2 and parts[1].lower() in ("стат", "статистика"):
        return _pvp_send_stats(message)

    if len(parts) < 2:
        bot.send_message(message.chat.id,
            "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>— П В П —</b>\n\n"
            "Вызови соперника на пошаговый бой!\n\n"
            "<b>Использование:</b>\n"
            "  <code>пвп @игрок</code> — вызов\n"
            "  <code>пвп @игрок 1000</code> — вызов со ставкой\n"
            "  <code>/pvp @игрок</code> — то же самое\n\n"
            "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Удары по частям тела: Голова 💥×1.5, Корпус ×1.0, Ноги ×0.8\n"
            "🛡️ Защити нужную часть — блок снижает урон на 80%\n"
            "📬 Бой проходит в личных сообщениях",
            parse_mode="HTML")
        return

    existing = _pvp_active_for(uid)
    if existing:
        bot.send_message(message.chat.id, "❌ У тебя уже есть активный бой!", parse_mode="HTML")
        return

    # Проверяем ставку — если есть ставка и налоговый долг, блокируем
    _pvp_bet_check = None
    for _p in parts[2:]:
        try:
            _pvp_bet_check = int(_p.replace("к","000").replace("К","000").replace("м","000000").replace("М","000000"))
            break
        except Exception:
            pass
    if _pvp_bet_check and _pvp_bet_check > 0 and _tax_is_restricted(uid):
        bot.send_message(message.chat.id,
            "🚫 <b>ПВП со ставкой заблокировано</b> — есть налоговый долг!\n"
            "Напиши <code>налог</code> чтобы оплатить.", parse_mode="HTML")
        return
    ...
    if not target_id:
        bot.send_message(message.chat.id,
            "❌ Игрок не найден. Укажи <code>@username</code> или ответь на его сообщение",
            parse_mode="HTML")
        return

    if target_id == uid:
        bot.send_message(message.chat.id, "❌ Нельзя вызвать самого себя", parse_mode="HTML")
        return
    if not is_registered(target_id):
        bot.send_message(message.chat.id, "❌ Этот игрок не зарегистрирован", parse_mode="HTML")
        return
    if _pvp_active_for(target_id):
        bot.send_message(message.chat.id, "❌ Этот игрок уже в бою", parse_mode="HTML")
        return
    ...
    for part in parts[1:]:
        if part.startswith("@"):
            continue
        try:
            bet = int(parse_bet_amount(part, float("inf")))
            break
        except Exception:
            pass
    ...
    if bet > 0:
        if get_balance(uid) < bet:
            bot.send_message(message.chat.id,
                f"❌ Недостаточно монет! Нужно {format_balance(bet)}", parse_mode="HTML")
            return

    battle_id = _pvp_create(uid, target_id, bet, message.chat.id)
    if bet > 0:
        update_balance(uid, -bet)

    a_name = _mkt_user_name_by_id(uid)
    d_name = _mkt_user_name_by_id(target_id)
    a_stats = _eq_calc_stats(uid)
    d_stats = _eq_calc_stats(target_id)

    bet_line = f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> Ставка: <b>{format_balance(bet)}</b> с каждого" if bet > 0 else ""

    # Текст приглашения с экипировкой через tg-emoji
    invite_text = (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Вызов на бой!</b>\n\n"
        f"🔴 <b>{a_name}</b>\n"
        f"{_pvp_eq_line(uid)}\n"
        f"<i>{a_stats['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {a_stats['defense']} 🛡</i>\n\n"
        f"🔵 <b>{d_name}</b> — это ты\n"
        f"{_pvp_eq_line(target_id)}\n"
        f"<i>{d_stats['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {d_stats['defense']} 🛡</i>"
        f"{bet_line}\n\n"
        f"Принимаешь вызов?"
    )
    kb = {"inline_keyboard": [[
        {"text": "✅ Принять",   "callback_data": f"pvp_accept_{battle_id}"},
        {"text": "❌ Отклонить", "callback_data": f"pvp_decline_{battle_id}"},
    ]]}

    # Отправляем приглашение в ЛС защитнику
    dm_result = _tg_api("sendMessage", chat_id=target_id, text=invite_text,
                         parse_mode="HTML", reply_markup=kb)
    dm_ok = dm_result.get("ok")
    dm_msg_id = dm_result.get("result", {}).get("message_id") if dm_ok else None

    # Сохраняем message_id ЛС-приглашения (используем message_id поля под это)
    if dm_msg_id:
        _pvp_update(battle_id, message_id=dm_msg_id)

    # В чате — просто уведомление что вызов отправлен
    if dm_ok:
        chat_note = (
            f"📬 <b>{a_name}</b> вызвал <b>{d_name}</b> на бой!\n"
            f"Приглашение отправлено в личку{bet_line}"
        )
    else:
        # Если ЛС не открыто — fallback в чат
        chat_note = (
            f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{a_name}</b> вызывает <b>{d_name}</b>!\n"
            f"⚠️ Не удалось отправить в ЛС — ответи тут{bet_line}"
        )
        try:
            sent = bot.send_message(message.chat.id, invite_text,
                                    parse_mode="HTML", reply_markup=kb)
            _pvp_update(battle_id, chat_id=message.chat.id, message_id=sent.message_id)
        except Exception as e:
            print(f"[pvp] fallback chat send error: {e}")
            _pvp_update(battle_id, status="cancelled")
            if bet > 0:
                update_balance(uid, bet)
            return
    ...
    try:
        _tg_api("sendMessage", chat_id=message.chat.id,
                text=chat_note, parse_mode="HTML")
    except Exception:
        pass
    ...
    def _cancel_pending():
        ...
        if bet > 0:
            update_balance(uid, bet)
        if dm_ok and dm_msg_id:
            try:
                _tg_api("editMessageText", chat_id=target_id, message_id=dm_msg_id,
                        text=f"⏰ Вызов от <b>{a_name}</b> истёк", parse_mode="HTML")
            except Exception:
                pass

    t = _threading.Timer(60, _cancel_pending)
    t.daemon = True
    t.start()
...
def cb_pvp_accept(call):
    uid = call.from_user.id
    try:
        battle_id = int(call.data[len("pvp_accept_"):])
    except ValueError:
        bot.answer_callback_query(call.id, "❌")
        return
    ...
    if b["status"] != "pending":
        bot.answer_callback_query(call.id, "❌ Вызов уже недействителен")
        return
    if uid != b["defender_id"]:
        bot.answer_callback_query(call.id, "❌ Этот вызов не тебе!")
        return

    if b["bet"] > 0:
        if get_balance(uid) < b["bet"]:
            bot.answer_callback_query(call.id, f"❌ Недостаточно монет! Нужно {format_balance(b['bet'])}", show_alert=True)
            return
        update_balance(uid, -b["bet"])

    _pvp_update(battle_id, status="active", last_action=int(time.time()))
    bot.answer_callback_query(call.id, "⚔️ Бой начался! Проверь личку 📬")

    a_name = _mkt_user_name_by_id(b["attacker_id"])
    d_name = _mkt_user_name_by_id(b["defender_id"])
    try:
        _tg_api("editMessageText",
                chat_id=b["chat_id"],
                message_id=b["message_id"],
                text=(f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{a_name}</b> vs <b>{d_name}</b>\n\n"
                      f"📬 Бой идёт в личных сообщениях!\nОба игрока получили сообщение в ЛС."),
                parse_mode="HTML")
    except Exception:
        pass

    b = _pvp_get(battle_id)
    a_mid, d_mid = _pvp_send_dm(battle_id, b, last_event="<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Бой начался!")

    if not a_mid:
        try:
            bot.send_message(b["attacker_id"], "⚠️ Не удалось отправить бой в ЛС. Напиши боту /start и попробуй снова.", parse_mode="HTML")
        except Exception:
            pass
    if not d_mid:
        try:
            bot.send_message(b["defender_id"], "⚠️ Не удалось отправить бой в ЛС. Напиши боту /start и попробуй снова.", parse_mode="HTML")
        except Exception:
            pass

    _pvp_start_timeout(battle_id)
@bot.callback_query_handler(func=lambda c: c.data.startswith("pvp_decline_"))
def cb_pvp_decline(call):
    uid = call.from_user.id
    try:
        battle_id = int(call.data[len("pvp_decline_"):])
    except ValueError:
        bot.answer_callback_query(call.id, "❌")
        return
    ...
    if not b:
        ...
        return
    if b["status"] != "pending":
        bot.answer_callback_query(call.id, "❌ Уже недействителен")
        return
    if uid not in (b["attacker_id"], b["defender_id"]):
        bot.answer_callback_query(call.id, "❌ Не твой вызов")
        return

    _pvp_update(battle_id, status="cancelled")
    if b["bet"] > 0:
        update_balance(b["attacker_id"], b["bet"])

    a_name = _mkt_user_name_by_id(b["attacker_id"])
    d_name = _mkt_user_name_by_id(b["defender_id"])
    bot.answer_callback_query(call.id, "Вызов отклонён")
    _pvp_edit(b, f"❌ <b>{d_name}</b> отклонил вызов от <b>{a_name}</b>")

@bot.callback_query_handler(func=lambda c: c.data == "pvp_noop")
def cb_pvp_noop(call):
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("pvp_action_"))
def cb_pvp_action(call):
    uid = call.from_user.id
    try:
        parts = call.data.split("_")
        battle_id = int(parts[2])
        expected_uid = int(parts[3])
        action_type = parts[4]
        body_part = parts[5]
    except Exception:
        bot.answer_callback_query(call.id, "❌")
    ...
    if not b or b["status"] != "active":
        bot.answer_callback_query(call.id, "❌ Бой завершён")
        return
    if b["whose_turn"] != uid:
        bot.answer_callback_query(call.id, "❌ Сейчас не твой ход!")
        return

    is_attacker = (uid == b["attacker_id"])
    opponent_id = b["defender_id"] if is_attacker else b["attacker_id"]
    bp_info = BODY_PARTS[body_part]

    if action_type == "guard":
        _pvp_cancel_timeout(battle_id)
        if is_attacker:
            _pvp_update(battle_id, attacker_pending_guard=body_part, last_action=int(time.time()))
        else:
            _pvp_update(battle_id, defender_pending_guard=body_part, last_action=int(time.time()))
        bot.answer_callback_query(call.id, f"🛡 {bp_info['name']} — теперь выбери куда бить")
        b = _pvp_get(battle_id)
        kb = _pvp_action_kb(battle_id, uid, b)
        my_msg = b.get("attacker_dm_msg_id" if is_attacker else "defender_dm_msg_id", 0)
        if my_msg:
            text = _pvp_battle_text(b, viewer_id=uid)
            try:
                _tg_api("editMessageText", chat_id=uid, message_id=my_msg,
                        text=text, parse_mode="HTML", reply_markup=kb)
            except Exception as e:
                print(f"[pvp] guard edit error: {e}")
        _pvp_start_timeout(battle_id)
        return

    elif action_type == "atk":
        _pvp_cancel_timeout(battle_id)
        my_pending = b.get("attacker_pending_guard" if is_attacker else "defender_pending_guard", "")
        if not my_pending:
            bot.answer_callback_query(call.id, "❌ Сначала выбери защиту!")
            return
        try:
            stats_atk = _eq_calc_stats(uid)
            stats_def = _eq_calc_stats(opponent_id)
            opp_guard = b.get("defender_guard" if is_attacker else "attacker_guard", "")
            is_blocked = (opp_guard == body_part)

            # Урон меча: base ± 3. Если меча нет — базовый урон 5
            sword_dmg = stats_atk["damage"] if stats_atk["damage"] > 0 else 5
            base = sword_dmg + random.randint(-3, 3)
            raw_dmg = max(1, int(base * bp_info["dmg_mult"]))

            # Официальная формула Minecraft
            ap = stats_def["total_ap"]
            tough = stats_def["total_toughness"]
            if ap > 0:
                eff_ap = min(20.0, max(ap / 5.0, ap - raw_dmg / (2.0 + tough / 4.0)))
                reduction = eff_ap / 25.0
            else:
                reduction = 0.0

            reduced = max(1, int(raw_dmg * (1 - reduction)))
            if is_blocked:
                reduced = max(1, int(reduced * (1 - PVP_BODYBLOCK_REDUCTION)))

            actor_name = _mkt_user_name_by_id(uid)
            block_note = " · 🛡 заблок!" if is_blocked else ""
            crit_note = " · 💥 КРИТ" if bp_info["dmg_mult"] > 1.0 else ""
            armor_note = f" · 🛡{int(reduction*100)}% погл." if reduction > 0 and not is_blocked else ""
            event = f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{actor_name}</b> → {bp_info['emoji']} {bp_info['name']} · <b>−{reduced}</b>{block_note}{armor_note}{crit_note}"

            # Износ меча атакующего
            sword_result = _eq_damage(uid, "sword", 1)
            if sword_result == "broken":
                event += "\n💔 <b>Меч сломался!</b>"

            # Износ брони защищающегося
            armor_slot = bp_info.get("armor_slot")
            if armor_slot:
                armor_result = _eq_damage(opponent_id, armor_slot, 1)
                if armor_result == "broken":
                    slot_ru = {"helmet": "Шлем", "chestplate": "Нагрудник", "leggings": "Поножи", "boots": "Сапоги"}.get(armor_slot, armor_slot)
                    event += f"\n💔 <b>{slot_ru} соперника сломался!</b>"

            if is_attacker:
                new_def_hp = max(0, b["defender_hp"] - reduced)
                _pvp_update(battle_id,
                    defender_hp=new_def_hp,
                    attacker_guard=my_pending,
                    attacker_pending_guard="",
                    defender_guard="",
                    whose_turn=opponent_id,
                    last_action=int(time.time()))
            else:
                new_atk_hp = max(0, b["attacker_hp"] - reduced)
                _pvp_update(battle_id,
                    attacker_hp=new_atk_hp,
                    defender_guard=my_pending,
                    defender_pending_guard="",
                    attacker_guard="",
                    whose_turn=opponent_id,
                    last_action=int(time.time()))
            ...
            if b["attacker_hp"] <= 0 or b["defender_hp"] <= 0:
                _pvp_finish(battle_id, b, event)
                return

            _pvp_edit_dm(b, last_event=event, battle_id=battle_id)
            _pvp_start_timeout(battle_id)

        except Exception as e:
            print(f"[pvp] atk error: {e}")
            bot.answer_callback_query(call.id, "❌ Ошибка хода, попробуй ещё раз")
            _pvp_start_timeout(battle_id)

    else:
        bot.answer_callback_query(call.id, "❌")
def _pvp_finish(battle_id, b, last_event=""):
    ...
    _pvp_update(battle_id, status="finished")
    _pvp_record_stat(winner_id, loser_id)

    # Арена: читаем arena_match_id напрямую из БД (dict b мог быть устаревшим)
    _arena_mid = 0
    try:
        with get_db_cursor() as _c:
            _c.execute("SELECT arena_match_id FROM pvp_battles WHERE id=?", (battle_id,))
            _r = _c.fetchone()
            if _r: _arena_mid = _r[0] or 0
    except Exception: pass
    if _arena_mid:
        try: _arena_elo_update_and_notify(winner_id, loser_id, _arena_mid)
        except Exception as _ae: print(f"[arena] elo: {_ae}")

    prize_note = ""
    if b["bet"] > 0:
        prize = b["bet"] * 2
        update_balance(winner_id, prize)
        prize_note = f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>+{format_balance(prize)}</b> победителю!"

    w_name = _mkt_user_name_by_id(winner_id)
    l_name = _mkt_user_name_by_id(loser_id)
    w_wins, _ = _pvp_get_stats(winner_id)
    _, l_losses = _pvp_get_stats(loser_id)
    finish_event = f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>{w_name}</b> победил! 💀 {l_name} повержен{prize_note}"
    b_updated = _pvp_get(battle_id)
    _pvp_edit_dm(b_updated, last_event=finish_event, battle_id=battle_id)
    try:
        if b.get("message_id"):
            prize_str = f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> {format_balance(b['bet'] * 2)}" if b["bet"] > 0 else ""
            _tg_api("editMessageText", chat_id=b["chat_id"], message_id=b["message_id"],
                    text=f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>{w_name}</b> победил!{prize_str}", parse_mode="HTML")
    except Exception:
        pass
def _pvp_start_timeout(battle_id):
    def _timeout():
        b = _pvp_get(battle_id)
        if not b or b["status"] != "active":
            return
        if int(time.time()) - b["last_action"] < PVP_TURN_TIMEOUT - 2:
            return
        loser_id = b["whose_turn"]
        winner_id = b["defender_id"] if loser_id == b["attacker_id"] else b["attacker_id"]
        _pvp_update(battle_id, status="finished")
        _pvp_record_stat(winner_id, loser_id)
        if b["bet"] > 0:
            update_balance(winner_id, b["bet"] * 2)
        w_name = _mkt_user_name_by_id(winner_id)
        l_name = _mkt_user_name_by_id(loser_id)
        prize_note = f"\n<tg-emoji emoji-id=\"5217871612321216821\">💰</tg-emoji> <b>+{format_balance(b['bet'] * 2)}</b> победителю!" if b["bet"] > 0 else ""
        timeout_event = f"⏰ <b>{l_name}</b> не успел — победа <b>{w_name}</b>!{prize_note}"
        b_upd = _pvp_get(battle_id)
        if b_upd:
            _pvp_edit_dm(b_upd, last_event=timeout_event, battle_id=battle_id)

    t = _threading.Timer(PVP_TURN_TIMEOUT, _timeout)
    t.daemon = True
    _pvp_timeout_timers[battle_id] = t
    t.start()

def _pvp_cancel_timeout(battle_id):
    t = _pvp_timeout_timers.pop(battle_id, None)
    if t:
        t.cancel()

def _pvp_send_stats(message):
    uid = message.from_user.id
    if not is_registered(uid):
        return
    wins, losses = _pvp_get_stats(uid)
    ...
    for slot in slot_order:
        set_id = equipped.get(slot)
        if set_id:
            eq_lines.append(f"  {_eq_emoji(set_id, slot)} {SLOT_NAMES[slot]}: <b>{EQUIPMENT_SETS[set_id]['name']}</b>")
        else:
            eq_lines.append(f"  {fallbacks[slot]} {SLOT_NAMES[slot]}: <i>пусто</i>")

    wrate_bar = "🟩" * (winrate // 10) + "⬜" * (10 - winrate // 10)
    text = (
        f"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>— П В П   С Т А Т И С Т И К А —</b>\n\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Побед:      <b>{wins}</b>\n"
        f"💀 Поражений:  <b>{losses}</b>\n"
        f"📊 Винрейт:    {wrate_bar}  <b>{winrate}%</b>\n\n"
        f"╠══ 🛡️ <b>СНАРЯЖЕНИЕ</b> ══╣\n"
        + "\n".join(eq_lines) +
        f"\n\n  <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Урон: <b>{stats['damage']}</b>   🛡️ Защита: <b>{stats['defense']}</b>\n"
        f"╚══════════════════════╝"
    )
    bot.send_message(message.chat.id, text, parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("пвп стат", "пвп статистика"))
def handle_pvp_stats(message):
    _pvp_send_stats(message)


# ══════════════════════════════════════════════════════════════
#   🏟 ПВП АРЕНА — СИСТЕМА ELO / MATCHMAKING
# ══════════════════════════════════════════════════════════════

import threading as _threading_arena

ARENA_ELO_START       = 1000
ARENA_ELO_K_NEWBIE    = 48
ARENA_ELO_K_BASE      = 32
ARENA_ELO_K_VETERAN   = 20
ARENA_MATCH_RANGE     = 200
ARENA_MATCH_RANGE_EXP = 400
ARENA_QUEUE_TIMEOUT   = 120

ARENA_RANK_NAMES = [
    (0,    "🪨 Камень"),
    (1100, "<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> Бронза"),
    (1250, "🥈 Серебро"),
    (1400, "🥇 Золото"),
    (1600, "💎 Платина"),
    (1800, "👑 Алмаз"),
...
def init_arena_db(cursor=None):
    def _exec(cur):
        cur.execute("""
            CREATE TABLE IF NOT EXISTS arena_ratings (
                user_id INTEGER PRIMARY KEY, elo INTEGER DEFAULT 1000,
                wins INTEGER DEFAULT 0, losses INTEGER DEFAULT 0,
                streak INTEGER DEFAULT 0, best_streak INTEGER DEFAULT 0,
                peak_elo INTEGER DEFAULT 1000, total_games INTEGER DEFAULT 0,
                last_game INTEGER DEFAULT 0
            )""")
        cur.execute("""
            CREATE TABLE IF NOT EXISTS arena_queue (
                user_id INTEGER PRIMARY KEY, elo INTEGER NOT NULL,
                entered_at INTEGER NOT NULL, chat_id INTEGER NOT NULL,
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS arena_matches (
                ...
                p1_elo_before INTEGER DEFAULT 0, p2_elo_before INTEGER DEFAULT 0,
                p1_elo_after INTEGER DEFAULT 0,  p2_elo_after INTEGER DEFAULT 0,
                p1_elo_change INTEGER DEFAULT 0, p2_elo_change INTEGER DEFAULT 0,
                created_at INTEGER DEFAULT 0, finished_at INTEGER DEFAULT 0,
                status TEXT DEFAULT 'active'
            )""")
        # Добавляем колонку arena_match_id в pvp_battles если нет
        try:
            cur.execute("ALTER TABLE pvp_battles ADD COLUMN arena_match_id INTEGER DEFAULT 0")
        except Exception:
            pass
        cur.execute("CREATE INDEX IF NOT EXISTS idx_arena_queue_elo ON arena_queue(elo)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_arena_ratings_elo ON arena_ratings(elo DESC)")
        print("🏟 Таблицы арены OK")
    if cursor:
        _exec(cursor)
    else:
        with get_db_cursor() as cur:
...
try:
    ...
    print(f"[arena] init: {_e}")

def _elo_expected(a, b):
    return 1.0 / (1.0 + 10 ** ((b - a) / 400.0))

def _elo_k(games):
    return ARENA_ELO_K_NEWBIE if games < 10 else (ARENA_ELO_K_VETERAN if games > 50 else ARENA_ELO_K_BASE)

def _arena_rank(elo):
    r = ARENA_RANK_NAMES[0][1]
    for t, n in ARENA_RANK_NAMES:
        if elo >= t: r = n
    return r

def _arena_get_rating(uid):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM arena_ratings WHERE user_id=?", (uid,))
        row = cur.fetchone()
        if row: return dict(row)
        cur.execute("INSERT OR IGNORE INTO arena_ratings (user_id) VALUES (?)", (uid,))
    return {"user_id": uid, "elo": 1000, "wins": 0, "losses": 0,
            "streak": 0, "best_streak": 0, "peak_elo": 1000, "total_games": 0}

def _arena_elo_update(winner_id, loser_id, match_id):
    w = _arena_get_rating(winner_id)
    l = _arena_get_rating(loser_id)
    w_exp = _elo_expected(w["elo"], l["elo"])
    l_exp = _elo_expected(l["elo"], w["elo"])
    w_change = max(1,  int(round(_elo_k(w["total_games"]) * (1 - w_exp))))
    l_change = min(-1, int(round(_elo_k(l["total_games"]) * (0 - l_exp))))
    w_new = max(0, w["elo"] + w_change)
    l_new = max(0, l["elo"] + l_change)
    w_streak = w.get("streak", 0) + 1
    w_best   = max(w.get("best_streak", 0), w_streak)
    w_peak   = max(w.get("peak_elo", 1000), w_new)
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO arena_ratings (user_id,elo,wins,losses,streak,best_streak,peak_elo,total_games,last_game)
            VALUES (?,?,1,0,1,1,?,1,?)
            ON CONFLICT(user_id) DO UPDATE SET
                elo=?,wins=wins+1,streak=?,best_streak=?,peak_elo=?,total_games=total_games+1,last_game=?
        """, (winner_id,w_new,w_peak,now, w_new,w_streak,w_best,w_peak,now))
        cur.execute("""
            INSERT INTO arena_ratings (user_id,elo,wins,losses,streak,best_streak,peak_elo,total_games,last_game)
            VALUES (?,?,0,1,0,0,?,1,?)
            ON CONFLICT(user_id) DO UPDATE SET
                elo=?,losses=losses+1,streak=0,total_games=total_games+1,last_game=?
        """, (loser_id,l_new,l_new,now, l_new,now))
        cur.execute("""
            UPDATE arena_matches SET winner_id=?,finished_at=?,status='finished',
                p1_elo_after=CASE WHEN player1_id=? THEN ? ELSE p1_elo_after END,
                p2_elo_after=CASE WHEN player2_id=? THEN ? ELSE p2_elo_after END,
                p1_elo_change=CASE WHEN player1_id=? THEN ? ELSE p1_elo_change END,
                p2_elo_change=CASE WHEN player2_id=? THEN ? ELSE p2_elo_change END
            WHERE id=?
        """, (winner_id,now,
              winner_id,w_new, winner_id,w_new,
              winner_id,w_change, winner_id,w_change,
              match_id))
    return w_change, l_change, w_new, l_new

def _arena_elo_update_and_notify(winner_id, loser_id, match_id):
    w_ch, l_ch, w_new, l_new = _arena_elo_update(winner_id, loser_id, match_id)
    try:
        _tg_api("sendMessage", chat_id=winner_id, parse_mode="HTML",
                text=f"🏟 <b>Итог арены</b>\n\n<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Победа!\n📈 ELO: <b>{w_new-w_ch} → {w_new}</b> (<b>+{w_ch}</b>)\n{_arena_rank(w_new)}")
    except Exception: pass
    try:
        _tg_api("sendMessage", chat_id=loser_id, parse_mode="HTML",
                text=f"🏟 <b>Итог арены</b>\n\n💀 Поражение\n📉 ELO: <b>{l_new-l_ch} → {l_new}</b> (<b>{l_ch}</b>)\n{_arena_rank(l_new)}")
    except Exception: pass

# ── Очередь ──
_arena_q_lock = _threading_arena.Lock()

def _arena_in_queue(uid):
    with get_db_cursor() as cur:
        cur.execute("SELECT * FROM arena_queue WHERE user_id=?", (uid,))
        r = cur.fetchone()
        return dict(r) if r else None

def _arena_join_queue(uid, chat_id, msg_id=0):
    r = _arena_get_rating(uid)
    now = int(time.time())
    with get_db_cursor() as cur:
        cur.execute("INSERT OR REPLACE INTO arena_queue (user_id,elo,entered_at,chat_id,msg_id) VALUES (?,?,?,?,?)",
                    (uid, r["elo"], now, chat_id, msg_id))

def _arena_leave_queue(uid):
    with get_db_cursor() as cur:
        cur.execute("DELETE FROM arena_queue WHERE user_id=?", (uid,))

def _arena_queue_size():
    with get_db_cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM arena_queue")
        return cur.fetchone()[0]

def _arena_find_opponent(uid, expand=False):
    r = _arena_get_rating(uid)
    diff = ARENA_MATCH_RANGE_EXP if expand else ARENA_MATCH_RANGE
    with get_db_cursor() as cur:
        cur.execute("""SELECT * FROM arena_queue WHERE user_id!=? AND elo BETWEEN ? AND ?
                       ORDER BY ABS(elo-?) ASC, entered_at ASC LIMIT 1""",
                    (uid, r["elo"]-diff, r["elo"]+diff, r["elo"]))
        row = cur.fetchone()
        return dict(row) if row else None

def _arena_update_queue_msg(uid, text):
    e = _arena_in_queue(uid)
    if not e or not e["msg_id"]: return
    try:
        kb = {"inline_keyboard": [[{"text": "❌ Выйти из очереди", "callback_data": "arena_leave_queue"}]]}
        _tg_api("editMessageText", chat_id=uid, message_id=e["msg_id"],
                text=text, parse_mode="HTML", reply_markup=kb)
    except Exception: pass

def _arena_searching_text(uid, secs=0):
    r = _arena_get_rating(uid)
    exp = "\n🔍 <i>Расширяем поиск...</i>" if secs > 30 else ""
    return (f"🏟 <b>АРЕНА — ПОИСК</b>\n\n{_arena_rank(r['elo'])}  ELO: <b>{r['elo']}</b>\n\n"
            f"⏳ Ожидание: <b>{secs}с</b>{exp}\n👥 В очереди: <b>{_arena_queue_size()}</b>\n\n"
            f"Бой начнётся автоматически в личке.")

def _arena_start_match(uid1, uid2):
    # Очередь уже очищена в _arena_try_match внутри лока
    r1 = _arena_get_rating(uid1)
    r2 = _arena_get_rating(uid2)
    now = int(time.time())

    # INSERT без arena_match_id — добавим отдельным UPDATE (баг 4 fix)
    with get_db_cursor() as cur:
        cur.execute("""
            INSERT INTO pvp_battles
            (attacker_id,defender_id,bet,status,attacker_hp,defender_hp,whose_turn,
             attacker_blocked,defender_blocked,chat_id,created_at,last_action,
             attacker_dm_msg_id,defender_dm_msg_id,attacker_guard,defender_guard)
            VALUES (?,?,0,'active',100,100,?,0,0,?,?,?,0,0,'','')
        """, (uid1, uid2, uid1, uid1, now, now))
        battle_id = cur.lastrowid

    with get_db_cursor() as cur:
        cur.execute("""INSERT INTO arena_matches
            (battle_id,player1_id,player2_id,p1_elo_before,p2_elo_before,created_at,status)
            VALUES (?,?,?,?,?,?,'active')""",
            (battle_id, uid1, uid2, r1["elo"], r2["elo"], now))
        match_id = cur.lastrowid

    # Обновляем arena_match_id отдельно — колонка точно есть после init_arena_db
    with get_db_cursor() as cur:
        cur.execute("UPDATE pvp_battles SET arena_match_id=? WHERE id=?", (match_id, battle_id))

    n1 = _mkt_user_name_by_id(uid1)
    n2 = _mkt_user_name_by_id(uid2)
    s1 = _eq_calc_stats(uid1)
    s2 = _eq_calc_stats(uid2)

    header = (f"🏟 <b>БОЙ НАЙДЕН!</b>\n\n"
              f"🔴 <b>{n1}</b>  {_arena_rank(r1['elo'])}  <i>({r1['elo']} ELO)</i>\n"
              f"{_pvp_eq_line(uid1)}\n<i>{s1['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {s1['defense']} 🛡</i>\n\n"
              f"🔵 <b>{n2}</b>  {_arena_rank(r2['elo'])}  <i>({r2['elo']} ELO)</i>\n"
              f"{_pvp_eq_line(uid2)}\n<i>{s2['damage']} <tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji>  {s2['defense']} 🛡</i>\n\n")

    b = _pvp_get(battle_id)
    kb1 = _pvp_action_kb(battle_id, uid1, b)
    r1a = _tg_api("sendMessage", chat_id=uid1, text=header+"<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Твой ход!</b>",
                  parse_mode="HTML", reply_markup=kb1)
    r2a = _tg_api("sendMessage", chat_id=uid2, text=header+"⏳ Ждём хода соперника...",
                  parse_mode="HTML", reply_markup={"inline_keyboard": []})
    a_mid = r1a.get("result",{}).get("message_id",0) if r1a.get("ok") else 0
    d_mid = r2a.get("result",{}).get("message_id",0) if r2a.get("ok") else 0
    _pvp_update(battle_id, attacker_dm_msg_id=a_mid, defender_dm_msg_id=d_mid)
    _pvp_start_timeout(battle_id)
    print(f"[arena] {uid1} vs {uid2} battle={battle_id} match={match_id}")

def _arena_try_match(uid, expand=False):
    # Лок только для атомарной проверки + удаления из очереди
    # _arena_start_match (отправка TG сообщений) — вне лока чтобы не блокировать других
    with _arena_q_lock:
        if not _arena_in_queue(uid): return False
        opp = _arena_find_opponent(uid, expand=expand)
        if not opp: return False
        opp_id = opp["user_id"]
        if not _arena_in_queue(opp_id): return False
        # Убираем обоих из очереди прямо здесь — внутри лока
        _arena_leave_queue(uid)
        _arena_leave_queue(opp_id)
    # Запускаем матч вне лока
    _threading_arena.Thread(target=_arena_start_match, args=(uid, opp_id), daemon=True).start()
    return True

def _arena_queue_ticker(uid, started_at):
    while True:
        time.sleep(5)
        elapsed = int(time.time()) - started_at
        if not _arena_in_queue(uid): return
        # Поиск бесконечный — отменить можно только кнопкой "Выйти из очереди"
        # После 30 секунд расширяем диапазон ELO для более быстрого матча
        if _arena_try_match(uid, expand=(elapsed > 30)): return
        _arena_update_queue_msg(uid, _arena_searching_text(uid, elapsed))

# ── Интерфейс арены ──
def _arena_main_text(uid):
    r = _arena_get_rating(uid)
    total = r["wins"] + r["losses"]
    wr = round(r["wins"] / total * 100) if total > 0 else 0
    wr_bar = "🟩"*(wr//10) + "⬜"*(10-wr//10)
    with get_db_cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM arena_ratings WHERE elo>?", (r["elo"],))
        place = cur.fetchone()[0] + 1
    return (f"🏟 <b>П В П — А Р Е Н А</b>\n\n{_arena_rank(r['elo'])}\n"
            f"📊 ELO: <b>{r['elo']}</b>   <tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> #{place} в рейтинге\n\n"
            f"Победы: <b>{r['wins']}</b>   Поражения: <b>{r['losses']}</b>\n"
            f"Винрейт: {wr_bar} <b>{wr}%</b>\n"
            f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> Серия: <b>{r['streak']}</b>   Рекорд: <b>{r['best_streak']}</b>\n"
            f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Пик ELO: <b>{r['peak_elo']}</b>\n\n"
            f"<i>Поиск подбирает соперника с близким рейтингом.\nПобеда даёт ELO, поражение — отнимает.</i>")

def _arena_main_kb(uid):
    in_q = _arena_in_queue(uid)
    # Баг 2 fix: блокируем только если есть НЕ-арена бой
    active = _pvp_active_for(uid)
    in_battle = active and not active.get("arena_match_id")
    rows = []
    if in_battle:
        rows.append([{"text": "⚔️ Ты уже в обычном бою!", "callback_data": "arena_noop"}])
    elif in_q:
        rows.append([{"text": "❌ Выйти из поиска", "callback_data": "arena_leave_queue"}])
    else:
        rows.append([{"text": "🔍 Найти бой", "callback_data": "arena_search"}])
    rows.append([{"text": "🏆 Топ арены", "callback_data": "arena_top"}])
    rows.append([{"text": "◀️ Назад", "callback_data": "pvp_menu_back"}])
    return {"inline_keyboard": rows}

@bot.callback_query_handler(func=lambda c: c.data == "arena_main")
def cb_arena_main(call):
    uid = call.from_user.id
    if not is_registered(uid):
        bot.answer_callback_query(call.id, "❌ Не зарегистрирован"); return
    _tg_api("editMessageText", chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=_arena_main_text(uid), reply_markup=_arena_main_kb(uid), parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "arena_search")
def cb_arena_search(call):
    uid = call.from_user.id
    # Быстрые проверки ДО answer_callback_query
    if not is_registered(uid):
        bot.answer_callback_query(call.id, "❌ Не зарегистрирован"); return
    active = _pvp_active_for(uid)
    if active and not active.get("arena_match_id"):
        bot.answer_callback_query(call.id, "❌ Сначала закончи текущий бой!", show_alert=True); return
    if _arena_in_queue(uid):
        bot.answer_callback_query(call.id, "❌ Ты уже в очереди!"); return

    # Отвечаем СРАЗУ — кнопка разблокируется мгновенно
    bot.answer_callback_query(call.id, "🔍 Ищем соперника...")

    def _do_search():
        # Встаём в очередь
        _arena_join_queue(uid, call.message.chat.id, 0)
        # Пробуем найти сразу
        found = _arena_try_match(uid, expand=False)
        if found:
            try:
                _tg_api("sendMessage", chat_id=uid, parse_mode="HTML",
                        text="<tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> <b>Соперник найден!</b> Бой начинается 👇")
            except Exception: pass
            return
        # Соперника нет — шлём сообщение "в поиске" в ЛС
        now = int(time.time())
        kb = {"inline_keyboard": [[{"text": "❌ Выйти из очереди", "callback_data": "arena_leave_queue"}]]}
        dm = _tg_api("sendMessage", chat_id=uid, text=_arena_searching_text(uid, 0),
                     parse_mode="HTML", reply_markup=kb)
        msg_id = dm.get("result",{}).get("message_id",0) if dm.get("ok") else 0
        with get_db_cursor() as cur:
            cur.execute("UPDATE arena_queue SET msg_id=? WHERE user_id=?", (msg_id, uid))
        _threading_arena.Thread(target=_arena_queue_ticker, args=(uid, now), daemon=True).start()
        # Обновляем меню
        try:
            _tg_api("editMessageText", chat_id=call.message.chat.id, message_id=call.message.message_id,
                    text=_arena_main_text(uid), reply_markup=_arena_main_kb(uid), parse_mode="HTML")
    ...
    _threading_arena.Thread(target=_do_search, daemon=True).start()

@bot.callback_query_handler(func=lambda c: c.data == "arena_leave_queue")
def cb_arena_leave_queue(call):
    uid = call.from_user.id
    if not _arena_in_queue(uid):
        bot.answer_callback_query(call.id, "Ты не в очереди"); return
    _arena_leave_queue(uid)
    bot.answer_callback_query(call.id, "✅ Вышел из очереди")
    try:
        _tg_api("editMessageText", chat_id=call.message.chat.id, message_id=call.message.message_id,
                text="❌ <b>Поиск отменён</b>", parse_mode="HTML",
                reply_markup={"inline_keyboard": []})
    except Exception: pass

@bot.callback_query_handler(func=lambda c: c.data == "arena_top")
def cb_arena_top(call):
    with get_db_cursor() as cur:
        cur.execute("SELECT user_id,elo,wins,losses FROM arena_ratings ORDER BY elo DESC LIMIT 10")
        rows = cur.fetchall()
    if not rows:
        bot.answer_callback_query(call.id, "Топ пока пуст"); return
    medals = ["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣","🔟"]
    lines = ["<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> <b>ТОП 10 АРЕНЫ</b>\n"]
    for i, (uid_r, elo, wins, losses) in enumerate(rows):
        name = _mkt_user_name_by_id(uid_r)
        total = wins + losses
        wr = round(wins/total*100) if total > 0 else 0
        lines.append(f"{medals[i]} <b>{name}</b>\n   {_arena_rank(elo)}  ELO: <b>{elo}</b>  WR: {wr}%")
    kb = {"inline_keyboard": [[{"text": "◀️ Назад", "callback_data": "arena_main"}]]}
    _tg_api("editMessageText", chat_id=call.message.chat.id, message_id=call.message.message_id,
            text="\n\n".join(lines), reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda c: c.data == "arena_noop")
def cb_arena_noop(call):
    bot.answer_callback_query(call.id, "⚔️ Сначала закончи текущий бой!")

@bot.message_handler(commands=["arena"])
@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("арена", "пвп арена"))
def handle_arena_cmd(message):
    uid = message.from_user.id
    if not is_registered(uid):
        bot.send_message(message.chat.id, "❌ Напиши /start"); return
    if message.chat.type == "private":
        _tg_api("sendMessage", chat_id=uid, text=_arena_main_text(uid),
                reply_markup=_arena_main_kb(uid), parse_mode="HTML")
    else:
        bot.send_message(message.chat.id,
            "🏟 <b>ПВП Арена</b>\n\nОткрой в личке бота:\n<code><tg-emoji emoji-id=\"5415965335192883624\">⚔️</tg-emoji> ПВП</code> → <b>Арена</b>",
            parse_mode="HTML")

@bot.message_handler(func=lambda m: (
    m.chat.type == "private" and m.text and
    m.text.strip().lower() in ("пвп", "/pvp")
))
def handle_pvp_private_arena(message):
    """Быстрый запуск: пишешь 'пвп' — сразу встаёшь в очередь поиска."""
    uid = message.from_user.id
    if not is_registered(uid): return

    # Если уже в бою или в очереди — просто показываем меню арены
    active = _pvp_active_for(uid)
    if active and not active.get("arena_match_id"):
        _tg_api("sendMessage", chat_id=uid, text=_arena_main_text(uid),
                reply_markup=_arena_main_kb(uid), parse_mode="HTML")
    ...
    # Сразу встаём в очередь
    def _quick_search():
        _arena_join_queue(uid, uid, 0)
        found = _arena_try_match(uid, expand=False)
        if found:
            try:
                _tg_api("sendMessage", chat_id=uid, parse_mode="HTML",
    ...
    _threading_arena.Thread(target=_quick_search, daemon=True).start()

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() in ("арена стат", "арена статистика"))
def handle_arena_stat(message):
    uid = message.from_user.id
    if not is_registered(uid): return
    r = _arena_get_rating(uid)
    total = r["wins"] + r["losses"]
    ...
    with get_db_cursor() as cur:
        ...
        place = cur.fetchone()[0] + 1
    bot.send_message(message.chat.id,
        f"🏟 <b>СТАТИСТИКА АРЕНЫ</b>\n\n{_arena_rank(r['elo'])}\n"
        f"📊 ELO: <b>{r['elo']}</b>   Место: <b>#{place}</b>\n"
        f"<tg-emoji emoji-id=\"5204065856560045377\">⭐</tg-emoji> Пик ELO: <b>{r['peak_elo']}</b>\n\n"
        f"<tg-emoji emoji-id=\"5213326931331659859\">🏆</tg-emoji> Победы: <b>{r['wins']}</b>\n💀 Поражения: <b>{r['losses']}</b>\n"
        f"📈 Винрейт: {wr_bar} <b>{wr}%</b>\n\n"
        f"<tg-emoji emoji-id=\"5424972470023104089\">🔥</tg-emoji> Текущая серия: <b>{r['streak']}</b>\n🏅 Рекорд: <b>{r['best_streak']}</b>",
        parse_mode="HTML")

print("🏟 Арена загружена")


# ══════════════════════════════════════════════════════════════
#   ⛏️ УПРАВЛЕНИЕ ШАНСАМИ РУД (АДМИН)
#   Команды:
#     руды шансы           — показать текущие шансы
#     руда <id> <шанс>     — изменить шанс руды
#     руды сброс           — вернуть дефолтные значения
# ══════════════════════════════════════════════════════════════

# Дефолтные шансы (берём из MINE_RESOURCES при старте)
_ORE_DEFAULT_CHANCES = {r["id"]: r["chance"] for r in MINE_RESOURCES}

def _ore_chances_text():
    total = sum(r["chance"] for r in MINE_RESOURCES)
    lines = ["⛏️ <b>Шансы выпадения руд</b>\n"]
    for r in MINE_RESOURCES:
        pct = r["chance"] / total * 100
        default = _ORE_DEFAULT_CHANCES[r["id"]]
        diff = r["chance"] - default
        diff_str = ""
        if diff > 0:   diff_str = f" <b>(+{diff:.1f} ✏️)</b>"
        elif diff < 0: diff_str = f" <b>({diff:.1f} ✏️)</b>"
        lines.append(
            f"  <code>{r['id']:10}</code>  {r['name']:12}  "
            f"<b>{r['chance']:.1f}</b> ({pct:.1f}%){diff_str}"
        )
    lines.append(f"\nСумма: <b>{total:.1f}</b>  (чем больше число — тем чаще выпадает)")
    lines.append("\n<i>Изменить: <code>руда &lt;id&gt; &lt;число&gt;</code></i>")
    lines.append("<i>Сброс:    <code>руды сброс</code></i>")
    return "\n".join(lines)

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "руды шансы" and is_admin(m.from_user.id))
def handle_ore_chances_show(message):
    bot.send_message(message.chat.id, _ore_chances_text(), parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "руды сброс" and is_admin(m.from_user.id))
def handle_ore_chances_reset(message):
    for r in MINE_RESOURCES:
        r["chance"] = _ORE_DEFAULT_CHANCES[r["id"]]
    bot.send_message(message.chat.id,
        "✅ <b>Шансы руд сброшены на дефолт</b>\n\n" + _ore_chances_text(),
        parse_mode="HTML")

@bot.message_handler(func=lambda m: m.text and re.match(r"^руда\s+\S+\s+[\d.]+$", m.text.strip().lower()) and is_admin(m.from_user.id))
def handle_ore_chance_set(message):
    parts = message.text.strip().split()
    # "руда <id> <число>"
    if len(parts) != 3:
        bot.send_message(message.chat.id, "❌ Формат: <code>руда &lt;id&gt; &lt;число&gt;</code>", parse_mode="HTML")
        return
    ore_id = parts[1].lower()
    try:
        new_chance = float(parts[2])
    except ValueError:
        bot.send_message(message.chat.id, "❌ Число должно быть числом, например: <code>руда diamond 0.8</code>", parse_mode="HTML")
        return
    if new_chance < 0:
        bot.send_message(message.chat.id, "❌ Шанс не может быть отрицательным"); return

    ore = next((r for r in MINE_RESOURCES if r["id"] == ore_id), None)
    if not ore:
        ids = ", ".join(f"<code>{r['id']}</code>" for r in MINE_RESOURCES)
        bot.send_message(message.chat.id, f"❌ Руда <code>{ore_id}</code> не найдена\nДоступные: {ids}", parse_mode="HTML")
        return

    old = ore["chance"]
    ore["chance"] = new_chance
    total = sum(r["chance"] for r in MINE_RESOURCES)
    pct = new_chance / total * 100 if total > 0 else 0
    bot.send_message(message.chat.id,
        f"✅ <b>{ore['name']}</b>\n"
        f"Шанс: <b>{old:.1f}</b> → <b>{new_chance:.1f}</b>\n"
        f"Теперь выпадает в <b>{pct:.1f}%</b> случаев\n\n"
        f"<i>Изменение активно сразу, до перезапуска бота.\n"
        f"Для постоянного эффекта измени MINE_RESOURCES в коде.</i>",
        parse_mode="HTML")

# Быстрый просмотр — просто "руды"

@bot.message_handler(func=lambda m: m.text and m.text.lower().strip() == "обновить" and is_admin(m.from_user.id))
def handle_refresh_usernames(message):
    """Обновить username/first_name всех пользователей по последнему сообщению."""
    bot.send_message(message.chat.id, "⏳ Обновляю данные пользователей...")
    updated = 0
    with get_db_cursor() as cur:
    ...
    for uid in uids:
        try:
            ...
            pass
    bot.send_message(message.chat.id, f"✅ Обновлено {updated} из {len(uids)} пользователей.")

@bot.message_handler(func=lambda m: m.text and m.text.strip().lower() == "руды" and is_admin(m.from_user.id))
def handle_ore_quick(message):
    bot.send_message(message.chat.id, _ore_chances_text(), parse_mode="HTML")
