...
def calculate_interest(user_id):
    ...
    if result and result[0] > 0:  # Если есть вклад
        ...
        if hours_passed >= 1:  # Прошёл минимум 1 час
            ...
            interest = int(bank_deposit * 0.01 * interest_hours)  # 0.5% за каждый час
...
# =============== ОБНОВЛЕННАЯ УБОРКА СНЕГА (5 КНОПОК + СНЕЖИНКА) ===============

# Глобальные переменные для снежной уборки
SNOW_COOLDOWN = {}  # {user_id: timestamp_end}
SNOW_JOBS = {}  # {user_id: {"clicks_left": X, "total_earnings": X, "completed": X}}
    ...
    try:
        ...
        if user_id in SNOW_COOLDOWN:
            ...
            if current_time < cooldown_end:
                ...
                cool_msg = "⏳ Отдых после уборки!\n\n"
                cool_msg += f"⏰ Еще: {minutes}м {seconds}с"
        ...
        if user_id in SNOW_JOBS:
            ...
            markup = create_snow_keyboard(job["clicks_left"])
        ...
        stats_msg = (
            f"❄️ УБОРКА СНЕГА\n\n"
            f"🎯 Задание: 150 кликов\n"
            f"💰 Заработок: {format_balance(earnings)}❄️\n"
            f"📈 Надбавка: +50❄️ за каждую уборку\n"
            f"🏆 Выполнено: {completed_jobs}\n\n"
            f"Кликай на снежинку чтобы убирать!"
        )
        
        markup = create_snow_keyboard(150)
...
def create_snow_keyboard(clicks_left):
    """Создает клавиатуру с 5 кнопками (1 снежинка + 4 точки)"""
    ...
    for i in range(5):
        if i == snow_position:
            # Снежинка
            buttons.append(InlineKeyboardButton("❄️", callback_data="snow_click"))
        else:
            # Точки
            buttons.append(InlineKeyboardButton("•", callback_data="snow_dummy"))
...
def get_snow_progress_message(job):
    ...
    clicks_done = 150 - job["clicks_left"]
    progress_percent = (clicks_done / 150) * 100
    ...
    message = (
        ...
        f"🎯 {clicks_done}/150 кликов\n"
        f"💰 Заработок: {format_balance(job['total_earnings'])}❄️\n\n"
        f"Кликай на снежинку!"
    )
    
    return message

@bot.callback_query_handler(func=lambda call: call.data in ["snow_click", "snow_dummy"])
def handle_snow_click(call):
    try:
        ...
        if call.data == "snow_dummy":
            # Клик на точку - ничего не происходит
            bot.answer_callback_query(call.id, "")
            return
        ...
        if job["clicks_left"] <= 0:
            ...
            result_msg = (
                f"✅ УБОРКА ЗАВЕРШЕНА!\n\n"
                f"🎯 Выполнено: 150 кликов\n"
                f"💰 Заработано: {format_balance(earnings)}❄️\n"
                f"📊 Баланс: {format_balance(new_balance)}❄️\n"
                f"🏆 Всего уборок: {job['completed']}\n\n"
                f"⏳ Следующая через 3 минуты"
            )
...
# =============== АВТООЧИСТКА СТАРЫХ РАБОТ ===============

def cleanup_snow_jobs():
    """Очищает старые работы (старше 30 минут)"""
    while True:
        ...
        for user_id, job in SNOW_JOBS.items():
            # Если работа начата больше 30 минут назад
            if current_time - job.get("start_time", current_time) > 1800:
                to_remove.append(user_id)
        
        for user_id in to_remove:
            del SNOW_JOBS[user_id]
            logging.info(f"Очищена старая работа снега для {user_id}")

# Запускаем очистку в отдельном потоке
snow_cleanup_thread = threading.Thread(target=cleanup_snow_jobs, daemon=True)
...
def handle_snow_stat(message):
    ...
    message_text = "❄️ СТАТИСТИКА УБОРКИ\n\n"
    ...
    if user_id in SNOW_JOBS:
        ...
        message_text += f"🎯 {clicks_done}/150 кликов\n"
        message_text += f"💰 Заработок: {format_balance(job['total_earnings'])}❄️\n"
    else:
        message_text += "📭 Нет активной уборки\n"
        message_text += "💡 Начните через меню 'Работа'"
